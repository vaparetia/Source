...
{
   /// <summary>
   /// Get the CRC-32 Value for a String, Byte array or Stream
   /// </summary>
   public class CRC32
   {
      readonly static uint[] skCRCLookup = new uint[] {
      ...
      //readonly static Encoding encoder = Encoding.GetEncoding(0); // Default codepage, can't use this otherwise CRC's would change for different computers
      readonly static Encoding skEncoder = Encoding.GetEncoding(437);  // Standard US OEM (IBM437 - OEM United States)
      readonly static uint skPoly = 4294967295;

      /// <summary>
      /// Get the crc32 value for an entire Stream, setting the position to zero.
      /// </summary>
      /// <param name="st">The stream to perform the crc32 calculation on.</param>
      /// <returns>The crc32 value.</returns>
      static public uint CalculateCRC(Stream st) 
      {
         return CalculateCRC(st, 0, st.Length); 
      } 
      ...
      /// <returns>The crc32 value.</returns>
      static public uint CalculateCRC(Stream st, long beginPosition, long length) 
      {
         uint crcVal = skPoly;
         ...
         try
         {
            ...
            { 
               ...
               crcVal = (crcVal >> 8) ^ skCRCLookup[(crcVal & 0xFF) ^ by];
            }
         }
         catch (Exception e)
         {
      ...
      /// </summary>
      /// <param name="text">The string you wish to calculate the crc32 value on.</param>
      /// <returns>The CRC-32 value.</returns>
      static public uint CalculateCRC(string text) 
      {
         byte[] buffer; 
         buffer = skEncoder.GetBytes(text);
         return CalculateCRC(buffer); 
      } 

      /// <summary>
      /// Get the CRC-32 value for a byte[].
      /// </summary>
      /// <param name="by">The byte[] you wish to calculate the crc32 value on</param>
      /// <returns>The CRC-32 value.</returns>
      static public uint CalculateCRC(byte[] by) 
      {
         uint crcVal = skPoly;
         long len;
         len = by.Length;
         ...
         { 
            crcVal = (crcVal >> 8) ^ skCRCLookup[(crcVal & 0xFF) ^ by[i]];
         }
         crcVal ^= skPoly;
         return crcVal;
      }
