//----------------------------------------------------------------------------
// DCCIndexBuffer.h
// Dreamcast stub — mirrors VTACIndexBuffer.h without Vita-specific deps
//----------------------------------------------------------------------------
...
public:
   CIndexBufferChunk()
      : mpIndexBuffer(NULL), mpMemory(NULL), mCount(0) {}

   CIndexBufferChunk(CIndexBuffer *pIndexBuffer, uint16 *pMemory, int const count)
      : mpIndexBuffer(pIndexBuffer), mpMemory(pMemory), mCount(count) {}
...
public:
   ...
   static CIndexBufferChunk const ChunkFromInts(int const *pIn) { return *reinterpret_cast<CIndexBufferChunk const *>(pIn); }
