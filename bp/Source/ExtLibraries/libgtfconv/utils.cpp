...
uint32_t utilStr2Remap(const char*str)
{
	uint32_t remap_pass_throw  = 
	...
	for(uint32_t i = 0; i < 4; ++i){
		uint32_t pass_throw_color = CELL_GCM_TEXTURE_REMAP_FROM_A + i;
		uint32_t flag = (CELL_GCM_TEXTURE_REMAP_ONE << 8) | pass_throw_color;
