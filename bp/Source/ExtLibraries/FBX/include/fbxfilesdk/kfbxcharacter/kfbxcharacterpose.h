...
public:
    ...
    bool GetLocalPosition(ECharacterNodeId pCharacterNodeId, KFbxVector4& pLocalT, KFbxVector4& pLocalR, KFbxVector4& pLocalS) const
    {
		...
		KFbxScene* lScene = lCharacterLink.mNode->GetScene();
		if( lScene )
		{
			pLocalT = lScene->GetEvaluator()->GetNodeLocalTransform(lCharacterLink.mNode).GetT();
			pLocalR = lScene->GetEvaluator()->GetNodeLocalTransform(lCharacterLink.mNode).GetR();
			pLocalS = lScene->GetEvaluator()->GetNodeLocalTransform(lCharacterLink.mNode).GetS();
	        return true;
		}
		return false;
    }
    ...
    bool GetGlobalPosition(ECharacterNodeId pCharacterNodeId, KFbxXMatrix& pGlobalPosition) const
    {
		...
		if( lScene )
		{
			pGlobalPosition = lScene->GetEvaluator()->GetNodeGlobalTransform(lCharacterLink.mNode);
	        return true;
		}
		return false;
    }
