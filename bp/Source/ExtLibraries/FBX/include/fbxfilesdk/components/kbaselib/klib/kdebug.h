    ...
    // Macros
	    ...
	    #define K_ASSERT( pCondition )   \
						    ...
						    {\
                                ...
                                if (!HideForEver) KFBX_ASSERT_NAMESPACE(KAssertFailed)( KSTRINGITIZE(__FILE__), __func__, __LINE__, KSTRINGITIZE(#pCondition),&HideForEver ); \
						    }((void)0)

	    #define K_ASSERT_MSG_NOW( pMessage )   \
						    { \
							    static bool HideForEver=false; \
							    if (!HideForEver) KFBX_ASSERT_NAMESPACE(KAssertFailed)( KSTRINGITIZE(__FILE__), __func__, __LINE__,  pMessage,&HideForEver ); \
						    }((void)0)
	    ...
	    #define K_ASSERT_MSG_NOT_IMPLEMENTED \
						    { \
							    static bool HideForEver=false; \
							    if (!HideForEver) KFBX_ASSERT_NAMESPACE(KAssertFailed)( KSTRINGITIZE(__FILE__), __func__, __LINE__,  "Not implemented",&HideForEver ); \
						    }((void)0)
...
#define KFBX_INCOMPATIBLE_WITH_KARRAYTEMPLATE(T)                \
    template<> struct KFbxIncompatibleWithKArrayTemplate<T> {   \
        union {                                                 \
            T t();                                              \
        } catcherr;                                             \
        enum {value = 1};                                       \
