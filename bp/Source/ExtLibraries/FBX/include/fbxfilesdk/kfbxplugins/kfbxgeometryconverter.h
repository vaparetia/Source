...
class KString;
/** 
* This class provides the functionnality to convert geometry nodes 
...
public:
    ...
    * \return                             \c true on success, or \c false if the node attribute is not a mesh, a patch or a nurb.
	* \remarks							  Altought this method is publicly available, its only use in the FBX SDK is internal to some of
...
public:
    ...
    */
    int TriangulatePolygon(KFbxMesh *pNewMesh, KFbxVector4 *pControlPoints, int* pVertexIndices, int * pPolyVertexIndices, int pStartVertexIndex, KFbxVector4& pNormal, int pMaterialIndex, int pPolygonSize, int polygonIndex);
