...
public:
    ...
    const static int LAYERELEMENT_TYPE_TEXTURE_END_INDEX = int(eDISPLACEMENT_TEXTURES);

    //! The count of texture type layer elements. 
    const static int LAYERELEMENT_TYPE_TEXTURE_COUNT = int(eLAST_ELEMENT_TYPE - eDIFFUSE_TEXTURES);
    ...
    const static int LAYERELEMENT_TYPE_NON_TEXTURE_COUNT = int(eDIFFUSE_TEXTURES - eUNDEFINED - 1);
...
/** 
  * \internal Layer element array readlock
  */
template <typename T>
...
/** \brief Layer element for mapping Materials to a geometry.
  * \nosubgrouping
  */
...
/** KFbxLayer class provides a base for the layering mechanism.
  ...
  * 
  * A typical layer for a Mesh contains Normals, UVs and Materials. A typical layer for NURBS contains only Materials. If you apply a texture
  * to a Mesh, a typical layer contains the Textures as well as the UVs. In the case of the NURBS, the NURBS' parameterization is
  * used for the UVs; no UVs should be specified.  The texture mapping is defined completely within the layer; there is no 
  ...
  * Take this into account when you fill the layer. For example, it is legal to define the Layer 0 with the Textures and UVs and then
  ...
  * 
  * You can achieve texture layering when you define more than one layer that contains Textures and UV elements. For example, a Mesh may have only Textures and
  * corresponding UV elements on Layer 0 to create the primary effect, and then another set of Textures and UVs on Layer 1. The way of the texture blending
  * is defined in the Texture layer element.
  *
  * Normally, you can access layer from KFbxLayerContainer like KFbxGeometry.
  * For example, KFbxLayer *lLayer0 = lGeometry->GetLayer(0);
  * \nosubgrouping
