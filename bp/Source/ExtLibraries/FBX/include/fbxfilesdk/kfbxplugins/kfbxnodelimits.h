...
class KFBX_DLL KFbxLimits
{
	public:
		/** Constructor.
		  * Set the default limited property to eTRANSLATION.
		  * If pMBTransform is NULL, create one.
		  * \param pMBTransform The actual object that contains the limit data.
		  *                     KMBTransform defines three members: mRotationSpace, mTLimit and mSLimit.
		  *                     They are the actual limit data when the KFbxLimits object is used for rotation, 
		...
		/** Set the property that is limited
		  * \param pProperty     The limited property: one of eTRANSLATION, eROTATION or eSCALE.
		  */
		...
		/** Get the property that is limited
		  ...
		  */
		inline ELimitedProperty  GetLimitedProperty() const
		{
			return mProperty;
		}
...
* Each transform component limit can be activated or deactivated. All three KFbxLimits 
* objects (mTranslationLimits, mRotationLimits, and mScalingLimits) reference the KMBTransform 
* object (mMBTransform). As a result, a change made with one of the KFbxLimits objects 
* (or the corresponding methods) will affect the respective fields on the other two. 
*
...
class KFBX_DLL KFbxNodeLimits
{
	...
	public:
		/** Constructor.
		  ...
		  * \param pMBTransform     Refer to comments in KFbxLimits.
		  *                         The object contains the actual data for all limits of transform components.
		  */
