...
This sample shows a very simple use of Edge Zlib decompression. It loads a compressed file and passes the compressed data in to
Edge Zlib to be decompressed by an SPU. It then checks that the output uncompressed data agrees with the master file which was
previously used to create the compressed version offline.
...
The program loads the compressed file ("assets/bs_cube.dae.gz") and parses it to remove the gz header and footer. Only the raw
compressed data without any header/footer is wanted for passing into Edge Zlib.
...
<Running the Program>
    - Set fileserving root to the zlib-basic-inflate-sample directory.
