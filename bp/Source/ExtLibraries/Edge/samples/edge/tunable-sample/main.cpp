...
const uint32_t EDGE_RING_OUTPUT_BUFFER_SIZE = 16*1024*1024; // per SPU
...
mSkinningMatrices(NULL)
{
	...
	mCurrentNumElephants = 128;
	mCurrentMaxGrab = 16;
	mCurrentActiveSpus = 1;
	mCurrentRingBufferSize = mCurrentActiveSpus * 1024;
	mMenuEnabled = false;
}
...
bool SampleApp::onUpdate()
{
	...
	bool xPress = mpCross && mpCross->getBoolValue();

	mMenuEnabled = (xPress && !prev) ? !mMenuEnabled : mMenuEnabled;
	...
	if (!mMenuEnabled) {
		...
		bool upPress = mpUp && mpUp->getBoolValue();
		bool downPress = mpDown && mpDown->getBoolValue();
		bool leftPress = mpLeft && mpLeft->getBoolValue();
		bool rightPress = mpRight && mpRight->getBoolValue();
		if (upPress && !upPrev)
			mMenuSelection = (mMenuSelection - 1) % 4;
		if (downPress && !downPrev)
			mMenuSelection = (mMenuSelection + 1) % 4;

		int32_t delta = 0;
		if (leftPress && !leftPrev) {
			delta -= 1;
		}
		if (rightPress && !rightPrev) {
			delta += 1;
		}
		if (delta) {
			switch (mMenuSelection) {
			case 0:
				mCurrentNumElephants += delta;
				mCurrentNumElephants = (mCurrentNumElephants >= 1) ? mCurrentNumElephants : 1;
				mCurrentNumElephants = (mCurrentNumElephants <= 128) ? mCurrentNumElephants : 128;
				break;
			case 1:
				mCurrentActiveSpus += delta;
				mCurrentActiveSpus = (mCurrentActiveSpus >= 1) ? mCurrentActiveSpus : 1;
				mCurrentActiveSpus = (mCurrentActiveSpus <= 5) ? mCurrentActiveSpus : 5;
				break;
			case 2:
				mCurrentMaxGrab += delta;
				mCurrentMaxGrab = (mCurrentMaxGrab >= 1) ? mCurrentMaxGrab : 1;
				mCurrentMaxGrab = (mCurrentMaxGrab <= 16) ? mCurrentMaxGrab : 16;
				break;
			case 3:
				if ( (mCurrentRingBufferSize < 1024) || (mCurrentRingBufferSize == 1024 && delta < 0)) {
					delta *= 128;
				}
				else
					delta *= 1024;

				mCurrentRingBufferSize += delta;
				mCurrentRingBufferSize = (mCurrentRingBufferSize >= 128) ? mCurrentRingBufferSize : 128;
				mCurrentRingBufferSize = (mCurrentRingBufferSize <= 16 * SPURS_SPU_NUM * 1024) ? mCurrentRingBufferSize : 16 * SPURS_SPU_NUM * 1024;
				break;
			}
		}
...
void SampleApp::RenderElephants()
{
	...
	static int frame = 0;
	++frame;
	for (uint32_t iElephant = 0; iElephant < mCurrentNumElephants; iElephant++) 
	{
		Matrix4 mat = Matrix4::rotationZYX(Vector3(0, 0, 0));
		mat.setTranslation(Vector3(5.f * (float)(iElephant/10), 0.0f, -5.0f - 8.0f * (float)(iElephant % 10)));
...
void SampleApp::onRender()
{
...

	{
		...
		FWDebugFont::print("Edge Tunable Sample Application\n");
		FWDebugFont::printf("FPS: %.2f\n\n", fFPS);
	}

	// "Menu"
	{
		FWDebugFont::printf((mMenuEnabled && mMenuSelection == 0) ? "> Elephants: %u\n" : "  Elephants: %u\n", mCurrentNumElephants);
		FWDebugFont::printf((mMenuEnabled && mMenuSelection == 1) ? "> SPUs: %u\n" : "  SPUs: %u\n", mCurrentActiveSpus);
		FWDebugFont::printf((mMenuEnabled && mMenuSelection == 2) ? "> Max Grab: %u\n" : "  Max Grab: %u\n", mCurrentMaxGrab);
		FWDebugFont::printf((mMenuEnabled && mMenuSelection == 3) ? "> Total Ring Buffer Size (KB): %u" : "  Total Ring Buffer Size (KB): %u", mCurrentRingBufferSize);
		FWDebugFont::printf("\tPer Spu (KB): %u\n", (mCurrentRingBufferSize / mCurrentActiveSpus));
	}
}
