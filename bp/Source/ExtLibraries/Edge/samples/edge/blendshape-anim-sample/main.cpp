 ...
 */
void SampleApp::CreateAnimJob(CellSpursJob256* jobAnim, void* outputMatrices, float* outputUserChannels, const struct EdgeAnimBlendBranch* blendBranches, uint32_t numBlendBranches, const struct EdgeAnimBlendLeaf* blendLeaves, uint32_t numBlendLeaves, const EdgeAnimJointTransform* rootJoint)
{
    int ret;
    ...
    jobAnim->header.sizeOut = EDGE_ALIGN(numJointsAligned * 48 + mSkeleton->numUserChannels * sizeof(float), 16U);
	...
	jobAnim->workArea.userData[userDataCount++] = static_cast<uint64_t>(reinterpret_cast<uintptr_t>(outputUserChannels)) << 32;
...
{
    // jobs
    static CellSpursJob256 jobAnim;
    static CellSpursJob256 jobEnd;
    CreateSendEventJob(&jobEnd, &mSpursEventFlag, EVENT_FLAG_BITS);

    // create anim blend leaf
	static EdgeAnimBlendLeaf leaf __attribute__((__aligned__(16)));
	leaf = EdgeAnimBlendLeaf(mAnimation, mAnimation->sizeHeader, mEvalTime);

    // create anim jobs
    CreateAnimJob(&jobAnim, mSkinningMatrices, mShapeAlphas, NULL, 0, &leaf, 1, &mRootJoint);
	// Test the job descriptor for validity -- if this fails, the
	// job will crash when it's executed.
	int ret = cellSpursCheckJob(&jobAnim, sizeof(CellSpursJob256),
    ...
    jobList.eaJobList = (uint64_t)&jobAnim;
    jobList.numJobs = 1;
    jobList.sizeOfJob = sizeof(jobAnim);
    ...
    ret = cellSpursJobChainAttributeInitialize(&jobChainAttributes, 
        command_list_anim, sizeof(jobAnim),
...
void SampleApp::CreateGeomJob(JobGeom256* job, CellGcmContextData *ctx, EdgeGeomPpuConfigInfo *info, void *skinningMatrices)
{
    // embedded SPU ELF symbols
	...
	if(info->blendShapes) {
		job->userData.eaBlendShapeInfo = (uintptr_t)&mShapeInfos[mNumShapeInfos];
		uint64_t numShapes = 0;
		for(uint32_t i = 0; i < info->numBlendShapes; ++i) {
			if(!info->blendShapes[i] || !mShapeAlphas[i])
				continue;
			uint64_t shapeSize = (uint64_t)(info->blendShapeSizes[i]);
			mShapeInfos[mNumShapeInfos].dmaTag = info->blendShapes[i] | (shapeSize << 32);
			mShapeInfos[mNumShapeInfos].alpha = mShapeAlphas[i];
			mNumShapeInfos++;
			numShapes++;
		}
...
void SampleApp::runGeomJobs()
{
    ...
    // init jobs
	// TODO: the skinning matrices aren't actually used since the mesh is not skinned
    int numJobs = 0;
    static JobGeom256 jobsGeom[MAX_SEGMENTS];
	for(uint32_t i = 0; i < mSceneData->numPpuConfigInfos; ++i)
	{
		EdgeGeomPpuConfigInfo *segmentInfo = ((EdgeGeomPpuConfigInfo*)(mSceneData->offsetToPpuConfigInfos)) + i;
		...
		CreateGeomJob(job, &pbContext, segmentInfo, mSkinningMatrices);
...
bool SampleApp::onInit(int argc, char **ppArgv)
{
    ...
    mSceneData = (FileHeader*)loadFile(SYS_APP_HOME"/assets/bs_cube2.edge");
    mColorTex = (uint8_t*)loadFile(SYS_APP_HOME"/assets/logo-color.bin", &mColorTexSize);
    mNormalTex = (uint8_t*)loadFile(SYS_APP_HOME"/assets/logo-normal.bin", &mNormalTexSize);
	...
	mCamera.setPosition(Point3(0, 0, 4.0f));
    ...
    mSkeleton = (EdgeAnimSkeleton*)loadFile(SYS_APP_HOME"/assets/bs_cube2.skel");
    mAnimation = (EdgeAnimAnimation*)loadFile(SYS_APP_HOME"/assets/bs_cube2.anim");

    // load inverse bind matrices
    mInvBindMatrices = loadFile(SYS_APP_HOME"/assets/bs_cube2.edge.invbind");

    // allocate skinning matrices (which are transposed, 3x4)
    mSkinningMatrices = (float*)memalign(16, 48 * mSkeleton->numJoints);
    // init all matrices to identity
    for(int j = 0; j < mSkeleton->numJoints; ++j) {
        Matrix4 m( Matrix4::identity() );
        m = transpose(m);
        memcpy(&mSkinningMatrices[j*12], &m, 48);
    }

    // init root joint
	mRootJoint.rotation = Quat(0,0,0,1);
	mRootJoint.translation = Point3(0, 0, 0);
	mRootJoint.scale = Vector4(1, 1, 1, 1);

	// allocate blend shape/user channel related buffers
	mShapeInfos = (EdgeGeomBlendShapeInfo*)memalign(128, MAX_SEGMENTS*MAX_NUM_BLENDSHAPES*sizeof(EdgeGeomBlendShapeInfo));

	// blend shape factors get updated by dma, so align size to 16 bytes
	mShapeAlphas = (float*)memalign(16, EDGE_ALIGN(MAX_NUM_BLENDSHAPES * sizeof(float), 16));
...
bool SampleApp::onUpdate()
{
    ...
    mEvalTime += deltaTime;
    if (mEvalTime >= mAnimation->duration)
        mEvalTime = 0.0f;
...
void SampleApp::onRender()
{
...

	{
		...
		FWDebugFont::printf("Edge Blend Shape Animation Sample Application\n\n");
		FWDebugFont::printf("FPS: %.2f\n", fFPS);
		FWDebugFont::printf("Blend factor 0: %.2f\n", mShapeAlphas[0]);
		FWDebugFont::printf("Blend factor 1: %.2f\n", mShapeAlphas[1]);
    }
}
