...
void lzo1x_inflate_retry_sample_main( CellSpurs* pSpurs );
...
int main()
{
	...
	lzo1x_inflate_retry_sample_main( &s_spurs );
...
void lzo1x_inflate_retry_sample_main( CellSpurs* pSpurs )
{
	...
	void* pUncompressedSizeAndCompressedBuf;
	const void* pCompressedBuf;
	...
	while ( true )
	{
		...
		pUncompressedSizeAndCompressedBuf = LoadFile( compressedFilename, &compressedFileSize );

		// get #uncompressed bytes
		expectedUncompressedSize = *(const uint32_t*)pUncompressedSizeAndCompressedBuf;
		// get ptr to compressed bytes
		pCompressedBuf = (const void*)((uint32_t)pUncompressedSizeAndCompressedBuf + 4);
		// get compressed size
		...
		if ( numLoadAttempts < numBadReadsBeforeWeGetASuccessfulRead )
		{
			...
			((char*)pCompressedBuf)[0] = 0x7F;
		}
		++numLoadAttempts;

		if ( !( compressedFileSize   <= kEdgeLzo1xInflateInputBufferMaxSize ) )
		{
			goto InvalidData;
		}

		if ( ! ( expectedUncompressedSize <= kEdgeLzo1xInflateOutputBufferMaxSize ) )
		{
			goto InvalidData;
		}
		...
		edgeLzo1xAddInflateQueueElement(		inflateQueue,
												pCompressedBuf, compressedFileSize,
		...
		printf( "PPU: Edge Lzo1x event acknowledged\n" );
...
InvalidData:
		...
		pUncompressedSizeAndCompressedBuf = NULL;
	...
	// since the first 4 bytes of compressed data stored a size,
	// reflect that in the compressed size for printing stats
	compressedFileSize += 4;
