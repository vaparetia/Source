...
void edgePostMain( EdgePostTileInfo* tileInfo )
{
	...
	// inplace convert color buffer from argb8888 to floating points
	edgePostArgb8ToFloats( src_address0 - back_offset, src_address0 - back_offset, numsrcpixels * numsrcscanline);
	...
	// run motionblur 
	for ( uint32_t j = 0; j < numscaline; ++j, dst_address += ostride, src_address0 += istride4, src_address1 += imotionstride )
		edgePostMotionblur( dst_address, src_address0, src_address1, istride4, numpixels );
}
