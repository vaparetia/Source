...
void SampleApp::CreateGeomJob(JobGeom256* job, CellGcmContextData *ctx, EdgeGeomPpuConfigInfo *info, 
			void *skinningMatrices, uint32_t elephantIndex) const
{
	// Steam descriptor
...
void SampleApp::initRenderTargets()
{
	uint32_t width = 1280;
	uint32_t height = 720;
	...
	uint32_t memSize = ((frameSize * kNumFramebuffers) + 0xffff) & ~0xffff;
	printf( "Allocating color buffers in VRAM: %d bytes\n", memSize );

	// allocate memory
	uint8_t* tiledMrtArea = (uint8_t*)cellGcmUtilAllocateLocalMemory( memSize, 0x10000);

	// setup gcm surface structure
	mRenderTarget.type = CELL_GCM_SURFACE_PITCH;
	mRenderTarget.antialias = CELL_GCM_SURFACE_CENTER_1;
	mRenderTarget.colorFormat	= CELL_GCM_SURFACE_A8R8G8B8;
	mRenderTarget.colorTarget = CELL_GCM_SURFACE_TARGET_0;
	...
	// 0 is color
	cellGcmAddressToOffset( tiledMrtArea, &mColorOffsets[0]);
	cellGcmAddressToOffset( tiledMrtArea + frameSize, &mColorOffsets[1]);
	mRenderTarget.colorPitch[0] = pitch;
	mRenderTarget.colorLocation[0] = CELL_GCM_LOCATION_LOCAL;
	mRenderTarget.colorOffset[0] = 0xffffffff;		// will be double-buffered

	// All other unused
	for ( int j = 1; j < 4; ++j )
	{
		mRenderTarget.colorLocation[j] = CELL_GCM_LOCATION_LOCAL;
		mRenderTarget.colorOffset[j] = 0;
		mRenderTarget.colorPitch[j] = 64;
	}

	// Depth buffer
	FWCellGCMWindow* pWindow = (FWCellGCMWindow*)FWWindow::getWindow();
	mRenderTarget.depthFormat = CELL_GCM_SURFACE_Z24S8;
	mRenderTarget.depthLocation = CELL_GCM_LOCATION_LOCAL;
	mRenderTarget.depthOffset	= pWindow->getFrameOffset(2);
	mRenderTarget.depthPitch = pWindow->getFramePitch(2);

	// set Tile info
	cellGcmSetTileInfo( 4, CELL_GCM_LOCATION_LOCAL, mColorOffsets[0], memSize, pitch, CELL_GCM_COMPMODE_DISABLED, 0, 0);
	cellGcmBindTile( 4);
}
...
bool SampleApp::onInit(int argc, char **ppArgv)
{
	...
	// Init textures, shaders and render targets
	initTextures();
	initShaders();
	initRenderTargets();
...
bool SampleApp::onUpdate()
{
	...
	case 4: mPostParams.m_middleGrey += delta; break;
	case 5: mPostParams.m_whitePoint += delta; break;
	}
...
void SampleApp::drawSkybox()
{
	...
	Matrix4 mvp = transpose( getProjectionMatrix() * skyMat);
...
void SampleApp::RenderTweakables()
{
	...
	SetTweakColor(4); FWDebugFont::printf("Middlegrey                : %f\n", mPostParams.m_middleGrey);
	SetTweakColor(5); FWDebugFont::printf("White point               : %f\n", mPostParams.m_whitePoint);
...
void SampleApp::onRender()
{
	...
	mRenderTarget.colorOffset[0] = mColorOffsets[frameIndex];

	// setup MRT
	cellGcmSetSurfaceWindow(&mRenderTarget, CELL_GCM_WINDOW_ORIGIN_BOTTOM, CELL_GCM_WINDOW_PIXEL_CENTER_HALF);
	...
	for (uint32_t iElephant = 0; iElephant < numElephants; iElephant++) 
	{
		...
		for(uint32_t i = 0; i < sizeof(allSegments) / sizeof(allSegments[0]); ++i)
		{
			...
			void* matrixPalette = &s_skinningMatrices[kNumBones*12*(frame%kNumFrames)];
			CreateGeomJob(job, &pbContext, allSegments[i], matrixPalette, iElephant);
...

	{
		...
		FWDebugFont::print("Edge Post Sample Bloom Application\n\n");
