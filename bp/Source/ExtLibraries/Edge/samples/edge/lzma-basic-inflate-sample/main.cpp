...
static void lzma_basic_inflate_sample_main( CellSpurs* pSpurs );
...
int main()
{
	...
	lzma_basic_inflate_sample_main( &s_spurs );
...
void lzma_basic_inflate_sample_main( CellSpurs* pSpurs )
{
	...
	assert( compressedFileSize <= kEdgeLzmaInflateInputBufferMaxSize );
	...
	SplitLzmaDataIntoComponents( pCompressedBuff, compressedFileSize,
								&pProperties, &propertiesSize,
								&uncompressedSize64,
								&pCompressedStream, &compressedStreamSize );

	assert( uncompressedSize64 <= kEdgeLzmaInflateOutputBufferMaxSize );

	uint32_t expectedUncompressedSize = (uint32_t)uncompressedSize64;
	...
	//////////////////////////////////////////////////////////////////////////
	//
	//	Add one item to the Inflate Queue. We need to pass in the "properties"
	//	and the compressed raw data.
