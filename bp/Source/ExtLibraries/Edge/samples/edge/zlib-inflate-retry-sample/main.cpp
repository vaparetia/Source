...
void zlib_inflate_retry_sample_main( CellSpurs* pSpurs );
...
int main()
{
	...
	zlib_inflate_retry_sample_main( &s_spurs );
...
void zlib_inflate_retry_sample_main( CellSpurs* pSpurs )
{
	...
	while ( true )
	{
		...
		const void* pRawData				= GzHeaderFindRawData( pCompressedBuf );
		if( NULL == pRawData )
		{
			goto InvalidData;
		}
		...
		rawDataSize				= GzHeaderGetRawDataSize( pCompressedBuf, compressedFileSize );
		expectedUncompressedSize	= GzHeaderGetUncompressedSize( pCompressedBuf, compressedFileSize );
