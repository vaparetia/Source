...
uint8_t g_DebugFlags = 0;

/*
 * Sample application for morphological AA.
 ...
 * selected texture down to XDR, have the MLAA task process it and then have RSX copy it into the back buffer.
 *
 * So see the integration of the MLAA task, opens spupostprocessing.cpp. The actual task and algorithm are found in
 * target/spu/src/edge/post-task-mlaa/
 * target/spu/src/edge/post/mlaa/
 *
 * USAGE:
 * -The program will go throught the APP_HOME/assets directory and load all 720p textures. Other texture will be ignored.
 * -You can switch between the active textures using the left/right buttons on the d-pad.
 * -MLAA uses a threshold for edge detection. You can increase/decrease this using the up/down bottons on the d-pad
 * -Two edge detection algorithms are provided, which can be toggled using CIRCLE
 * -To visualize the edges found, press SQUARE
 * -MLAA can be toggled on and off useing TRIANGLE
 */
...
SampleApp::SampleApp() :
	mMlaaEnabled(true),
	mMlaaShowEdges(false),
	mMlaaRelativeEdgeDetection(true),
	mMlaaThresholdBase(0x0a), // these are pretty good threshold values, but you might find better ones...
	mMlaaThresholdFactor(0x59), 
	mMlaaAbsoluteThreshold(0x20),
	mTextureWidth(1280),
	mTextureHeight(720),
	mCurrentTexture(0)
{
...
bool SampleApp::initTexture( CellGcmTexture& texture, const char* textureName)
{
	...
	if( (0 != ret) || 
		(mTextureWidth != attribute.tex.width) ||
		(mTextureHeight != attribute.tex.height) ||
...
void SampleApp::initTextures()
{
	...
	{
		if(
			entry.d_namlen>5 && /*(0 == textures_found) &&*/
...
bool SampleApp::onInit(int argc, char **ppArgv)
{
	...
	InitSpuPostprocessing(mTextureWidth, mTextureHeight);
	return true;
}
...
bool SampleApp::onUpdate()
{
	...
	if ( delay == 0)
	{
		...
		{
			mMlaaEnabled = (mMlaaEnabled+1)%2;
		}

		if ( mpSquare->getBoolTrue() )
		{
			mMlaaShowEdges = !mMlaaShowEdges;
		}

		if ( mpCircle->getBoolTrue() )
		{
			mMlaaRelativeEdgeDetection = !mMlaaRelativeEdgeDetection;
			printf("%s\n",mMlaaRelativeEdgeDetection?"relative edge detection":"absolute edge detection");
		}

		if ( mpCross->getBoolTrue() && mMlaaRelativeEdgeDetection)
		{
			g_DebugFlags ^= 4;
			mMlaaThresholdFactor=(g_DebugFlags&4)?0x84:0x59;
			printf("threshold scale: 0x%02x\n",mMlaaThresholdFactor);
		}
		...
		if(mMlaaRelativeEdgeDetection)
		{
			if ( mpUp->getBoolTrue() && mMlaaThresholdFactor<0xFF )
			{
				++mMlaaThresholdFactor;
				printf("threshold scale: 0x%02x\n",mMlaaThresholdFactor);
			}

			if ( mpDown->getBoolTrue() && mMlaaThresholdFactor>0 )
			{
				--mMlaaThresholdFactor;
				printf("threshold scale: 0x%02x\n",mMlaaThresholdFactor);
			}
		...
		{
			if ( mpUp->getBoolTrue() && mMlaaAbsoluteThreshold<0xFF )
			{
				++mMlaaAbsoluteThreshold;
				printf("absolute threshold: 0x%02x\n",mMlaaAbsoluteThreshold);
			}

			if ( mpDown->getBoolTrue() && mMlaaAbsoluteThreshold>0 )
			{
				--mMlaaAbsoluteThreshold;
				printf("absolute threshold: 0x%02x\n",mMlaaAbsoluteThreshold);
			}
...
void SampleApp::onRender()
{
	...
	cellGcmSetClearColor(0x000000FF);
