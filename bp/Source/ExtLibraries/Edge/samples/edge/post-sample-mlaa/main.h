...
private:
	...
	void	ShutDownSpuProcessing();
	void	StartSpuPostprocessing();
	void	SwapSpuPostprocessBuffers();

	// spu stages
	void	SetupMlaaTask(uint8_t* destEa, uint8_t* srcEa, uint32_t imagePitch, uint32_t imageHeight, uint32_t transposeHeight, uint32_t mode);
...
private:
	...
	bool						mMlaaEnabled;
	bool						mMlaaShowEdges;
	bool						mMlaaRelativeEdgeDetection;

	int32_t						mMlaaThresholdBase;
	int32_t						mMlaaThresholdFactor;
	int32_t						mMlaaAbsoluteThreshold;
	...
	// images
	const uint32_t				mTextureWidth;
	const uint32_t				mTextureHeight;
	...
	void*						mPostprocessArena;	// postprocessing work area (just MLAA in our case)
