...
static uint8_t s_segmentTransferBuffer[kEdgeZlibInflateInputBufferMaxSize + kEdgeSegCompSampleOverAllocateBufferSize] __attribute__((aligned(128)));
...
void MoveBuffersAndAddZlibSegsFileItemsToInflateQueue(
...
{
	TestValidZlibSegsFile( pSegsHeader, NULL, segsFileSize );	//Check this is a legitimate file header before we do anything else
	...
	{
		EDGE_PRINTF( "EDGE ZLIB ERROR: Segs file will decompress to bigger than output buffer size (%d > %d)\n", totalUncompressedSize, outputBuffMaxSize );
		EDGE_ASSERT( false );
	}
	...
	//	AddZlibSegsFileItemsToInflateQueue instead.
	//
	//////////////////////////////////////////////////////////////////////////
	...
	if ( ( eaSegsFileDataEnd <= eaOutputUncompBuff ) || ( eaSegsFileData >= eaOutputUncompBuffEnd ) )
	{
		...
		//So just call AddZlibSegsFileItemsToInflateQueue to do this.
		AddZlibSegsFileItemsToInflateQueue(	pSegsHeader,
											eaSegsFileData,
											segsFileSize,
											eaInflateQueue,
											eaOutputUncompBuff,
	...
	for ( int segNo = numSegments-1; segNo >= 0; --segNo )
	{
		...
		if ( isStoredCompressed )
		{
			...
			edgeZlibAddInflateQueueElement(	eaInflateQueue,
	...
	if ( numNonCompressedSegs > 0 )
	{
		...
		if ( eaNumElementsToDecompress )
		{
			...
			{
				EDGE_PRINTF( "EDGE ZLIB ERROR: Attempted to decrement a work-to-do counter by more than counters current value (%d > %d)\n", numNonCompressedSegs, old );
				EDGE_ASSERT( false );
			}
...
void AddZlibSegsFileItemsToInflateQueue(
			const EdgeSegCompSampleFileHeader* pSegsHeader,	//Pointer to where the header and TOC have been pre-loaded into LS
			uint32_t eaSegsFileData,						// Effective Address of segs file data, 16 aligned
			uint32_t segsFileSize,							// size of the input segs file
			uint32_t eaInflateQueue,						// Effective Address of the Inflate Queue
...
void TestValidZlibSegsFile( const EdgeSegCompSampleFileHeader* pSegsHeader, const char* segFileName, uint32_t segsFileSize )
{
	EDGE_ASSERT( (((uint32_t)pSegsHeader) & 0xF) == 0 );

	if ( EdgeSegCompSampleFileHeader::kMagic != pSegsHeader->m_magic )
	{
		EDGE_PRINTF( "EDGE ZLIB ERROR: Invalid segs file\n" );
		EDGE_ASSERT( false );
	}
						...
						register uint32_t dmaTagId )
{
	...
	//	more than kEdgeZlibInflateInputBufferMaxSize.
	//
	//////////////////////////////////////////////////////////////////////////

	EDGE_ASSERT( size <= kEdgeZlibInflateInputBufferMaxSize );
	...
	_edgeZlibUnalignedLargeDmaPut( pLsBufferDest, eaDest, size, dmaTagId );
	spu_idisable();
	EDGE_DMA_WAIT_TAG_STATUS_ALL( 1 << dmaTagId );
	if ( interruptsEnabled )
		spu_ienable();
