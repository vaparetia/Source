...
#include "file.h"
#include "lzma/edgelzma_compressor.h"
...
int main( int argc, char* argv[] )
{
	...
	{
		printf( "Usage: lzma-segcomp-sample <input filename> [-o <output filename>] [-<level>] [-seg-size=<size>]\n" );
...
int ConvertFile( const char* outputFileName, const char* masterFilename, uint32_t level, int maxSegmentSize )
{
	...
	pHeader->m_decompressionType			= EdgeSegCompSampleFileHeader::kDecompressionType_EdgeLzma;
	...
	//space than we'll need. Add 5% to the master file size.
	uint32_t tempCompBufSize = ((masterFileSize * 21) / 20);
	...
	for ( uint32_t segNo = 0; segNo < numSegments; ++segNo )
	{
		...
		// execute lzma decompression, with 2 byte header and 4 byte footer removed
		uint32_t ret = lzmaCompress(	&pTempCompBuf[tempCompBufOffset],
										(uint32_t*)&compSegRawSize, // returns compressed size here
										(const uint8_t*)pSegmentData,
										uncompSegSize,
										level );
		if( ret != 0 )
		{
			printf( "ERROR: Failed lzma compression, segment=%d, ret=%d\n", (unsigned int)segNo, (unsigned int)ret );
			free( pHeader );
			free( pTempCompBuf );
			FreeFile( pMasterData );
			return 1;
