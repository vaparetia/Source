...
PPU_SRCS		=	main.cpp $(VPSHADER_PPU_OBJS) $(FPSHADER_PPU_OBJS) \
					spu/$(OBJS_DIR)/job_send_event.spu.elf spu/$(OBJS_DIR)/job_geom.spu.elf 
PPU_TARGET		=	runtime-partitioning-sample.$(EDGE_BUILD).ppu.elf

PPU_INCDIRS		+=	-I$(CELL_FW_DIR)/include \
					-I$(GCMCOMMONDIR) \
					-I$(EDGE_TARGET_ROOT)/../host-common/include

PPU_LIBS		+=	$(CELL_FW_DIR)/libfwgcm.a \
					$(GCMCOMMONDIR)/gcmutil.a \
					$(EDGE_SAMPLE_COMMON_DIR)/libedgesamplecommon$(EDGE_BUILD_SUFFIX).a \
					$(EDGE_TARGET_ROOT)/ppu/lib/libedgegeomtool$(EDGE_BUILD_SUFFIX).a
