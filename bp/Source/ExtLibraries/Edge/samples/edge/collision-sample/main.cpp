...
extern char _binary_job_job_intersect_spu_bin_start[];
extern char _binary_job_job_intersect_spu_bin_size[];
...
static Vector4 lookAt;
static Point3 position;
static DebugVert *vertexData = NULL;
static uint16_t *indexData = NULL;
static RayResult rayResults[2] __attribute__((__aligned__(128)));
...
void SampleApp::CreateCollisionJob(JobIntersect256* job, EdgeGeomPpuConfigInfo *info, void *skinningMatrices, uint32_t seg) const
{
	// Steam descriptor
	job->inputDmaList[kJobIntersectDmaOutputStreamDesc].eal  = (uintptr_t)info->spuOutputStreamDesc;
	job->inputDmaList[kJobIntersectDmaOutputStreamDesc].size = info->spuOutputStreamDescSize;

	// Indexes
	job->inputDmaList[kJobIntersectDmaIndexes_0].eal  = (uintptr_t)info->indexes;
	job->inputDmaList[kJobIntersectDmaIndexes_0].size = info->indexesSizes[0];
	job->inputDmaList[kJobIntersectDmaIndexes_1].eal  = job->inputDmaList[kJobIntersectDmaIndexes_0].eal + job->inputDmaList[kJobIntersectDmaIndexes_0].size;
	job->inputDmaList[kJobIntersectDmaIndexes_1].size = info->indexesSizes[1];
	
	// Skinning Matrices
	job->inputDmaList[kJobIntersectDmaSkinMatrices_0].eal  = info->skinMatricesByteOffsets[0] + (uintptr_t)skinningMatrices;
	job->inputDmaList[kJobIntersectDmaSkinMatrices_0].size = info->skinMatricesSizes[0];
	job->inputDmaList[kJobIntersectDmaSkinMatrices_1].eal  = info->skinMatricesByteOffsets[1] + (uintptr_t)skinningMatrices;
	job->inputDmaList[kJobIntersectDmaSkinMatrices_1].size = info->skinMatricesSizes[1];

	// Skinning Indexes & Weights
	job->inputDmaList[kJobIntersectDmaSkinIndexesAndWeights_0].eal  = (uintptr_t)info->skinIndexesAndWeights;
	job->inputDmaList[kJobIntersectDmaSkinIndexesAndWeights_0].size = info->skinIndexesAndWeightsSizes[0]; 
	job->inputDmaList[kJobIntersectDmaSkinIndexesAndWeights_1].eal  = job->inputDmaList[kJobIntersectDmaSkinIndexesAndWeights_0].eal + job->inputDmaList[kJobIntersectDmaSkinIndexesAndWeights_0].size;
	job->inputDmaList[kJobIntersectDmaSkinIndexesAndWeights_1].size = info->skinIndexesAndWeightsSizes[1];

	// Vertexes (stream 1)
	job->inputDmaList[kJobIntersectDmaVertexes1_0].eal  = (uintptr_t)info->spuVertexes[0];
	job->inputDmaList[kJobIntersectDmaVertexes1_0].size = info->spuVertexesSizes[0];
	job->inputDmaList[kJobIntersectDmaVertexes1_1].eal  = job->inputDmaList[kJobIntersectDmaVertexes1_0].eal + job->inputDmaList[kJobIntersectDmaVertexes1_0].size;
	job->inputDmaList[kJobIntersectDmaVertexes1_1].size = info->spuVertexesSizes[1];
	job->inputDmaList[kJobIntersectDmaVertexes1_2].eal  = job->inputDmaList[kJobIntersectDmaVertexes1_1].eal + job->inputDmaList[kJobIntersectDmaVertexes1_1].size;
	job->inputDmaList[kJobIntersectDmaVertexes1_2].size = info->spuVertexesSizes[2];

	// Vertexes (stream 2)
	job->inputDmaList[kJobIntersectDmaVertexes2_0].eal  = (uintptr_t)info->spuVertexes[1];
	job->inputDmaList[kJobIntersectDmaVertexes2_0].size = info->spuVertexesSizes[3];
	job->inputDmaList[kJobIntersectDmaVertexes2_1].eal  = job->inputDmaList[kJobIntersectDmaVertexes2_0].eal + job->inputDmaList[kJobIntersectDmaVertexes2_0].size;
	job->inputDmaList[kJobIntersectDmaVertexes2_1].size = info->spuVertexesSizes[4];
	job->inputDmaList[kJobIntersectDmaVertexes2_2].eal  = job->inputDmaList[kJobIntersectDmaVertexes2_1].eal + job->inputDmaList[kJobIntersectDmaVertexes2_1].size;
	job->inputDmaList[kJobIntersectDmaVertexes2_2].size = info->spuVertexesSizes[5];

	// Triangle Culling Data
	job->inputDmaList[kJobIntersectDmaViewportInfo].eal  = (uintptr_t)&mViewportInfo;
	job->inputDmaList[kJobIntersectDmaViewportInfo].size = sizeof(EdgeGeomViewportInfo);
	job->inputDmaList[kJobIntersectDmaLocalToWorldMatrix].eal  = (uintptr_t)&mLocalToWorldMatrix;
	job->inputDmaList[kJobIntersectDmaLocalToWorldMatrix].size = sizeof(EdgeGeomLocalToWorldMatrix);
	
	// SpuConfigInfo
	job->inputDmaList[kJobIntersectDmaSpuConfigInfo].eal	 = (uintptr_t)info;
	job->inputDmaList[kJobIntersectDmaSpuConfigInfo].size = sizeof(EdgeGeomSpuConfigInfo);
	
	// Fixed Point Attribute Offsets
	job->inputDmaList[kJobIntersectDmaFixedPointOffsets1].eal  = (uintptr_t)info->fixedOffsets[0];	 
	job->inputDmaList[kJobIntersectDmaFixedPointOffsets1].size = info->fixedOffsetsSize[0];
	job->inputDmaList[kJobIntersectDmaFixedPointOffsets2].eal  =	(uintptr_t)info->fixedOffsets[1];
	job->inputDmaList[kJobIntersectDmaFixedPointOffsets2].size = info->fixedOffsetsSize[1];

	// Input Streams Descriptors
	job->inputDmaList[kJobIntersectDmaInputStreamDesc1].eal  = (uintptr_t)info->spuInputStreamDescs[0];
	job->inputDmaList[kJobIntersectDmaInputStreamDesc1].size = info->spuInputStreamDescSizes[0];
	job->inputDmaList[kJobIntersectDmaInputStreamDesc2].eal  = (uintptr_t)info->spuInputStreamDescs[1];
	job->inputDmaList[kJobIntersectDmaInputStreamDesc2].size = info->spuInputStreamDescSizes[1];

	// Ray description
	job->inputDmaList[kJobIntersectDmaRay].eal = (uintptr_t)ray;
	job->inputDmaList[kJobIntersectDmaRay].size = sizeof(ray);
	...
	// Ray results and segment index
	job->userData.eaRayResults = (uintptr_t)(rayResults);
	job->userData.segmentIndex = seg;

	job->header.eaBinary = (uintptr_t)_binary_job_job_intersect_spu_bin_start;
	job->header.sizeBinary = CELL_SPURS_GET_SIZE_BINARY(_binary_job_job_intersect_spu_bin_size);
	job->header.sizeDmaList = kJobIntersectInputDmaEnd*8;
	job->header.eaHighInput = 0;
	job->header.useInOutBuffer = 1;
	job->header.sizeInOrInOut = info->ioBufferSize;
	//job->header.sizeOut = 0;
...
bool SampleApp::onInit(int argc, char **ppArgv)
{
	...
	// Collision-specific data buffers
	indexData = (uint16_t *)memalign(1024*1024, 1024*1024);
	vertexData = (DebugVert *)memalign(1024*1024, 1024*1024);
	cellGcmMapMainMemory(indexData, 1024*1024, &dummy);
	cellGcmMapMainMemory(vertexData, 1024*1024, &dummy);
...
void SampleApp::onRender()
{
	...
	Matrix4 invV = inverse(getViewMatrix());
	...
	rayResults[0].t = 10000.f;
	rayResults[0].uid = 0xffffffff;

	for(uint32_t i = 0; i < sizeof(allSegments) / sizeof(allSegments[0]); ++i)
	{
		// Build the collision job
		JobIntersect256 *collisionJob = (JobIntersect256*) &jobs[numSegments++];
		CreateCollisionJob(collisionJob, allSegments[i], &mSkinningMatrices[kNumBones*12*(frame%kNumFrames)], i);

		int ret = cellSpursCheckJob((CellSpursJob256*)collisionJob, sizeof(JobIntersect256),
			sizeof(CellSpursJob256));
		if ( ret != CELL_OK){
			printf("cellSpursCheckJob failed : 0x%x\n", ret);
		}

		// Bind the RSX-only vertex attributes for this geometry job before it's created
		...
		JobGeom256 *geomJob = (JobGeom256*) &jobs[numSegments++];
		CreateGeomJob(geomJob, &pbContext, allSegments[i], &mSkinningMatrices[kNumBones*12*(frame%kNumFrames)]);

		ret = cellSpursCheckJob((CellSpursJob256*)geomJob, sizeof(JobGeom256),
			sizeof(CellSpursJob256));
		if ( ret != CELL_OK){
			printf("cellSpursCheckJob failed : 0x%x\n", ret);
		}
	...
	assert(sizeof(JobIntersect256)==sizeof(jobs[0]));
	...
	// Render a visual indication of the intersection point's normal
	vertexData[0].m_pos[0] = rayResults[0].intersection[0];
	vertexData[0].m_pos[1] = rayResults[0].intersection[1];
	vertexData[0].m_pos[2] = rayResults[0].intersection[2];
	vertexData[0].m_norm[0] = 0.f;
	vertexData[0].m_norm[1] = 1.f;
	vertexData[0].m_norm[2] = 0.f;
	vertexData[0].m_tan[0] = 0.f;
	vertexData[0].m_tan[1] = 0.f;
	vertexData[0].m_tan[2] = 1.f;
	vertexData[0].m_tan[3] = 0.f;
	vertexData[0].m_tex[0] = 0.f;
	vertexData[0].m_tex[1] = 0.f;

	vertexData[1].m_pos[0] = rayResults[0].intersection[0] + 2.f * rayResults[0].normal[0];
	vertexData[1].m_pos[1] = rayResults[0].intersection[1] + 2.f * rayResults[0].normal[1];
	vertexData[1].m_pos[2] = rayResults[0].intersection[2] + 2.f * rayResults[0].normal[2];
	vertexData[1].m_norm[0] = 0.f;
	vertexData[1].m_norm[1] = 1.f;
	vertexData[1].m_norm[2] = 0.f;
	vertexData[1].m_tan[0] = 0.f;
	vertexData[1].m_tan[1] = 0.f;
	vertexData[1].m_tan[2] = 1.f;
	vertexData[1].m_tan[3] = 0.f;
	vertexData[1].m_tex[0] = 0.f;
	vertexData[1].m_tex[1] = 0.f;

	indexData[0] = 0;
	indexData[1] = 1;

	float mvp_float[16];
	memcpy(mvp_float, &mvp, 64);

	cellGcmSetVertexProgramConstants(0, 16, mvp_float);
	uint32_t vertexesOffset, indexesOffset;
	cellGcmAddressToOffset(vertexData, &vertexesOffset);
	cellGcmAddressToOffset(indexData, &indexesOffset);
	cellGcmSetVertexDataArray(0, 0, sizeof(DebugVert), 3, CELL_GCM_VERTEX_F, CELL_GCM_LOCATION_MAIN, vertexesOffset + 0);
	cellGcmSetVertexDataArray(2, 0, sizeof(DebugVert), 3, CELL_GCM_VERTEX_F, CELL_GCM_LOCATION_MAIN, vertexesOffset + 12);
	cellGcmSetVertexDataArray(6, 0, sizeof(DebugVert), 4, CELL_GCM_VERTEX_F, CELL_GCM_LOCATION_MAIN, vertexesOffset + 24);
	cellGcmSetVertexDataArray(8, 0, sizeof(DebugVert), 2, CELL_GCM_VERTEX_F, CELL_GCM_LOCATION_MAIN, vertexesOffset + 40);
	cellGcmSetDrawIndexArray(CELL_GCM_PRIMITIVE_LINES, 2, CELL_GCM_DRAW_INDEX_ARRAY_TYPE_16, CELL_GCM_LOCATION_MAIN, indexesOffset);
...

	{
		...
		FWDebugFont::print("Edge Collision Sample Application\n\n");
		FWDebugFont::printf("FPS: %.2f\n", fFPS);

		FWDebugFont::setPosition(596, 314);
		FWDebugFont::setColor(1.f, 0.f, 0.f, 1.0f);

		FWDebugFont::printf("+", fFPS);
	}
}
