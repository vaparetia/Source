...
void SampleApp::InitVRamBuffers( uint32_t width, uint32_t height )
{
	...
	mOriginalTexture.pitch = mMrtTarget.colorPitch[0];
...
void SampleApp::Tonemap( const CellGcmTexture& original, const CellGcmTexture& accumulator, const CellGcmTexture& mask )
{
	// set tonemap parameters
	CGparameter tonemapParameters = cellGcmCgGetNamedParameter( mTonemapCompositeFp.mCgProgram, "parameters" );
	float v[] = { mAvgLuminance, mPostParams.m_whitePoint, mPostParams.m_middleGrey, .0f };
	...
	cellGcmSetTextureFilter( 2, 0, CELL_GCM_TEXTURE_LINEAR, CELL_GCM_TEXTURE_LINEAR, CELL_GCM_TEXTURE_CONVOLUTION_QUINCUNX );
	cellGcmSetTextureControl( 0, CELL_GCM_TRUE, 0, 0, 0);
	cellGcmSetTextureControl( 1, CELL_GCM_TRUE, 0, 0, 0);
	cellGcmSetTextureControl( 2, CELL_GCM_TRUE, 0, 0, 0);
	...
	cellGcmSetTextureAddress( 2, CELL_GCM_TEXTURE_CLAMP_TO_EDGE, CELL_GCM_TEXTURE_CLAMP_TO_EDGE, CELL_GCM_TEXTURE_CLAMP_TO_EDGE, CELL_GCM_TEXTURE_UNSIGNED_REMAP_NORMAL, CELL_GCM_TEXTURE_ZFUNC_LESS, 0);
	cellGcmSetTexture( 0, &original );
	cellGcmSetTexture( 1, &accumulator );
	cellGcmSetTexture( 2, &mask );
...
void SampleApp::DepthDownsample( const CellGcmSurface& src, const CellGcmSurface& dst )
{
