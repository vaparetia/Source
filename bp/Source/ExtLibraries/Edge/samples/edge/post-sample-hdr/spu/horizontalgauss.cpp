...
extern "C"
void Filter7x1_fx16_fx16( void* output, const void* input, uint32_t stride, const vec_float4* weights, uint32_t count );
...
void edgePostMain( EdgePostTileInfo* tileInfo )
{
	...
	case POST_GAUSS_FX16:
		...
		for ( uint32_t j = 0; j < numscaline; ++j, dst_address += ostride * 2, src_address += istride * 2 )
			Filter7x1_fx16_fx16( dst_address, src_address, istride * 2, weights_splats, numpixels );
