...
void SampleApp::initShader( Shader& shader, void* pData )
{
	...
	// move it into vram if fragment shader
...
{
#ifdef SN_TARGET_PS3
#	define SHADER( name ) _binary_##name##_start
#else
#	define SHADER( name ) _binary_shaders_##name##_start
#endif

	// scene shaders
	extern char SHADER( vpshader_vpo)[];
	extern char SHADER( fpshader_fpo)[];
	extern char SHADER( vpsky_vpo)[];
	extern char SHADER( fpsky_fpo)[];
	initShader( mSkyVp, SHADER( vpsky_vpo) );
	initShader( mSkyFp, SHADER( fpsky_fpo) );
	initShader( mSceneVp, SHADER( vpshader_vpo) );
	initShader( mSceneFp, SHADER( fpshader_fpo) );

	// post shaders
	extern char SHADER( postvp_vpo)[];
	extern char SHADER( post_depthdownsamplefp_fpo)[];
	extern char SHADER( tonemap_compositefp_fpo)[];
	extern char SHADER( downsample_luvfp_fpo)[];
	initShader( mResampleVp, SHADER( postvp_vpo) );
	initShader( mDepthdownsampleFp, SHADER( post_depthdownsamplefp_fpo) );
	initShader( mTonemapCompositeFp, SHADER( tonemap_compositefp_fpo) );
	initShader( mLuvDownsampleFp, SHADER( downsample_luvfp_fpo) );
}
...
bool SampleApp::onInit(int argc, char **ppArgv)
{
	...
	InitVRamBuffers( mDispInfo.mWidth, mDispInfo.mHeight );
...
bool SampleApp::onUpdate()
{
	...
	case 10: mPostParams.m_middleGrey += delta; break;
	case 11: mPostParams.m_whitePoint += delta; break;
	case 12: mPostParams.m_ilrStrenght += delta; break;
	}
...
void SampleApp::drawSkybox( const Matrix4& mvp, const Matrix4& pmvp, const Matrix4& mv )
{
	...
	cellGcmSetTextureAddress(0,	CELL_GCM_TEXTURE_CLAMP_TO_EDGE, CELL_GCM_TEXTURE_CLAMP_TO_EDGE, CELL_GCM_TEXTURE_CLAMP_TO_EDGE, 
		CELL_GCM_TEXTURE_UNSIGNED_REMAP_NORMAL, CELL_GCM_TEXTURE_ZFUNC_LESS, CELL_GCM_TEXTURE_GAMMA_R | CELL_GCM_TEXTURE_GAMMA_G | CELL_GCM_TEXTURE_GAMMA_B);
...
void SampleApp::renderGround( const Matrix4& oldViewProj )
{
	...
	cellGcmSetTextureAddress( 0, CELL_GCM_TEXTURE_WRAP, CELL_GCM_TEXTURE_WRAP, CELL_GCM_TEXTURE_CLAMP_TO_EDGE, 
		CELL_GCM_TEXTURE_UNSIGNED_REMAP_NORMAL, CELL_GCM_TEXTURE_ZFUNC_LESS, CELL_GCM_TEXTURE_GAMMA_R | CELL_GCM_TEXTURE_GAMMA_G | CELL_GCM_TEXTURE_GAMMA_B);
	cellGcmSetTextureFilter( 0, 0, CELL_GCM_TEXTURE_LINEAR, CELL_GCM_TEXTURE_LINEAR, CELL_GCM_TEXTURE_CONVOLUTION_QUINCUNX);
...
void SampleApp::RenderTweakables()
{
	...
	SetTweakColor(10); FWDebugFont::printf("Middlegrey                : %f\n", mPostParams.m_middleGrey);
	SetTweakColor(11); FWDebugFont::printf("White point               : %f\n", mPostParams.m_whitePoint);
	SetTweakColor(12); FWDebugFont::printf("Ilr strenght              : %f\n", mPostParams.m_ilrStrenght);

	FWDebugFont::setPosition( 1, 400);
	FWDebugFont::setColor( .8f, .8f, .8f, 1.0f);
	FWDebugFont::printf("Average Luminance  : %f\n", mAvgLuminance);
	FWDebugFont::printf("Max Luminance      : %f\n", mMaxLuminance);
...
void SampleApp::onRender()
{
	...
	// kick rsx
    ProfileBars::AddPpuMarker(0x00ff0000);
	cellGcmFlush();
...

	{
		...
		FWDebugFont::print("Edge Post Sample HDR Application\n\n");
