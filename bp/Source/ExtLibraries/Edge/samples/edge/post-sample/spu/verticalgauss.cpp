...
void edgePostMain( EdgePostTileInfo* tileInfo )
{
	...
	switch ( stage.user0 )
	{
	case POST_GAUSS_8: // blur an argb8 image
		for ( uint32_t j = 0; j < numscaline; ++j, dst_address += ostride, src_address += istride )
			edgePostGauss1x7_8( dst_address, src_address, istride, numpixels, weights );
		break;

	case POST_GAUSS_F: // blur a single channel floating point image ( used for near fuzziness dilation )
		for ( uint32_t j = 0; j < numscaline; ++j, dst_address += ostride, src_address += istride )
