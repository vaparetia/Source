...
float SampleApp::mBloomWeights[4] = { 0.3125f, 0.234375f, 0.09375f, 0.015625f};
float SampleApp::mDoubleWeights[4] = { 0.3125f * 2.f, 0.234375f * 2.f, 0.09375f * 2.f, 0.015625f * 2.f};
...
mMoveRate(5.f),
mUseMlaa(true),
mShowMlaaEdges(false)
{
	mNear = 0.2f;
	mFar = 500.f;
	mPostprocessingMode = kModeSpuPostprocesing;
	mAnimPause = false;
...
#ifdef SN_TARGET_PS3
#	define POSTSHADER( name ) _binary_##name##_start
#else
#	define POSTSHADER( name ) _binary_shaders_##name##_start
#endif

	extern char POSTSHADER( postvp_vpo)[];
	extern char POSTSHADER( post_copyfp_fpo)[];
	extern char POSTSHADER( post_motionblurfp_fpo)[];
	extern char POSTSHADER( post_spu_compositefp_fpo)[];
	extern char POSTSHADER( post_nearfuzzinessfp_fpo)[];
	extern char POSTSHADER( post_farfuzzinessfp_fpo)[];
	extern char POSTSHADER( post_doffp_fpo)[];
	extern char POSTSHADER( post_gauss7x1fp_fpo)[];
	extern char POSTSHADER( post_gauss1x7fp_fpo)[];
	extern char POSTSHADER( post_bloomcapturefp_fpo)[];
	extern char POSTSHADER( post_bloomcompositefp_fpo)[];
	extern char POSTSHADER( post_depthdownsamplefp_fpo)[];
	extern char POSTSHADER( post_ilrfp_fpo)[];

	// post shaders
	initShader( mResampleVp, POSTSHADER( postvp_vpo) );
	initShader( mResampleFp, POSTSHADER( post_copyfp_fpo) );
	initShader( mDepthdownsampleFp, POSTSHADER( post_depthdownsamplefp_fpo) );
	initShader( mMotionblurFp, POSTSHADER( post_motionblurfp_fpo) );
	initShader( mSpuCompositeFp, POSTSHADER( post_spu_compositefp_fpo) );
	initShader( mNearFuzzinessFp, POSTSHADER( post_nearfuzzinessfp_fpo) );
	initShader( mFarFuzzinessFp, POSTSHADER( post_farfuzzinessfp_fpo) );
	initShader( mDofFp, POSTSHADER( post_doffp_fpo) );
	initShader( mGauss7x1Fp, POSTSHADER( post_gauss7x1fp_fpo) );
	initShader( mGauss1x7Fp, POSTSHADER( post_gauss1x7fp_fpo) );
	initShader( mBloomCaptureFp, POSTSHADER( post_bloomcapturefp_fpo) );
	initShader( mBloomCompositeFp, POSTSHADER( post_bloomcompositefp_fpo) );
	initShader( mIlrFp, POSTSHADER( post_ilrfp_fpo) );
}
...
bool SampleApp::onInit(int argc, char **ppArgv)
{
	...
	uint8_t* colorTex = (uint8_t*)loadFile(SYS_APP_HOME"/assets/elephant-color.bin", colorTexSize);
	
	uint32_t normalTexSize = 0;
	uint8_t* normalTex = (uint8_t*)loadFile(SYS_APP_HOME"/assets/elephant-normal.bin", normalTexSize);
	...
	InitGpuPostprocessing( mDispInfo.mWidth, mDispInfo.mHeight );
...
bool SampleApp::onUpdate()
{
	...
	// mode...
	static int firstHack = 6;
	if ( firstHack == 0)
	{
		if ( mpCircle->getBoolTrue() )
		{
			mPostprocessingMode = ( mPostprocessingMode + 1) % 3;
			firstHack = 4;
		}
		...
		if ( mpR1->getBoolTrue() )
		{
			mShowMlaaEdges = !mShowMlaaEdges;
			printf("%s\n",mShowMlaaEdges?"on":"off");
		}

	} else --firstHack;

	// select parameter under edit
	static int secondHack = 10;
	if ( --secondHack < 0 )
	{
		mCurrentTweak += mpRight->getBoolValue() ? 1 : 0;
		mCurrentTweak -= mpLeft->getBoolValue() ? 1 : 0;
		mCurrentTweak = mCurrentTweak < 0 ? mCurrentTweak + kNumParameters : mCurrentTweak;
		mCurrentTweak %= kNumParameters;
		secondHack = 10;
	}

	// get delta to be applied
	const float kDeltaIncr = 0.002f;
	float delta = 0;
	if ( mpTriangle->getBoolValue() )
		delta = kDeltaIncr;
	else if ( mpCross->getBoolValue() )
		delta = -kDeltaIncr;
	
	switch ( mCurrentTweak )
	{
	case 0:	mPostParams.m_bloomStrength += delta; break;
	case 1:	mPostParams.m_exposureLevel += delta; break;
	case 2:	mPostParams.m_minLuminance += delta; break;
	case 3:	mPostParams.m_maxLuminance += delta; break;
	case 4:	mPostParams.m_nearFuzzy += delta; break;
	case 5:	mPostParams.m_nearSharp += delta; break;
	case 6:	mPostParams.m_farSharp += delta; break;
	case 7:	mPostParams.m_farFuzzy += delta; break;
	case 8:	mPostParams.m_maxFuzziness += delta; break;
	case 9:	mPostParams.m_motionScaler += delta; break;
	case 10: mPostParams.m_ilrStrenght += delta; break;
	}

	// make sure some values are appropriate
	mPostParams.m_nearFuzzy = clamp( mPostParams.m_nearFuzzy, 0.f, 1.f );
	mPostParams.m_nearSharp = clamp( mPostParams.m_nearSharp, .00001f, 1.f );
	mPostParams.m_farSharp = clamp( mPostParams.m_farSharp, .0f, 1.1f );
	mPostParams.m_farFuzzy = clamp( mPostParams.m_farFuzzy, .0f, 1.1f );
	mPostParams.m_nearSharp = mPostParams.m_nearSharp < mPostParams.m_nearFuzzy ? mPostParams.m_nearFuzzy : mPostParams.m_nearSharp;
	mPostParams.m_farSharp = mPostParams.m_farSharp < mPostParams.m_nearSharp ? mPostParams.m_nearSharp : mPostParams.m_farSharp;
	mPostParams.m_farFuzzy = mPostParams.m_farFuzzy < mPostParams.m_farSharp ? mPostParams.m_farSharp : mPostParams.m_farFuzzy;
	mPostParams.m_maxFuzziness = clamp( mPostParams.m_maxFuzziness, 0, 1 );
	
	// clamp below zero
	if ( mPostParams.m_bloomStrength < .0f )
		mPostParams.m_bloomStrength = .0f;
	if ( mPostParams.m_exposureLevel < .0f )
		mPostParams.m_exposureLevel = .0f;
	if ( mPostParams.m_minLuminance < .0f )
		mPostParams.m_minLuminance = .0f;
	if ( mPostParams.m_maxLuminance < .0f )
		mPostParams.m_maxLuminance = .0f;
	if ( mPostParams.m_motionScaler < .0f )
		mPostParams.m_motionScaler = .0f;
...
void SampleApp::SetTweakColor( int inx )
{
	if ( inx == mCurrentTweak )
		FWDebugFont::setColor( .3f, .3f, .3f, 1.0f);
	else
		FWDebugFont::setColor( .5f, .5f, .5f, 1.0f);
}

void SampleApp::RenderTweakables()
{
	FWDebugFont::setPosition( 1, 40);

	FWDebugFont::printf("Mode: %s (circle to toggle)\n", 
		mPostprocessingMode == kModeNoPostprocesing ? "NONE" :
		( mPostprocessingMode == kModeSpuPostprocesing ? "SPU" : "GPU"));

	SetTweakColor(0); FWDebugFont::printf("Bloom strength            : %f\n", mPostParams.m_bloomStrength);
	SetTweakColor(1); FWDebugFont::printf("Bloom exposure level      : %f\n", mPostParams.m_exposureLevel);
	SetTweakColor(2); FWDebugFont::printf("Bloom min luminance       : %f\n", mPostParams.m_minLuminance);
	SetTweakColor(3); FWDebugFont::printf("Bloom max luminance       : %f\n", mPostParams.m_maxLuminance);
	SetTweakColor(4); FWDebugFont::printf("Dof near fuzzy            : %f\n", mPostParams.m_nearFuzzy);
	SetTweakColor(5); FWDebugFont::printf("Dof near sharp            : %f\n", mPostParams.m_nearSharp);
	SetTweakColor(6); FWDebugFont::printf("Dof far sharp             : %f\n", mPostParams.m_farSharp);
	SetTweakColor(7); FWDebugFont::printf("Dof far fuzzy             : %f\n", mPostParams.m_farFuzzy);
	SetTweakColor(8); FWDebugFont::printf("Dof max fuzziness         : %f\n", mPostParams.m_maxFuzziness);
	SetTweakColor(9); FWDebugFont::printf("Motion blur scaler        : %f\n", mPostParams.m_motionScaler);
	SetTweakColor(10); FWDebugFont::printf("Ilr strenght              : %f\n", mPostParams.m_ilrStrenght);
...
void SampleApp::onRender()
{
	...
	float ms[4] = { mPostParams.m_motionScaler, getNear(), 1.f / ( getFar() - getNear()), 0}; 
	...
	switch (mPostprocessingMode)
	{
	case kModeNoPostprocesing:
		
		//CopyMlaaBufferUp();
		// Present the framebuffer unaltered
	...
	case kModeSpuPostprocesing:
		SwapSpuPostprocessBuffers();
		break;

	case kModeGpuPostprocesing:
		DoGpuPostprocess();
		break;
