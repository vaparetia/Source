...
extern "C" 
{
	void cellSpursJobMain2(CellSpursJobContext2* stInfo,  CellSpursJob256 *job256)
	{
		...
		const uint32_t eaReport = job->userData.eaReport;

		CellGcmReportData reportData __attribute__((aligned(16)));
		if (eaReport != 0xFFFFFFFF) {
			//read the report once
			{
				...
				spu_writech(MFC_LSA, (uint32_t)&reportData);
				spu_writech(MFC_EAL, eaReport);
				spu_writech(MFC_Size, sizeof(CellGcmReportData));
		...
		if (eaReport != 0xFFFFFFFF)
		{
			...
			//kill the job if needed, otherwise test again before filling in the hole
			if (reportData.zero != 0xFFFFFFFF && reportData.value == 0) {
				edgeGeomSetFreePtr(&ctx, stInfo->ioBuffer);
				//spu_printf("report: %08x %08x\n", reportData.value, reportData.zero);
		...
		if (eaReport != 0xFFFFFFFF)
		{
			...
			while (1) {
				...
				if (reportData.zero != 0xFFFFFFFF) {
					break;
				}

				// Wait ~1000 cycles to avoid flooding the GPU with label
			...
			if (reportData.value == 0) {
				//spu_printf("Killing job!\n");
				edgeGeomSetFreePtr(&ctx, stInfo->ioBuffer);
