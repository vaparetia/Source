...
#define ELEPHANT_REPORTS				(512) 					/*768*/
...
extern unsigned long _binary_vpshadersimple_vpo_start;
extern unsigned long _binary_vpshadersimple_vpo_end;
extern unsigned long _binary_fpshadersimple_fpo_start;
extern unsigned long _binary_fpshadersimple_fpo_end;
...
static uint32_t bbVertexesOffset;
static uint32_t planeVertexesOffset;
static bool useConditionalRendering = false;
static bool reportsInMainMemory = true;
...
void SampleApp::CreateGeomJob(JobGeom256* job, CellGcmContextData *ctx, EdgeGeomPpuConfigInfo *info, void *skinningMatrices, uint32_t elephantIndex) const
{
	...
	job->userData.eaReport = (useConditionalRendering) ? (uint32_t)cellGcmGetReportDataAddressLocation(ELEPHANT_REPORTS + elephantIndex, reportsInMainMemory ? CELL_GCM_LOCATION_MAIN : CELL_GCM_LOCATION_LOCAL) : 0xFFFFFFFF;
...
void SampleApp::initShader()
{
	...
	mCGVertexProgramSimple   = (CGprogram)(void*)&_binary_vpshadersimple_vpo_start;
	mCGFragmentProgramSimple = (CGprogram)(void*)&_binary_fpshadersimple_fpo_start;
	...
	// init
	cellGcmCgInitProgram(mCGVertexProgramSimple);
	cellGcmCgInitProgram(mCGFragmentProgramSimple);
	...
	// allocate video memory for simple fragment program
	cellGcmCgGetUCode(mCGFragmentProgramSimple, &ucode, &ucodeSize);

	mFragmentProgramSimpleUCode = (void*)cellGcmUtilAllocateLocalMemory(ucodeSize, 1024);
	cellGcmAddressToOffset(mFragmentProgramSimpleUCode, &mFragmentProgramSimpleOffset);
	memcpy(mFragmentProgramSimpleUCode, ucode, ucodeSize); 

	// get and copy 
	cellGcmCgGetUCode(mCGVertexProgramSimple, &ucode, &ucodeSize);
	mVertexProgramSimpleUCode = ucode;
...
bool SampleApp::onInit(int argc, char **ppArgv)
{
	...
	mCamera.setPosition(Point3(-8.27728f, 1.94f, 30.2424f));
	...
	/*
	  Reserve and Map Main Memory for report to main memory.
	  Report Main Memory address must be mapped to Io address 0x0e000000.
	  Please use cellGcmMapEaIoAddress for mapping address explicitly.
	*/
	void* report_address_main = memalign(1024*1024, 16 * 1024 * 1024);
	if(cellGcmMapEaIoAddress(report_address_main,0x0e000000, 16 * 1024 * 1024) != CELL_OK){
		printf("Report to Main memory, Map Ea address to Io Address  failed\n");
		exit(1);
	}
	...
	//copy bounding box data over to VRAM
	float *bbVertexes = (float*)cellGcmUtilAllocateLocalMemory(8 * 3 * 4, 16);
	bbVertexes[0] = -6.9f; 		bbVertexes[1] = -.06f; 		bbVertexes[2] = 7.5f;
	bbVertexes[3] = -2.5f; 		bbVertexes[4] = -.06f; 		bbVertexes[5] = 7.5f;
	bbVertexes[6] = -2.5f; 		bbVertexes[7] = 4.4f; 		bbVertexes[8] = 7.5f;
	bbVertexes[9] = -6.9f; 		bbVertexes[10] = 4.4f; 		bbVertexes[11] = 7.5f;
	bbVertexes[12] = -2.5f; 	bbVertexes[13] = -.06f; 	bbVertexes[14] = 15.4f;
	bbVertexes[15] = -6.9f; 	bbVertexes[16] = -.06f; 	bbVertexes[17] = 15.4f;
	bbVertexes[18] = -6.9f; 	bbVertexes[19] = 4.4f; 		bbVertexes[20] = 15.4f;
	bbVertexes[21] = -2.5f; 	bbVertexes[22] = 4.4f; 		bbVertexes[23] = 15.4f;
	cellGcmAddressToOffset(bbVertexes, &bbVertexesOffset);

	//copy occluding plane data over to VRAM
	float *planeVertexes = (float*)cellGcmUtilAllocateLocalMemory(4 * 3 * 4, 16);
	planeVertexes[0] = -10.f; 	planeVertexes[1] = -1.f; 	planeVertexes[2] = 17.f;
	planeVertexes[3] = 0.f; 	planeVertexes[4] = -1.f; 	planeVertexes[5] = 17.f;
	planeVertexes[6] = 0.f; 	planeVertexes[7] = 8.f; 	planeVertexes[8] = 17.f;
	planeVertexes[9] = -10.f; 	planeVertexes[10] = 8.f; 	planeVertexes[11] = 17.f;
	cellGcmAddressToOffset(planeVertexes, &planeVertexesOffset);
...
bool SampleApp::onUpdate()
{
	...
	useConditionalRendering = (mpTriangle && mpTriangle->getBoolValue()) || (mpSquare && mpSquare->getBoolValue());
	reportsInMainMemory = !(mpSquare && mpSquare->getBoolValue());

	return FWGCMCamApplication::onUpdate();
}

void SampleApp::SetupOccluder()
{
	cellGcmSetVertexProgram(mCGVertexProgramSimple, mVertexProgramSimpleUCode);
	cellGcmSetFragmentProgram(mCGFragmentProgramSimple, mFragmentProgramSimpleOffset);

	cellGcmSetDepthTestEnable(CELL_GCM_TRUE);
	cellGcmSetDepthFunc(CELL_GCM_LESS);
	cellGcmSetShadeMode(CELL_GCM_SMOOTH);

	//Occluder drawn in wireframe if conditional rendering is not to be used
	cellGcmSetFrontPolygonMode(useConditionalRendering ? CELL_GCM_POLYGON_MODE_FILL : CELL_GCM_POLYGON_MODE_LINE);
	cellGcmSetBackPolygonMode(useConditionalRendering ? CELL_GCM_POLYGON_MODE_FILL : CELL_GCM_POLYGON_MODE_LINE);
}

void SampleApp::RenderOccluder()
{
	const static uint16_t planeIndexes[] = {0, 1, 2, 3};
	const static float red[] = {1.f, 0.f, 0.f, 1.f};
	Matrix4 mat = Matrix4::identity();
	Matrix4 mvp = transpose(getProjectionMatrix() * getViewMatrix() * mat);
	cellGcmSetVertexProgramConstants(0, 16, (float*)&mvp);
	cellGcmSetVertexProgramConstants(4, 4, red);

	cellGcmSetVertexDataArray(0/* POSITION */, 0, 12 /* stride */,
							  3 /* component count */, CELL_GCM_VERTEX_F, CELL_GCM_LOCATION_LOCAL,
							  planeVertexesOffset);
	cellGcmSetDrawInlineIndexArray16(CELL_GCM_PRIMITIVE_QUADS, 0, 4, planeIndexes);
}

void SampleApp::SetupBoundingBoxes()
{
	// we're not writing to the color or depth buffers
	// just testing
...
void SampleApp::RenderBoundingBoxes()
{
	//render bounding boxes
	const static uint16_t bbIndexes[] = {	0, 1, 2, 3,
											1, 0, 5, 4,
											3, 2, 7, 6,
											2, 1, 4, 7,
											0, 3, 6, 5,
	...
	for (uint32_t iElephant = 0; iElephant < numElephants; iElephant++) 
	{
		...
		mat.setTranslation(Vector3(0.0f, 0.0f, -5.0f - 8.0f * (float)iElephant));
		Matrix4 mvp = transpose(getProjectionMatrix() * getViewMatrix() * mat);
		cellGcmSetVertexProgramConstants(0, 16, (float*)&mvp);
		...
		cellGcmSetVertexDataArray(0/* POSITION */, 0, 12 /* stride */,
								  3 /* component count */, CELL_GCM_VERTEX_F, CELL_GCM_LOCATION_LOCAL,
								  bbVertexesOffset);
		cellGcmSetDrawInlineIndexArray16(CELL_GCM_PRIMITIVE_QUADS, 0, 24, bbIndexes);

		cellGcmSetReport(CELL_GCM_ZPASS_PIXEL_CNT, ELEPHANT_REPORTS + iElephant);
	}
	cellGcmSetZpassPixelCountEnable(CELL_GCM_FALSE);

	// kick rsx to get this pass started
	cellGcmFlush();
}


void SampleApp::SetupElephants()
{
	cellGcmSetInvalidateVertexCache();

	cellGcmSetRenderEnable(CELL_GCM_TRUE, 0);
...
void SampleApp::RenderElephants()
{
	// Reset Parameters
	int numSegments = 0;
	numShapeInfos = 0;
	...
	cellGcmSetReportLocation(CELL_GCM_LOCATION_LOCAL);
...
void SampleApp::onRender()
{
	...
	// grey rsx bar until the start of the prepass point (clear etc)
	ProfileBars::AddRsxMarker(0x20202000);		
	//clear reports
	cellGcmSetReportLocation(reportsInMainMemory ? CELL_GCM_LOCATION_MAIN : CELL_GCM_LOCATION_LOCAL);
	memset((void *)(cellGcmGetReportDataAddressLocation(ELEPHANT_REPORTS + 0, reportsInMainMemory ? CELL_GCM_LOCATION_MAIN : CELL_GCM_LOCATION_LOCAL)), 0xff, 16 * numElephants);
	...
	// profile bar reports are kept in VRAM
	cellGcmSetReportLocation(CELL_GCM_LOCATION_LOCAL);
	// yellow bar for first pass
...

	{
		...
		FWDebugFont::print("Edge Conditional Rendering Sample Application\n");
		if (useConditionalRendering) {
			if (reportsInMainMemory) {
				FWDebugFont::print("Conditional rendering with reports in main memory.\n");
			}
			else{
				FWDebugFont::print("Conditional rendering with reports in video memory.\n");
			}
