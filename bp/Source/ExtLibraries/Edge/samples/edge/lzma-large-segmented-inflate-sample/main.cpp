...
#include "../lzma-segcomp-sample/ppu/lzma_segs_ppu.h"
...
void lzma_large_segmented_inflate_sample_main( CellSpurs* pSpurs );
...
int main()
{
	...
	lzma_large_segmented_inflate_sample_main( &s_spurs );
...
void lzma_large_segmented_inflate_sample_main( CellSpurs* pSpurs )
{
	...
	const char segsFileName[]		= "/app_home/assets/elephant-color.bin.lzma.segs";
	...
	TestValidLzmaSegsFile( pSegsFileData, segsFileName, segsFileSize );

	uint32_t numSegments					= GetLzmaSegsFileNumSegments( pSegsFileData );
	uint32_t totalUncompressedSize			= GetLzmaSegsFileTotalUncompressedSize( pSegsFileData );

	//PrintLzmaSegsHeaderAndToc( pSegsFileData, segsFileName, segsFileSize );
	...
	uint32_t numNonCompressedSegs = AddLzmaSegsFileItemsToInflateQueue(
											pSegsFileData,
											segsFileSize,
											inflateQueue,
											pOutputUncompBuff,
