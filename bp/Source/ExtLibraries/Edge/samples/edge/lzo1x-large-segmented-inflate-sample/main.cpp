...
#include "../lzo1x-segcomp-sample/ppu/lzo1x_segs_ppu.h"
...
void lzo1x_large_segmented_sample_main( CellSpurs* pSpurs );
...
int main()
{
	...
	lzo1x_large_segmented_sample_main( &s_spurs );
...
void lzo1x_large_segmented_sample_main( CellSpurs* pSpurs )
{
	...
	const char segsFileName[]	= "/app_home/assets/elephant-color.bin.lzo1x.segs";
	...
	TestValidLzo1xSegsFile( pSegsFileData, segsFileName, segsFileSize );

	uint32_t numSegments					= GetLzo1xSegsFileNumSegments( pSegsFileData );
	uint32_t totalUncompressedSize			= GetLzo1xSegsFileTotalUncompressedSize( pSegsFileData );

	//PrintLzo1xSegsHeaderAndToc( pSegsFileData, segsFileName, segsFileSize );
	...
	assert( CELL_OK == ret );
	ret = cellSpursTasksetAttributeSetName( &taskSetAttribute, "edgeLzo1xInflateTaskSet" );
	assert( CELL_OK == ret );
	...
	uint32_t numNonCompressedSegs = AddLzo1xSegsFileItemsToInflateQueue(
											pSegsFileData,
											segsFileSize,
											inflateQueue,
											pOutputUncompBuff,
