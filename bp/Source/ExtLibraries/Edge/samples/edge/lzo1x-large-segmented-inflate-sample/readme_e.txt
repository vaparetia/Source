...
"lzo1x-basic-inflate-sample" showed a simple example of decompressing a single small file using Edge LZO.
"lzo1x-large-segmented-inflate-sample" now shows how a larger file could have been segmented and compressed offline (see
"lzo1x-segcomp-sample") and then decompressed at run-time and recombined to give the original master file again.
...
The program loads the compressed segmented file ("assets/elephant-color.bin.lzo1x.segs") which will be being decompressed.

This sample creates the Inflate Queue and five Inflate Tasks (one for every SPU the decompression will be running on).

The program then calls "AddLzo1xSegsFileItemsToInflateQueue()" which will parse the TOC (Table of Contents)
...
<Running the Program>
    - Set fileserving root to the lzo1x-large-segmented-inflate-sample directory.
