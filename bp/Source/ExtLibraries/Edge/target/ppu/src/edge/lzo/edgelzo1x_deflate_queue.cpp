...
extern char _binary_task_edgelzo1x_deflate_task_spu_elf_start[];
 ...
 *								"edgeLzo1xGetDeflateTaskContextSaveSize()" or
 *								"EDGE_LZO1X_GET_DEFLATE_TASK_CONTEXT_SAVE_SIZE()".
		...
		EdgeLzo1xDeflateQHandle	handle )
{
	...
	EDGE_ASSERT( EDGE_LZO1X_GET_DEFLATE_TASK_CONTEXT_SAVE_SIZE() == edgeLzo1xGetDeflateTaskContextSaveSize() );	//Just checking our macro is correct

	int ret = cellSpursCreateTask(	pTaskSet,
									&taskId,
									_binary_task_edgelzo1x_deflate_task_spu_elf_start,
									pTaskContext,
 ...
 *								can be queried by calling "edgeLzo1xGetDeflateQueueSize".
 *
...
EdgeLzo1xDeflateQHandle edgeLzo1xCreateDeflateQueue(
			...
			uint32_t					bufferSize )
{
	EDGE_ASSERT( bufferSize == edgeLzo1xGetDeflateQueueSize( maxNumQueueEntries ) );
	EDGE_ASSERT( bufferSize == EDGE_LZO1X_GET_DEFLATE_QUEUE_SIZE( maxNumQueueEntries ) );	//Just checking our macro is correct
	...
	int ret = cellSpursLFQueueInitializeIWL(	pSpurs,
												pLfQueue,
												pLfQueueBuffer,
												sizeof(EdgeLzo1xDeflateQueueElement),
												maxNumQueueEntries,
...
uint32_t edgeLzo1xGetDeflateQueueSize( uint32_t maxNumQueueEntries )
{
	EDGE_ASSERT( maxNumQueueEntries > 0 );
	uint32_t size = sizeof(CellSpursLFQueue) + (maxNumQueueEntries * sizeof(EdgeLzo1xDeflateQueueElement));
	return size;
}
 ...
 * @param	pCompressedSize				The Effective Address of the uint32_t into which the output
		...
		EdgeLzo1xDeflateTaskProcessing	processing)
{
	...
	EDGE_ASSERT_MSG ( !sys_process_is_stack((void*)pInputUncompressedData),
		( "EDGE LZO1X ERROR: Input uncompressed data is on stack (which is illegal)\n") );
	
	EDGE_ASSERT_MSG ( !sys_process_is_stack(pOutputCompressedData),
		( "EDGE LZO1X ERROR: Output compressed data is on stack (which is illegal)\n") );

	EDGE_ASSERT_MSG ( !sys_process_is_stack((void*)pOutputCompressedSize),
		( "EDGE LZO1X ERROR: Ptr to output size & flag output data is on stack (which is illegal)\n") );
	...
	EDGE_ASSERT_MSG ( maxCompressedOutputSize <= kEdgeLzo1xDeflateOutputBufferMaxSize,
		( "EDGE LZO1X ERROR: Size of compressed data is beyond maximum allowed\n") );
		...
		EdgeLzo1xDeflateTaskProcessing	processing)
{
	...
	EDGE_ASSERT_MSG ( uncompressedSize <= kEdgeLzo1xDeflateInputBufferMaxSize,
		( "EDGE LZO1X ERROR: Size of uncompressed data is beyond maximum allowed\n") );
...
void edgeLzo1xShutdownDeflateQueue( EdgeLzo1xDeflateQHandle handle )
{
	CellSpursLFQueue* pLfQueue = (CellSpursLFQueue*) handle;
