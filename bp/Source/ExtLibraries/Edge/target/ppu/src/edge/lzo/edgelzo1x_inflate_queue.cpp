...
extern char _binary_task_edgelzo1x_inflate_task_spu_elf_start[];
 ...
 *								"edgeLzo1xGetInflateTaskContextSaveSize()" or
 *								"EDGE_LZO1X_GET_INFLATE_TASK_CONTEXT_SAVE_SIZE().
		...
		EdgeLzo1xInflateQHandle	handle )
{
	...
	EDGE_ASSERT( EDGE_LZO1X_GET_INFLATE_TASK_CONTEXT_SAVE_SIZE() == edgeLzo1xGetInflateTaskContextSaveSize() );	//Just checking our macro is correct

	int ret = cellSpursCreateTask(	pTaskSet,
									&taskId,
									_binary_task_edgelzo1x_inflate_task_spu_elf_start,
									pTaskContext,
 ...
 *								can be queried by calling "edgeLzo1xGetInflateQueueSize".
 *
...
EdgeLzo1xInflateQHandle edgeLzo1xCreateInflateQueue(
			...
			uint32_t				    bufferSize )
{
	EDGE_ASSERT( bufferSize == edgeLzo1xGetInflateQueueSize( maxNumQueueEntries ) );
	EDGE_ASSERT( bufferSize == EDGE_LZO1X_GET_INFLATE_QUEUE_SIZE( maxNumQueueEntries ) );	//Just checking our macro is correct
	...
	int ret = cellSpursLFQueueInitializeIWL(	pSpurs,
												pLfQueue,
												pLfQueueBuffer,
												sizeof(EdgeLzo1xInflateQueueElement),
												maxNumQueueEntries,
...
uint32_t edgeLzo1xGetInflateQueueSize( uint32_t maxNumQueueEntries )
{
	EDGE_ASSERT( maxNumQueueEntries > 0 );
	uint32_t size = sizeof(CellSpursLFQueue) + (maxNumQueueEntries * sizeof(EdgeLzo1xInflateQueueElement));
	return size;
}
		...
		EdgeLzo1xInflateTaskProcessing	processing )
{
	...
	EDGE_ASSERT_MSG( !sys_process_is_stack((void*)pInputCompressedData),
		("EDGE LZO1X ERROR: Input compressed data is on stack (which is illegal)\n") );
	
	EDGE_ASSERT_MSG( !sys_process_is_stack(pOutputUncompPartialBuff),
		("EDGE LZO1X ERROR: Output uncompressed data is on stack (which is illegal)\n") );

	EDGE_ASSERT_MSG( compressedSize <= kEdgeLzo1xInflateInputBufferMaxSize,
		("EDGE LZO1X ERROR: Size of compressed data is greater than allowed maximum\n") );

	EDGE_ASSERT_MSG( expectedUncompressedSize <= kEdgeLzo1xInflateOutputBufferMaxSize,
		("EDGE LZO1X ERROR: Size of uncompressed data is greater than allowed maximum\n") );
		...
		EdgeLzo1xInflateTaskProcessing	processing )
{
	...
	EDGE_ASSERT_MSG( compressedSize <= kEdgeLzo1xInflateInputBufferMaxSize,
		("EDGE LZO1X ERROR: Size of compressed data is greater than maximum allowed\n") );

	EDGE_ASSERT_MSG( expectedUncompressedSize <= kEdgeLzo1xInflateOutputBufferMaxSize,
		("EDGE LZO1X ERROR: Size of uncompressed data is greater than maximum allowed\n") );
...
void edgeLzo1xShutdownInflateQueue( EdgeLzo1xInflateQHandle handle )
{
	CellSpursLFQueue* pLfQueue = (CellSpursLFQueue*) handle;
