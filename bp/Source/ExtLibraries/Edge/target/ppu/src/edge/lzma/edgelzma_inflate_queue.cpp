...
extern char _binary_task_edgelzma_inflate_task_spu_elf_start[];
 ...
 *								"edgeLzmaGetInflateTaskContextSaveSize()" or
 *								"EDGE_LZMA_GET_INFLATE_TASK_CONTEXT_SAVE_SIZE()".
		...
		EdgeLzmaInflateQHandle	handle )
{
	...
	EDGE_ASSERT( EDGE_LZMA_GET_INFLATE_TASK_CONTEXT_SAVE_SIZE() == edgeLzmaGetInflateTaskContextSaveSize() );	//Just checking our macro is correct

	int ret = cellSpursCreateTask(	pTaskSet,
									&taskId,
									_binary_task_edgelzma_inflate_task_spu_elf_start,
									pTaskContext,
									EDGE_LZMA_GET_INFLATE_TASK_CONTEXT_SAVE_SIZE(),
 ...
 *								can be queried by calling "edgeLzmaGetInflateQueueSize".
 *
...
EdgeLzmaInflateQHandle edgeLzmaCreateInflateQueue(
			...
			uint32_t				    bufferSize )
{
	EDGE_ASSERT( bufferSize == edgeLzmaGetInflateQueueSize( maxNumQueueEntries ) );
	EDGE_ASSERT( bufferSize == EDGE_LZMA_GET_INFLATE_QUEUE_SIZE( maxNumQueueEntries ) );	//Just checking our macro is correct
	...
	int ret = cellSpursLFQueueInitializeIWL(	pSpurs,
												pLfQueue,
												pLfQueueBuffer,
												sizeof(EdgeLzmaInflateQueueElement),
												maxNumQueueEntries,
...
uint32_t edgeLzmaGetInflateQueueSize( uint32_t maxNumQueueEntries )
{
	EDGE_ASSERT( maxNumQueueEntries > 0 );
	uint32_t size = sizeof(CellSpursLFQueue) + (maxNumQueueEntries * sizeof(EdgeLzmaInflateQueueElement));
	return size;
}
		...
		EdgeLzmaInflateTaskProcessing	processing )
{
	...
	EDGE_ASSERT_MSG( !sys_process_is_stack((void*)pInputCompressedData), ("EDGE LZMA ERROR: Input compressed data is on stack (which is illegal)\n") );

	EDGE_ASSERT_MSG( !sys_process_is_stack(pOutputUncompPartialBuff), ("EDGE LZMA ERROR: Output uncompressed data is on stack (which is illegal)\n") );
	...
	case kEdgeLzmaInflateTask_Inflate:
		{
			...
			uint32_t dictSize = ((uint32_t)pProperties[4] << 24)
	...
	queueEntry.m_eaWorkToDoCounter				= ((uint32_t)pWorkToDoCounter) | processing;	//Bottom bit is a flag to do decompression (1), or memcpy (0)
		...
		EdgeLzmaInflateTaskProcessing	processing )
{
	...
	EDGE_ASSERT_MSG( !sys_process_is_stack((void*)pInputCompressedData),
		( "EDGE LZMA ERROR: Input compressed data is on stack (which is illegal)\n") );
	
	EDGE_ASSERT_MSG( !sys_process_is_stack(pOutputUncompPartialBuff),
		( "EDGE LZMA ERROR: Output uncompressed data is on stack (which is illegal)\n") );
...
void edgeLzmaShutdownInflateQueue( EdgeLzmaInflateQHandle handle )
{
	CellSpursLFQueue* pLfQueue = (CellSpursLFQueue*) handle;
