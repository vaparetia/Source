...
/** 
 * @brief   Convert joint transforms (R/T/S) to 3x4 matrices (transposed)
 *          SPU intrinisics implementation.
 *
 * @code
 * Matrix = | Sx * (0.5 - Rx*Ry - Rz*Rz)   Sy * (Rx*Ry - Rw*Rz)         Sz * (Rx*Rz + Rw*Ry)         Tx |
 *          | Sx * (Rx*Ry + Rw*Rz)         Sy * (0.5 - Rx*Rx - Rz*Rz)   Sz * (Ry*Rz - Rw*Rx)         Ty |
 *          | Sx * (Rx*Rz - Rw*Ry)         Sy * (Ry*Rz + Rw*Rz)         Sz * (0.5 - Rx*Rx - Ry*Ry)   Tz |
 * @endcode
 *
 * @note    A reference C++ implementation of this function is available in
 *          target/reference/src/edge/anim/edgeanim_joints_to_matrices3x4.cpp
 *
...
void edgeAnimJointsToMatrices3x4(void* outputMatrices, 
								 ...
								 unsigned int count)
{
	EDGE_BOOKMARK(EDGE_BOOKMARK_ANIM_JOINTS_TO_MATRICES_4X4);
	...
	for (; count4; count4--) {
		...
		const vec_float4 ts0 = spu_shuffle(t0, t1, shufAaBb);
		const vec_float4 ts1 = spu_shuffle(t0, t1, shufCcDd);
		const vec_float4 ts2 = spu_shuffle(t2, t3, shufBbAa);
		const vec_float4 ts3 = spu_shuffle(t2, t3, shufDdCc);
		const vec_float4 m30 = spu_sel(ts0, ts2, selABcd);
		const vec_float4 m31 = spu_shuffle(ts0, ts2, shufCDab);
		const vec_float4 m32 = spu_sel(ts1, ts3, selABcd);
		...
		const vec_float4 smx0 = spu_shuffle(m00, m10, shufAaBb);
		const vec_float4 smx1 = spu_shuffle(m00, m10, shufCcDd);
		const vec_float4 smx2 = spu_shuffle(m20, m30, shufBbAa);
		const vec_float4 smx3 = spu_shuffle(m20, m30, shufDdCc);
		...
		// (X0X1X2X3 Y0Y1Y2Y3 Z0Z1Z2Z3 W0W1W2W3 -> X0Y0Z0W0 X1Y1Z1W1 X2Y2Z2W2 X3Y3Z3W3)
		const vec_float4 smy0 = spu_shuffle(m01, m11, shufAaBb);
		const vec_float4 smy1 = spu_shuffle(m01, m11, shufCcDd);
		const vec_float4 smy2 = spu_shuffle(m21, m31, shufBbAa);
		const vec_float4 smy3 = spu_shuffle(m21, m31, shufDdCc);
		...
		// (X0X1X2X3 Y0Y1Y2Y3 Z0Z1Z2Z3 W0W1W2W3 -> X0Y0Z0W0 X1Y1Z1W1 X2Y2Z2W2 X3Y3Z3W3)
		const vec_float4 smz0 = spu_shuffle(m02, m12, shufAaBb);
		const vec_float4 smz1 = spu_shuffle(m02, m12, shufCcDd);
		const vec_float4 smz2 = spu_shuffle(m22, m32, shufBbAa);
		const vec_float4 smz3 = spu_shuffle(m22, m32, shufDdCc);
