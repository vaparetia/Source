...
/** 
 * @brief   Convert 4x4 matrices to joint transforms (R/T/S)
 ...
 * Ty = m31
 * Tz = m32
 * Sx = sqrt(m00*m00 + m01*m01 + m02*m02)
 * Sy = sqrt(m10*m10 + m11*m11 + m12*m12)
 * Sz = sqrt(m20*m20 + m21*m21 + m22*m22)
 * @endcode
 *
 * @note    A reference C++ implementation of this function is available in
 *          target/reference/src/edge/anim/edgeanim_matrices4x4_to_joints.cpp
 *
...
void edgeAnimMatrices4x4ToJoints(EdgeAnimJointTransform* outputJoints, 
								 ...
								 unsigned int count)
{
	EDGE_BOOKMARK(EDGE_BOOKMARK_ANIM_MATRICES_4X4_TO_JOINTS);
	...
	for (; count4; count4--) {
		...
		const vec_float4 m0row3 = input[3];
		const vec_float4 m1row0 = input[4];
		const vec_float4 m1row1 = input[5];
		const vec_float4 m1row2 = input[6];
		const vec_float4 m1row3 = input[7];
		const vec_float4 m2row0 = input[8];
		const vec_float4 m2row1 = input[9];
		const vec_float4 m2row2 = input[10];
		const vec_float4 m2row3 = input[11];
		const vec_float4 m3row0 = input[12];
		const vec_float4 m3row1 = input[13];
		const vec_float4 m3row2 = input[14];
		const vec_float4 m3row3 = input[15];
		...
		const vec_float4 m01 = spu_shuffle(s00, s02, shufCDab);
		const vec_float4 m02 = spu_sel(s01, s03, selABcd);
		...
		const vec_float4 m10 = spu_sel(s10, s12, selABcd);
		const vec_float4 m11 = spu_shuffle(s10, s12, shufCDab);
		const vec_float4 m12 = spu_sel(s11, s13, selABcd);
		...
		const vec_float4 m20 = spu_sel(s20, s22, selABcd);
		const vec_float4 m21 = spu_shuffle(s20, s22, shufCDab);
		...
		// Lookup shuffles
		vec_uint4 cnt0tmp = spu_rlmask(cmp0, -29);	  
		vec_uint4 cnt1tmp = spu_rlmask(cmp1, -30);	  
		vec_uint4 cnt2tmp = spu_rlmask(cmp2, -31);	  
