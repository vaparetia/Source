										   ...
										   unsigned int count)
{
	...
	Vectormath::Aos::Transform3* outputMat4 = (Vectormath::Aos::Transform3*) outputMatrices;
	...
	for(; count4; count4--, jointLinkage+=8) {
		for(int i=0; i<4; i++) {
			...
			const Vectormath::Aos::Transform3& parent = isRoot ? rootMatrix : outputMat4[idxParent];
			...
			// Output Matrix
			outputMat4[idxJoint] = worldMatrix;
		}
	}
}
