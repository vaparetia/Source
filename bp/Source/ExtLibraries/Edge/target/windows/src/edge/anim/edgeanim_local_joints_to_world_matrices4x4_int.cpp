										   ...
										   unsigned int count)
{
	...
	for( unsigned int count4 = (count + 3) >> 2; count4; count4--, jointLinkage+=8 )
	{
		...
		// Joint offsets
		__m128i jointInIdx = _mm_add_epi16( jointLink, _mm_slli_epi16( jointLink, 1 ) );
		__m128i jointOutIdx = _mm_slli_epi16( jointLink, 2 );
		...
		__m128i parentJointIdx = _mm_add_epi16( parentIdx, _mm_slli_epi16( parentIdx, 1 ) );
		__m128i parentMatIdx = _mm_slli_epi16( parentIdx, 2 );
		...
		__m128 lrx = inputBase[ _mm_extract_epi16( jointInIdx, 0 ) ];
		__m128 lry = inputBase[ _mm_extract_epi16( jointInIdx, 2 ) ];
		__m128 lrz = inputBase[ _mm_extract_epi16( jointInIdx, 4 ) ];
		__m128 lrw = inputBase[ _mm_extract_epi16( jointInIdx, 6 ) ];
		_MM_TRANSPOSE4_PS( lrx, lry, lrz, lrw );

		__m128 ltx = inputBase[ _mm_extract_epi16( jointInIdx, 0 ) + 1 ];
		__m128 lty = inputBase[ _mm_extract_epi16( jointInIdx, 2 ) + 1 ];
		__m128 ltz = inputBase[ _mm_extract_epi16( jointInIdx, 4 ) + 1 ];
		__m128 ltw = inputBase[ _mm_extract_epi16( jointInIdx, 6 ) + 1 ];
		_MM_TRANSPOSE4_PS( ltx, lty, ltz, ltw );

		__m128 lsx = inputBase[ _mm_extract_epi16( jointInIdx, 0 ) + 2 ];
		__m128 lsy = inputBase[ _mm_extract_epi16( jointInIdx, 2 ) + 2 ];
		__m128 lsz = inputBase[ _mm_extract_epi16( jointInIdx, 4 ) + 2 ];
		__m128 lsw = inputBase[ _mm_extract_epi16( jointInIdx, 6 ) + 2 ];
		...
		__m128 lpsx = inputBase[ _mm_extract_epi16( parentJointIdx, 1 ) + 2 ];
		__m128 lpsy = inputBase[ _mm_extract_epi16( parentJointIdx, 3 ) + 2 ];
		__m128 lpsz = inputBase[ _mm_extract_epi16( parentJointIdx, 5 ) + 2 ];
		__m128 lpsw = inputBase[ _mm_extract_epi16( parentJointIdx, 7 ) + 2 ];
		...
		__m128 world0Row0 = outputBase[ _mm_extract_epi16( parentMatIdx, 1 ) ];
		__m128 world1Row0 = outputBase[ _mm_extract_epi16( parentMatIdx, 3 ) ];
		__m128 world2Row0 = outputBase[ _mm_extract_epi16( parentMatIdx, 5 ) ];
		__m128 world3Row0 = outputBase[ _mm_extract_epi16( parentMatIdx, 7 ) ];
		__m128 world0Row1 = outputBase[ _mm_extract_epi16( parentMatIdx, 1 ) + 1 ];
		__m128 world1Row1 = outputBase[ _mm_extract_epi16( parentMatIdx, 3 ) + 1 ];
		__m128 world2Row1 = outputBase[ _mm_extract_epi16( parentMatIdx, 5 ) + 1 ];
		__m128 world3Row1 = outputBase[ _mm_extract_epi16( parentMatIdx, 7 ) + 1 ];
		__m128 world0Row2 = outputBase[ _mm_extract_epi16( parentMatIdx, 1 ) + 2 ];
		__m128 world1Row2 = outputBase[ _mm_extract_epi16( parentMatIdx, 3 ) + 2 ];
		__m128 world2Row2 = outputBase[ _mm_extract_epi16( parentMatIdx, 5 ) + 2 ];
		__m128 world3Row2 = outputBase[ _mm_extract_epi16( parentMatIdx, 7 ) + 2 ];
		__m128 world0Row3 = outputBase[ _mm_extract_epi16( parentMatIdx, 1 ) + 3 ];
		__m128 world1Row3 = outputBase[ _mm_extract_epi16( parentMatIdx, 3 ) + 3 ];
		__m128 world2Row3 = outputBase[ _mm_extract_epi16( parentMatIdx, 5 ) + 3 ];
		__m128 world3Row3 = outputBase[ _mm_extract_epi16( parentMatIdx, 7 ) + 3 ];

		// Swizzle World Matrices
		__m128 world_m00 = world0Row0, world_m01 = world1Row0, world_m02 = world2Row0, world_m03 = world3Row0;
		__m128 world_m10 = world0Row1, world_m11 = world1Row1, world_m12 = world2Row1, world_m13 = world3Row1;
		__m128 world_m20 = world0Row2, world_m21 = world1Row2, world_m22 = world2Row2, world_m23 = world3Row2;
		__m128 world_tx  = world0Row3, world_ty  = world1Row3, world_tz  = world2Row3, world_tw  = world3Row3;
		_MM_TRANSPOSE4_PS( world_m00, world_m01, world_m02, world_m03 );
		_MM_TRANSPOSE4_PS( world_m10, world_m11, world_m12, world_m13 );
		_MM_TRANSPOSE4_PS( world_m20, world_m21, world_m22, world_m23 );
		_MM_TRANSPOSE4_PS( world_tx,  world_ty,  world_tz,  world_tw  );
		...
		// Deswizzle Matrices
		world0Row0 = world_m00_t; world1Row0 = world_m01_t; world2Row0 = world_m02_t; world3Row0 = _mm_setzero_ps();
		world0Row1 = world_m10_t; world1Row1 = world_m11_t; world2Row1 = world_m12_t; world3Row1 = _mm_setzero_ps();
		world0Row2 = world_m20_t; world1Row2 = world_m21_t; world2Row2 = world_m22_t; world3Row2 = _mm_setzero_ps();
		world0Row3 = world_tx   ; world1Row3 = world_ty   ; world2Row3 = world_tz   ; world3Row3 = k1_0f ;
		_MM_TRANSPOSE4_PS( world0Row0, world1Row0, world2Row0, world3Row0 );
		_MM_TRANSPOSE4_PS( world0Row1, world1Row1, world2Row1, world3Row1 );
		_MM_TRANSPOSE4_PS( world0Row2, world1Row2, world2Row2, world3Row2 );
		_MM_TRANSPOSE4_PS( world0Row3, world1Row3, world2Row3, world3Row3 );

		// Output
		outputBase[ _mm_extract_epi16( jointOutIdx, 0 ) ] = world0Row0;
		outputBase[ _mm_extract_epi16( jointOutIdx, 2 ) ] = world1Row0;
		outputBase[ _mm_extract_epi16( jointOutIdx, 4 ) ] = world2Row0;
		outputBase[ _mm_extract_epi16( jointOutIdx, 6 ) ] = world3Row0;

		outputBase[ _mm_extract_epi16( jointOutIdx, 0 ) + 1 ] = world0Row1;
		outputBase[ _mm_extract_epi16( jointOutIdx, 2 ) + 1 ] = world1Row1;
		outputBase[ _mm_extract_epi16( jointOutIdx, 4 ) + 1 ] = world2Row1;
		outputBase[ _mm_extract_epi16( jointOutIdx, 6 ) + 1 ] = world3Row1;

		outputBase[ _mm_extract_epi16( jointOutIdx, 0 ) + 2 ] = world0Row2;
		outputBase[ _mm_extract_epi16( jointOutIdx, 2 ) + 2 ] = world1Row2;
		outputBase[ _mm_extract_epi16( jointOutIdx, 4 ) + 2 ] = world2Row2;
		outputBase[ _mm_extract_epi16( jointOutIdx, 6 ) + 2 ] = world3Row2;

		outputBase[ _mm_extract_epi16( jointOutIdx, 0 ) + 3 ] = world0Row3;
		outputBase[ _mm_extract_epi16( jointOutIdx, 2 ) + 3 ] = world1Row3;
		outputBase[ _mm_extract_epi16( jointOutIdx, 4 ) + 3 ] = world2Row3;
		outputBase[ _mm_extract_epi16( jointOutIdx, 6 ) + 3 ] = world3Row3;
	}
}
