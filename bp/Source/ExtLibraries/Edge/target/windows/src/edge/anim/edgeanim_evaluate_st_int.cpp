						  ...
						  __m128* output )
{
	...
	for( uint32_t count4 = (animCount + 3) >> 2; count4; count4-- )
	{
		...
		jointOfs = _mm_add_epi32( _mm_slli_epi32( jointOfs, 3 ), _mm_slli_epi32( jointOfs, 2 ) ); // *kStride = *12
		...
		// decompress + swizzle jointA, jointB
		__m128 joint0X = _mm_loadu_ps( (float*) key0adr0 );
		__m128 joint0Y = _mm_loadu_ps( (float*) key0adr1 );
		__m128 joint0Z = _mm_loadu_ps( (float*) key0adr2 );
		__m128 joint0W = _mm_loadu_ps( (float*) key0adr3 );
		_MM_TRANSPOSE4_PS( joint0X, joint0Y, joint0Z, joint0W );

		__m128 joint1X = _mm_loadu_ps( (float*) key1adr0 );
		__m128 joint1Y = _mm_loadu_ps( (float*) key1adr1 );
		__m128 joint1Z = _mm_loadu_ps( (float*) key1adr2 );
		__m128 joint1W = _mm_loadu_ps( (float*) key1adr3 );
		_MM_TRANSPOSE4_PS( joint1X, joint1Y, joint1Z, joint1W );

		// lerp, set w = 1.0f + save result
		__m128 oneMinusT = _mm_sub_ps( k1_0f, t );
		__m128 joint0 = _mm_madd_ps( t, joint1X, _mm_mul_ps( oneMinusT, joint0X ) );
		__m128 joint1 = _mm_madd_ps( t, joint1Y, _mm_mul_ps( oneMinusT, joint0Y ) );
		__m128 joint2 = _mm_madd_ps( t, joint1Z, _mm_mul_ps( oneMinusT, joint0Z ) );
		__m128 joint3 = k1_0f;
		_MM_TRANSPOSE4_PS( joint0, joint1, joint2, joint3 );

		// write out
		output[ animTable[0] * 3 ] = joint0;
		output[ animTable[1] * 3 ] = joint1;
		output[ animTable[2] * 3 ] = joint2;
		output[ animTable[3] * 3 ] = joint3;
		...
		intraInc = _mm_add_epi32( _mm_slli_epi32( intraInc, 3 ), _mm_slli_epi32( intraInc, 2 ) );
