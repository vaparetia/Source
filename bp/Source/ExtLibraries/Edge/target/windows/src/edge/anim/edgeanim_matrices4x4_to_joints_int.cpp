                                 ...
                                 unsigned int count)
{
	...
	for( unsigned int count4 = (count + 3) >> 2; count4; count4-- )
	{
		...
		const __m128 m0row3 = input[3];
		const __m128 m1row0 = input[4];
		const __m128 m1row1 = input[5];
		const __m128 m1row2 = input[6];
		const __m128 m1row3 = input[7];
		const __m128 m2row0 = input[8];
		const __m128 m2row1 = input[9];
		const __m128 m2row2 = input[10];
		const __m128 m2row3 = input[11];
		const __m128 m3row0 = input[12];
		const __m128 m3row1 = input[13];
		const __m128 m3row2 = input[14];
		const __m128 m3row3 = input[15];

		// Swizzle matrices
		__m128 m00 = m0row0, m01 = m1row0, m02 = m2row0, m03 = m3row0;
		__m128 m10 = m0row1, m11 = m1row1, m12 = m2row1, m13 = m3row1;
		__m128 m20 = m0row2, m21 = m1row2, m22 = m2row2, m23 = m3row2;
		_MM_TRANSPOSE4_PS( m00, m01, m02, m03 );
		_MM_TRANSPOSE4_PS( m10, m11, m12, m13 );
		_MM_TRANSPOSE4_PS( m20, m21, m22, m23 );

		// Set W=1.0f + output translations
		__m128 t0 = _mm_shuffle_ps( m0row3, _mm_shuffle_ps( m0row3, k1_0f, _MM_SHUFFLE( 3, 3, 2, 2 ) ), _MM_SHUFFLE( 3, 1, 1, 0 ) );
		__m128 t1 = _mm_shuffle_ps( m1row3, _mm_shuffle_ps( m1row3, k1_0f, _MM_SHUFFLE( 3, 3, 2, 2 ) ), _MM_SHUFFLE( 3, 1, 1, 0 ) );
		__m128 t2 = _mm_shuffle_ps( m2row3, _mm_shuffle_ps( m2row3, k1_0f, _MM_SHUFFLE( 3, 3, 2, 2 ) ), _MM_SHUFFLE( 3, 1, 1, 0 ) );
		__m128 t3 = _mm_shuffle_ps( m3row3, _mm_shuffle_ps( m3row3, k1_0f, _MM_SHUFFLE( 3, 3, 2, 2 ) ), _MM_SHUFFLE( 3, 1, 1, 0 ) );
		output[1]  = t0;
		output[4]  = t1;
		output[7]  = t2;
		output[10] = t3;
