...
extern "C"
void edgeAnimBlendJointsRelative(struct EdgeAnimJointTransform* outputJoints, 
                                 ...
                                 unsigned int count)
{
	...
	const __m128 mulDelta = _mm_sel_ps( k1_0f, kNeg1_0f, subtract );
	...
	for( unsigned int count4 = (count + 3) >> 2; count4; count4-- )
	{
		...
		// Blend factor
		__m128 t = _mm_sel_ps( _mm_mul_ps( rightWeight, splatAlpha ), k1_0f, compose );
		...
		// Force left / right
		__m128i forceLeftI = _mm_andnot_si128( bothValidI, _mm_andnot_si128( subtractI, leftValid ) );
		__m128i forceRightI = _mm_andnot_si128( bothValidI, _mm_andnot_si128( subtractI, _mm_and_si128( composeI, rightValid ) ) );
		...
		*outputW = *(uint32_t*)(&outputWeight);
		...
		// Blend translations
		__m128 blendedTransX = _mm_madd_ps( rightTransX, mulDelta, leftTransX );
		__m128 blendedTransY = _mm_madd_ps( rightTransY, mulDelta, leftTransY );
		__m128 blendedTransZ = _mm_madd_ps( rightTransZ, mulDelta, leftTransZ );
		blendedTransX = _mm_madd_ps( blendedTransX, t, _mm_mul_ps( leftTransX, oneMinusT ) );
		blendedTransY = _mm_madd_ps( blendedTransY, t, _mm_mul_ps( leftTransY, oneMinusT ) );
		blendedTransZ = _mm_madd_ps( blendedTransZ, t, _mm_mul_ps( leftTransZ, oneMinusT ) );
		...
		// Blend scales
		__m128 blendedScaleX = _mm_sel_ps( _mm_mul_ps( leftScaleX, rightScaleX ), _mm_div_ps( leftScaleX, rightScaleX ), subtract );
		__m128 blendedScaleY = _mm_sel_ps( _mm_mul_ps( leftScaleY, rightScaleY ), _mm_div_ps( leftScaleY, rightScaleY ), subtract );
		__m128 blendedScaleZ = _mm_sel_ps( _mm_mul_ps( leftScaleZ, rightScaleZ ), _mm_div_ps( leftScaleZ, rightScaleZ ), subtract );
		blendedScaleX = _mm_madd_ps( blendedScaleX, t, _mm_mul_ps( leftScaleX, oneMinusT ) );
		blendedScaleY = _mm_madd_ps( blendedScaleY, t, _mm_mul_ps( leftScaleY, oneMinusT ) );
		blendedScaleZ = _mm_madd_ps( blendedScaleZ, t, _mm_mul_ps( leftScaleZ, oneMinusT ) );
		...
		rightRotX = _mm_mul_ps( rightRotX, mulDelta );
		rightRotY = _mm_mul_ps( rightRotY, mulDelta );
		rightRotZ = _mm_mul_ps( rightRotZ, mulDelta );

		// Swap left & right rotations for subtract mode
		__m128 aRotX = _mm_sel_ps( rightRotX, leftRotX, subtract );
		__m128 aRotY = _mm_sel_ps( rightRotY, leftRotY, subtract );
		__m128 aRotZ = _mm_sel_ps( rightRotZ, leftRotZ, subtract );
		__m128 aRotW = _mm_sel_ps( rightRotW, leftRotW, subtract );
		__m128 bRotX = _mm_sel_ps( leftRotX, rightRotX, subtract );
		__m128 bRotY = _mm_sel_ps( leftRotY, rightRotY, subtract );
		__m128 bRotZ = _mm_sel_ps( leftRotZ, rightRotZ, subtract );
		__m128 bRotW = _mm_sel_ps( leftRotW, rightRotW, subtract );

		// Compose target rotation 
		__m128 composedRotXa = _mm_mul_ps( aRotX, bRotW );
		__m128 composedRotYa = _mm_mul_ps( aRotY, bRotW );
		__m128 composedRotZa = _mm_mul_ps( aRotZ, bRotW );
		__m128 composedRotWa = _mm_mul_ps( aRotW, bRotW );

		__m128 composedRotXb = _mm_madd_ps( aRotW, bRotX, composedRotXa );
		__m128 composedRotYb = _mm_nmsub_ps( aRotZ, bRotX, composedRotYa );
		__m128 composedRotZb = _mm_madd_ps( aRotY, bRotX, composedRotZa );
		__m128 composedRotWb = _mm_nmsub_ps( aRotX, bRotX, composedRotWa );

		__m128 composedRotXc = _mm_madd_ps( aRotZ, bRotY, composedRotXb );
		__m128 composedRotYc = _mm_madd_ps( aRotW, bRotY, composedRotYb );
		__m128 composedRotZc = _mm_nmsub_ps( aRotX, bRotY, composedRotZb );
		__m128 composedRotWc = _mm_nmsub_ps( aRotY, bRotY, composedRotWb );

		__m128 composedRotX = _mm_nmsub_ps( aRotY, bRotZ, composedRotXc );
		__m128 composedRotY = _mm_madd_ps( aRotX, bRotZ, composedRotYc );
		__m128 composedRotZ = _mm_madd_ps( aRotW, bRotZ, composedRotZc );
		__m128 composedRotW = _mm_nmsub_ps( aRotZ, bRotZ, composedRotWc );
			   
		// Slerp rotations : compute dot product
		__m128 dotRot = _mm_mul_ps( leftRotX, composedRotX );
		dotRot = _mm_madd_ps( leftRotY, composedRotY, dotRot );
		dotRot = _mm_madd_ps( leftRotZ, composedRotZ, dotRot );
		dotRot = _mm_madd_ps( leftRotW, composedRotW, dotRot );
		...
		__m128 sinaEval3 = _mm_add_ps( _mm_mul_ps( sin3, sqAngle ), sin2 );
		__m128 sinaEval2 = _mm_add_ps( _mm_mul_ps( sinaEval3, sqAngle ), sin1 );
		__m128 sinaEval1 = _mm_add_ps( _mm_mul_ps( sinaEval2, sqAngle ), sin0 );
		__m128 sinaEval0 = _mm_add_ps( _mm_mul_ps( sinaEval1, sqAngle ), k1_0f );
		...
		// Do the actual blend
		__m128 blendedRotX = _mm_madd_ps( composedRotX, fRight, _mm_mul_ps( leftRotX, fLeft ) );
		__m128 blendedRotY = _mm_madd_ps( composedRotY, fRight, _mm_mul_ps( leftRotY, fLeft ) );
		__m128 blendedRotZ = _mm_madd_ps( composedRotZ, fRight, _mm_mul_ps( leftRotZ, fLeft ) );
		__m128 blendedRotW = _mm_madd_ps( composedRotW, fRight, _mm_mul_ps( leftRotW, fLeft ) );
