...
// Asserts instead of exception handling
execution_error::execution_error(int i_)
{
...
expression_result xpath_processor::er_compute_xpath ()
{
   ...
   XPATH_CATCH (syntax_error)
   {
      expression_result er_null;
      er_result = er_null;
      e_error = e_error_syntax;
   }
   XPATH_CATCH (syntax_overflow)
   {
      expression_result er_null;
      er_result = er_null;
      e_error = e_error_overflow;
   }
   XPATH_CATCH (execution_error e)
   {
      expression_result er_null;
      er_result = er_null;
   ...
   bool o_skip_only)             ///< True if we only need to skip rules and not act on the data stack
{
   ...
   switch (xc_action)
   {
      ...
      case xpath_function_call :
               ...
               for (u_arg = 0; u_arg < u_variable; u_arg++)
               {
                  ...
                  if (! o_skip_only)
                  {
                     if (! xs_stack . u_get_size ())
                        XPATH_THROW execution_error (10);
         ...
         if (o_error)
            XPATH_THROW execution_error (11);
         break;
      ...
      default :
         XPATH_THROW execution_error (12);
   }
}
   ...
   bool o_skip_only)
{
   ...
   TiXmlElement const* XEp_child;
   TiXmlElement const* XEp_elem;
   TiXmlElement const* XEp_father;
   TiXmlAttribute const * XAp_attrib;
   TiXmlNode const * XNp_next;
   TiXmlNode const * XNp_parent;
...
bool xpath_processor::o_check_predicate (TiXmlElement const * XEp_child, bool o_by_name)
{
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (S_name == "true")
      ...
      XPATH_THROW execution_error (13);
}
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (u_nb_arg != 1)
      XPATH_THROW execution_error (14);
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (! u_nb_arg)
      XPATH_THROW execution_error (15);
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (u_nb_arg != 2)
      XPATH_THROW execution_error (16);
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (! u_nb_arg)
      XPATH_THROW execution_error (17);
...
{
   if (u_nb_arg)
      XPATH_THROW execution_error (18);
   v_push_bool (false);
}
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (u_nb_arg != 1)
      XPATH_THROW execution_error (19);
   switch (erpp_arg [0] -> e_type)
   {
      case e_int :
      case e_bool :
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (u_nb_arg)
      XPATH_THROW execution_error (20);
   XEp_context = XEp_get_context ();
   if (! XEp_context)
      XPATH_THROW execution_error (21);
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (u_nb_arg != 1)
      XPATH_THROW execution_error (22);
   S_res = "";
   if (erpp_arg [0] -> e_type == e_node_set)
   {
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (u_nb_arg != 1)
      XPATH_THROW execution_error (23);
...
{
   if (u_nb_arg != 1)
      XPATH_THROW execution_error (24);
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (u_nb_arg)
      XPATH_THROW execution_error (25);
   XEp_context = XEp_get_context ();
   if (! XEp_context)
      XPATH_THROW execution_error (26);
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (u_nb_arg != 2)
      XPATH_THROW execution_error (27);
   S_arg_1 = erpp_arg [0] -> S_get_string ();
   S_arg_2 = erpp_arg [1] -> S_get_string ();
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (u_nb_arg != 1)
      XPATH_THROW execution_error (28);
   S_arg = erpp_arg [0] -> S_get_string ();
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (u_nb_arg != 2 && u_nb_arg != 3)
      XPATH_THROW execution_error (29);
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (u_nb_arg != 1)
      XPATH_THROW execution_error (30);
   i_sum = 0;
   d_sum = 0.0;
   o_out_double = false;
   if (erpp_arg [0] -> e_type != e_node_set)
      XPATH_THROW execution_error (31);
   nsp_set = erpp_arg [0] -> nsp_get_node_set ();
   assert (nsp_set);
   ...
   expression_result ** erpp_arg)   ///< Argument list
{
   ...
   if (u_nb_arg)
      XPATH_THROW execution_error (38);
   XEp_context = XEp_get_context ();
   if (! XEp_context)
      XPATH_THROW execution_error (39);
...
{
   if (u_nb_arg)
      XPATH_THROW execution_error (32);
   v_push_bool (true);
}
      ...
      switch (u_sub)
      {
         ...
         case xpath_multiplicative_expr_div :
            if (fabs (d_arg_2) < 1.0e-6)
               XPATH_THROW execution_error (33);
            d_res = d_arg_1 / d_arg_2;
            break;
         case xpath_multiplicative_expr_mod :
...
void xpath_processor::v_order_recurs (TiXmlNode const * Np_base, int & i_current)
{
   TiXmlNode const * Np_child;

   const_cast<TiXmlNode*>(Np_base) -> SetUserData ((void *) i_current++);
...
void xpath_processor::v_set_context (
   TiXmlElement const * XEp_in,     ///< Context node
