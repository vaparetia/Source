...
#include "Win32GenericHTTPClient.h"
#include <winreg.h>

GenericHTTPClient::GenericHTTPClient(){
	_hHTTPOpen=_hHTTPConnection=_hHTTPRequest=NULL;
   _dwError = ERROR_SUCCESS;
}
...
BOOL GenericHTTPClient::Open(LPCTSTR szURL)
{
   BOOL bReturn=TRUE;
   DWORD dwPort=0;
   TCHAR szProtocol[__SIZE_BUFFER]="";
   ...
   if(Connect(szAddress, __DEFAULT_AGENT_NAME, dwPort)){
      if(!RequestOfURI(szURI, GenericHTTPClient::RequestGetMethod)){
         bReturn=FALSE;
      }
   }else{
      bReturn=FALSE;
...
#if 0// #ifdef __HTTP_DEBUG // AndyO, No exceptions
   catch(CException *e)
   {
		TRACE("\nEXCEPTION\n");
		TCHAR szERROR[1024];
