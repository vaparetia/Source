...
namespace rv_compat {
   ...
   inline void Construct(T* p, V const& v) { ::new(static_cast<void*>(p)) T(v); }
   template<class T>
   inline void Destroy(T* p) { p->~T(); }
   template<class It>
   inline void DestroyRange(It first, It last) { for (; first != last; ++first) Destroy(first); }
}
#endif
...
#else
         rv_compat::Construct(data() + mSize, what);
#endif
         mSize++;
      }
...
#else
            rv_compat::DestroyRange( begin() + n, end() );
#endif
         }
         else
         {
...
#else
         rv_compat::Destroy( data() + mSize - 1 );
#endif
         mSize--;
      }
...
#else
         rv_compat::DestroyRange( data(), data() + mSize );
#endif
      }
