...
#include "StdAfx.h"
#include "Engine/Resource/Win32CResourceFactoryLoadItem.h"
...
static void _cook_asset(std::string const &filePath)
{
   CSystemVar assetTool("BPE_ASSETCOOKER");
   std::string args(CStringExtras::Stringize("%s /t /c", filePath.c_str()));
   ...
   shellInfo.lpFile = assetTool.GetValue().c_str();
   ...
   {
      {
         ...
         if (CResourceFactory::GetAutoCookOnLoad()) _cook_asset(filePath);
