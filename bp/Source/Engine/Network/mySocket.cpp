...
bool myTcpSocket::connectToServer(string const& serverNameOrAddr,hostType hType)
{ 
   /* 
...
int myTcpSocket::XPrecieveMessage(string& message)
{
	...
	catch(myException* excp)
	{
		...
		{
			bpe_console_printf("!! your party has shut down the connection... \n");
			return -99;
		}
		excp->response();
		delete excp;
...
char const * mySocket::getErrorString() const
{
   if (mpError)
   {
...
int mySocket::getErrorNumber() const
{
   if (mpError)
   {
