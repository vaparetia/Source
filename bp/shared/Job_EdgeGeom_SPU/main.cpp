...
void cellSpursJobMain2(CellSpursJobContext2* stInfo,  CellSpursJob256 *job256)
{
   ...
   uint32_t totalMatrixCount  = (job->inputDmaList[kJobGeomDmaSkinMatrices_0].size + job->inputDmaList[kJobGeomDmaSkinMatrices_1].size) / 48;
   ...
   bool hasSecondStream = (job->inputDmaList[kJobGeomDmaVertexes2_0].size + job->inputDmaList[kJobGeomDmaVertexes2_1].size + job->inputDmaList[kJobGeomDmaVertexes2_2].size) > 0;
   if (!hasSecondStream)
   {
   ...
   uint32_t numVisibleIdxs = edgeGeomCullTriangles(&ctx, EDGE_GEOM_CULL_FRUSTUM);
