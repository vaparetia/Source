//----------------------------------------------------------------------------
// BP_NetworkPS3.cpp
//----------------------------------------------------------------------------

#include "Engine/Stdafx.h"
#include "Engine/Math/BPETypeConversion.inl"
#include "Engine/System/COsContext.h"

#include "BP_Network.h"

#include <stdio.h>
#include <sdk_version.h>
#include <np.h>

#include <cell/sysmodule.h>
#include <sysutil/sysutil_userinfo.h>

#include <netex/libnetctl.h>
#include "BP_PSNDialogSignalFunctions.h"

static int sce_ok( char const *str, int var )
{
   if ( var < 0 )
   {
      printf( "Error code: %s - %d\n", str, var );
      BPE_VERIFYA( false, "sce not ok!" );
   }

   return var;
}

#define SCE_CHECK_OK(x) sce_ok( #x, (x) )

#define SCE_OK (0)

#if MGS_VERSION==2
static const SceNpCommunicationId sNpCommunicationId = {
   {'N', 'P', 'W', 'R', '0', '2', '3', '8', '0'},
   '\0',
   0,
   0
};

/***
SceNpCommunicationPassphrase


***/

static const SceNpCommunicationPassphrase sNpCommunicationPassphrase = {
   {
      
   }
};

/***
SceNpCommunicationSignature


***/

static const SceNpCommunicationSignature sNpCommunicationSignature = {
   {
      
   }
};

static const SceNpCommunicationConfig s_npCommunicationConfig =
{
   &sNpCommunicationId,
   &sNpCommunicationPassphrase,
   &sNpCommunicationSignature
};
#endif

//----------------------------------------------------------------------------

#if MGS_VERSION==3

static const SceNpCommunicationId sNpCommunicationId = {
   {'N', 'P', 'W', 'R', '0', '2', '3', '8', '1'},
   '\0',
   0,
   0
};

/***
SceNpCommunicationPassphrase


***/

static const SceNpCommunicationPassphrase sNpCommunicationPassphrase = {
   {
      
   }
};

/***
SceNpCommunicationSignature


***/

static const SceNpCommunicationSignature sNpCommunicationSignature = {
   {
      
   }
};

static const SceNpCommunicationConfig s_npCommunicationConfig =
{
   &sNpCommunicationId,
   &sNpCommunicationPassphrase,
   &sNpCommunicationSignature
};

#endif

typedef enum
{
   kPSN_Uninitialized = 0,
   kPSN_DialogStarted,
   kPSN_DialogLoaded,
   kPSN_Error,
}
EPSNDialogState;

namespace
{
   int sServiceState = SCE_NP_MANAGER_STATUS_OFFLINE;
   bool sUserNPIDValid = false;
   SceNpId sUserNPID;
   char sUserName[CELL_USERINFO_USERNAME_SIZE]; // Local user name
   static bool sNetworkInitialized = false;
   static int sNetcheckDialogHandle = 0;

   void update_service_state( int state )
   {
      if ( state == SCE_NP_MANAGER_STATUS_ONLINE && sServiceState != SCE_NP_MANAGER_STATUS_ONLINE )
      {
         if ( SCE_OK != sceNpManagerGetNpId( &sUserNPID ) )
         {
            printf( "NP ERROR: NP Service transition without NP ID?!\n" );

            sUserNPIDValid = true;
            sServiceState = SCE_NP_MANAGER_STATUS_OFFLINE;
            return;
         }
      }
      sServiceState = state;
   }

   void set_initial_service_state()
   {
      int initialState;
      sceNpManagerGetStatus( &initialState );
      update_service_state(initialState);
   }

   void myNpServiceStateCallback( int state, int retCode, void *userdata )
   {
      update_service_state(state);
   }
   
   int const skPsnDialogHandleId = 1;
   EPSNDialogState sPsnDialogState;
   struct CellNetCtlNetStartDialogResult sPsnDialogResult;
}

void BP_Network_Init()
{
   if ( !sNetworkInitialized )
   {
      sNetworkInitialized = true;

      // Get the cached np id from flash memory if there is a cached NP ID
      SceNpManagerCacheParam param;
      if ( sceNpManagerGetCachedInfo(CELL_SYSUTIL_USERID_CURRENT, &param) == SCE_OK)
      {
         sUserNPIDValid = true;
         sUserNPID = param.npId;
      }
      else
      {
         // If there's an error getting the ID, then say it's invalid
         // AND zero out the NP ID for good measure.
         sUserNPIDValid = false;
         memset( &sUserNPID, 0, sizeof( sUserNPID ) );
      }

      // NP was already initialized in PS3COSContext
      SCE_CHECK_OK( sceNpManagerRegisterCallback( &myNpServiceStateCallback, NULL ) );
      set_initial_service_state();

      int ret = cellSysmoduleLoadModule(CELL_SYSMODULE_SYSUTIL_USERINFO);

      CellUserInfoUserStat userInfo;
      ret = cellUserInfoGetStat(CELL_SYSUTIL_USERID_CURRENT, &userInfo);
      strcpy(sUserName, userInfo.name);

      ret = cellSysmoduleUnloadModule(CELL_SYSMODULE_SYSUTIL_USERINFO);

      sPsnDialogResult.size = sizeof( sPsnDialogResult );
      sPsnDialogState = kPSN_Uninitialized;
   }
}

void BP_Network_HeartBeat()
{
   SCE_CHECK_OK( cellSysutilCheckCallback() );
}

int BP_Network_IsSignedIn()
{
   return sServiceState == SCE_NP_MANAGER_STATUS_ONLINE;
}

SceNpCommunicationConfig const *BP_Network_GetTUSNPConfig()
{
   return &s_npCommunicationConfig;
}

SceNpId const *BP_Network_GetNPID()
{
   // Return NP ID if the user is signed in, or NULL
   // This is useful to pass NULL to NP functions

   return BP_Network_IsSignedIn() ? ( &sUserNPID ) : NULL;
}

void BP_Network_GetNPIDCached( SceNpId *id, int *pIsValid )
{
   *id = sUserNPID;
   *pIsValid = sUserNPIDValid;
}

int BP_Network_Dialog_BeginPSN()
{
   int ret = 0;
   struct CellNetCtlNetStartDialogParam netstart_param;

   memset( &netstart_param, 0, sizeof( netstart_param ) );
   netstart_param.size = sizeof( netstart_param );
   netstart_param.type = CELL_NET_CTL_NETSTART_TYPE_NP; 

   ret = cellNetCtlNetStartDialogLoadAsync( &netstart_param );

   if( ret != 0 )
   {
      //TODO: Handle error here?
      sPsnDialogState = kPSN_Error;
      ret = 0;
   }
   else
   {
      sPsnDialogState = kPSN_DialogStarted;
      //Fake a handle in case of success.
      ret = skPsnDialogHandleId;
   }
   return ret;
}

int BP_Network_Dialog_IsStillProcessing( int handle )
{
   if( handle == skPsnDialogHandleId
      && ( sPsnDialogState == kPSN_DialogStarted || sPsnDialogState == kPSN_DialogLoaded ) )
   {
      return 1;
   }
   return 0;
}

int BP_Network_Dialog_GetResults( int handle, int *result )
{
   *result = sPsnDialogResult.result;
   if( sPsnDialogState == kPSN_Uninitialized )
   {
      return 1;
   }
   return 0;
}

const char* BP_Network_GetCurrentUserName()
{
   return sUserName;
}

void BP_OnNetDialogFinished( uint64_t status )
{
   int const ret = cellNetCtlNetStartDialogUnloadAsync( &sPsnDialogResult );

   if( ret < 0 || sPsnDialogResult.result != 0 )
   {
      //TODO: Handle error here?
      sPsnDialogState = kPSN_Error;
   }
   else
   {
      sPsnDialogState = kPSN_DialogLoaded;
   }
}

void BP_OnNetDialogUnloaded( uint64_t status )
{
   sPsnDialogState = kPSN_Uninitialized;
}
