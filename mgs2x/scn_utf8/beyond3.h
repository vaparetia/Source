/*
	...
	2001/05/21 H.Yoshiike		 
	$Id: beyond3.h,v 1.14 2001/09/04 23:27:05 usr00725 Exp $				      
...
/*
※このファイルをincludeする際は、srcにrai_beyond.mlsとrai_beyond2.mlsの両方を書く必要があります。
*/
...
// $:強制移動座標２：ビハインド用の移動座標
	...
	proc 北向き柵なしビヨンド発動 $:トラップ名 $:手すり位置 $:プレイヤーＸ座標 $:トラップ左端 $:トラップ右端 $:床の高さ $:フラグ {
		// 通常状態からのビヨンド
		//ビヨンドモーション開始座標に移動
		if ($:プレイヤーＸ座標 < $:トラップ左端+d:ADJ_DIS) {
			mesg プレイヤー d:PLAYER motion \
				d:モーションリスト:non_drop_start_nofence \	//モーション番号
				2048 \	//初期方向
				-1 \	//終了方向
				4 \	//終了姿勢（４はビヨンド）
		...
		command エルード設定 -values \
			...
			d:モーションリスト:non_drop1hand_end2 \		// 戻りモーション
			6 \											// 床チェックする関節（ここでは右腕）
			0 \											// 開始時向き
			0 \											// 終了時向き
			($:フラグ | d:BY_NO_RETURNSTAMP) \			// フラグ 戻り時踏み付け攻撃なし
			d:モーションリスト:non_drop1hand_end2 \		// 戻りモーション（壁有り
			14 \										// 戻り時に壁チェックする関節番号
	...
	proc 南向き柵なしビヨンド発動 $:トラップ名 $:手すり位置 $:プレイヤーＸ座標 $:トラップ左端 $:トラップ右端 $:床の高さ $:フラグ {
		// 通常状態からのビヨンド
		//ビヨンドモーション開始座標に移動
		if ($:プレイヤーＸ座標 < $:トラップ左端+d:ADJ_DIS) {
			mesg プレイヤー d:PLAYER motion \
	...
	proc 東向き柵なしビヨンド発動 $:トラップ名 $:手すり位置 $:プレイヤーＺ座標 $:トラップ上端 $:トラップ下端 $:床の高さ $:フラグ {
		// 通常状態からのビヨンド
		//ビヨンドモーション開始座標に移動
	...
	proc 西向き柵なしビヨンド発動 $:トラップ名 $:手すり位置 $:プレイヤーＺ座標 $:トラップ上端 $:トラップ下端 $:床の高さ $:フラグ {
		// 通常状態からのビヨンド
		//ビヨンドモーション開始座標に移動
		if ($:プレイヤーＺ座標 < $:トラップ上端+d:ADJ_DIS) {
			mesg プレイヤー d:PLAYER motion \
...
// ----------------------------------------------------
// 利用上の注意：
// 必ずＡ点Ｂ点の指定は、プレイヤーから見て時計回りに座標を指定してください。
// 外積により内外チェックを施しているためです。松花 2001.6.6
// -----------------------------------------------------
	proc 斜め柵無しビヨンド発動 $:トラップ名 $:プレイヤーＸ位置 $:プレイヤーＺ位置 $:手すりＡ点Ｘ座標 $:手すりＡ点Ｚ座標 $:手すりＢ点Ｘ座標 $:手すりＢ点Ｚ座標 $:床の高さ $:フラグ {
		...
		eval( $w:デルタＸ座標３ = $:手すりＡ点Ｘ座標 - $:手すりＢ点Ｘ座標  ) ;
		eval( $w:デルタＺ座標３ = $:手すりＡ点Ｚ座標 - $:手すりＢ点Ｚ座標  ) ;
		eval( $w:垂線角度 = ((`command atan $w:デルタＺ座標３ $w:デルタＸ座標３` - 1024 ) & 4095 )) ;
		...
		eval( $i:v1x = $:手すりＢ点Ｘ座標-$:手すりＡ点Ｘ座標);
		eval( $i:v1y = $:手すりＢ点Ｚ座標-$:手すりＡ点Ｚ座標);
		eval( $i:v2x = $:プレイヤーＸ位置-$:手すりＡ点Ｘ座標);
		eval( $i:v2y = $:プレイヤーＺ位置-$:手すりＡ点Ｚ座標);
		if ((($i:v1x * $i:v2y) - ($i:v1y * $i:v2x)) > 0) {
			// 正
			return -1;
		} else if ((($i:v1x * $i:v2y) - ($i:v1y * $i:v2x)) < 0) {
			// 負 (エリア外へ出ました)
		} else {
			// 線上
			return  0;
...
print 'beyond3.h double include!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
