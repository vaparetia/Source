/*
	...
	$Id: paddemo.h,v 1.16 2002/06/25 09:13:25 usr03682 Exp $                      
...
proc パッドデモ開始 $:p_デモファイル名  $:p_表示フラグ {
	#if d:DEBUG_PRINT 
	...
	if (($:p_表示フラグ & d:MENU_ON) == 1) {
		command メニュー設定 -menu on
	}

	if (($:p_表示フラグ & d:GAGE_ON) == 1) {
		command メニュー設定 -gage on
	}

	if (($:p_表示フラグ & d:RADAR_ON) == 1) {
		command メニュー設定 -radar on
	}

	if (($:p_表示フラグ & d:SUBWIN_ON) == 1) {
...
// キャンセルありバージョン
// 終了時姿勢 ： ０／１／２／３ ： 立ち／しゃがみ／匍匐／イントルード
proc パッドデモ開始（キャンセルあり） $:p_デモファイル名 $:p_表示フラグ $:p_終了時位置Ｘ $:p_終了時位置Ｙ $:p_終了時位置Ｚ $:p_終了時方向 $:p_終了時姿勢 {
	#if d:DEBUG_PRINT 
		print 'pad_demo_start'
	#endif
