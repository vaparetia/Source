/*
	...
	$Id: boss_stdproc.h,v 1.1 2002/02/01 06:08:31 usr01475 Exp $
...
//-------------------------------------------------------------------------------------
	...
	proc ボスラッシュ開始時キャラ設定 {
		...
		// ボス戦専用ステージキャラ
		chara ボスラッシュ結果 リザルト画面 -n リザルト画面終了時プロック \
			-v	$b:バージョン情報 \
		...
		command ゲームオーバー呼びわけ -c 2			//ボスラッシュ用
		...
		#elifdef d:STAGE_W00B || d:STAGE_W20C || d:STAGE_W25A || d:STAGE_W31C || d:STAGE_W46A || d:STAGE_W61A
		// 元のファイル用のextern宣言
		...
		extern chara ボスラッシュ結果
		extern command ゲームオーバー呼びわけ
		extern chara ボスラッシュテロップ表示
		extern chara ボスタイマー
		...
		extern command ボスラッシュ結果後書き
		extern command ボスラッシュ音声交換
	...
	proc ボスラッシュ終了時処理 {
		...
		if (`command ゲームオーバーチェック` == 0 ) {
			...
			if ( $b:ボスラッシュステージ == d:boss_no:B_OLGA ) {
				...
				// WIN表示
				chara ボスラッシュテロップ表示 テロップ君 -m 2 -n ボスラッシュ終了プロック
				// 気絶で倒したかどうかチェック
				...
				// WIN表示
				chara ボスラッシュテロップ表示 テロップ君 -m 3 -n ボスラッシュ終了プロック
				// 気絶で倒したかどうかチェック
				if ( $w:殺傷人数 == $w:ボスラッシュキルカウント ) {
				...
				command ボスラッシュ音声交換 -c t:vc030120 
			}
	...
	proc テロップ表示 {
			...
			print 'telop_start!!!!!'
		#endif
		chara ボスラッシュテロップ表示 テロップ君 -m 1 -n ゲームスタート
	}
...
#define	リスタートしたらオルガ戦	1
	...
	proc ボスラッシュ終了プロック {
		...
		if ( $b:ボスラッシュステージ <= d:boss_no:B_RAY ) {
			...
			if ( $s:ボスラッシュプレイヤー == スネーク ) {
				chara ボスラッシュテロップ表示 テロップ君 -m 0 -b $b:ボスラッシュステージ -n 次のステージをロード -p 0
			} else {
				chara ボスラッシュテロップ表示 テロップ君 -m 0 -b $b:ボスラッシュステージ -n 次のステージをロード -p 1
			}
