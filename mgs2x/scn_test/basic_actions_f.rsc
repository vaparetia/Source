/*
	...
	$Id: basic_actions_f.rsc,v 1.1 2002/02/01 06:08:31 usr01475 Exp $
...
//		'ぱなしにすれば敵を引きずって移動することもできる。'
	...
	中級３説明:
		{
			En Vue en coin, actionnez la touche L2/R2
			pour jeter un coup d'&@eil derri&`ere le coin.
			L'ennemi vous verra peut-&^etre vous pencher
			sur le c&^ot&'e, attention !
		}
...
//		'ところを敵に見つからないように注意が必要。'
	...
	中級４説明:
		{
			En Observation &`a la premi&`ere personne,
			actionnez la touche L2/R2 pour vous d&'eplacer
			lat&'eralement. Actionnez ensemble les touches
			L2 et R2 pour vous &'etirer.
		}
	中級４説明_1:
		{
			Cette man&@euvre est utile lors des fusillades
			o&`u les protagonistes se cachent derri&`ere
			des objets.
		}
...
//		'回避モードや危険モードでは効果がない。'
	中級８説明:
		{
			Pour d&'es&'electionner les armes ou objets dont
			vous &^etes &'equip&'e(e), actionnez rapidement la
			touche L2 ou R2. Pour les r&'ecup&'erer, actionnez
			une nouvelle fois cette touche.
		}
	中級８説明_1:
		{
			Cette man&@euvre est utile avec
			les &'equipements d'Observation
			&`a la premi&`ere personne comme la lunette.
		}
/*
...
//		'その間に逃げることも可能。'
	...
	上級４説明:
		{
			Pour viser, avec votre pistolet, actionnez la
			touche Arme. Simultan&'ement, actionnez la
			touche L1 pour vous d&'eplacer tout en visant.
			Il est &'egalement possible de tirer en bougeant.
		}
