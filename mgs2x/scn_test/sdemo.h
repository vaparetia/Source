/*
	...
	$Id: sdemo.h,v 1.1 2002/02/01 06:08:32 usr01475 Exp $
...
proc シナリオカメラデモ $:時間 $:PX1 $:PY1 $:PZ1 $:TX1 $:TY1 $:TZ1 $:AGL1 $:PX2 $:PY2 $:PZ2 $:TX2 $:TY2 $:TZ2 $:AGL2 {
...
proc シナリオカメラデモ（シネマ） $:時間 $:PX1 $:PY1 $:PZ1 $:TX1 $:TY1 $:TZ1 $:AGL1 $:PX2 $:PY2 $:PZ2 $:TX2 $:TY2 $:TZ2 $:AGL2 {
	if ( $f:シナリオカメラデモ起動中 == 0  ) {		// ワークの処理の関係で多重呼び出しには対応していません。
		eval( $f:シナリオカメラデモ起動中 = 1 ) ;
		eval( $w:時間 = $:時間 ) ;
		eval( $i:PX1 = $:PX1 ) ;
