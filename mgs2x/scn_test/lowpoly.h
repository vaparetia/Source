/*
	...
	$Id: lowpoly.h,v 1.1 2002/02/01 06:08:31 usr01475 Exp $
...
proc キャストセレクト共通 {
	//	ビジネスマン
	if ($1 == d:BUS) {
//		eval ($s:新キャスト[ $2 ] = cit_maley_ct_sh)
...
proc キャスト代入ＥＶＭ（リネーム） {
	...
	//	ビジネスマン／ＯＬ／おばちゃん／ハイテク兵はＫＭＳのみ
		...
		//	ビジネスマンに
		if (	($s:新キャスト[ $1 ] == cit_maley_ct_sh) ||	\
				($s:新キャスト[ $1 ] == cit_maley_ctg_sh)	)	{
			command デモローポリ劇場 \
				-change	$s:元キャスト[ $1 ]	$s:元モデル種類[ $1 ]	\
						$s:新キャスト[ $1 ]	$s:新モデル種類[ $1 ]
...
//#########################################################################################
	...
	if ($:デモ == d:DEMO1) {
		...
		//	オセロットを変更する
			//	スネークはここではsna_def_mh.evmが無いので、sna_def_sh.kmsを使用する
		if ($:新キャスト２ == d:SNA) {
			eval ($s:新キャスト[ 2 ] = sna_def_sh)
			eval ($s:新モデル種類[ 2 ] = kms)
...
//			eval ($s:新キャスト[ 1 ] = for_def_addhand_mh_mt)
...
//			eval ($s:新キャスト[ 2 ] = for_def_addhand_mh_mt)
	...
	//	タンカー編オープニング
		...
		//	オセロット				rev_def_mh_mt.evm
			...
			eval ($s:元モデル種類[ 2 ] = evm)
				...
				//	スネークはsna_def_sh.kmsを使用する
			if ($s:新キャスト[ 2 ] == sna_def_sh) {
					command デモローポリ劇場 \
	...
	//	オルガＶＳスネーク
		...
		//	追加処理		オルガ（指付き）も変更する
			...
			if ($s:新キャスト[ 2 ] != ori_mdl) {
				...
				//	ビジネスマンに
				} else if (	($s:新キャスト[ 2 ] == cit_maley_ct_sh) ||	\
							($s:新キャスト[ 2 ] == cit_maley_ctg_sh)	)	{
					command デモローポリ劇場 \
	...
	//	ヴァンプ遭遇
		...
		//	追加処理		プリスキン（腕に血）も変更する
			...
			if ($s:新キャスト[ 4 ] != ori_mdl) {
				//	ビジネスマンに
				if (	($s:新キャスト[ 4 ] == cit_maley_ct_sh) ||	\
						($s:新キャスト[ 4 ] == cit_maley_ctg_sh)	)	{
					command デモローポリ劇場 \
...
//#########################################################################################
		...
		r_plt6
			...
			ソリダス
			sol_def_mh_mt.evm
			sol_gantai_mh_mt.evm

			フォーチュン
			for_def_sh_mt.kms
			for_def_mh_mt.evm
//			for_def_addhand_mh_mt.evm
