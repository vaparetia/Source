/*
	...
	$Id: tales_comment_e.rsc,v 1.8 2002/08/27 03:08:46 usr03005 Exp $
...
the Colonel, Ames is in Strut F.
EOF
...
And according to the Colonel, there are 
hostages tied up on Level B1, Shell 1 Core. 
Which site I choose is up to my judgment.
EOF
...
Fatman selected me as his next target. 
That massive body of his -- I have no 
...
*/
A_人質部屋内＿マイクなし: << EOF
I arrived at the room with the hostages. The 
Colonel says that I need a directional mic to 
identify Jennifer. I guess I'll have to find a 
...
hostages from moving. Fatman is probably 
on the heliport.
EOF
...
*/
A_爆弾無線後ファットマン前: << EOF
I have defused the explosives. The hostages 
are all safe now. The other half of the 
mission is suppressing the terrorist. Fatman 
is probably on the heliport.
EOF
...
has selected me as his next target. 
That massive body of his -- I have no 
choice but to take him out.
EOF
...
of hostages tied up in Shell 2. Emma and I 
decided to go from Strut D to Shell 2 to save 
the hostages.
EOF
...
*/
B_Ｅ脚脱出中: << EOF
We found where the drugs had been hidden. 
But then in rushed a large enemy unit. 
I WILL get past the surrounding enemies and 
escape to the DE connecting bridge together 
with Emma.
EOF
...
enemies inside Strut E. The two of us headed 
for Shell 2 to rescue the hostages. Then 
appeared a Harrier right in front of us. We 
have no choice but to bring the Harrier down.
EOF
...
I cannot let Gurlukovich take Metal Gear. 
According to Meryl, Metal Gear is in the hold.
EOF
...
*/
C_長廊下戦中: << EOF
As I headed for the hold, I fell into an enemy 
...
*/
C_長廊下脱出中: << EOF
I managed to escape the hold, but the 
enemies are still after me. I have to get 
out of Deck 2. Maybe if I go back to the 
engine room there will be a way out.
EOF
...
*/
C_長廊下脱出後メリル戦前: << EOF
The Colonel says that Gurlukovich might join 
up with his people and get a hold of Metal 
Gear. I must get to the bridge before that 
happens and change the ship's course.
EOF
...
intentions of her late father. As I expressed 
my doubts toward the facts Gurlukovich spoke 
...
Vamp's next target is O'Brien, who is on 
the first floor of the Shell 2 Core. I head for 
Strut D to get to Shell 2.
EOF
...
ＸＸＸＸＸＸＸＸｘ長い！ＸＸＸＸＸｘ
According to the last words of the SEAL, 
Vamp's next target is O'Brien, who is on the 
first floor of the Shell 2 Core.  The door from 
Strut D to Shell 2 is shut. The only way now 
is to cross the oil fence on the water. The 
ladder to the oil fence is on Level B1, Strut E.
*/
D_Ｄ脚後Ｅ脚前: << EOF
...
*/
D_Ｅ脚後シェル２中央棟１Ｆ前: << EOF
I arrived at Shell 2. Vamp has already 
entered in Shell 2. O'Brien is in danger. 
O'Brien must still be on the first floor in 
the Shell 2 Core.
EOF
...
and destroy the power source in the air 
purification room.
EOF
...
a mysterious monster.  Supposedly, Gurlugon 
has been spotted many times near the AB 
connecting bridge.
EOF
...
*/
E_回想Ｃ脚前: << EOF
I have come to Big Shell to look for 
Gurlugon. I must find Gurlugon.
EOF
...
E_回想Ｃ脚後ゴルルゴン登場前: << EOF
I have come to Big Shell to look for 
Gurlugon. I must find Gurlugon. According to 
Otacon, Gurlugon is often spotted near the 
...
*/
E_敵兵ラッシュ中: << EOF
After I took a photo, I was attacked by an 
armed group. Who the hell are they? Now 
is no time for questions. I'll have to take 
them out first.
EOF
...
*/
E_ＲＡＹ戦中: << EOF
When I arrived at the singularity, a bunch 
of RAYs sent by Solidus appeared in front 
of me. Otacon needs time to complete his 
operation. I have no choice but to fight these 
RAYs.
EOF
...
*/
E_ソリダス戦中: << EOF
The universe is now being repaired. As I 
wait for my moment of return, Solidus stands 
in front of me to challenge me to the final 
battle. He no longer cares to listen to reason.
EOF
