/*
	ファイル名		a_stagename_j.h
	作成日			2002/07/04
	作者			S.Yamashita
	説明			AlternativeMissions用ステージ名日本語版リソース
	$Id: a_stagename_j.h,v 1.7 2002/09/24 05:56:15 usr03700 Exp $
*/

/*
 * 注意：
 * 注意：リソースの順番を変えると「chara ＶＲ全体マップ表示３Ｄ」に影響が出ます！！！
 * 注意：間に追加などをする場合はプログラムの対応作業が必要になります。
 * 注意：
 */

resource ステージ名日本語 {
	0:
	'船尾甲板',
	1:
	'船橋一階居住区',
	2:
	'船橋一階リフレッシュルーム',
	3:
	'船橋二階居住区',
	4:
	'船橋三階居住区',
	5:
	'船橋四階居住区',
	6:
	'船橋五階操舵室',
	7:
	'航海甲板ウィング',
	8:
	'機関室',
	9:
	'第二甲板右舷',
	10:
	'第二甲板左舷',
	11:
	'第一船倉',
	12:
	'第二船倉',
	13:
	'第三船倉',
	14:
	'Ａ脚底部海底ドック',
	15:
	'Ａ脚屋上',
	16:
	'Ａ脚ポンプ室',
	17:
	'ＡＢ連絡橋',
	18:
	'Ｂ脚変電室',
	19:
	'ＢＣ連絡橋',
	20:
	'Ｃ脚食堂',
	21:
	'ＣＤ連絡橋',
	22:
	'Ｄ脚第一沈殿池',
	23:
	'ＤＥ連絡橋',
	24:
	'Ｅ脚集配場',
	25:
	'Ｅ脚ヘリポート',
	26:
	'ＥＦ連絡橋',
	27:
	'Ｆ脚倉庫',
	28:
	'ＦＡ連絡橋',
	29:
	'シェル１中央棟１Ｆ',
	30:
	'シェル１中央棟Ｂ１',
	31:
	'シェル１中央棟Ｂ１集会場',
	32:
	'シェル１中央棟Ｂ２電算室',
	33:
	// 'ＤＧ連絡橋',
	'シェル１シェル２連絡橋',
	34:
	// 'ＤＧ連絡橋〜ＬＧ連絡橋',
	'シェル１シェル２連絡橋〜ＬＧ連絡橋',
	35:
	'Ｌ脚外周〜ＫＬ連絡橋',
	36:
	'ＫＬ連絡橋',
	37:
	'Ｌ脚汚水処理場',
	38:
	'シェル２中央棟１Ｆ空気清浄室',
	39:
	'シェル２中央棟Ｂ１第一ろ過室',
	40:
	'シェル２中央棟Ｂ１第二ろ過室',
	41:
	'Ｌ脚下部オイルフェンス',
	42:
	'アーセナルギア胃',
	43:
	'アーセナルギア空腸',
	44:
	'アーセナルギア上行結腸',
	45:
	'アーセナルギア回腸',
	46:
	'アーセナルギアＳ状結腸',
	47:
	'アーセナルギア直腸',
	48:
	'フェデラルホール',
	49:
	'プラント編スタート',
	49A:
	'タンカー編クリア',
	49B:
	'プラント編クリア',
	49C:
	'スネークテイルズＡクリア',
	49D:
	'スネークテイルズＢクリア',
	49E:
	'スネークテイルズＣクリア',
	49F:
	'スネークテイルズＤクリア',
	49G:
	'スネークテイルズＥクリア',
	49H:
	'フィランソロピーのアジト',		
	49I:		
	'並行宇宙の狭間',		
	49J:
	'雷電テイルズ'
	49K:
	'特異点'
	49L:
	'-'	//予備 		
			

	  /* 全体マップ用ステージ名 */
	50:
      '船尾甲板',
	51:
	  '船橋一階居住区',
	52:
	  '船橋一階リフレッシュルーム',
	53:
	  '船橋二階居住区',
	54:
	  '船橋三階居住区',
	55:
	  '船橋四階居住区',
	56:
	  '船橋五階操舵室',
	57:
	  '航海甲板ウィング',
	58:
	  '機関室',
	59:
	  '第二甲板',
	60:
	  '第一船倉',
	61:
	  '第二船倉',
	62:
	  '第三船倉',
	/* プラント */
	63:
	  'Ａ脚ポンプ施設'
	64:
	  'ＡＢ連絡橋'
	65:
	  'Ｂ脚変電室'
	66:
	  'ＢＣ連絡橋'
	67:
	  'Ｃ脚食堂'
	68:
	  'ＣＤ連絡橋'
	69:
	  'Ｄ脚第一沈殿池'
	70:
	  'ＤＥ連絡橋'
	71:
	  'Ｅ脚貨物集配施設'
	72:
	  'ＥＦ連絡橋'
	73:
	  'Ｆ脚倉庫'
	74:
	  'ＦＡ連絡橋'
	75:
	  'シェル１中央棟'
	76:
	  'シェル１シェル２連絡橋'
	77:
	  'Ｇ脚油処理施設'
	78:
	  'ＧＨ連絡橋'
	79:
	  'Ｈ脚倉庫'
	80:
	  'ＨＩ連絡橋'
	81:
	  'Ｉ脚機材組立施設'
	82:
	  'ＩＪ連絡橋'
	83:
	  'Ｊ脚発電施設'
	84:
	  'ＪＫ連絡橋'
	85:
	  'Ｋ脚生物化学実験施設'
	86:
	  'ＫＬ連絡橋'
	87:
	  'Ｌ脚汚水処理場'
	88:
	  'ＬＧ連絡橋'
	89:
	  'シェル２中央棟'
	90:
	  'Ｌ脚下部オイルフェンス'
	91:
	  'Ｈ脚下部オイルフェンス'
	92:
	  'アーセナルギア胃'
	93:
	  'アーセナルギア空腸'
	94:
	  'アーセナルギア上行結腸'
	95:
	  'アーセナルギア回腸'
	96:
	  'アーセナルギアＳ状結腸'
	97:
	  'アーセナルギア直腸'
	98:
	  '麻布十番'
	99:
	  '船橋市'
	99A:
	  '青山二丁目'
	99B:
	  'ニューヨーク52番街'
	100:
	  'Strut A Pump Facility'
	101:
	  'AB connecting bridge'
	102:
	  'Strut B Transformer Room'
	103:
	  'BC connecting bridge'
	104:
	  'Strut C Dining Hall'
	105:
	  'CD connecting bridge'
	106:
	  'Strut D Sediment Pool'
	107:
	  'DE connecting bridge'
	108:
	  'Strut E Material Distribution Facility'
	109:
	  'EF connecting bridge'
	110:
	  'Strut F warehouse'
	111:
	  'FA connecting bridge'
	112:
	  'Shell 1 core'
	113:
	  'Shell 1 - 2 connecting bridge'
	114:
	  'Strut G Oil Processing Facility'
	115:
	  'GH connecting bridge'
	116:
	  'Strut H Warehouse'
	117:
	  'HI connecting bridge'
	118:
	  'Strut I Assembly Facility'
	119:
	  'IJ connecting bridge'
	120:
	  'Strut J Power Plant'
	121:
	  'JK connecting bridge'
	122:
	  'Strut K Biochem Lab'
	123:
	  'KL connecting bridge'
	124:
	  'Strut L Sewage Treatment Facility'
	125:
	  'LG connecting bridge'
	126:
	  'Shell 2 Core'
	127:
	  'Strut L Oil Fence'
	128:
	  'Strut H Oil Fence'
	129:
	  'Arsenal Gear-Stomach'
	130:
	  'Arsenal Gear-Jejunum'
	131:
	  'Arsenal Gear-Ascending Colon'
	132:
	  'Arsenal Gear-Ileum'
	133:
	  'Arsenal Gear-Sigmoid Colon'
	134:
	  'Arsenal Gear-Rectum'
	135:
	  'Azabu-Juban'
	136:
	  'Hunabashi City'
	137:
	  'Aoyama Ni-Chome'
	138:
	  'New York 52nd Street'
	139:
		'Aft deck',
	140:
		{Deck-A, crew's quarters},
	141:
		{Deck-A, crew's lounge},
	142:
		{Deck-B, crew's quarters},
	143:
		{Deck-C, crew's quarters},
	144:
		{Deck-D, crew's quarters},
	145:
		'Deck-E, the bridge',
	146:
		'Navigational deck,wing',
	147:
		'Engine room',
	148:
		'Deck 2, starboard',
	149:
		'Deck 2, port',
	150:
		'Hold No.1',
	151:
		'Hold No.2',
	152:
		'Hold No.3',
	59A:
	  '第二甲板右舷',
	59B:
	  '第二甲板左舷',

	/* これ以降バグマップ */
};

/*
resource 全体マップ用ステージ名 {
	0:
	Aft deck'
	1:
	Deck-A, crew's quarters'
	2:
	Deck-A, crew lounge
	3:
	Deck-B, crew's quarters
	4:
	Deck-C, crew's quarters
	5:
	Deck-D, crew's quarters
	6:
	Deck-E, the bridge
	7:
	Navigational deck,wing
	8:
	Engine room
	9:
	Deck 2
	10:
	Hold No. 1
	11:
	Hold No. 2
	12:
	Hold No. 3
};
*/

resource 難易度名日本語 {
	easy:
	'EASY',
	normal:
	'NORMAL',
	hard:
	'HARD',
	vhard:
	'VERY HARD',
	extream:
	'EXTREME',
};
