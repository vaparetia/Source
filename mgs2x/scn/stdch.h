/*
	...
	$Id: stdch.h,v 1.135 2002/09/18 13:38:26 usr03682 Exp $
	...
	if( $s:d_num == not_demo ) {
		// 薬莢管理
			...
			eval( $i:ＡＫＳ薬莢数 = 32 );			// ライデン、タンカーゴル兵
				...
				eval( $i:ＵＳＰ薬莢数 = 32 );		// 盾兵
			#endif
			#ifdef d:STAGE_PLANT || d:STAGE_VR
		...
		// ゲームオーバー処理
		#ifdef d:STAGE_VR || d:STAGE_ALT
			// ＶＲ用
			command システムコールバック -gameover A_ゲームオーバー処理 -continue A_コンティニュー処理
		#else
			// 本編用
			...
			command ゲームオーバー背景設定 -m 0
		#endif
		#ifdef d:STAGE_VR || d:STAGE_ALT
			// ＶＲとオルタナティブミッションの時はこのゲームオーバー
			command ゲームオーバー呼びわけ -c 3
		#endif

		#ifdef d:STAGE_VR || d:STAGE_ALT
			// アイテムの取得位置を変更していた人がいたら困るので、これもデフォルトに戻す
			command アイテム収得可能範囲設定 300 1500 300
		#endif
...
*/
		...
		// ノード端末
			...
			// レーダー（オンオフに関わらずアラート表示を行うため必要）
				...
				chara レーダー レーダー -d 341
			#elifdef d:STAGE_A61A || d:STAGE_W61A
			// ソリダス戦は危険ゲージを出したくないのでレーダーを起動しない 2002.09.17 吉池
			#else
				chara レーダー レーダー
			#endif
		#endif
