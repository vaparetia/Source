...
/*
	quat2.c
	クォータニオン処理ルーチン（汎用低速ルーチン）

	1999/07/07 K.Takabe
	$Id: quat2.c,v 1.1.1.3 2002/11/19 11:42:53 Yoshizawa1 Exp $
...
void MT_EulerToQuatXYZ( FVECTOR *quat, FVECTOR *rot)
{
   ...
   BP_SinCos(&sin_cos, 0.5f*rot->vx);
   sr = sin_cos.vx;
   cr = sin_cos.vy;
   BP_SinCos(&sin_cos, 0.5f*rot->vy);
   sp = sin_cos.vx;
   cp = sin_cos.vy;
   BP_SinCos(&sin_cos, 0.5f*rot->vz);
   sy = sin_cos.vx;
   cy = sin_cos.vy;
...
void MT_MatToQuat( FVECTOR *quat, FMATRIX *mat )
{
	...
	/* 符号チェック */
	if ( tr > 0.0F ){
		s = bp_sqrtf( tr + 1.0F ); //BP_MATH - emulate PS2 sqrtf
		...
		s = bp_sqrtf( ( mat->m[i][i] - ( mat->m[j][j] + mat->m[k][k] ) ) + 1.0F ); //BP_MATH - emulate PS2 sqrtf
...
void MT_QuatSlerp( FVECTOR *res, FVECTOR *from, FVECTOR *to, float t )
{
	...
	if ( cosom < 1.0f - DELTA )
   {
		...
		sinom = sinf( omega );
#else
		sinom = sqrtf( 1.0F - cosom * cosom ); //BP_MATH - no need to emulate PS2 sqrtf, given the conditions above
...
void MT_QuatNormalize( FVECTOR *res, FVECTOR *quat )
{
	...
	if ( square > 0.00001F ){
		dist = (float)( 1.0F / bp_sqrtf( square ) ); //BP_MATH - emulate PS2 sqrtf
		res->vx = quat->vx * dist ;
		res->vy = quat->vy * dist ;
		res->vz = quat->vz * dist ;
		res->vw = quat->vw * dist ;
...
void MT_QuatGetValue( FVECTOR *res, FVECTOR *quat )
{
    ...
    if (len > DELTA){
		len = DG_RSQRT( len );	/* DG_RSQRT() == 1.0f / sqrt() */
		res->vx = tx * len ;
		res->vy = ty * len ;
		res->vz = tz * len ;
	    res->vw = (float)( 2.0F * acosf( tw ) );
    } else {
		res->vx = 0.0F ;
		res->vy = 0.0F ;
		res->vz = 1.0F ;
...
void MT_QuatSetValue( FVECTOR *quat, FVECTOR *param )
{
	...
	quat->vw = MA_Cos( param->vw / 2.0F );
	dist = bp_sqrtf( ( 1.0f - quat->vw * quat->vw ) / temp ) ;  //BP_MATH - emulate PS2 sqrtf
...
void MT_QuatSetFromAx( FVECTOR *quat, FVECTOR *from, FVECTOR *to )
{
	...
	if ( cost > 0.99999f ){
		...
		ty = from->vx ;
		tz = -from->vy ;
		len = bp_sqrtf( ty * ty + tz * tz );   //BP_MATH - emulate PS2 sqrtf
		if ( len < DELTA ){
...
#ifndef PSX2
		if ( temp == 0.0f )temp = 0.0001f ; // Added by T.Morita 2002.03.04
#endif
		dist = (float)( 1.0F / bp_sqrtf( temp ) ) ;  //BP_MATH - emulate PS2 sqrtf
		tx *= dist ;
		ty *= dist ;
		tz *= dist ;
	...
	ss = (float)bp_sqrtf( 0.5F * ( 1.0F - cost ) ); //BP_MATH - emulate PS2 sqrtf
	tx *= ss;
	ty *= ss;
	tz *= ss;
	...
	quat->vw = (float)bp_sqrtf( 0.5F * ( 1.0F + cost ) ); //BP_MATH - emulate PS2 sqrtf
...
void MT_QuatSqrt( FVECTOR *res, FVECTOR *q1 )
{
	...
	length = bp_sqrtf( q1->vw * q1->vw + q1->vx * q1->vx + q1->vy * q1->vy );  //BP_MATH - emulate PS2 sqrtf
	if ( length != 0.0F ){
		length = 1.0F / length ;
	} else {
		length = 1.0F ;
	...
	m = 1.0F / bp_sqrtf( r.vw * r.vw + r.vx * r.vx );  //BP_MATH - emulate PS2 sqrtf
	r1 = bp_sqrtf( ( 1.0F + r.vy ) * 0.5F );  //BP_MATH - emulate PS2 sqrtf
	r2 = bp_sqrtf( ( 1.0F - r.vy ) * 0.5F );  //BP_MATH - emulate PS2 sqrtf

	res->vx = bp_sqrtf(length) * r2 * r.vx * m ; //BP_MATH - emulate PS2 sqrtf
	res->vy = bp_sqrtf(length) * r1 ;            //BP_MATH - emulate PS2 sqrtf
	res->vz = q1->vz ;
	res->vw = bp_sqrtf(length) * r1 * r.vw * m ; //BP_MATH - emulate PS2 sqrtf
...
float MT_QuatLength( FVECTOR *q1 )
{
	return bp_sqrtf( q1->vw * q1->vw + q1->vx * q1->vx + q1->vy * q1->vy + q1->vz * q1->vz ); //BP_MATH - emulate PS2 sqrtf
}
...
void MT_QuatExp( FVECTOR *res, FVECTOR *q1 )
{
	...
	len1 = (float) bp_sqrtf( q1->vx * q1->vx + q1->vy * q1->vy + q1->vz * q1->vz );  //BP_MATH - emulate PS2 sqrtf
	if ( len1 > 0.0F ){
		len2 = BP_Sin( len1 ) / len1; 
	} else {
		len2 = 1.0F ;
	}
...
void MT_QuatLog( FVECTOR *res, FVECTOR *q1 )
{
	...
	length = bp_sqrtf( q1->vx * q1->vx + q1->vy * q1->vy + q1->vz * q1->vz );  //BP_MATH - emulate PS2 sqrtf
...
void MT_QuatLnDif( FVECTOR *res, FVECTOR *q1, FVECTOR *q2 )
{
	...
	len = bp_sqrtf( dif.vx * dif.vx + dif.vy * dif.vy + dif.vz * dif.vz );  //BP_MATH - emulate PS2 sqrtf
	s = dif.vw ;
...
/* クオータニオン分解
   あるクオータニオンを、任意のクオータニオンとそれを含まないクオータニオンに
   分解する。
   計算 :
     クオータニオンqを、クオータニオンrとクオータニオンnに分解するものとする。
     q = rn
     q n^-1 = r

     YqZn - ZqYn + WqXn + WnXq = Xr
     ZqXn - XqZn + WqYn + WnYq = Yr
     XqYn - YqXn + WqZn + WnZq = Zr

     また問題より、rとnは垂直であるから、r・n = 0
     XrXn + YrYn + ZrZn = 0

     (YqZn - ZqYn + WqXn + WnXq)Xn
      + (ZqXn - XqZn + WqYn + WnYq)Yn
      + (XqYn - YqXn + WqZn + WnZq)Zn = 0

     Wq(Xn^2 + Yn^2 + Zn^2) + Wn(XqXn + YqYn + ZqZn) = 0

     Wn = Wq(Xn^2 + Yn^2 + Zn^2)/(XqXn + YqYn + ZqZn)

     (Xn^2 + Yn^2 + Zn^2) = (1-Wn^2)(Xnn^2 + Ynn^2 + Znn^2) = (1-Wn^2)
     (XqXn + YqYn + ZqZn) = (XqXnn + YqYnn + ZqZnn) * sqrt(1-Wn^2) とおくと、
     Wn = Wq(1-Wn^2)/((XqXnn + YqYnn + ZqZnn) * sqrt(1-Wn^2))
     Wn^2 = (Wq/(XqXnn + YqYnn + ZqZnn))^2 * (1-Wn^2)
     (1+(Wq/(XqXnn + YqYnn + ZqZnn))^2)Wn^2 = (Wq/(XqXnn + YqYnn + ZqZnn))^2
     (((XqXnn + YqYnn + ZqZnn)^2 + (Wq^2))/(XqXnn + YqYnn + ZqZnn)^2)Wn^2
        = Wq^2/(XqXnn + YqYnn + ZqZnn)^2
     Wn^2 = Wq^2/((XqXnn + YqYnn + ZqZnn)^2 + (Wq^2))

     Wn   = Wq/sqrt((XqXnn + YqYnn + ZqZnn)^2 + (Wq^2)) */

#undef DELTA
#undef FLT_EPSILON
#define FLT_EPSILON	(1.19209290E-07F)
#define DELTA	(FLT_EPSILON*FLT_EPSILON)

static inline void MT_QuatSeparateSub(FVECTOR *s,FVECTOR *n)
{
    float x,p,y;

    x=s->vx*n->vx+s->vy*n->vy+s->vz*n->vz;
	...
	if(x<s->vw){
	    y/=s->vw*s->vw;
	    p=1.0f/bp_sqrtf(y); //BP_MATH - emulate PS2 sqrtf
	    n->vw=p;
	    p*=x/s->vw;
	}
	else{
...
/* クオータニオンsを、rとnに分解する。
   ...
   Input  : s , n.v
   Output : r , n.w
   |s| = 1.0 , |n.v| = 1.0である必要あり。*/

void MT_QuatSeparate(FVECTOR *s,FVECTOR *r,FVECTOR *n)
{
    FVECTOR tmp;
    MT_QuatSeparateSub(s,n);
    MT_QuatInverse(&tmp,n);
    MT_QuatMul(r,s,&tmp);
}
...
void MT_QuatSeparate2(FVECTOR *s,FVECTOR *n,FVECTOR *r)
{
    FVECTOR tmp;
    MT_QuatSeparateSub(s,n);
    MT_QuatInverse(&tmp,n);
