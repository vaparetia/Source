/*
	...
	1999/05/28 K.Uehara
	$Id: stageld.c,v 1.1.1.3 2002/11/19 11:42:40 Yoshizawa1 Exp $
*/
	  ...
	  case INIT_DAR :
		...
		*/
		...
		do {
			...
			buf += ( 3 & ( 4 - ( 3 & (u_int)buf ) ) ) ;
			size = *( ( (u_int *)buf ++ ) ) ;
			buf += ( 15 & ( 16 - ( 15 & (u_int)buf ) ) ) ;
			/*
				返り値が０の場合は次のターンにリトライする
			*/
			id = GV_CacheID3( work->load_name );
printf( "[%s] : size %d id %08x addr %x \n", work->load_name, size, id, buf ) ;
#if 1
			stat = LoadInit( buf, id, work, size ) ;
			if ( stat == 0 ) return 1;
			if ( stat < 0 ) {
				/* エラー */
...
void *FS_LoadStageRequest( char *dirname )
{
	...
	info = GV_MallocLoad( sizeof( FS_STAGE_INFO ) );
	...
	info->load_conf = GV_MallocLoad( size + 1 );
