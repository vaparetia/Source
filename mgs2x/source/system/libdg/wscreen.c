/*
	...
	1999/07/07 K.Takabe
	$Id: wscreen.c,v 1.7 2003/01/05 04:46:33 takaki Exp $
...
void		DG_ScreenChanl( DG_CHANL *cp, int which )
{
...
#ifndef _WINDOWS
    unsigned int    flag;
    unsigned int    invisible_flag;
#endif

	DG_PerfStartD(DGPERF_CTGR_SCREEN) ;
	...
	*EYE_PERS = cp->eye_pers ;
...
#ifndef _WINDOWS
		flag = objs->flag ;
		if ( ( flag & invisible_flag ) || ( objs->bound_mode == 2 ) ){
	...
	DG_PerfEndD(DGPERF_CTGR_SCREEN) ;
}
