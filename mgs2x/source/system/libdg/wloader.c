/*
	...
	2002/03/12 K.Takabe
	$Id: wloader.c,v 1.9 2003/01/05 04:46:33 takaki Exp $
...
static void KmsMakePrePacket( DG_DEF *def )
{
	...
	int		def_n_x_models, mdl_n_packs ; 

	ASSERT(0) ;	// ここを通る事が有るのか調べるためのTrap(02/09/11 Takaki Eiji)
	...
	def_n_x_models = (int)def->n_x_models ;
	mdl = def->models ;
	for ( i = 0 ; i < def_n_x_models ; i++, mdl++ ){
		/* 必要な頂点バッファ量を算出 */
		mdl_n_packs = (int)mdl->n_packs ;
		pack = mdl->packs ;
		total_verts = 0 ;
		for ( j = 0 ; j < mdl_n_packs ; j++, pack++ ){
	...
	for ( i = def->n_x_models ; i > 0 ; i--, mdl++ ){
		...
		for ( j = mdl->n_packs ; j > 0 ; j--, pack++ ){
			...
			if( pack->n_indices )
			{
				...
				idx_ptr = (WORD *)pack->index ;
				for(i=pack->n_indices; i>0; i--, idx_ptr++)
				{
					if( n_verts < *idx_ptr ){ n_verts = *idx_ptr ; }
				}
			}
#endif
		}
		...
		if( n_verts >= 0 )
		{
			int	i ;
			short	wt ;
			BOOL	chk_wt0, chk_wt1 ;

			chk_wt0 = TRUE ;
			chk_wt1 = TRUE ;
			stride = mdl->stride ;
			vert   = (DG_VERTEX_KMSS *)mdl->vbuff ;
			for(i=n_verts+1; i>0; i--)
			{
				wt = 32767 - vert->wt ;	// Weight反転

				if( wt != 0     ){ chk_wt0 = FALSE ; }
				if( wt != 32767 ){ chk_wt1 = FALSE ; }
			...
			if( chk_wt0 ){ mdl->type |= DG_TYPE_REVWT_WT0 ; }
			if( chk_wt1 ){ mdl->type |= DG_TYPE_REVWT_WT1 ; }
		}

		mdl->type |= DG_TYPE_REVWT ;	// 頂点変更済みフラグ
#endif

#ifdef _WINDOWS
		/* Direct3D用 VertexBufferの作成 */
		//DG_MakeDGMdlVertexBuffer(mdl) ;
#endif
	}
}
...
void DG_UnloadKms( void *buf )
{
	...
	DG_MDL	*mdl ;
	int		i ;
	if( def->n_x_models ){ DG_ExeBufferSync(FALSE) ; }	// 安全策
	mdl = def->models ;
	for ( i = 0 ; i < def->n_x_models ; i++, mdl++ ){
