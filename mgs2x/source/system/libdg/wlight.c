/*
	...
	2002/03/13 K.Takabe
	$Id: wlight.c,v 1.4 2002/10/31 02:19:12 takaki Exp $
...
int			DG_LightClock ;
static FVECTOR	max_vec = {99999999.0f,99999999.0f,99999999.0f,0.0f},min_vec = {-99999999.0f,-99999999.0f,-99999999.0f,0.0f};
...
void		DG_SetFixedLightMap( LIT_DEF *light, int map_id )
{
	...
	for( i = MAX_FIX_LIGHTS - 1; i > 0; i--, p++ ){
		...
		if( p->FixedLights == NULL ){
			...
			if( DG_CheckLightMatrixNormalize() )
			{
				DG_NormalizeLightMatrix2(&DG_LightMatrix, &DG_ColorMatrix) ;
			}
			return;
		}
	}
...
int		DG_SetTmpSpotLight( FVECTOR *point, FVECTOR *dir, float range, float umbra, float penumbra, int color, int flag )
{
	...
	*(int*)&lit->color = color ;
	...
	/* バウンディング対応バージョン */
#if 1
	r = sqrtf( 1.0f - penumbra * penumbra ) * range * 2 ;
	{
		FVECTOR	end_pos ;
...
{
	DG_NormalizeLightMatrix2( &light[0], &light[1] ) ;
}
...
void	DG_NormalizeLightMatrix2( FMATRIX *light, FMATRIX *col_mtx )
{
	...
	dir = (float *)&light->m[0] ;
	col = (float *)&col_mtx->m[0] ;
	for(i=3;i>0; i--, dir++, col+=4)
	{
		vec.vx = dir[0] ;
		vec.vy = dir[4] ;
		vec.vz = dir[8] ;
		if( (vec.vx == 0.0f) && (vec.vy == 0.0f) && (vec.vz == 0.0f) )
		{
			col[0] = 0.0f ;
			col[1] = 0.0f ;
			col[2] = 0.0f ;
		...
		{
			coef = DG_SQRT((vec.vx*vec.vx)+(vec.vy*vec.vy)+(vec.vz*vec.vz)) ;
			if( coef < (1.0f/256.0f) )
			{
				col[0] = 0.0f ;
				col[1] = 0.0f ;
...
9:						vsub.xyz		vf02,vf06,vf03	# vf02 = pos - point
	...
	/* マトリクスの正規化処理 */
	if( DG_CheckLightMatrixNormalize() ){ DG_NormalizeLightMatrix(light) ; }
