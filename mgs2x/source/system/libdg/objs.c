...
/*
	...
	1999/07/07 K.Takabe
	$Id: objs.c,v 1.1.1.3 2002/11/19 11:42:15 Yoshizawa1 Exp $
...
DG_OBJS		*DG_MakeObjs( DG_DEF *def, int flag, int chanl )
{
	...
	int		buf_size ;
	int		i, j, n_models ;
   unsigned const bpMeshPtr = BP_GetMeshByMGSAddr((unsigned int)def);
   ...
   // Checks if we're in the "NO MSAA" list for this tri file
   if ( BP_Obj_CheckForceNoMSAAForTRIFile( def->texture ) )
   {
      flag |= DG_FLAG_NOMSAA;
   }

   objs->BP_VAnimBufferTimestamp = -1;

   // Arm fix:
   // Identify a particular model in a particular scene
   // then in chain.c, identify a unit and packet and ignore it
   // fixes MGSTWO-2893
   if (!strcmp(GM_GetArea(), "d053p01"))
   {
      int p = 0;
      mdl = def->models ;
	...
	for ( i = 0; i < n_models; ++i ) 
   {
      ...
      obj->BP_PrecomputedChainBuffer = 0;
		...
		for ( j = 0 ; j < mdl->n_packs ; j++ ){
			...
			//if ( flag & DG_FLAG_MULTITEX ) printf("%0x %d \n", pack->flag, pack->n_verts );
			if ( mdlpack->tex_id[0] != 0 ) pack->tex_ptr[0] = (DG_TEX*)mdlpack->tex_id[0];
			if ( mdlpack->tex_id[1] != 0 ) pack->tex_ptr[1] = (DG_TEX*)mdlpack->tex_id[1];
			if ( mdlpack->tex_id[2] != 0 ) pack->tex_ptr[2] = (DG_TEX*)mdlpack->tex_id[2];
			pack++ ;
			mdlpack++ ;
		}
		obj ++ ;
...
/* 関節毎にモデルを指定 */
DG_OBJS		*DG_MakeObjs2( DG_DEF *def, int flag, int chanl, DG_DEFMDLPAIR *defmdl_list, int n_list )
{
	DG_OBJS		*objs ;
	DG_OBJ		*obj ;
	DG_MDL		*mdl ;
	...
	int		buf_size ;
	int		i, j, n_models ;
   unsigned bpMeshPtr = BP_GetMeshByMGSAddr((unsigned int)def);
   DG_DEF *pLastDef = def;
   ...
   {
		DG_MDLPACK	*mdlpack ;
      DG_DEF *currentDef = defmdl_list->def ? defmdl_list->def : def;

      if ( currentDef != pLastDef )
      {
         pLastDef = currentDef;
         bpMeshPtr = BP_GetMeshByMGSAddr((unsigned int)currentDef);
      }
		...
		obj->light = &DG_LightMatrix ;
		obj->flag = objs->flag ;
		if ( n_list > 0 && defmdl_list->mdl != NULL ){
			obj->model = defmdl_list->mdl ;
		} else {
			obj->model = mdl ;
		}
      ...
      obj->BP_Model = bpMeshPtr;
      obj->BP_ObjIndex = obj->model - currentDef->models;
...
/* 部分的にモデルを変更する */
void DG_ChangeModelObj( DG_OBJS* changeObjs, DG_OBJ *obj, DG_DEF* mdlDef, DG_MDL *mdl )
{
	int		i ;
	DG_OBJ_PACKET	*pack ;
	DG_MDLPACK		*mdlpack ;
   ...
   obj->BP_DGObjs = changeObjs;
   
   if ( obj->BP_PrecomputedChainBuffer )
   {
      DG_AS_FreePrecomputedChainBuffers( obj );
   }

   obj->BP_Model = BP_GetMeshByMGSAddr((unsigned int)mdlDef);
   obj->BP_ObjIndex = mdl - mdlDef->models;
...
void DG_ConnectObjs( DG_OBJS *parent, DG_OBJS *child )
{
	...
	while ( parent->next_dgobjs != NULL ){
		//printf("objs.c: next = %08x\n", parent->next );
		parent = parent->next_dgobjs ;
	}
	//printf("objs.c: connect objs end(%08x)\n", child->next );
	parent->next_dgobjs = child ;
}
...
void DG_DisconnectObjs( DG_OBJS *parent, DG_OBJS *child )
{
	...
	while ( parent->next_dgobjs != child ){
		parent = parent->next_dgobjs ;
		if ( parent == NULL ){
	...
	parent->next_dgobjs = parent->next_dgobjs->next_dgobjs ;
}
...
{
#if 0 //BP_PS2
	objs->fog_param[0] = 255.0f / ( near - far );
	objs->fog_param[1] = 255.0f - objs->fog_param[0] * near ;
#else
   objs->fog_param[0] = -1.0f / (near - far);
   objs->fog_param[1] = -objs->fog_param[0] * near;
#endif
}
...
DG_OBJS		*DG_MakeObjs2D( DG_DEF *def, int flag, int chanl, DG_DEFMDLPAIR *mdl_list, int n_list, char *fname )
{
	DG_OBJS		*objs ;
