/*
	...
	2002/03/28 K.Takabe
	$Id: waplha.c,v 1.6 2002/10/06 10:32:51 takaki Exp $
*/
...
#define FIX			D3DBLEND_CONSTANTALPHA
#define INVFIX		D3DBLEND_INVCONSTANTALPHA
...
{
#ifndef _WINDOWS
		IDirect3DDevice8_SetRenderState( g_pd3dDevice, D3DRS_ALPHABLENDENABLE, mode );
		IDirect3DDevice8_SetRenderState( g_pd3dDevice, D3DRS_BLENDOP, op );
		IDirect3DDevice8_SetRenderState( g_pd3dDevice, D3DRS_SRCBLEND, src );
		IDirect3DDevice8_SetRenderState( g_pd3dDevice, D3DRS_DESTBLEND, dst );
#else
		DG_SetRenderState( D3DRS_ALPHABLENDENABLE, mode );
		DG_SetRenderState( D3DRS_BLENDOP, op );
		DG_SetRenderState( D3DRS_SRCBLEND, src );
		DG_SetRenderState( D3DRS_DESTBLEND, dst );
#endif
}
...
*/
void SetAlphaMode( DG_ALPHA_STATE *state, u_long type)
{
...
void DG_SetAlphaMode(u_long type)
{
	...
	if (type == 0) {
		...
		if ( reset_chanl_mask ){
#ifndef _WINDOWS
			IDirect3DDevice8_SetRenderState( g_pd3dDevice, D3DRS_COLORWRITEENABLE, D3DCOLORWRITEENABLE_ALL );
#else
...
#ifndef _WINDOWS
		IDirect3DDevice8_SetRenderState( g_pd3dDevice, D3DRS_ALPHATESTENABLE, state->mode );
		IDirect3DDevice8_SetRenderState( g_pd3dDevice, D3DRS_ALPHABLENDENABLE, state->mode );
		IDirect3DDevice8_SetRenderState( g_pd3dDevice, D3DRS_BLENDOP, state->op );
		IDirect3DDevice8_SetRenderState( g_pd3dDevice, D3DRS_SRCBLEND, state->src );
		IDirect3DDevice8_SetRenderState( g_pd3dDevice, D3DRS_DESTBLEND, state->dst );
#else
		DG_SetRenderState( D3DRS_ALPHATESTENABLE, state->mode );
		DG_SetRenderState( D3DRS_ALPHABLENDENABLE, state->mode );
		DG_SetRenderState( D3DRS_BLENDOP, state->op );
		DG_SetRenderState( D3DRS_SRCBLEND, state->src );
		DG_SetRenderState( D3DRS_DESTBLEND, state->dst );
#endif
		alpha_fix_mode = state->fix_flag ;
		if ( state->init_flag == 2 ){
			/* カラーマスク有効化 */
			DG_SetRenderState( D3DRS_COLORWRITEENABLE, D3DCOLORWRITEENABLE_ALPHA );
			reset_chanl_mask = 1 ;
		} else {
			/* カラーマスク復元チェック */
			if ( reset_chanl_mask ){
...
#ifdef _WINDOWS
	DG_ConstAlpha.act = alpha_fix_mode ;
#endif
	if ( alpha_fix_mode ){
#ifndef _WINDOWS
		alpha <<= 1 ;
		if ( alpha > 255 ) alpha = 255 ;
		IDirect3DDevice8_SetRenderState( 0, D3DRS_BLENDCOLOR, alpha << 24 );
#else
		DG_ConstAlpha.alpha = alpha ;	// 重いので、最終的にはFIXを使用しない様に修正したい
#endif
	}
}
