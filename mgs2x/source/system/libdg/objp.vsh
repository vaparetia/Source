; OBJ描画用頂点シェーダ
;
; 2001/05/23  F.Miyauchi
; $Id: objp.vsh,v 1.1.1.3 2002/11/19 11:42:15 Yoshizawa1 Exp $
;
; 平行光源    OFF
; ウェイト    OFF
; 点光源      ON
...
#include "plight.vsh"							; 点光源計算 / 重かったら分岐する

add oD0.xyz, V_DIFFUSE.xyz, R_PLIGHT_COL.xyz	; 点光源カラー出力
mov oD0.w, V_DIFFUSE.w							; アルファは入力のまま

; ピクセルシェーダで利用するため、R_VECをoT1に書き込む
rcp R_TMP.x, c[CV_PLIGHTCOL].w					; R_TMP.x = 1 / 光源有効範囲
max R_VEC, R_VEC, -R_VEC						; R_VEC = abs(R_VEC)
mul R_VEC, R_VEC, R_TMP.x						; R_VEC *= 1 / RANGE

mov oT2.xyz, c[CV_PLIGHTCOL].xyz				; oT2に光源色を出力
mov oT2.w, c[CV_ONE]
mov oT3.xyz, R_VEC								; oT3に座標を出力
mov oT3.w, c[CV_ONE]
