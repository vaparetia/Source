...
/*
	...
	1999/07/07 K.Takabe
	$Id: light.c,v 1.1.1.3 2002/11/19 11:42:10 Yoshizawa1 Exp $
...
int		DG_SetTmpLight( FVECTOR *point, float r_range, float e_range )
{
   ...
   // AS(JM) - Do not set tmp lights in skip frames
   ...
   lit->color.r = lit->color.g = lit->color.b = 0xff;
   lit->color.cd = 0;
	/* バウンディング設定 */
...
int		DG_SetTmpLight2( FVECTOR *point, float r_range, float e_range, int color, int flag )
{
   ...
   if ( DG_Arm_SkipThisFrame() )
   {
	...
	lit->e_range = e_range ;
	lit->flag = flag ;
   lit->color.r = color & 0xFF;
   lit->color.g = (color >> 8) & 0xFF;
   lit->color.b = (color >> 16) & 0xFF;
   lit->color.cd = (color >> 24) & 0xFF;
	/* バウンディング設定 */
	max.vx = point->vx + e_range ;
	max.vy = point->vy + e_range ;
	max.vz = point->vz + e_range ;
...
int		DG_SetTmpSpotLight( FVECTOR *point, FVECTOR *dir, float range, float umbra, float penumbra, int color, int flag )
{
   ...
   lit->color.cd = (color >> 24) & 0xFF;
	...
	asm( "
		...
		vaddx.xyz		vf06,vf05,vf01	# vf06 = スポット半径のバウンディング最大値
		vsubx.xyz		vf07,vf05,vf01	# vf07 = スポット半径のバウンディング最小値
		vmax.xyz		vf06,vf04,vf06	# vf06 = バウンディング最大値（始点を考慮に入れて最終値を算出）
		vmini.xyz		vf07,vf04,vf07	# vf07 = バウンディング最小値（始点を考慮に入れて最終値を算出）
		sqc2			vf06,0x00(%2)	# 
		sqc2			vf07,0x00(%3)	# 
...
#else
                  memcpy(&light[ 1 ].m[i][0], &light[ 1 ].m[i-1][0], sizeof(u_long128));
#endif
					}
