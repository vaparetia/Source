/*
	...
	2002/03/13 K.Takabe
	$Id: wpshade.c,v 1.9 2002/12/12 14:21:25 takaki Exp $
...
/* ベクトルの長さを求める */
#define VEC_LEN(_x,_y,_z) (sqrt((_x) * (_x) + (_y) * (_y) + (_z) * (_z)))
...
#undef	_SetMatrix
#define	__SetMatrix(m_)		{ DG_SSE_CurrentMatrix = *(FMATRIX *)(m_) ; }
#else
...
#else
	if ( work->hit_flag == 0 ) return ( NULL );	// NULLで非変更通知
#endif
...
#else
		if( ReshadeRGB( mdl, NO_CACHED(obj->rgbs), lit_def, first_flag ) )
		{
			/* 変更していたら更新 */
...
void DG_TempPreshadeRGB( DG_OBJ *obj, LIT_DEF *lit_def, int which, int start_flag )
{
...
#else
	/* メモリ確保（Windows版は静的に確保している頂点バッファメモリから確保） */
#ifdef DEBUG_MODE
	if( !obj->d3d_cvbuff )
	{
		printf("[ Warning ] obj->d3d_cvbuff:NULL %s(%d)\n", __FILE__, __LINE__) ;
	}
#endif
	...
	if( obj->d3d_cvbuff )
	{
...
#else
		if( DG_CheckIndexPrimitiveUseable() )	// Index頂点使用可能
		{
			IDirect3DVertexBuffer8_Lock(obj->d3d_cvbuff, 0, sizeof(CVECTOR) * packet_size,
								(BYTE **)&tmp_color, 0) ;
		}
		else	// Index頂点使用不可能
		{
			ASSERT( (sizeof(CVECTOR) * obj->n_verts) < DG_WORKBUFFER_SIZE ) ;
			tmp_color = DG_GetWorkBuffer();
		}
		...
		if( !DG_CheckIndexPrimitiveUseable() )
		{
			...
			size = sizeof(CVECTOR) * n_indices ;

			IDirect3DVertexBuffer8_Lock(obj->d3d_cvbuff, 0, size, &dst, 0) ;
			DG_ArrangeVertexByIndex(dst, tmp_color, sizeof(CVECTOR), obj->index, n_indices) ;

			obj->norms = dst ;	// 特に意味はありませんが、Index対応版に合わせる為の処理。
		}
...
#else
			if ( obj->vanime_flag & DG_VANIME_DELNORMS ) {
				BYTE	*dst ;
				DWORD	size ;
