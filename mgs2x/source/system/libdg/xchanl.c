...
/*
	...
	2002/02/08 K.Takabe
	$Id: xchanl.c,v 1.4 2002/11/23 11:36:55 Yoshizawa1 Exp $
...
void		DG_InitChanlSystem( int flag )
{
	...
	*/
	{
		extern void DG_AddPluginComdl();
		extern void DG_AddPluginDmapack();
		extern void DG_AddPluginOptcmf();
		//DG_AddPluginComdl();
		DG_AddPluginDmapack();
		DG_AddPluginOptcmf();
	}
...
void		DG_SortChanlSystem( int which )
{
	...
	*/
	...
	if ( DG_XBoxDebugFlag & DG_XBOXDEBUG_SAFETYAREA ){
		...
		DG_OpenDmaTask();
		//DG_Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0, 0.0f, 0);
		DG_SetDmapackLine( &packet.line[0], (int)x0, (float)y0, 0x80ffffff, (float)x1, (float)y0, 0x80ffffff );
...
static int		QueueObjs( DG_OBJ_BUFFER *obj_buff, void *objs )
{
	...
	*/
	...
	if ( n >= obj_buff->max_queue ){
      //printf("QueueObjs: queue buffer over!!\n");
		return -1 ;
	}
