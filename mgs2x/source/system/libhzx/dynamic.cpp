/*
   ...
   $Id: dynamic.cpp,v 1.1.1.3 2002/11/19 11:42:47 Yoshizawa1 Exp $
   */
...
/* 動的ハザードコールバックを登録 */
HZX_D_CALLBACK		*HZX_SetDynamicCallback(
		void				*hzd ,
		HZX_D_CALLBACK		*cb ,
...
/* コールバックを削除 */
void		HZX_RemoveDynamicCallback(
		void				*hzd ,
		HZX_D_CALLBACK		*cb )
{
	HZX_D_SEGMENT		*seg ;
...
static	void	AddIVec( IVECTOR* v3, IVECTOR* v1, IVECTOR* v2 )
{
	v3->vx = v1->vx + v2->vx ;
	v3->vy = v1->vy + v2->vy ;
	v3->vz = v1->vz + v2->vz ;
		...
		SVECTOR*		rot )
{
    ...
    DG_PutVector( p, p, 2 ) ;
    GV_FVtoIV( &p[ 0 ], &ivNew[ 0 ], 7 ) ;
    ivNew[ 0 ].vw = seg->def[ 0 ].vw ;
    ivNew[ 1 ].vw = seg->def[ 1 ].vw ;
    ...
    HZX_MoveDynamicSegment( seg, &ivNew[ 0 ], &ivNew[ 1 ] ) ;
}

void 	HZX_RotateDynamicSegment2(
		HZX_D_SEGMENT*	seg ,
		IVECTOR*		axis , /* 回転中心 */
		FMATRIX*		mat )
{
...
HZX_D_SEGMENT	*HZX_AddDynamicSegment( HZX_GROUP_ID id, IVECTOR *p1, IVECTOR *p2, u_int atr )
{
    ...
    grp = hzd->grp + group ;
    seg = (HZX_D_SEGMENT*)GV_Malloc( sizeof( HZX_D_SEGMENT ) ) ;
    if ( seg == NULL ) {
...
/* 長方形チェック */
static	int		CheckRect( IVECTOR* v1, IVECTOR* v2, IVECTOR* v3, IVECTOR* v4 )
{
...
/* フラットチェック */
static	int		CheckFlat3( IVECTOR* v1, IVECTOR* v2, IVECTOR* v3 )
{
    if ( v1->vy == v2->vy &&
...
static	int		CheckFlat4( IVECTOR* v1, IVECTOR* v2, IVECTOR* v3, IVECTOR* v4 )
{
    if ( v1->vy == v2->vy &&
		v2->vy == v3->vy && 
...
/* 法線長さ32000 */
static	void 	GetNormal( IVECTOR* res, IVECTOR* v1, IVECTOR* v2, IVECTOR* v3, IVECTOR* v4, int n )
{
    FVECTOR	normal ;
    IVECTOR	tmp ;
    float	len ;

    normal.vx = (float)(( v3->vy - v2->vy ) * ( v2->vz - v1->vz ) -
		( v2->vy - v1->vy ) * ( v3->vz - v2->vz ) ) ;
    normal.vy = (float)(( v3->vz - v2->vz ) * ( v2->vx - v1->vx ) -
		( v2->vz - v1->vz ) * ( v3->vx - v2->vx ) ) ;
    normal.vz = (float)(( v3->vx - v2->vx ) * ( v2->vy - v1->vy ) -
    ...
    len = bp_sqrtf( normal.vx * normal.vx +  //BP_MATH - emulate PS2 sqrtf
				normal.vy * normal.vy +
				normal.vz * normal.vz ) ;
	if( len == 0.0f ){ len = 0.00000001f; } /*0DIV yano 2002.04.01*/
		...
		SVECTOR*		rot )
{
    ...
    HZX_MoveDynamicFloor( seg, &ivNew[ 0 ], &ivNew[ 1 ], &ivNew[ 2 ], &ivNew[ 3 ] ) ;
}

/* 動的床回転２ */
void 	HZX_RotateDynamicFloor2(
		HZX_D_FLOOR*	seg ,
		IVECTOR*		axis , /* 回転中心 */
		FMATRIX*		mat )
{
								 ...
								 IVECTOR *p3, IVECTOR *p4, int n, u_int atr )
{
    ...
    grp = hzd->grp + group ;
    flr = (HZX_D_FLOOR*)GV_Malloc( sizeof( HZX_D_FLOOR ) ) ;
    if ( flr == NULL ) {
		...
		FVECTOR*	pvNew )
{
	...
	GV_MatToVec( now, &v1 ) ;
	_sceVu0AddVector( pvNew, &v2, &v1 ) ;
}

void	HZX_CallbackUpdateMatrix( void *hzd, FMATRIX *old, FMATRIX* pmatNew )
{
	HZX_D_SEGMENT		*seg ;
	HZX_D_FLOOR			*flr ;
	FMATRIX				pre, now, matThis, t ;
	...
	DG_COPY_MAT( &matThis, old ) ;
	GV_VecToMat( &DG_ZeroVector, &matThis ) ;	

	_sceVu0TransposeMatrix( &t, &pre ) ;
	_sceVu0MulMatrix( &matThis, &t, &matThis ) ;
	_sceVu0MulMatrix( &matThis, &now, &matThis ) ;

	GV_MatToVec( old, &old_pos ) ;
	HZX_CallbackUpdatePos( hzd, &old_pos, &new_pos ) ;

	DG_COPY_MAT( pmatNew, &matThis ) ;
	GV_VecToMat( &new_pos, pmatNew ) ;
}
								 ...
								 int atr )
{
	...
	trp = (HZX_D_TRP*)GV_Malloc( sizeof( HZX_D_TRP ) ) ;
	if ( trp == NULL ) {
