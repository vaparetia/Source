...
static int cdct_huff(unsigned char *image,int width , int height ,
	       unsigned char *code );
static int yc_rgb(unsigned char  *image , int width , int height);
...
#else
	{
		unsigned char *codeY, *codeC;

		codeY = code + sizeof( int ) * 2;
		codeC = code + sizeof( int ) * 2 + nbyte0;
	    nbyte0 = dct_huff(image ,width , height , codeY, Y);
	    nbyte1 = dct_huff(image ,width , height , codeC, C);
	}
#endif
    if(( nbyte0 != -1) && (nbyte1 != -1)){
		int res;
		res = yc_rgb(image , width , height);
		if( res < 0 ){
			printf("yc_rgb err!\n");
			return -1;
		}
...
static int yc_rgb(unsigned char *image ,int width , int height)
{
    ...
    int  r, g, b, y, cr, cb;
#ifdef BP_PSX2_GCC
    int ccr[2][width+1],ccb[2][width+1]; /*removed by T.Morita 2002.02.01*/
#else
	int *ccr[2], *ccb[2] ;
	ccr[0] = STACK_ALLOC( sizeof(int) * (width+1) * 2 ) ; /* gcc拡張表現から Mallocに変更 T.Morita 2002.02.01 */
	if ( ccr[0] == NULL )
	  return -1 ;
	ccr[1] = ccr[0]+ (width+1) ;

	ccb[0] = STACK_ALLOC( sizeof(int) * (width+1) * 2 ) ; /* gcc拡張表現から Mallocに変更 T.Morita 2002.02.01 */
	if ( ccb[0] == NULL )
	  return -1 ;
	ccb[1] = ccb[0]+ (width+1) ;
#endif
...
#else /*XBOX*/
		*(image + i*width*4 + j*4   ) = b;
		*(image + i*width*4 + j*4+1 ) = g;
		*(image + i*width*4 + j*4+2 ) = r;
#endif
#endif
		   ...
		   int width ,int height)
{
	...
	for (xs = 0; xs < width ; xs += 8){
	    ...
	    q[0] = i;
	    for (k=1; k<64 ;){
		i = huffman_load( code , &kbyte , &kbit , (int *)pc_ac ,
				 (int *)bits_ac , 16*16 );
		if (i == -1) {
		    return -1;
		}
...
{
	return cdct_huff( image, width, height, code );
}
