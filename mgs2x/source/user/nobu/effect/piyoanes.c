/*
	...
	2000/02/11 N.Tanaka
	$Id: piyoanes.c,v 1.1.1.3 2002/11/19 11:46:48 Yoshizawa1 Exp $
...
static int GetResources(Work *work, void* lpParam, LPMGSDEMOMODEL lpModel)
{
	...
	nScene = ((LPDE2EFFECTPIYORIANES)lpParam)->nScene;
	nNode  = ((LPDE2EFFECTPIYORIANES)lpParam)->nNode;
	lpDGObject = (lpModel + nScene)->object.objs;

	work->nCount = ((LPDE2EFFECTPIYORIANES)lpParam)->nCount;

	/* キャラ呼び出し */
	work->lpWork = NewPiyori_Anes(&(lpDGObject->objs[nNode].world));
 ...
 * 登録部分
 */
void *NewDemoPiyoriAnes(void *lpParam, LPMGSDEMOMODEL lpModel)
{
	Work *work;
