/*
	...
	2000/02/11 N.Tanaka
	$Id: piyori.c,v 1.1.1.3 2002/11/19 11:46:48 Yoshizawa1 Exp $
...
static int GetResources(Work *work, void* lpParam, LPMGSDEMOMODEL lpModel)
{
	...
	nScene = ((LPDE2EFFECTPIYORI)lpParam)->nScene;
	nNode  = ((LPDE2EFFECTPIYORI)lpParam)->nNode;
	lpDGObject = (lpModel + nScene)->object.objs;

	work->nCount = ((LPDE2EFFECTPIYORI)lpParam)->nCount;

	/* キャラ呼び出し */
	work->lpWork = NewPiyori(&(lpDGObject->objs[nNode].world));
 ...
 * 登録部分
 */
void *NewDemoPiyori(void *lpParam, LPMGSDEMOMODEL lpModel)
{
	Work *work;
