/*
	...
	2000/07/24	Y.Kira
	$Id: handycam.c,v 1.1.1.3 2002/11/19 11:45:07 Yoshizawa1 Exp $
*/
#include <sys/types.h>
#include <stdio.h>
#include <stdlib.h>
...
static void Act(Work * work)
{
  ...
  if(work->x_pulse_len <= 0)
    if(((rand() >> 16) & 63) == 0)
      work->x_pulse_len = 100 + (rand() >> 16) % 20;

  if(work->y_pulse_len <= 0)
    if(((rand() >> 16) & 63) == 0)
      work->y_pulse_len = 50 + (rand() >> 16) % 20;
...
{
  work->x_body_action = (float)((rand()>>16) % 100) / 50.0F * F_PI;
  work->y_body_action = (float)((rand()>>16) % 100) / 50.0F * F_PI;
  work->x_pulse_len = 0;
  work->y_pulse_len = 0;
  return 0;
}
