/*
  ...
  $Id: c_indemo.c,v 1.1.1.3 2002/11/19 11:45:04 Yoshizawa1 Exp $
*/
/*
...
static void scrn_snake_face_position(Work * work)
{
  ...
  if(x[1] >= (float)DRAW_WIDTH)
    x[0] -= x[1] - (float)(DRAW_WIDTH-1), x[1] -= (float)(DRAW_WIDTH-1);
  if(y[1] >= (float)DRAW_HEIGHT)
    y[0] -= y[1] - (float)(DRAW_HEIGHT-1), y[1] -= (float)(DRAW_HEIGHT-1);
...
static int scrn_dma_setup(Work * work)
{
  static codecBG_DMA def_bg_dma = {
	...
	{
#ifndef TEX_16BPP
#define _TEXMODE SCE_GS_PSMCT32
#else
#define _TEXMODE SCE_GS_PSMCT16
#endif /* TEX_16BPP */
	  reg: SCE_GS_TEX0_1,
