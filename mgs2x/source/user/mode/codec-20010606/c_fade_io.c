/*
	...
	$Id: c_fade_io.c,v 1.1.1.3 2002/11/19 11:45:03 Yoshizawa1 Exp $
*/
...
#define	TEXTURE_CENTER		( GV_StrCode( "drop01_msk" ) )
#define	TEXTURE_DROP		( GV_StrCode( "drop01_msk" ) )
...
static	void	Act( Work *work )
{
	...
	work->count--;
	over_count=(work->count>=0)?work->count:0;
...
{
	if( work->prim_second != NULL ) GM_FreePrim2( work->prim_second );
}
...
//		prim->prim_env.alpha.data = SCE_GS_SET_ALPHA( 2, 0, 0, 1, 0x00 ) ;
	...
	for ( j = 0 ; j < num_poly ; j++ ){
		center.vx = rnd()*SCREEN_NEAR - SCREEN_NEAR/2.0f;
		center.vy = rnd()*SCREEN_NEAR - SCREEN_NEAR/2.0f;
...
static int GetResources( Work *work )
{
	...
	{
		extern void *NewCodecFadeInOut_Demo( int col_r, int col_g, int col_b, int col_a, int time );
		NewCodecFadeInOut_Demo( (int)work->init_color.vx, (int)work->init_color.vy, (int)work->init_color.vz, ALPHA_MAX-(int)work->alpha_max, 0 );
		NewCodecFadeInOut_Demo( (int)work->init_color.vx, (int)work->init_color.vy, (int)work->init_color.vz, (int)work->alpha_max, work->count );
	}
...
//---------------------------------
void *NewCodecFadeInOutScn( void )
{
	Work		*work ;

	work = (Work *)GV_NewEffect( GV_ACTOR_MANAGER, sizeof( Work ) ) ;
	if ( work != NULL ) {
