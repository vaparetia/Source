...
/*
  ...
  $Id: c_ffocus.c,v 1.6 2002/11/23 12:09:26 Yoshizawa1 Exp $
*/
/*
...
typedef ALIGN16_DECL(struct) _frame_set {
	DG_GIFTAG		giftag ;
						 ...
						 int z, int dir )
{
	static SCREEN_DRAW	def_screen_draw = {
      ...
      //data
      {
         ...
         { SCE_GS_SET_RGBAQ(128,128,128,128,0), SCE_GS_RGBAQ }, //rgb1
         //sprt[DRAW_DIV]
         { 0 }
         //test2
      }
...
/* パケット用の構造体を宣言しているが、実際にはは先頭から動的に使用しているので注意！ */
typedef struct {
...
void ffocusSetFocus(void * workp, int t, int type, int near, int zfar)
{
	if ( workp ) {
		Work * work = workp;
...
static void Die( Work *work )
{
	/* パケットメモリ開放 */
#ifdef PSX2
	if ( work->dmapack->packet[0] != NULL ) FREE( work->dmapack->packet[0] );
#else
