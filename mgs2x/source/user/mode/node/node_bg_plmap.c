...
/*
  ...
  2001/07/09 Y.Kira
  $Id: node_bg_plmap.c,v 1.1.1.3 2002/11/19 11:45:18 Yoshizawa1 Exp $
*/
#ifdef PSX2
#include <sys/types.h>
#include <stdio.h>
...
#define CODE_OPENPLMAP		0x002c15a6		/* openPlantMap */
#define CODE_IDLEPLMAP		0x00031277		/* idlePlantMap */

#if 0
#define CODE_CLOSEPLMAP		0x00e94422		/* closePlantMap-1 */
#else
#define CODE_CLOSEPLMAP		0x00109576		/* GV_StrCode("closePlantMap-1") */
#endif
...
static int ReceiveSignal(void * workp, int signal, int value)
{
	...
	case NODE_BG_SIGNAL_OPENPLMAP:
		if(L2D_EvokeAction(work->l2d_handle,CODE_OPENPLMAP)!=L2D_STAT_ACK){
			work->action_strcode=CODE_OPENPLMAP;
		}
		work->start_flag=1;
		break;
	case NODE_BG_SIGNAL_CLOSEPLMAP:
		if(L2D_EvokeAction(work->l2d_handle,CODE_CLOSEPLMAP)!=L2D_STAT_ACK){
			work->action_strcode=CODE_CLOSEPLMAP;
		}
		work->start_flag=0;
		break;
	default:
...
static void Act(Work * work)
{
	...
	if(work->start_flag){
		work->action_strcode=CODE_IDLEPLMAP;
	}
}
...
static int GetResources(Work * work, int l2d_name)
{
   float safeZoneOffsetY=0;
	...
	printf("node_bg_plmap.l2d: l2d_name = %d\n", l2d_name);
	...
	work->l2d_handle = L2D_SetupLayout2(l2d_ptr,
										NODE_BG_CHANL, NODE_BG_PRI_PLMAP,
										SPR_FLAG_PRIV, 0, safeZoneOffsetY);
  
	printf("node_bg_plmap.c: l2d handle = %d\n", work->l2d_handle);
