...
/*
	 ...
	 $Id: control_headmark3.c,v 1.1.1.3 2002/11/19 11:48:52 Yoshizawa1 Exp $
*/
...
static  int GetResources( Work *work, FMATRIX *world, TARGET *trg )
{
	work->world = world ;
...
void CallActHeadMarks( void *pwork, int call_type )
{
	Work *work = pwork;
	int type = call_type;
	if( !work ) return ;
	...
	if( type & HMK2_TYPE_MARK){
		...
		printf("type = %x\n",type );
		if( !(work->ctrl->map&GM_CurrentStageMap) ) return;
...
void  *NewControl_Headmark3( FMATRIX *world, TARGET *trg, CONTROL *cntrl )
{
	Work *work ;
//printf("\n\nptrg = %p\n\n",trg);
	...
	if(work != NULL){
		...
		work->ctrl = cntrl;
		if(GetResources( work, world, trg ) < 0){
			GV_DestroyActor( work ) ;
			return NULL ;
		}
	}
