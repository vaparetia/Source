...
/*
	...
	$Id: scr_goggles.c,v 1.1.1.3 2002/11/19 11:48:40 Yoshizawa1 Exp $
	...
	for( k = 0; k < BUNKATSU; k++ ){
		...
		for( i = 0; i < mdl->n_packs; i++ ){
			...
			for( j = 0; j < objpack->n_verts; j++ ){
				...
				svx = (short)((float)((k&1)?-sverts->vx:sverts->vx) * 1.5f );
				svy = (short)((float)(-((k&2)?-sverts->vy:sverts->vy) * 1.5f));
...
static int GetResources( Work *work, int mode, int camera_num )
{
	...
	InitMVerts( work->mverts, def, work->n_mverts );
#else
	{
		int xbox_packet_size = 32000;/* 下の＊＊＊のprintf文のサイズ分が必要 */
		...
		prim = InitMVerts( prim, def, work->n_mverts );
		DG_SetDmapackEnd( prim );
		
      printf( "scr_goggles.c need Size: %d\n",  (int)prim-(int)work->packet_mem  ) ; /* ＊＊＊ */
...
二重起動はしません
*/
void *NewScrGoggles( int mode, int camera_num )
{
	Work		*work ;
