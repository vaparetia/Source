...
/*
	...
	$Id: plant_sun.c,v 1.1.1.3 2002/11/19 11:48:38 Yoshizawa1 Exp $
...
extern void *NewOnlineCheck2_Static( int map_id,
...
static void Flare_Act(Work *work)
{
	...
	if( (IT_Thermal     == GM_Item)
	 || (IT_NightVision == GM_Item) ){ //|| !GM_CheckPlayerStatus( PLAYER_WATCH ) ){
		Flare_Invisible(work);
		return ;
	}
...
static int InitSunPrim( Work *work )
{
	...
	tex_code = GCL_GetOptionValue( 't', 1126305 );
...
#if 0
//ハレーション
static int InitHalationPrim( Work *work )
{
	return 0;
}
#endif
static void GetOptions( Work *work )
{
	...
	if(GCL_GetOption('f')){
		...
		fvtemp.vx = GCL_GetNextInt();
		fvtemp.vy = GCL_GetNextInt();
		fvtemp.vz = GCL_GetNextInt();
		fvtemp.vw = 0.0f;
		_sceVu0Normalize(&fvtemp,&fvtemp);
		_sceVu0ScaleVector(&work->center,&fvtemp,-9000.0f*work->scale);
		work->center.vw = 0.0f;
	}else{
		if(GCL_GetOption('d')){
			FVECTOR		fvtemp = { 0.0f, 0.0f, -9000.0f*work->scale,0.0f};
			SVECTOR		rot;
			...
			DG_RotVector(&fvtemp,&work->center,1);
		}else{
			work->center.vx = 0.0f;
			work->center.vy = 0.0f;
			work->center.vz = 0.0f;
	...
	work->alpha = (float)GCL_GetOptionValue( 'a', 18 );
	
	work->w = (float)GCL_GetOptionValue( 'w', (int)(240.0f * work->scale ) );
	work->h = (float)GCL_GetOptionValue( 'h', (int)(240.0f * work->scale ) );

	//PRINT_PFVEC(0,&work->center);
}
...
//	ASSERT( work->hzx_id != 0 );
	...
	hzx_group = GCL_GetOptionValue( 'g', -1 );
	if( hzx_group < 0 ){
		hzx_group = GM_GetHzxGroupID( GM_CurrentMap );
	}else{
		hzx_group = ( HZX_GROUP_ID )GV_GetBit( hzx_group );
	}
	GV_SetActorChild(work,NewOnlineCheck2_Static( hzx_group,
												 &work->line_check_pos[0], (FVECTOR*)DG_Chanls[0].eye.m[3],
												 HZX_CHK_ALL|HZX_CHK_RECOIL_TYPE_ONLY, 0, 0,
												 &work->result[0],
												 4, 4, 100.0f  ));
	GV_SetActorChild(work,NewOnlineCheck2_Static( hzx_group,
...
#if 0
	if( GCL_GetOption( 'c' ) ){
		col = (GCL_GetNextInt()&0xff)<<24;
		col |= (GCL_GetNextInt()&0xff)<<16;
		col |= (GCL_GetNextInt()&0xff)<<8;
		col |= (GCL_GetNextInt()&0xff)<<0;
	}else{
		col = 0;
	}

	if( col ){		// 海面にきらきら
		extern void *NewKirariWaterSun( FMATRIX *world, int col );
		void	*p0;
		p0 = NewKirariWaterSun( &work->world, col );
		if( p0 ) GV_SetActorChild( work, p0 );
	}
#else
	if( GCL_GetOption( 'c' ) ){		// 海面にきらきら
		extern void *NewKirariWaterSun( FMATRIX *world, int col );
		void	*p0;
		int	amb_col;
		...
		amb_col  = ((int)(DG_ColorMatrix.m[3][0])&255)<<24;
		amb_col |= ((int)(DG_ColorMatrix.m[3][1])&255)<<16;
		amb_col |= ((int)(DG_ColorMatrix.m[3][2])&255)<< 8;
		amb_col |= (col&255);
		p0 = NewKirariWaterSun( &work->world, amb_col );
		if( p0 ) GV_SetActorChild( work, p0 );
	}
#endif
...
void *NewPlantSunMain( int name, int map )
{
	Work *work = NULL;
