...
/*
	...
	$Id: lens_flr_gm.c,v 1.1.1.3 2002/11/19 11:48:37 Yoshizawa1 Exp $
...
static int CheckMesgParam( Work *work )
{
	...
	while( --mes_num >= 0 ){
		...
		switch( num ){
		  case 2:
			//alpha変更
			work->to_alpha = msg->message[1];
			work->timer = msg->message[2];
			work->diff_alpha = (work->to_alpha-work->max_alpha)/work->timer;
			break;
		default:
...
static void Flare_Act(Work *work)
{
	...
	temp = temp_x - 1.1f;
	if(temp > 0.0f){
		alpha = (float)work->max_alpha - temp*(float)work->max_alpha/0.3f;
	}
	temp = temp_y - 1.1f;
	if(temp > 0.0f){
		alpha = (float)work->max_alpha - temp*(float)work->max_alpha/0.3f;
...
static void Act(Work *work)
{
	...
	TS_MakeMatrix(&world,(FVECTOR*)DG_Chanls[0].eye.m[2],&work->center);
	DG_SetPos(&world);
	DG_PutVector(work->init_check_pos,work->line_check_pos,4);
	//AN_Test_Eye2( &work->line_check_pos[0], 2 );
	//AN_Test_Eye2( &work->line_check_pos[1], 2 );
	//AN_Test_Eye2( &work->line_check_pos[2], 2 );
	//AN_Test_Eye2( &work->line_check_pos[3], 2 );
	CheckMesgParam( work );
	Flare_Act(work);

	if( work->timer > 0 ){

		work->max_alpha += work->diff_alpha;
		if( --work->timer < 0 ){
			work->timer = -1;
			work->max_alpha = work->to_alpha;
		}
	}
...
void *NewLensFlr_Game( FVECTOR *center, float size, int max_alpha )
{
	Work *work = NULL;
	...
	if(work){
		...
		printf("new lenz_flr_gm.c\n");
	}

	return (void *)work ;
}

static void GetOption( Work *work )
{
	FVECTOR	pos;
	...
	work->size = (float)GCL_GetOptionValue( 's', 0 );
	work->max_alpha = GCL_GetOptionValue( 'a', 0 );
}

void *NewLensFlr_GameScn( int name, int map )
{
	Work *work = NULL;
