...
/*
	...
	$Id: sp_menu_eng.c,v 1.1.1.3 2002/11/19 11:48:45 Yoshizawa1 Exp $
*/
...
#define		PUT_TXT_X0	(270)
#define		PUT_TXT_X1	(512+96)//(512)
#define 	TXT_SIZE_X	((float)(PUT_TXT_X1 - PUT_TXT_X0))
...
static void PrintMenu( Work *work, int cate_num, int alpha, int mode )
{
	...
	for( i = 0; i < MaxMenuNum[cate_num]; i++ ){
		...
		MENU_PutTextScreen( work->exp_work,
							...
							384, 1+(FONT_BUFFER_HEIGHT(1,0)+2)*(i+1), (alpha<<24)|color );
...
static void SpMenu_VisMenu( Work *work )
{
	...
	for( i = 0; i < N_MAXMENU0; i++ ){
		...
		MENU_PutTextScreen( work->exp_work,
							...
							0, 1+(FONT_BUFFER_HEIGHT(1,0)+2)*i,
							384, 1+(FONT_BUFFER_HEIGHT(1,0)+2)*(i+1), color );
	...
	if(++work->timer > 4){
		...
		for( i = 0; i < MaxMenuNum[work->menu_to_num]; i++ ){
			...
			wide = (float)MENU_CreateTextTexture( work->exp_work, 2, 2 + (FONT_BUFFER_HEIGHT(1,0)+2)*i,
												  384-4, FONT_BUFFER_HEIGHT(1,0)+2,
												  0, 0, 0, work->name_str[i+Flag_Offset[work->menu_to_num]] );

			MenuSize_X[work->menu_to_num][i] = wide * TXT_SIZE_X / 384.0f;
		}
	}
...
static void SpMenu_Menu_Movie(Work *work)
{
	...
	if(++work->timer>32){
		...
		pos.y = TRN2DFIG_Y(246.0f)-3;

		SPR_SetPosSprite(work->menu_name, &pos );
		SPR_ObjSetTexture(work->ctrler, Title_Sp_TexCode[work->now_category], work->tri_id);
...
//	int	i;
	...
	case 1:
      ...
      lowerYPos = 182;
		...
		locate.x = 28.0f;
		locate.y = (192.0f - TRN2DFIG_Y(ftemp0))+8;
	...
	case 2:
		...
		MENU_PutTextScreen( work->exp_work,
							(int)ftemp2, (int)(240.0f*384.0f/448.0f)-8,
							(int)ftemp3, (int)TRN2DFIG_Y(240.0f+24.0f)-8,
...
static void SpMenu_Movie(Work *work)
{
	...
	if( GV_PadDataDirect[0].release & PAD_OK ){ 
...
static void SpMenu_MovieClose(Work *work)
{
   ...
   SPR_POS	locate;
	...
	case 3:
		...
		MENU_ClearTextTexture( work->exp_work );
		for( i = 0; i < MaxMenuNum[work->now_category]; i++ ){
			MENU_CreateTextTexture( work->exp_work, 2, 2 + (FONT_BUFFER_HEIGHT(1,0)+2)*i,
									384-4, FONT_BUFFER_HEIGHT(1,0)+2,
...
static int GetResources( Work *work )
{
	...
	if(GCL_GetOption('d')){
		for( i = 0; i < N_ALLMENU; i++ ){
			...
			work->exp_str[i] = BP_GCL_LOOKUP_NEW_FONT_STRING((char*)GCL_GetNextInt());
...
#endif
	SPR_SetSizeSprite(sprite, 14.0f+140.0f, TRN2DFIG_Y(15.0f));
	sprite->head.alpha = SCE_GS_SET_ALPHA(0, 1, 0, 1, 64);
	sprite->head.flags |= SPR_FLAG_ALPHA;
	SET_COLOR_2DPRIM(sprite,0x00808080);
	SPR_SetPriority( sprite, 2);
	...
	for( i = 0; i < 4; i++ ){
		...
		if(sprite == NULL){ printf("ERR!! Make2DObj[sprite]\n"); return -1; }
		SPR_ObjSetTexture(sprite, Category_Tex_Code[i], tri_id);
#ifdef BP_PS2
	...
	for( i = 0; i < N_MAXMENU0; i++ ){
		...
		MenuSize_X[0][i] = wide * TXT_SIZE_X / 384.0f;
//		printf(" str wide [%f]\n",wide);
...
#else
	{
		SPR_POS tmp = {200, TRN2DFIG_Y(29.0f)+8} ;
		SPR_SetPosSprite(sprite, &tmp );
	}
#endif

	SPR_SetSizeSprite(sprite, 105.0f, TRN2DFIG_Y(10.0f) );
	sprite->head.alpha = SCE_GS_SET_ALPHA(0, 1, 0, 1, 64);
	sprite->head.flags |= SPR_FLAG_ALPHA;
...
#else
	{
		SPR_POS tmp = {58.0f, TRN2DFIG_Y(57.0f)} ;
		SPR_SetPosSprite(sprite, &tmp );
	}
#endif
	
	SPR_SetSizeSprite(sprite, 147, TRN2DFIG_Y(160.0f) );
	sprite->head.alpha = SCE_GS_SET_ALPHA(0, 1, 0, 1, 64);
	sprite->head.flags |= SPR_FLAG_ALPHA;
	SET_COLOR_2DPRIM(sprite, 0x00808080);
	SPR_SetPriority( sprite, 2);
...
#else
	{
		SPR_POS tmp = {228.0f, TRN2DFIG_Y(402.0f)-8} ;
		SPR_SetPosSprite(sprite, &tmp );
	}
#endif
...
#else
	{
		SPR_POS tmp = {223, TRN2DFIG_Y(402.0f-8.0f)-8} ;
		SPR_SetPosSprite(sprite, &tmp );
	}
#endif

	SPR_SetSizeSprite(sprite, 67, TRN2DFIG_Y(24.0f+16.0f) );
	sprite->head.alpha = SCE_GS_SET_ALPHA(0, 1, 0, 1, 64);
	sprite->head.flags |= SPR_FLAG_ALPHA;
	SET_COLOR_2DPRIM(sprite, 0);
...
void *NewSpecialMenu_Eng( int name, int map )
{
	Work	*work;
