...
/*
	...
	$Id: scr_drop_demo.c,v 1.1.1.3 2002/11/19 11:48:34 Yoshizawa1 Exp $
...
#ifndef BP_PS2
FVECTOR	_scale_norm, _key, _scale_vec ;
#endif
...
#else
	FVECTOR	vec1, vec2, tmp04, tmp05, tmp06 ;

	_sceVu0ScaleVectorXYZ( &vec1, ls_verts, _key.vx ) ;
	_sceVu0ScaleVectorXYZ( &vec2, lt_verts, _key.vy ) ;
	_sceVu0AddVector( &tmp04, &vec1, &vec2 );

	_sceVu0ScaleVectorXYZ( &vec1, ms_verts, _key.vx ) ;
	_sceVu0ScaleVectorXYZ( &vec2, mt_verts, _key.vy ) ;
	_sceVu0AddVector( &tmp05, &vec1, &vec2 );

	_sceVu0ScaleVectorXYZ( &vec1, &tmp04, _key.vz ) ;
	_sceVu0ScaleVectorXYZ( &vec2, &tmp05, _key.vw ) ;
	_sceVu0AddVector( &tmp06, &vec1, &vec2 );

	_sceVu0MulVector( out, &tmp06, &_scale_vec ) ;
#endif
}
...
#else
	FVECTOR	vec1, vec2, tmp06 ;

	_sceVu0ScaleVectorXYZ( &vec1, l_verts, _key.vz ) ;
	_sceVu0ScaleVectorXYZ( &vec2, m_verts, _key.vw ) ;
	_sceVu0AddVector( &tmp06, &vec1, &vec2 );
									...
									FVECTOR *ms_verts, FVECTOR *mt_verts )
{
	...
	_sceVu0MulVector( out, &tmp06, &_scale_norm ) ;
#endif
}
...
static void Act( Work *work )
{
	...
	TS_VecToRot( &cam_rot, work->cam_rot );
	rot_diff.vx = cam_rot.vx - work->pre_cam_rot.vx;
	rot_diff.vy = cam_rot.vy - work->pre_cam_rot.vy;
	rot_diff.vz = cam_rot.vz - work->pre_cam_rot.vz;
	...
	//ActDrops( work->drop_data, N_DROPS, 0, 0, work->puru_timer );

	ActDrops( work->drop_data, N_DROPS, work->drops_move_x, work->drops_move_y, work->puru_timer );
...
#if 1
	work->cam_rot = (FVECTOR*)DG_Chanls[0].eye.m[2];//rotate;
	TS_VecToRot( &work->pre_cam_rot, work->cam_rot );
#else
...
//	giftag->regs = GS_REGS_AD;
	...
	//printf( "scr_drop_demo.c need Size%d\n",  (int)prim-(int)work->packet_mem  ) ;
