...
/*
	...
	$Id: debris_cm_snd.c,v 1.1.1.3 2002/11/19 11:48:30 Yoshizawa1 Exp $
...
//	p_randam = (float *)MEM_SCR_POS+sizeof(FVECTOR)*work->n_comdl;
	TS_Scr_Mem( MEM_SCR_POS, work->dvec, sizeof(FVECTOR), work->n_comdl );
	max_height = (float*)(work->dvec+work->n_comdl);
...
#			vmul.xyz		vf16,vf8,vf8
			...
			to_pos.vy = temp;
#endif
			if( (*max_height) - dvec->vw > 500.0f ){
				GM_SeSetMode( work->se_tbl[(int)(irnd()>>7)%work->n_se], (FVECTOR*)comdl_pos->world.m[3], GM_SEMODE_NORMAL );
				//printf("%f: %f\n",(*max_height),dvec->vw);
				//*max_height = dvec->vw;
			}
			*max_height = dvec->vw;
	    }else{
			if( *max_height < comdl_pos->world.m[3][1] ) *max_height = comdl_pos->world.m[3][1];
...
//		}
		if(dvec->vw > to_pos.vy){
			...
			if( *max_height - dvec->vw > 500.0f ){
				GM_SeSetMode( work->se_tbl[(int)(irnd()>>7)%work->n_se], (FVECTOR*)comdl_pos->world.m[3], GM_SEMODE_BOMB );
				//printf("かっかっかっかっかっかっ\n");
			}
			*max_height = dvec->vw;
		}else{
						 ...
						 int color, float scale, int flags )
{
	...
	max_height = (float*)(work->dvec+num);
...
void *NewDebris_Cm_Snd( FVECTOR *bound, FVECTOR *force,
					int num, int objcode,
					int color, float scale, int flags, int *se_tbl, int n_se )
{
	Work		*work ;

	work = (Work *)GV_NewActor( GV_ACTOR_EFFECT, sizeof( Work ) + sizeof(FVECTOR)*num+ sizeof(float)*num );
