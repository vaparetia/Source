...
/*
	...
	$Id: mesg_bomb3.c,v 1.1.1.3 2002/11/19 11:48:32 Yoshizawa1 Exp $
...
//ドッポーン
extern void *NewSphereSplush( FVECTOR *pos, float radius, float intense, int alpha );
//波涛
...
extern void *NewBubbleMany_Demo( FVECTOR *center, int life, int col );
//波涛
extern void *NewWaveSplash_Demo2( FMATRIX *root, float scale, int base_alp );
#if 0
...
#define	PRINT_PFVEC(_i,_fv) printf("[%d] vx %f:vy %f:vz %f:vw %f:\n",_i,(_fv)->vx,(_fv)->vy,(_fv)->vz,(_fv)->vw)
		FMATRIX temp;
		DG_SetPos2( work->mov, &work->control.rot );
		DG_GetPos( &temp );
	...
	//HZX_ViewMatrix( &work->world, 500.0F );
	...
	while( --mes_num >= 0 ){
		...
		switch( num ){
		  case BOMB_EFT:
			NewBombEffect( &work->control.mov, msg->message[1] );
			break;
		  case FLYING_SMK:
			...
			NewFlyingSmoke( &work->control.mov, &fvtemp, msg->message[1] );
			break;
		  case BOMB_GAS_EFT:
			NewBombGasEffect( &work->control.mov, &work->control.rot );
			break;
		  case CRUSH_FORCE:
			NewCrushWithForce( &work->control.mov );
			break;
		  case REC:
...
#endif
			NewTs_Spark( &work->control.mov, &fvtemp, (float)msg->message[1] );
			break;
		  case BLOOD_MIST:
			AN_Blood_Mist( &work->control.mov );
			break;
		  case SPHERE_SPLUSH:
			
			NewSphereSplush( &work->control.mov, (float)msg->message[1], (float)msg->message[2], msg->message[3] );
			break;
		  case WAVE_SPLUSH:
			DG_SetPos2( &work->control.mov, &work->control.rot );
			DG_GetPos( &world );
		  ...
		  case POINT_TIDAL:
			...
			break;
		  case BLOOD_WATER:
			NewBloodWater_demo( &work->control.mov, (FVECTOR*)work->world.m[2], (float)msg->message[1], 0 );
			break;
		  case BUBBLE_MANY:
			temp = msg->message[2]<<24;
			temp |= msg->message[3]<<16;
			temp |= msg->message[4]<<8;
			NewBubbleMany_Demo( &work->control.mov, msg->message[1], temp );
//			NewBubbleMany( &work->control.mov, msg->message[1], temp );
			break;
		  case WAVE_SPLUSH2:
			DG_SetPos2( &work->control.mov, &work->control.rot );
			DG_GetPos( &world );
			NewWaveSplash_Demo2( &world, (float)msg->message[1],
								(msg->message[2]<<24)|(msg->message[3]<<16)|(msg->message[4]<<8)|((msg->message[5]&0xff)<<0) );
			break;
		  default:
