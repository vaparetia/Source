...
/*
	...
	$Id: cinema_scr_e3.c,v 1.4 2002/11/23 12:16:43 Yoshizawa1 Exp $
...
//	int				top = 0,bottom = 0;
...
#else
	{
		...
		prim = DG_SetDmapackQuad( prim,
								  0.0f  , 0.0f,                        col[0], 
								  DRAW_WIDTH, 0.0f, 			           col[1], 
								  DRAW_WIDTH, 0.0f + (float)work->top_pix, col[3],
								  0.0f,   0.0f + (float)work->top_pix, col[2] ); 
		prim = DG_SetDmapackQuad( prim,
								  0.0f  , DRAW_HEIGHT - (float)work->bottom_pix, col[4], 
								  DRAW_WIDTH, DRAW_HEIGHT - (float)work->bottom_pix, col[5], 
								  DRAW_WIDTH, DRAW_HEIGHT,                           col[7],
								  0.0f  , DRAW_HEIGHT,                           col[6] ); 
#endif
...
{
#ifndef PSX2 /* T.morita Added 2002.03.29 */
	/* パケットメモリ開放 */
...
void *NewCinemaScreen_e3( int name, int where )
{
	Work		*work ;
	if(CineWork) return (void*)1;
