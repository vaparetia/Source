...
/*
	...
	$Id: sea_slater.c,v 1.1.1.3 2002/11/19 11:48:50 Yoshizawa1 Exp $
...
#define		SCALE_RATIO		(226.2741700f)		//160.0f* (root 2.0f)
//#define		N_SSLTS			(512)
...
typedef struct _work {
	...
	FMATRIX			*root;					//親から貰う
	FMATRIX			*inv_root;				//親から貰う
	FVECTOR			*all_center;			//親から貰う
	FVECTOR			*all_wide;				//親から貰う
	FVECTOR			*refuge_center;			//親から貰う
	FVECTOR			*refuge_wide;			//親から貰う
...
static inline float _Vu0VecLenXYZ( FVECTOR *vec1, FVECTOR *vec2 )
{
	...
	_sceVu0MulVectorXYZ( &tmp, &tmp, &tmp );
...
#ifndef BP_PS2
static FMATRIX *MatRoot, *Mat2, *Mat3 ;
...
static void LoadMatHermiteLerp( FMATRIX *root )
{
	...
	asm volatile("
		...
		lqc2			vf28,0x00(%2)	# 親マトリクス読み込み
		lqc2			vf29,0x10(%2)	# 親マトリクス読み込み
		lqc2			vf30,0x20(%2)	# 親マトリクス読み込み
		lqc2			vf31,0x30(%2)	# 親マトリクス読み込み

	"::"r"(&c_mat2),"r"(&c_mat3),"r"(root));
#else
	Mat2 = &c_mat2;
	Mat3 = &c_mat3;
	MatRoot = root ;
								...
								FVECTOR *outer, FVECTOR *center, FVECTOR *dvec, float scale, FVECTOR *debug )
{
	...
	asm volatile("
		...
		vmove.xyz	vf13 ,vf29		#outer	root->m[1]
		#lqc2		vf13 ,0x00(%3)	#outer
		qmtc2.ni	%7,vf18			#scale

		vadd.xyzw	vf26, vf25,  vf17	#グループの移動量を考慮

		#sqc2		vf25 ,0x00(%8)	#debug
		#sqc2		vf17 ,0x10(%8)	#debug
		#sqc2		vf26 ,0x20(%8)	#debug
		...
		vwaitq
		vmulq.xyz	vf27, vf26, Q

		#root * vec
		vmulax.xyzw			ACC, vf28,vf27
		vmadday.xyzw		ACC, vf29,vf27
		vmaddz.xyzw			vf14, vf30,vf27


		vopmula.xyz	ACC,vf14,vf13
		vopmsub.xyz	vf12,vf13,vf14

		vadd.xyz	vf27,vf24,vf16
		vmulw.w		vf27,vf0,vf0	#out_mat w.vw = 1.0f
		#root * pos
		vmulax.xyzw			ACC, vf28,vf27
		vmadday.xyzw		ACC, vf29,vf27
		vmaddaz.xyzw		ACC, vf30,vf27
		vmaddw.xyzw			vf15, vf31,vf27
...
static int BoundCheckFlag( FVECTOR *bound_center, FVECTOR *bound_wide, int flag )
{
	...
	asm volatile("
		...
		vsub.xyzw		vf23, vf20, vf21

		#root * vec0
		vmulax.xyzw			ACC, vf28,vf22
		vmadday.xyzw		ACC, vf29,vf22
		vmaddaz.xyzw		ACC, vf30,vf22
		vmaddw.xyzw			vf12, vf31,vf22

		#root * vec1
		vmulax.xyzw			ACC, vf28,vf23
		vmadday.xyzw		ACC, vf29,vf23
		vmaddaz.xyzw		ACC, vf30,vf23
		vmaddw.xyzw			vf19, vf31,vf23
		...
		vmove.xz		vf17,vf19
		vmove.yz		vf18,vf19
	"::"r"(bound_center),"r"(bound_wide) : "memory");
#else
	_sceVu0AddVector( &_tmpvf[0], bound_center, bound_wide );
	_sceVu0SubVector( &_tmpvf[7], bound_center, bound_wide );
	_sceVu0ApplyMatrix( &_tmpvf[0], MatRoot, &_tmpvf[0] );
	_sceVu0ApplyMatrix( &_tmpvf[7], MatRoot, &_tmpvf[7] );
									 ...
									 FVECTOR *bound_pos, FVECTOR *bound_size )
{
	...
	len = (1.0f - len/(90.0f*90.0f))*200.0f;
	//printf("len = %f\n",len);
...
static inline void _SetSsVector( FVECTOR *vec, FVECTOR *now, FVECTOR *to, float ratio )
{
	...
	vec->vy = 0.0f;
	_sceVu0ScaleVector( &fvtemp, vec, ratio );
...
static VOLUMECURVE nor_volcurves = {
	2,
	SE_COS_75,
	SE_COS_90,
	{ 6000, 12000, -1, -1, },
...
static VOLUMECURVE own_volcurves = {
	2,
	SE_COS_60,
	SE_COS_90,
	{ 500, 4000, -1, -1, },
...
static void SeaSlaterAct(Work *work)
{
...
//	DG_COMDL_POS 	*comdl_pos_top = work->low_cm->pos;
	...
	SSLTR_GROUP_DATA	*ssltr_group = work->ssltr_group;
	...
	FVECTOR			local_bul[16],nzl_pos,nzl_vec;
	int				n_local_bul;
...
#if 1
	if( (BP_IsPAL()==TRUE && (GM_WaterLevel < DG_Chanls[0].eye.m[3][1] && !(GV_Time%15))) ||
       (BP_IsPAL()!=TRUE && (GM_WaterLevel < DG_Chanls[0].eye.m[3][1] && !(GV_Time%18))) )
   {
       float tmp;
		int	check = 0;
		FVECTOR		local_eye;
		...
		len = FLOAT_MAX;
		_RotTrans( &local_eye, work->inv_root, (FVECTOR*)DG_Chanls[0].eye.m[3] );
		
		for( i = 0,n_act = 0; i < work->n_group; i++ ){
			tmp = _Vu0VecLenXYZ( &ssltr_group[i].now_pos, &local_eye);//(FVECTOR*)DG_Chanls[0].eye.m[3] );
			if( ((ssltr_group[i].flags & GROUP_FLAG_REFUGE) && !(ssltr_group[i].flags & GROUP_FLAG_ESCAPE)) ){
				if( !check || len > tmp ){
					len = tmp;
					n_act = ssltr_group[i].n_act;
					_RotTrans( &nzl_pos, work->root, &ssltr_group[i].now_pos );					
				}
				check = 1;
			}else if( len > tmp && !check ){
				len = tmp;
				n_act = ssltr_group[i].n_act;
				_RotTrans( &nzl_pos, work->root, &ssltr_group[i].now_pos );					
		...
		if( check ){	
			int se_table[] = {
				SD_A_FNALPES1, //船虫わさわさ速3
				SD_A_FNALPES2, //船虫わさわさ速4(交互18fps連)
			};
			//GM_SeSetMode( se_table[(int)g_SeFrip], &nzl_pos, GM_SEMODE_NORMAL );
			GM_SeSetFromVolCurve( se_table[(int)g_SeFrip], &nzl_pos, &extra_volcurves );			
			//AN_Test_Eye2( &nzl_pos, 2 );
			//printf("funa 4\n");
		}else if( work->n_in_group/3 > n_act ){
			int se_table[] = {
				SD_A_FNALOP01, //船虫わさわさ遅１
				SD_A_FNALOP02, //船虫わさわさ遅２(交互18fps連)
			};
			GM_SeSetMode( se_table[(int)g_SeFrip], &nzl_pos, GM_SEMODE_NORMAL );
		}else if( work->n_in_group*2/3 > n_act ){
			int se_table[] = {
				SD_A_FNALOP11, //船虫わさわさ中１
				SD_A_FNALOP12, //船虫わさわさ中２(交互18fps連)
			};
			GM_SeSetMode( se_table[(int)g_SeFrip], &nzl_pos, GM_SEMODE_NORMAL );
		}else{
			int se_table[] = {
				SD_A_FNALOP21, //船虫わさわさ速１
				SD_A_FNALOP22, //船虫わさわさ速２(交互18fps連)
			};
			GM_SeSetMode( se_table[(int)g_SeFrip], &nzl_pos, GM_SEMODE_NORMAL );
		}
		g_SeFrip ^= 1;
	...
	if( !(GV_Time%18) ){
		...
		if( work->n_ssltr/3 > n_act ){
			int se_table[] = {
				SD_A_FNALOP01, //船虫わさわさ遅１
				SD_A_FNALOP02, //船虫わさわさ遅２(交互18fps連)
			};
			GM_SeSetMode( se_table[(int)g_SeFrip], &work->center, GM_SEMODE_NORMAL );
		}else if( work->n_ssltr*2/3 > n_act ){
			int se_table[] = {
				SD_A_FNALOP11, //船虫わさわさ中１
				SD_A_FNALOP12, //船虫わさわさ中２(交互18fps連)
			};
...
#if 1
	//足
	n_foot = work->allwork->n_foot;
	foot = foot_top = work->allwork->foot_pos;

#endif	
	n_bullet = work->allwork->pre_n_bullet;
	bullet_top = work->allwork->bullet_list;

	
	if( GM_WeaponFire == WP_ColdSpray && GM_PlayerSubWeaponBody ){
		_RotTrans( &nzl_pos, GM_PlayerSubWeaponBody->objs->root, &NozzlePoint );
		_RotTrans( &nzl_pos, work->inv_root, &nzl_pos );
		_RotTrans( &nzl_vec, work->inv_root, (FVECTOR*)GM_PlayerSubWeaponBody->objs->root->m[1] );

		//PRINT_PFVEC(0,&nzl_pos);
		//PRINT_PFVEC(1,&nzl_vec);
	...
	for( i = 0; i < work->n_group; i++ ){
		...
		if( !(work->flags&0x0002) &&
			GM_WeaponFire == WP_ColdSpray &&
			GM_PlayerSubWeaponBody &&
			!(ssltr_group->flags&GROUP_FLAG_REFUGE) ){
			//float 		len;
			FVECTOR		diff;
			_sceVu0SubVector( &diff, &ssltr_group->now_pos, &nzl_pos );
			//printf("len = %f\n",_sceVu0InnerProduct( &diff, &diff ));
			...
			if( _sceVu0InnerProduct( &diff, &diff ) < 2000.0f * 2000.0f ){
				...
				if( _sceVu0InnerProduct( &nzl_vec, &diff ) < -0.95f/*-0.995f*/ ){
					...
					ssltr_group->flags |= GROUP_FLAG_REFUGE;
					ssltr_group->wait = 60*60* 2;
					//printf("いででーよ\n");
					for( j = 0; j < work->n_refuge; j++ ){
						float temp = _Vu0VecLenXYZ( &ssltr_group->now_pos, &work->refuge_center[j*2] );
						if( temp < t_len ){
							t_len = temp;
							DG_COPY_VEC( &ssltr_group->to_pos, &work->refuge_center[j*2] );
		...
		AllCheckSltrBlast( &ssltr_group->now_pos, &ssltr_group->wide, work->allwork, work->n_in_group );
		
		//エリアを撃たれた
		if(ssltr_group->flags & GROUP_FLAG_HASTY){
			ssltr_group->flags++;
			if( (ssltr_group->flags&0x00ff) > 120 ){
				ssltr_group->flags = (ssltr_group->flags&0xff00)&(~GROUP_FLAG_HASTY);
			}
		}else if( _Vu0VecLenXYZ( &ssltr_group->now_pos, &ssltr_group->to_pos ) >= 10.0f * 10.0f ){
			//移動中は全員動く
			if( ssltr_group->n_act < work->n_in_group ) ssltr_group->n_act++;
		}else{
			if( ssltr_group->n_act > ssltr_group->max_act ) ssltr_group->n_act--;
			if( ssltr_group->n_act < ssltr_group->max_act ) ssltr_group->n_act++;
		}

		len = GetAllCheckNearPosLen( &check_pos, &check_vec, &ssltr_group->now_pos, work->allwork );
#if 0
		{
			FVECTOR		debug_pos;
			
			_RotTrans( &debug_pos, work->root, &check_pos );
			AN_Test_Eye2( &debug_pos, 2 );
			//PRINT_PFVEC( 0, &check_pos );
		}
#endif
#if PAL
		if( ssltr_group->flags & GROUP_FLAG_REFUGE ){
			if( !(ssltr_group->flags & GROUP_FLAG_ESCAPE) &&
				_Vu0VecLenXYZ( &ssltr_group->now_pos, &ssltr_group->to_pos ) < 500.0f * 500.0f ){
				
				float	t_len = FLOAT_MAX;
				ssltr_group->flags |= GROUP_FLAG_ESCAPE;
				for( j = 0; j < work->n_refuge; j++ ){
					float temp = _Vu0VecLenXYZ( &ssltr_group->now_pos, &work->refuge_center[j*2+1] );
					if( temp < t_len ){
						t_len = temp;
						DG_COPY_VEC( &ssltr_group->to_pos, &work->refuge_center[j*2+1] );
					}
				}
			}
			_SetSsVector( &dvec, &ssltr_group->now_pos, &ssltr_group->to_pos, 0.01f*1.2f );
			if( --(ssltr_group->wait) < 0 ){
				ssltr_group->flags &= ~(GROUP_FLAG_REFUGE|GROUP_FLAG_ESCAPE);
			}
		}else if( len > 4000.0f*4000.0f ){
			DG_COPY_VEC( &ssltr_group->to_pos, &ssltr_group->center );
			_SetSsVector( &dvec, &ssltr_group->now_pos, &ssltr_group->to_pos, 0.008f*1.2f );
			ssltr_group->max_act = work->n_in_group/16;
			ssltr_group->wait = 0;
		}else if( len > 2000.0f*2000.0f ){
			DG_COPY_VEC( &ssltr_group->to_pos, &ssltr_group->center );
			_SetSsVector( &dvec, &ssltr_group->now_pos, &ssltr_group->to_pos, 0.01f*1.2f );
			ssltr_group->max_act = work->n_in_group/8;
			ssltr_group->wait = 0;
		}else if( len > 1600.0f*1600.0f ){
			DG_COPY_VEC( &ssltr_group->to_pos, &ssltr_group->center );
			_SetSsVector( &dvec, &ssltr_group->now_pos, &ssltr_group->to_pos, 0.01f*1.2f );
			ssltr_group->max_act = work->n_in_group/4;
			ssltr_group->wait = 0;
			
		}else if( len > 1000.0f*1000.0f ){
			//DG_COPY_VEC( to_pos, center );
			_SetSsVector( &dvec, &ssltr_group->now_pos, &ssltr_group->to_pos, 0.02f*1.2f );
			ssltr_group->max_act = work->n_in_group/2;
			ssltr_group->wait = 0;
		}else{
			//２メーター以内
			ssltr_group->max_act = work->n_in_group;

			_SetSsVector( &dvec, &ssltr_group->now_pos, &ssltr_group->to_pos, 0.04f*1.2f );
			_SetSsToPosition( &ssltr_group->to_pos, &ssltr_group->now_pos, &ssltr_group->wide,
							  &check_pos, &check_vec,
							  &DG_ZeroVector, work->all_wide );
		}
#else
		if( ssltr_group->flags & GROUP_FLAG_REFUGE ){
			if( !(ssltr_group->flags & GROUP_FLAG_ESCAPE) &&
			...
			_SetSsVector( &dvec, &ssltr_group->now_pos, &ssltr_group->to_pos, 0.01f );
			if( --(ssltr_group->wait) < 0 ){
				ssltr_group->flags &= ~(GROUP_FLAG_REFUGE|GROUP_FLAG_ESCAPE);
			}
		}else if( len > 4000.0f*4000.0f ){
			DG_COPY_VEC( &ssltr_group->to_pos, &ssltr_group->center );
			_SetSsVector( &dvec, &ssltr_group->now_pos, &ssltr_group->to_pos, 0.008f );
			ssltr_group->max_act = work->n_in_group/16;
			ssltr_group->wait = 0;
		}else if( len > 2000.0f*2000.0f ){
			DG_COPY_VEC( &ssltr_group->to_pos, &ssltr_group->center );
			...
			//DG_COPY_VEC( to_pos, center );
			_SetSsVector( &dvec, &ssltr_group->now_pos, &ssltr_group->to_pos, 0.02f );
			ssltr_group->max_act = work->n_in_group/2;
			ssltr_group->wait = 0;
		}else{
			//２メーター以内
			...
			_SetSsVector( &dvec, &ssltr_group->now_pos, &ssltr_group->to_pos, 0.04f );
			_SetSsToPosition( &ssltr_group->to_pos, &ssltr_group->now_pos, &ssltr_group->wide,
							  &check_pos, &check_vec,
							  &DG_ZeroVector, work->all_wide );
		}
...
//		_SetSsVector( &dvec, now_pos, to_pos, 0.1f );
		...
		foot = foot_top;
		for( j = 0; j < n_foot; j++ ){
			if( (DG_FABS( ssltr_group->now_pos.vx - foot->vx ) < ssltr_group->wide.vx) &&
				(DG_FABS( ssltr_group->now_pos.vz - foot->vz ) < ssltr_group->wide.vz) ){
				//AN_Test_Eye2( foot, 2 );
				FVECTOR		foot_pos;
				_RotTrans( &foot_pos, work->root, foot );
				SetTrmplSlater( &foot_pos, work->root, 20.0f*rnd()+40.0f );
			}
			foot++;
		}
#endif
		...
		for( j = 0; j < n_bullet; j++ ){
			if( (DG_FABS( ssltr_group->now_pos.vx - bullet->vx ) < ssltr_group->wide.vx) &&
				(DG_FABS( ssltr_group->now_pos.vz - bullet->vz ) < ssltr_group->wide.vz) ){

				//DG_COPY_VEC( &local_bul[n_local_bul], bullet );
				_RotTrans( &local_bul[n_local_bul], work->root, bullet );
				ssltr_group->n_act = work->n_in_group;
				ssltr_group->flags = ((ssltr_group->flags&0xff00)|GROUP_FLAG_HASTY);
				n_local_bul++;
			}
			bullet++;
		...
		LoadMatHermiteLerp( work->root );
		if( !(BoundCheckFlag( &ssltr_group->now_pos, &ssltr_group->wide, 1 )&1)){
//		if( 1 ){
#if 0
			{
				//デバック表示
				FVECTOR	bound[2];
				_sceVu0AddVector( &bound[0], &ssltr_group->now_pos, &ssltr_group->wide );
				_sceVu0SubVector( &bound[1], &ssltr_group->now_pos, &ssltr_group->wide );
			...
			for( j = 0; j < work->n_in_group; j++ ){
				...
				world.m[1][1] = 1.0f;
				world.m[0][0] = ssltr_group->scale*TS_COSs(srot_data);
				world.m[2][2] = ssltr_group->scale*TS_COSs(srot_data);
				world.m[2][0] = ssltr_group->scale*TS_SINs(srot_data);
				world.m[0][2] = -ssltr_group->scale*TS_SINs(srot_data);

				{
					...
					SetSsMatrix_Refuge( (FMATRIX*)comdl_pos,
										...
										//-0.5f*( TS_COSs( data->timer/2 ) - 1.0f ),
										&outer, &ssltr_group->now_pos, &dvec, 1.0f, NULL );//RndTable[j&7] );

#if 0
					//デバック
					if( i == 0 ){
						printf( "t = %f\n",-0.5f*( TS_COSs( data->timer/2 ) - 1.0f ) );
						PRINT_PFVEC(0,(FVECTOR*)((FMATRIX*)comdl_pos)->m[0]);
						PRINT_PFVEC(1,(FVECTOR*)((FMATRIX*)comdl_pos)->m[1]);
						PRINT_PFVEC(2,(FVECTOR*)((FMATRIX*)comdl_pos)->m[2]);
						PRINT_PFVEC(3,(FVECTOR*)((FMATRIX*)comdl_pos)->m[3]);
				...
				if( (ssltr_group->index <= j && j < ssltr_group->index+ssltr_group->n_act) ||
					( j <= ssltr_group->index + ssltr_group->n_act - work->n_in_group ) ){
			...
			ssltr_group->timer += MOVE_TIME;
			if( ssltr_group->timer >= 0x1000 ){
				ssltr_group->index += ssltr_group->n_act;
				if( ssltr_group->index >= work->n_in_group )
					ssltr_group->index -= work->n_in_group;
				ssltr_group->timer &= 0x0fff;

				//printf("%d[ %d, %d ]\n",i,ssltr_group->index,ssltr_group->n_act);
			}

			n_act += work->n_in_group;
		}else{
			data += work->n_in_group;
		}
		...
		if( foot->vw > 0.0f ){
			for( i = 0; i < work->n_group; i++ ){
				if( (DG_FABS( ssltr_group->now_pos.vx - foot->vx ) < ssltr_group->wide.vx) &&
					...
					FVECTOR		foot_pos;
					_RotTrans( &foot_pos, work->root, foot );
					SetTrmplSlater( &foot_pos, &DG_UnitMatrix, 20.0f*rnd()+40.0f );
					foot->vw = -1.0f;
					break;
				}
				ssltr_group++;
...
static void Die(Work *work)
{
	...
	if(work->ssltr_data) GV_DelayedFree(work->ssltr_data);
	if(work->ssltr_group) GV_DelayedFree(work->ssltr_group);
}
...
static int GetResources( Work *work )
{
	...
	//ここで親のチェック君を取ってくる
	//親からＡＬＬトラップ情報及び逃げトラップ情報を取ってくる
	//原点計算して最後に親のマトリックスに掛けるほうこうで
	if( !AllSlater_CurrentWork ){ printf("リーダーを起動してからにして下さい\n"); return -1; }
	work->allwork = AllSlater_CurrentWork;
	work->root = &AllSlater_CurrentWork->root;							//親から貰う
	work->inv_root = &AllSlater_CurrentWork->inv_root;					//親から貰う
	work->n_refuge = AllSlater_CurrentWork->n_refuge;					//親から貰う
	work->all_center = &AllSlater_CurrentWork->center;					//親から貰う
	work->all_wide = &AllSlater_CurrentWork->wide;						//親から貰う
	work->refuge_center = AllSlater_CurrentWork->refuge_center[0];		//親から貰う
	work->refuge_wide = AllSlater_CurrentWork->refuge_wide[0];			//親から貰う

	//ここでフラグを取る
	work->flags = AllSlater_CurrentWork->main_flags;					//親から貰う

	work->n_group = GCL_GetOptionValue( 'g', 0 );
	work->n_in_group = GCL_GetOptionValue( 'i', 0 );

	work->n_ssltr = n_ssltr = work->n_group*work->n_in_group;

	if( !n_ssltr ){ printf("ゼロって意味ないジャン<sea_slater>\n"); return -1; };

	ssltr_data = work->ssltr_data = (SSLTR_DATA*)GV_Malloc( sizeof(SSLTR_DATA) * n_ssltr );
	if( !ssltr_data ) { printf("NO MEM <sea_slater>\n"); return -1; }

	ssltr_group = work->ssltr_group = (SSLTR_GROUP_DATA*)GV_Malloc( sizeof(SSLTR_GROUP_DATA) * work->n_group );
	if( !ssltr_group ) { printf("NO MEM <sea_slater>\n"); return -1; }
	...
	//グループ情報の初期化
	for( i = 0; i < work->n_group; i++ ){
		if( GCL_GetNextOption() != 'd' ){ printf("Err Wrong GCL Param -data <sea_slater>\n"); return -1; }
		ssltr_group->center.vx = (float)GCL_GetNextInt();
		ssltr_group->center.vy = 1.0f;
		ssltr_group->center.vz = (float)GCL_GetNextInt();
		ssltr_group->center.vw = 1.0f;
		ssltr_group->wide.vx = ssltr_group->wide.vz = (float)GCL_GetNextInt();
		ssltr_group->wide.vy = 0.0f;
		ssltr_group->wide.vw = 0.0f;

		//この時点でバンダリチェック入れます。
		if( ssltr_group->center.vx + ssltr_group->wide.vx > work->all_wide->vx ){
			ssltr_group->center.vx = work->all_wide->vx - ssltr_group->wide.vx;
			printf("!!!!slater baund over CHECK!! SCN!!\n");
		}else if( ssltr_group->center.vx - ssltr_group->wide.vx < -work->all_wide->vx ){
			ssltr_group->center.vx = -work->all_wide->vx + ssltr_group->wide.vx;
			printf("!!!!slater baund over CHECK!! SCN!!\n");
		}
		if( ssltr_group->center.vz + ssltr_group->wide.vz > work->all_wide->vz ){
			ssltr_group->center.vz = work->all_wide->vz - ssltr_group->wide.vz;
			printf("!!!!slater baund over CHECK!! SCN!!\n");
		}else if( ssltr_group->center.vz - ssltr_group->wide.vz < -work->all_wide->vz ){
			ssltr_group->center.vz = -work->all_wide->vz + ssltr_group->wide.vz;
			printf("!!!!slater baund over CHECK!! SCN!!\n");
		}
		
		DG_COPY_VEC( &ssltr_group->now_pos, &ssltr_group->center );
		DG_COPY_VEC( &ssltr_group->to_pos, &ssltr_group->center );
		ssltr_group->scale = ssltr_group->wide.vx/SCALE_RATIO;
		ssltr_group->timer = 0;
		ssltr_group->index = 0;
		ssltr_group->n_act = 2;
		ssltr_group->max_act = 2;
		ssltr_group->wait = 0;
		ssltr_group->flags = 0;

		_sceVu0AddVector( &center, &center, &ssltr_group->center );

		ssltr_group++;
	}

	_sceVu0ScaleVector( &center, &center, 1.0f/(float)work->n_group );

	_RotTrans( &work->center, work->root, &center );
#if 0
	{
		FMATRIX			light[2];
		DG_GetLightMatrix( &work->center, light );
		color = GetRGBFromLightNorm( (FVECTOR*)work->root->m[1], light );
	}
#endif
...
void *NewSeaSlater( int name, int map )
{
	Work *work = NULL;
...
//	work = (Work*)GV_NewEffect(GV_ACTOR_EFFECT,sizeof(Work));
	...
	if(work){
		GV_SetActor(&(work->actor),SeaSlaterAct,Die) ;
