...
/*
	...
	$Id: emm_slater.c,v 1.1.1.3 2002/11/19 11:48:49 Yoshizawa1 Exp $
...
#define		MOVE_TIME		(BP_AdjustTick2(4096/64))
...
typedef struct _emm_slater_work {
	GV_ACT_EX		actor;
...
#define     GROUP_FLAG_WAITRENDER (0x0800)
...
#ifndef BP_PS2 //yano modified 2002.03.04
								...
								FVECTOR *outer, FVECTOR *data, float scale )
{
	...
	out_mat->m[3][3] = 1.0f ;//yano 2002.03.04

	_sceVu0ScaleVectorXYZ( (FVECTOR *)out_mat->m[2], &vec2, scale ); //yano 2002.03.04

	_sceVu0ScaleVectorXYZ( (FVECTOR *)out_mat->m[1], outer, scale ); //yano 2002.03.04

	out_mat->m[0][3] = 0.0f ;//yano 2002.03.04
	_sceVu0ScaleVectorXYZ( (FVECTOR *)out_mat->m[0], &opvec, scale ); //yano 2002.03.04
									  ...
									  FVECTOR *bound_pos, FVECTOR *bound_size )
{
	...
	n_len = bp_sqrtf(1400.0f*1400.0f-p_len*p_len);  //BP_MATH - emulate PS2 sqrtf
...
static void EmmSlaterAct(Work *work)
{
...
//	DG_COMDL_POS 	*comdl_pos_top = work->low_cm->pos;
       ...
       (BP_IsPAL()!=TRUE) && GM_WaterLevel < DG_Chanls[0].eye.m[3][1] && !(GV_Time%18) )
   {
		...
		for( i = 0,n_act = 0; i < N_SSLT_GROUP; i++ ){
			tmp = _Vu0VecLenXYZ( &now_pos[i], (FVECTOR*)DG_Chanls[0].eye.m[3] );

			if( ((group_flags[i] & GROUP_FLAG_REFUGE) && !(group_flags[i] & GROUP_FLAG_ESCAPE)) ){
				if( !check || len > tmp ){
					len = tmp;
					n_act = group_n_act[i];
					DG_COPY_VEC( &nzl_pos, &now_pos[i] );
				}
				check = 1;
		...
		if( check ){
			...
			//printf("funa 4\n");
		}else if( N_SSLT_IN_GROUP/3 > n_act ){
			int se_table[] = {
			...
			GM_SeSetMode( se_table[(int)g_SeFrip], &nzl_pos, GM_SEMODE_NORMAL );
			//printf("funa 1\n");
		}else if( N_SSLT_IN_GROUP*2/3 > n_act ){
			int se_table[] = {
	...
	for( i = 0; i < N_SSLT_GROUP; i++ ){
      ...
      if ( BP_IsPAL()==TRUE )
      {
		   if( group_flags[0] & GROUP_FLAG_REFUGE ){
			   ...
			   //DG_COPY_VEC( to_pos, center );
			   _SetSsVector( &dvec, now_pos, to_pos, 0.02f*1.2f );
			   group_max_act[0] = 8;
		   }else{
			   group_max_act[0] = N_SSLT_IN_GROUP;
			   //逃げる処理いれるとこなのね
			   _SetSsVector( &dvec, now_pos, to_pos, 0.04f*1.2f );
...
//		_SetSsVector( &dvec, now_pos, to_pos, 0.1f );
		...
		//
#endif
      if ( !BoundCheckFlag( now_pos, wide, 1 ) )
      {
//		if( i == 0 ){
#if 0
			{
...
{
	int				cnt = 0,check,i,j,k;
	HZX_GRP			*group;
	HZX_BLOCK		*block;
	HZX_TRP			*trap;
	FVECTOR			bound0,bound1,fvtemp0;
	IVECTOR			check_verts[N_SSLT_GROUP][2];
	int				check_block[N_SSLT_GROUP];
...
*/
#if 1
		for( k = 0,check = 0; k < cnt; k++ ){
			if( check_verts[k][0].vx == trap->b1.vx + block->tx &&
				check_verts[k][0].vy == trap->b1.vy + block->ty &&
				check_verts[k][0].vz == trap->b1.vz + block->tz &&
				check_verts[k][1].vx == trap->b2.vx + block->tx &&
				check_verts[k][1].vy == trap->b2.vy + block->ty &&
				check_verts[k][1].vz == trap->b2.vz + block->tz /* && check_block[k] != j */ ){
		...
		check_verts[cnt][0].vx = trap->b1.vx + block->tx;
		check_verts[cnt][0].vy = trap->b1.vy + block->ty;
		check_verts[cnt][0].vz = trap->b1.vz + block->tz;
		check_verts[cnt][1].vx = trap->b2.vx + block->tx;
		check_verts[cnt][1].vy = trap->b2.vy + block->ty;
		check_verts[cnt][1].vz = trap->b2.vz + block->tz;
		...
		//center[cnt].vy += 20.0f;
		//PRINT_PFVEC(cnt,&center[cnt]);
		_sceVu0AddVector( all_cnt, all_cnt, &center[cnt] );

#define		SCALE_RATIO		(226.2741700f)		//160.0f* (root 2.0f)
		scale[cnt].vx = wide[cnt].vx/SCALE_RATIO;
		scale[cnt].vz = wide[cnt].vz/SCALE_RATIO;
...
static int GetResources( Work *work )
{
	...
	if( (ret = GetTrapInfo( work->trap_center, work->trap_wide, work->trap_scale, &center, hzx_group, trap_code )) ){
...
void *NewEmmaSlater( int name, int map )
{
	Work *work = NULL;
...
//	work = (Work*)GV_NewEffect(GV_ACTOR_EFFECT,sizeof(Work));
	...
	if(work){
		GV_SetActor(&(work->actor),EmmSlaterAct,Die) ;
