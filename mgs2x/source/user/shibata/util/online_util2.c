...
/*
   ...
   $Id: online_util2.c,v 1.1.1.3 2002/11/19 11:48:54 Yoshizawa1 Exp $
...
//printf("to vx %f: vy %f: vz = %f\n\n",to.vx,to.vy,to.vz);
	...
	hzx_result = HZX_OnlineHazardCheck( 0, &from, &to,
										HZX_CHK_DYNAMIC, HZX_SEG_NO_BULLET, HZX_SEG_NO_BULLET );
	
	tmp = HZX_CurrentGroupID ;
	HZX_CurrentGroupID = 0 ;
	hzx_result |= HZX_OnlineHazardCheck( work->hzx_id, &from, &to,
...
static int GetResources( Work *work, int hzx_id,
...
*/
void *NewOnlineCheck2_Static( int map_id,
							  FVECTOR *from, FVECTOR *to,
							  int chk_flag, int seg_flag, int flr_flag,
							  int *result,
							  int turn, int phase, float max_len  )
