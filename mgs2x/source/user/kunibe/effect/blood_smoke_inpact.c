...
/*
    ...
    $Id: blood_smoke_inpact.c,v 1.1.1.3 2002/11/19 11:44:33 Yoshizawa1 Exp $
*/
...
#else
#define COL_R		(255)//(48)
#define COL_G		(243)//(230)//(0)
#define COL_B		(243)//(230)//(0)
#define ALPHA		(16)//(64)
#endif
...
static void Act( Work *work )
{
	...
	if ( work->cnt < 6 ) {					// 60
	    ...
	    _sceVu0ScaleVector( vec, vec, 0.6f );
	    uvrgbwh->w = uvrgbwh_pre->w + 20;
	    uvrgbwh->h = uvrgbwh_pre->h + 20;
	    uvrgbwh->a = work->alpha[i];// * work->cnt / 6;	// 60
	}
	else {
	    _sceVu0AddVector( pos, pos_pre, vec );
	    ...
	    if ( uvrgbwh_pre->a > 0 ) {
		uvrgbwh->w = uvrgbwh_pre->w + 5;
		uvrgbwh->h = uvrgbwh_pre->h + 5;
		if ( i%4 == work->cnt%4 ) {
...
static int InitPacket( Work *work, FVECTOR* center, DG_PRIM2 *prim, DG_TEX *tex )
{
	...
	for ( i = 0 ; i < N_SPRTS ; i++ ) {
	    ...
	    uvrgbwh->b  = ~COL_B;
	    uvrgbwh->a  = 0;
	    work->alpha[i] = ALPHA + irnd()%16;

	    work->rot[i] = frnd();
	    work->rad[i] = 300.0f + frnd()*50.0f;
	    uvrgbwh->w  = work->rad[i] * vu0_Cos( work->rot[i]*PI );
	    uvrgbwh->h  = work->rad[i] * vu0_Sin( work->rot[i]*PI );
	    
	    ftmp = SPEED * ( 0.60f + frnd() * 0.40f );
	    angle1 = PI * frnd();
	    angle2 = PI * frnd();
	    vec->vx = ftmp * vu0_Cos( angle1 ) * vu0_Cos( angle2 );
	    vec->vy = ftmp * vu0_Sin( angle1 );
	    vec->vz = ftmp * vu0_Cos( angle1 ) * vu0_Sin( angle2 );
