...
/*
  ...
  $Id: poly_plasma2.c,v 1.1.1.3 2002/11/19 11:44:46 Yoshizawa1 Exp $
*/
...
#define	RADIUS			(50.0f)//(100.0f)
#define SPEED			(20.0f)//(40.0f)
...
static void Act( Work *work )
{
	...
	/* 繋ぎ目 */	
	    ...
	    DG_COPY_VEC( &work->bottom[i], &m_bottom );

	    _sceVu0AddVector( &center, &center, &vec_x2 );
	    DG_COPY_VEC( &work->bottom[i+1], &center );
...
static int InitPacket( Work *work, DG_PRIM2 *prim, DG_TEX *tex, FVECTOR *color )
{
	...
	for ( i = 0 ; i < N_PRIMS; i++ ) {
		...
		if ( j % 2 ) {
			...
			_sceVu0ScaleVector( &work->wave[n], (FVECTOR *)mat.m[0], RADIUS );
...
/*
  ...
  FVECTOR	*color
*/  
void *NewPolyPlasma2( FVECTOR *from, FVECTOR *middle, FVECTOR *to, float width, FVECTOR *color )
{
	Work	*work;
