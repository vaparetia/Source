...
/*
  ...
  $Id: fatman_scratch_spark.c,v 1.1.1.3 2002/11/19 11:44:39 Yoshizawa1 Exp $
*/
		       ...
		       SVECTOR *rot_wide )
{
	...
	for ( i = 0; i < n_prims; i++ ) {
	    ...
	    /* 位置決定 */
	    /* blade方向にランダム */
	    _sceVu0ScaleVector( &rnd_width, (FVECTOR *)work->node_mat.m[2], frnd()*200.0f );
	    _sceVu0AddVector( &init_pos, (FVECTOR *)work->node_mat.m[3], &rnd_width );
	    init_pos.vy -= 100.0f;	    
	    
	    _sceVu0AddVector( &pos[1], speed, &init_pos );
	    pos[1].vw = 1.0F;
		...
		int	  count )
{
	...
	if ( work != NULL ) {
		...
		DG_COPY_MAT( &work->node_mat, node_mat );
