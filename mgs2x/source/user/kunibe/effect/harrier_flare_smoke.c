...
/*
    ...
    $Id: harrier_flare_smoke.c,v 1.1.1.3 2002/11/19 11:44:41 Yoshizawa1 Exp $
*/
...
static void Act( Work *work )
{
	...
	alpha = (int)( (float)( ALPHA * ( N_SPRTS - j ) / N_SPRTS ) * work->alpha_param );	

	if ( alpha > 0 ) {
	    ftmp = work->radius[i] * (float)( N_SPRTS - j ) / (float)N_SPRTS
		+ work->radius[i] * 3.0f * (float)j / (float)N_SPRTS;
    ...
    _sceVu0SubVector( &vectmp, &prevpos, work->pos );
	...
	work->initsize = work->initsize * ( 1.0f + frnd()*0.1f );
	...
	else if ( work->initsize < work->size * 0.9f ) {
	    work->initsize = work->size * 0.9f;
	}    
	...
	for ( i = 0 ; i < n ; i++ ) {
	    ...
	    uvrgbwh[work->id].a = (short)( (float)uvrgbwh[work->id].a * work->alpha_param );
...
static int InitPacket( Work *work, DG_PRIM2 *prim, DG_TEX *tex )
{
	...
	for ( i = 0 ; i < N_SPRTS ; i++ ){
	    ...
	    ftmp = (short)( (float)COLOR * ( 1.0f * frnd()*0.25f ) );
	    uvrgbwh->r  = ftmp;
	    uvrgbwh->g  = ftmp;
	    uvrgbwh->b  = ftmp;
	    uvrgbwh->a  = 0;
	    ...
	    angle = TPI * frnd();
	    uvrgbwh->w  = (short)( ftmp * vu0_Cos( angle ) );
	    uvrgbwh->h  = (short)( ftmp * vu0_Sin( angle ) );
