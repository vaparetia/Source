...
/*
    ...
    $Id: missile_exp.c,v 1.1.1.3 2002/11/19 11:44:44 Yoshizawa1 Exp $
*/
...
static int GetResources( Work *work, FVECTOR* center, float size )
{
...
void *NewMissileExplosion( FVECTOR* center, float size )
{
	Work	*work;
	...
	if ( work != NULL ) {
		...
		if ( GetResources( work, center, size ) < 0 ) {
			GV_DestroyActor( work );
			return NULL;
		}
	}
