...
/*
  ...
  $Id: sprt_plasma2.c,v 1.1.1.3 2002/11/19 11:44:52 Yoshizawa1 Exp $
*/
...
#define	WAVE_HEIGHT		(40.0f)//(80.0f)//(100.0f)
#define SPEED			(40.0f)//(40.0f)//(40.0f)//(40.0f)//(100.0f)//(100.0f)//(50.0f)
	...
	for ( l = 0 ; l < 4 ; l++ ) {
		...
		for ( j = 0 ; j < N_VERTS ; j++ ) {
			...
			if ( n < HALF_NUM ) {
			    ...
			    _sceVu0ScaleVector( &p_param->height,
						&height, (float)( HALF_NUM*HALF_NUM - (n-HALF_NUM)*(n-HALF_NUM) ) / (float)(HALF_NUM*HALF_NUM) );
			    _sceVu0ScaleVector( &p_param->wave, (FVECTOR *)mat.m[0], WAVE_HEIGHT );
...
static int GetResources( Work *work, FVECTOR *from, FVECTOR *middle, FVECTOR *to, float width, FVECTOR *color, int life )
{
...
void *NewSpritePlasma2( FVECTOR *from, FVECTOR *middle, FVECTOR *to, float width, FVECTOR *color, int life )
{
	...
	if ( work != NULL ) {
		...
		if ( GetResources( work, from, middle, to, width, color, life ) < 0 ) {
			GV_DestroyActor( work );
			return NULL;
		}
	}
