...
/*
    ...
    $Id: c_box_fire.c,v 1.1.1.3 2002/11/19 11:44:34 Yoshizawa1 Exp $
*/
...
static int ActFire( Work *work )
{
    ...
    if ( work->pre_flag == 1 ) {
	if ( *work->break_flag == 2 ) {
	    DG_COPY_MAT( &work->mat, work->matrix );
	    work->matrix = &work->mat;
	}
	work->pre_flag = *work->break_flag;
    }
	...
	//NewBoxSmoke( (FVECTOR *)work->matrix->m[3], 300.0f );
	NewBoxSmoke( &work->smoke_pos, 300.0f );	/* いいのか？びみょー */
    }    
	...
	if ( work->rate > 0.40f ) {
	    if ( work->se_count <= 0 ) {
		GM_SeSetMode( CBOX_FIRE_SE[work->se_id],
			      (FVECTOR *)work->matrix->m[3],
			      GM_SEMODE_NORMAL );	
...
static int GetResources( Work *work, FMATRIX *mat, FVECTOR *bound_max, FVECTOR *bound_min, int *break_flag, int se_flag )
{
    ...
    DG_COPY_VEC( &work->bound_max, bound_max );
    DG_COPY_VEC( &work->bound_min, bound_min );
    
    work->break_flag = break_flag;
    work->pre_flag   = *work->break_flag;
...
/*-------- void *NewBoxFire( FMATRIX *mat, FVECTOR *bound_max, FVECTOR *bound_min, int *break_flag ) ----------
                               電撃床用ダンボール燃えエフェクト
     FMATRIX *mat        : ダンボールブロックのマトリクス
     FVECTOR *bound_max  : ダンボールのバウンディングボックス(最大)
     FVECTOR *bound_min  : ダンボールのバウンディングボックス(最小)
     int     *break_flag : ダンボールの壊れ状態フラグ (#user/okajima/effect/c_box_body.c参照)
-------------------------------------------------------------------------------------------------------------*/
void *NewBoxFire( FMATRIX *mat, FVECTOR *bound_max, FVECTOR *bound_min, int *break_flag, int se_flag )
{
	...
	if ( work != NULL ) {
		...
		if ( GetResources( work, mat, bound_max, bound_min, break_flag, se_flag ) < 0 ) {
			GV_DestroyActor( work );
			return NULL;
		}
	}
