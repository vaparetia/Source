...
/*
    ...
    $Id: c_box_smoke.c,v 1.1.1.3 2002/11/19 11:44:34 Yoshizawa1 Exp $
*/
...
static void Act( Work *work )
{
	...
	j = work->id - i;
	if ( j < 0 ) j += N_PRIMS;
	alpha = work->alpha_param * ALPHA * ( N_PRIMS - j ) / N_PRIMS;
	
	ftmp = 1.0f + frnd() * 0.2f;
	_sceVu0ScaleVector( &vectmp, &work->wind, ftmp );
	_sceVu0AddVector( &pos[i], &pos_pre[i], &vectmp );

	pos[i].vy += work->rising_spd * ( 1.0f + 3.0f * (float)j / (float)N_PRIMS );

	
	uvrgbwh[i].a = alpha;
	if ( uvrgbwh[i].a > 0 ) {
	ftmp = work->radius[i] * (float)( N_PRIMS - j ) / (float)N_PRIMS + work->radius[i] * 2.0f * (float)j / (float)N_PRIMS;
	...
	uvrgbwh[work->id].a *= work->alpha_param;
    ...
    if ( work->cnt >= 120 ) {
	work->size *= 0.995f;
	work->initsize *= 0.995f;
    }

    if ( work->cnt >= 300 ) {
	work->alpha_param = 1.0f * (float)( 420-work->cnt ) / 120.0f;
    }
...
static int GetResources( Work *work, FVECTOR* pos, float size )
{
    ...
    work->size = size;
    work->initsize = size;
    work->rising_spd = 20.f * size * (0.8f + frnd()*0.2f) / 300.f;//15.f// 400.f
    work->id  = 0;
    ...
    prim = work->prim = GM_MakePrim2( DG_PRIM2_RSPRT|DG_PRIM2_TEX|DG_PRIM2_ALPHA, N_PRIMS/8, 8 );
	if ( prim == NULL ) {
		return -1;
	}	
