...
/*
    ...
    $Id: emma_body_fire.c,v 1.1.1.3 2002/11/19 11:44:38 Yoshizawa1 Exp $
*/
...
static int ActFire( Work *work )
{
	...
	if ( uvrgbwh_pre->a > ALPHA_MIN ) {
		...
		_sceVu0ScaleVector( &vectmp, (FVECTOR *)work->body->objs->objs[param->node1].world.m[3], param->param );

		_sceVu0ScaleVector( &init_pos, (FVECTOR *)work->body->objs->objs[param->node2].world.m[3], ( 1.0f-param->param ) );
		
		_sceVu0AddVector( &init_pos, &init_pos, &vectmp );

		if ( param->node1 == 0 || param->node1 == 1 || param->node1 == 2 ) {
	...
	/* スプライト初期化 */
	    ...
	    if ( i > ( work->cnt - LIFE ) && ( work->cnt-LIFE ) < N_SPRT*3/4 ) {
		...
		_sceVu0ScaleVector( &vectmp, (FVECTOR *)work->body->objs->objs[param->node1].world.m[3], ftmp );
		_sceVu0ScaleVector( &init_pos, (FVECTOR *)work->body->objs->objs[param->node2].world.m[3], ( 1.0f-ftmp ) );
...
static int ActPowder( Work *work )
{
	...
	if ( work->cnt%2 ) {
	    ...
	    DG_COPY_VEC( pos, (FVECTOR *)work->body->objs->objs[0].world.m[3] );
...
static int InitPacket( Work *work )
{
	...
	for ( i = 0 ; i < N_SPRT ; i++ ){
	    ...
	    uvrgbwh->b  = COL_B;
	    uvrgbwh->a  = ALPHA_MIN + irnd()%ALPHA_WIDTH;//32 + irnd()%32;
...
static int GetResources( Work *work, OBJECT *body, int se_flag )
{
...
void *NewEmmaBodyFire( OBJECT *body, int se_flag )
{
	...
	if ( work != NULL ) {
		...
		if ( GetResources( work, body, se_flag ) < 0 ) {
			GV_DestroyActor( work );
			return NULL;
		}
	}
