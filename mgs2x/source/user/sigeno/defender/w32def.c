...
/*
	...
	拠点防御イベント兵
	2000/03/01 K.Sigeno
	$Id: w32def.c,v 1.1.1.3 2002/11/19 11:49:11 Yoshizawa1 Exp $
*/
...
extern void W32MedicationStart(ENETHINK *) ;

void	SIG_DefenderResurrect( ENETHINK * ) ;



static int w32_Noactive(ENETHINK *entk){
	if(entk->act->bodyp.life<=0) return 1 ;
	if(CheckNoActive(entk)) return 1 ;
	return 0 ;
}
...
*********************/
//int EMA_CommandEneAttack( int name ) ;攻撃開始時に呼ぶ
...
printf("ATTACK END!!!!!\n");
//		EMA_CommandEneDied( entk->ctrl->name ) ;
	}
}
...
printf("NOTICE\n");
		entk->think1 = ENE_TH1_ALERT ;
printf("DAM ATTACK START!!!!!\n");
		if(eve_w->find_flag & W32_FIND){ 
//			EMA_CommandEneAttack( entk->ctrl->name ) ;
		}
#if 1
		W32MedicationStart(entk) ;
#else
...
#define W32_MSG_ROUTE_CHANGE (1)
#define W32_MSG_ATTACK_MODE (2)
...
//		entk->alert = ALERT_LEVEL_MAX ;
	...
	if(w32_Noactive(entk)) EMA_CommandEneDied( entk->ctrl->name ) ;
...
printf("BODY FLAG [%x]\n",work->body.flag) ;
printf("OBJS FLAG [%x]\n",work->body.objs->flag) ;
printf("OBJ FLAG [%x]\n",work->body.objs->objs[0].flag) ;
printf("BODY MAP_NAME [%x]\n",work->body.map_name) ;
...
static int w32_GetResources( Work *work, int name, int type ,int *mt_array)
{
	...
	ACTION		*act ;
	OBJECT		*body, *weapon;
	int		root, node, bodyname, wpname ,sub_wpname ,life, faint, value;
	int	flag ;
//	FVECTOR	d_pos;
	char	*opt ;
...
#if 0
ENE_MDL_NAME_GPS /* GV_StrCode( "gps_def_mt" ) */
ENE_MDL_NAME_GPA /* GV_StrCode( "gpa_def_mt" ) */
ENE_MDL_NAME_GBA 
ENE_MDL_NAME_GBS 
#endif
...
#ifndef W32_TEST_MODE
	act->bodyp.type |= (ENE_TYPE_ATTACKER|ENE_TYPE_DMG_HAND_LEG|ENE_TYPE_ABAKAN
		|ENE_TYPE_NO_HEAR|ENE_TYPE_NO_BLURR|ENE_TYPE_EVENT_32) ;
//		|ENE_TYPE_NO_HEAR|ENE_TYPE_NO_BLURR) ;
#else
	act->bodyp.type |= (ENE_TYPE_NO_HEAR|ENE_TYPE_NO_BLURR|ENE_TYPE_EVENT_32|ENE_TYPE_ABAKAN|ENE_TYPE_DMG_HAND_LEG) ;
#endif
...
/******/
#if 0
	entk->sense.eye_s_s[ALERT_MODE_SNEAK] = 0 ;
	entk->sense.eye_s_s[ALERT_MODE_ALERT] = 0 ;
	entk->sense.eye_s_s[ALERT_MODE_AVOID] = 0 ;
	entk->sense.eye_s_s[ALERT_MODE_SEARCH] = 0 ;
#endif
/************/
	/*装備品*/
	/*こっちは兵士の種類を決定*/
...
//	int	i,mode;
	...
	w32_GetResources(work ,name ,ENE_TYPE_EVENT_A,NULL) ;
	work->eve_w.find_flag = 0; 
	entk->character = eve_w = &work->eve_w ;
	act = &work->action ;
//ASSERT(entk->character != NULL);
...
/*W32_GetResourc内で設定された目標をすり替え*/
	if(t_ctrl != NULL ){
		ENE_SetEyeInfo( &(entk->pl_eyei), &t_ctrl->mov, 
		&t_ctrl->addr, NULL, &t_ctrl->map ) ;
	}
	/*守備兵に必須の非通報フラグ*/
...
void *NewW32Defender( int name, int where )
{
	Work		*work ;
