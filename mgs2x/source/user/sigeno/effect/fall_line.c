...
/*
	fall_line.c
	//void	*NewVR_Fall_Line( int flag,FVECTOR *pos,float hankei ,float height,int time)
	VR 光降る
	2002/04/18 K.Sigeno
	$Id: fall_line.c,v 1.1.1.3 2002/11/19 11:49:46 Yoshizawa1 Exp $
*/
#include <sys/types.h>
#include <stdlib.h>
#include <stdio.h>
...
#if 1
//降雨
				tmp.vx = pos->vx = (hankei)*cosf(((float)M_PI*2.0f)*(float)rot/4095.0f) ;
				tmp.vy = ((irnd()>>8)%(work->hankei*2)) - (work->hankei);
				tmp.vz = pos->vz = (hankei)*sinf(((float)M_PI*2.0f)*(float)rot/4095.0f) ;

				pos->vy = tmp.vy + work->height ;
...
//	work->line_num = LINE_NUM * STD_DIS /GetCameraDis(&work->pos) ;
	work->line_num = LINE_NUM ;
