...
/*	説明:	ルートを作成							*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
void make_kas_rout(Kas_Work *work){
    int		loop;
    float	num;
    Work	*hwork;
	...
	work->kas_data[loop].time = kas_data_near[loop].time;
	work->kas_data[loop].action = kas_data_near[loop].action;
    }
...
/*	説明:	ルートを自動表示						*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
void view_auto_root_ks(ACRO_POINT_EX *acropoint, int number, int scale, int maxnum){
    FVECTOR	*FROM_POS;
    FVECTOR	*TO_POS;
    FVECTOR	FROM_VEC;
    FVECTOR	TO_VEC;
...
/*	説明:	アクロバット飛行						*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static void hermite_move_ks(Kas_Work *work, ACRO_POINT_EX *acropoint, int number, int max_num, float time){		
    short	num_plus;
    FVECTOR	*FROM_POS;
    FVECTOR	*TO_POS;
    FVECTOR	FROM_VEC;
	...
	if (HAR_ViewKacRout == 1){
	    ...
	    view_auto_root_ks(acropoint, number, max_time0, max_num);
	    view_auto_root_ks(acropoint, num_plus, max_time1, max_num);
	}
    }
#endif
...
int move_kasaka(Kas_Work *work){		
    ...
    hermite_move_ks(work, work->kas_data, work->rout_number, work->rout_num_max, work->rout_time);
