...
/*	$Id: eyecheck.c,v 1.1.1.3 2002/11/19 11:48:16 Yoshizawa1 Exp $		*/
/********************************************************************************/
...
/*	説明:									*/
/********************************************************************************/
void CYP_HMK_Check(EYEPARAM *eye , FMATRIX *head, void* p){
    
    if(eye->discv_time == HEADMARK_SET){
	GM_SeSetMode( SD_E_BIKKRI01 , eye->eyepos,
		      GM_SEMODE_BOMB );

	CallActHeadMarks( p, HMK2_TYPE_RED_AT_MIN);
	COM_DetectVibration( );
    }else if(eye->discv2 == HEADMARK_SET){

	    CallActHeadMarks( p, HMK2_TYPE_WHT_AT_MIN);
...
void DEV_CameraSe(EYEPARAM *eye ){
	/*首振り効果音*/
	if((DG_TickCount - eye->se_tick)*TIME_BASE > CAM_SE_INT){	//音を鳴らした瞬間を保存
...
/*	説明:	ダメージ兵と敵死兵を探す					*/
/********************************************************************************/
int DEV_EnemyCheck(EYEPARAM *eye , HZX_GROUP_ID hzx_id ){
    ...
    if (com == NULL){
	printf ("監視カメラ/サイファーより先にコマンダーを起動して下さい\n");
    }
...
void DEV_Bullet(EYEPARAM *eye ,FMATRIX *head){
    ...
    NewBullet( &mat,type, PLAYER_SIDE, 25, 5, 20000, 1000, WP_m4 ) ;
