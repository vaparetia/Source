...
/*
	ef_smoke.c
	量産型ＲＡＹ用ダミーエフェクト（ミサイル煙）

	2001/05/19 K.Takabe
	$Id: ef_smoke.c,v 1.1.1.3 2002/11/19 11:51:21 Yoshizawa1 Exp $
...
#if 0
#define START_SIZE		( 125 )
#define ZOOM_OFFSET		( 20 )	/* １フレームあたりの拡大サイズ */
#define SMOKE_SPEED		( 60 )
#define SMOKE_SCATTER	( 20 )
#define SMOKE_LOSE		( 1.0f )
#endif
#if 0
#define START_SIZE		( 500 )
#define ZOOM_OFFSET		( 30 )	/* １フレームあたりの拡大サイズ */
#define SMOKE_SPEED		( 200 )
#define SMOKE_SCATTER	( 32 )
#define SMOKE_LOSE		( 1.0f )
#endif
#if 1
#define START_SIZE		( 250 )
#define ZOOM_OFFSET		( 34 )	/* １フレームあたりの拡大サイズ */
...
static int GetResources( Work *work, FMATRIX *parent )
{
	...
	{/* バックファイア用プリミティブ初期化 */
		...
		work->prim2 = prim = GM_MakePrim2( PRIM_FLAG2, N_PRIMS2, N_VERTS2 );
		DG_ConfigPrim2Tex( prim, tex );
		DG_SetPrim2Alpha( prim, SCE_GS_SET_ALPHA( 0, 2, 0, 1, 0 ) );
		prim->root = work->parent ;
		for ( k = 0 ; k < 2 ; k++ ){
			pos = prim->pos[ k ];
...
/* ---------------------------------------------------------------- */
void *NewRAYMissileSmoke( FMATRIX *parent )
{
	Work		*work ;
