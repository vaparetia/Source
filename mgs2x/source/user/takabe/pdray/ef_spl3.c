...
/*
	...
	2001/07/29 K.Takabe
	$Id: ef_spl3.c,v 1.1.1.3 2002/11/19 11:51:22 Yoshizawa1 Exp $
...
/* ---------------------------------------------------------------- */
/* シグナル関連 */
#define SIGNAL_RAYJUMPSPLASH			(0xf569e200)	/* GV_StrCode("RayJumpSplash")<<8 */
#define SIGNAL_RAYJUMPSPLASH_STOP		(SIGNAL_RAYJUMPSPLASH|0x01)
/* ローカル用再定義 */
#define SIGNAL_THIS_EFFECT	(SIGNAL_RAYJUMPSPLASH)
#define SIGNAL_STOP			(SIGNAL_THIS_EFFECT|0x01)
/* ---------------------------------------------------------------- */
/* エフェクトパラメータ */
#define CONTINUE_TIME	(60)	/* エフェクト持続時間 */

#define GEN_NUM			(4)		/* １フレームの発生数 */
#define ALIVE_FRAME		(16*4)	/* スプライトの生存フレーム数 */
#define START_SIZE		( 600 )
#define ZOOM_OFFSET		( 20 )	/* １フレームあたりの拡大サイズ */
#define SMOKE_SPEED		( 900 )
#define SMOKE_SCATTER	( 80 )
#define SMOKE_LOSE		( 0.90f )
...
#define PRIM_FLAG2			(DG_PRIM2_SPRT|DG_PRIM2_ALPHA|DG_PRIM2_TEX)
#define N_PRIMS2			(1)
#define N_VERTS2			(32)
...
typedef struct _scrpad{/* 合計１６ＫＢ以下であること！ */
	FVECTOR				pos[ ALIVE_FRAME * GEN_NUM ] ;
	FVECTOR				vvec[ ALIVE_FRAME * GEN_NUM ] ;
	DG_PRIM2_UVRGBWH	uvrgbwh[ ALIVE_FRAME * GEN_NUM ] ;
} ScrpadWork ;
...
static void Act( Work *work )
{
	...
	alpha = 1.0f - DG_MIN( 1.0f, (float)work->continue_time / CONTINUE_TIME ) ;
   ...
   if( !DG_SwitchBuffPrim2( prim ) )
   {
	...
	for ( j = 0 ; j < DM_FrameSkip + 1 ; j++ ){
		  ...
		  case 0:/* 煙生成フェーズ */
			for ( i = 0 ; i < GEN_NUM ; i++ ){
				...
				{/* 回転スプライトの大きさ及び角度を決定する */
					...
					GTE_SinCos( &sincos, GTE_PS2RAD( 512 ) );
					GTE_ScaleVector( &size, &sincos, START_SIZE * ( alpha + 0.2f ) );
					GTE_ScaleVector( &offset, &sincos, ZOOM_OFFSET * ( alpha + 0.2f ) );
				...
				GTE_ZeroVector( &spr_vvec[i] );
				spr_vvec[i].vx = RND( SMOKE_SCATTER ) - SMOKE_SCATTER/2 ;
				spr_vvec[i].vz = RND( SMOKE_SCATTER ) - SMOKE_SCATTER/2 ;
				spr_vvec[i].vw = 0.0f ;
				GTE_AddVector( &spr_pos[i], &spr_pos[i], &spr_vvec[i] );
			}
			break ;
		  case 1:/* フェードフェーズ */
			for ( i = 0 ; i < GEN_NUM ; i++ ){
				GTE_ZeroVector( &spr_pos[i] );	/* 以降の頂点データは原点へ */
				spr_uvrgbwh[i].a = 0 ;
				spr_uvrgbwh[i].w = 0 ;
				spr_uvrgbwh[i].h = 0 ;
				spr_vvec[i].vw = 0.0f ;
		...
		{/* 着水水しぶき発生チェック */
			...
			vvec = work->vvec ;
			GTE_UnitMatrix( &mat );
			GTE_ScaleVector( (FVECTOR*)mat.m[0], (FVECTOR*)mat.m[0], 0.05f );
			GTE_ScaleVector( (FVECTOR*)mat.m[1], (FVECTOR*)mat.m[1], 0.05f );
			GTE_ScaleVector( (FVECTOR*)mat.m[2], (FVECTOR*)mat.m[2], 0.05f );
			for ( i = 0 ; i < ALIVE_FRAME * GEN_NUM ; i++ ){
				if ( vvec->vw == 1.0f ){
					*(FVECTOR*)mat.m[3] = *vvec ;
	...
	work->continue_time += TIME_BASE ;
	if ( work->continue_time > CONTINUE_TIME ){
		work->phase = 1 ;
	}
...
/* ---------------------------------------------------------------- */
void *NewRayJumpSplash( FMATRIX *parent )
{
	Work		*work ;
