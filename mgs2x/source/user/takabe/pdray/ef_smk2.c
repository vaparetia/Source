...
/*
	...
	2001/07/19 K.Takabe
	$Id: ef_smk2.c,v 1.1.1.3 2002/11/19 11:51:21 Yoshizawa1 Exp $
...
/* ローカル用再定義 */
#define SIGNAL_THIS_EFFECT	(SIGNAL_RAYBREAKSMOKE)
#define SIGNAL_STOP			(SIGNAL_THIS_EFFECT|0x01)
/* ---------------------------------------------------------------- */
/* エフェクトパラメータ */
#define GEN_NUM			(1)		/* １フレームの発生数 */
#define ALIVE_FRAME		(16*4)	/* スプライトの生存フレーム数 */
#define START_SIZE		( 1500 )
#define ZOOM_OFFSET		( 10 )	/* １フレームあたりの拡大サイズ */
#define SMOKE_SPEED		( 60 )	/*  */
#define SMOKE_SCATTER	( 60 )	/* 散らばり具合 */
#define SMOKE_LOSE		( 0.95f )

/* ---------------------------------------------------------------- */
/* プリミティブパラメータ */
#define PRIM_FLAG	(DG_PRIM2_RSPRT|DG_PRIM2_TEX|DG_PRIM2_ALPHA|DG_PRIM2_BOUNDCHECK)
#define N_VERTS		(16)
...
static void Act( Work *work )
{
   ...
   if( !DG_SwitchBuffPrim2( prim ) )
   {
	...
	for ( j = 0 ; j < DM_FrameSkip + 1 ; j++ ){
		...
		GTE_PutVector1( (FVECTOR*)world.m[3], &work->offset );
		...
		switch ( work->phase ){
		  case 0:/* 煙生成フェーズ */
			{/* 頂点移動量の算出 */
				FVECTOR		base_vec ;
				base_vec.vy = SMOKE_SPEED + RND( SMOKE_SCATTER );
				base_vec.vy = SMOKE_SPEED + RND( SMOKE_SPEED / 4 );
				base_vec.vx = RND( SMOKE_SCATTER ) - SMOKE_SCATTER / 2 ;
				base_vec.vz = RND( SMOKE_SCATTER ) - SMOKE_SCATTER / 2 ;
				base_vec.vw = 0.0f ;
		...
		for ( i = 0 ; i < ALIVE_FRAME * GEN_NUM ; i++ ){
			if ( spr_uvrgbwh->a != 0 ){/* データがあるもののみ更新するようにする */
				...
				spr_vvec->vx *= SMOKE_LOSE ;
				spr_vvec->vz *= SMOKE_LOSE ;
				/* サイズ変更 */
				...
				//if ( a < 0 ) a = 0 ;
				a = 12 * ( i + 1 ) / ( ALIVE_FRAME * GEN_NUM ) ;
				GTE_MaxVector( &max, &max, spr_pos ) ;
				GTE_MinVector( &min, &min, spr_pos ) ;
				spr_uvrgbwh->r = a ;
				spr_uvrgbwh->g = a ;
				spr_uvrgbwh->b = a ;
				//a = spr_uvrgbwh->a - 64 / ALIVE_FRAME ;
				//if ( a < 0 ) a = 0 ;
				a = 32 * ( i + 1 ) / ( ALIVE_FRAME * GEN_NUM ) ;
				spr_uvrgbwh->a = a ;
			}
			spr_uvrgbwh++ ;
			spr_pos++ ;
		...
		prim->bound_max = max ;
		prim->bound_min = min ;
		prim->bound_max.vx += 2000.0f ;
		prim->bound_max.vy += 2000.0f ;
		prim->bound_max.vz += 2000.0f ;
		prim->bound_max.vw = 1.0f ;
		prim->bound_min.vx -= 2000.0f ;
		prim->bound_min.vy -= 2000.0f ;
		prim->bound_min.vz -= 2000.0f ;
		prim->bound_min.vw = 1.0f ;
	}
...
/* ---------------------------------------------------------------- */
static int GetResources( Work *work, FMATRIX *parent, FVECTOR *offset )
{
	DG_PRIM2	*prim ;
...
/* ---------------------------------------------------------------- */
void *NewRAYBreakSmoke( FMATRIX *parent, FVECTOR *offset )
{
	Work		*work ;
	...
	if ( work != NULL ) {
		...
		if ( GetResources( work, parent, offset ) < 0 ) {
