/*
	...
	2001/02/15 K.Takabe
	$Id: f_focus2.c,v 1.1.1.3 2002/11/19 11:51:09 Yoshizawa1 Exp $
...
{
	/* パケットメモリ開放 */
	GV_DelayedFree( work->dmapack->packet[0] );
	/* ＤＭＡパケットオブジェクト開放 */
...
static int GetResources_S( Work *work, int name, int where )
{
	...
	work->disable = 1 ;
	work->max_plane = 4 ;
	work->target_focus_near = DRAW_Z_MAX ;	/* フォーカス開始距離 */
	work->target_focus_far = DRAW_Z_MAX ;	/* フォーカス終了距離 */
...
static int GetResources_P( Work *work, int name, int max_plane, int near, int far )
{
	work->disable = 0 ;
	work->max_plane = max_plane ;
	work->target_focus_near = CalcDepth( near ) ;
	work->target_focus_far = CalcDepth( far ) ;
	return GetResources( work, name, GM_CurrentMap );
}

/* ---------------------------------------------------------------- */
/* シナリオ呼び出しインターフェイス */
void *NewFarFocusEffectSet2( int name, int where )
{
	Work		*work ;
...
/* プログラム呼び出しインターフェイス */
void *NewFarFocusEffect2( int name, int max_plane, int near, int far )
{
	Work		*work ;
	...
	if ( work != NULL ) {
		GV_SetActor( &( work->actor ), Act, Die ) ;
		GV_ActorEX( &work->actor );
		if ( GetResources_P( work, name, max_plane, near, far ) < 0 ) {
			GV_DestroyActor( work ) ;
			return NULL ;
		}
	}
