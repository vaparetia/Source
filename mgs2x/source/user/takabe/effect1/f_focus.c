...
/*
	...
	2000/03/02 K.Takabe
	$Id: f_focus.c,v 1.5 2002/11/23 12:09:25 Yoshizawa1 Exp $
...
static void Act( Work *work )
{
	...
	if( ( n_msg = GV_ReceiveMessage( work->name, &msg ) ) == 0 ){
		...
		/* メッセージチェック */
		for ( msg+=n_msg-1 ; n_msg > 0 ; n_msg--, msg-- ){
...
static void *_farfocus_buffer_callback(void *pParam, DG_DMAPACK *pDstDmaPack)
{
   Work* pWork = (Work*)pParam;
   SFarFocusRenderWork *dst_work = (SFarFocusRenderWork *)DG_AS_SceneBufferAlloc(sizeof(SFarFocusRenderWork));
   dst_work->max_plane = pWork->max_plane;
   dst_work->focus_near = pWork->focus_near;
   dst_work->focus_far = pWork->focus_far;

   return dst_work;
}

static void BP_FarFocusCallback(void* param)
{
   SFarFocusRenderWork* pWork = (SFarFocusRenderWork*)param;
   SBP_PFX_FarFocus* pPacket = (SBP_PFX_FarFocus*)BP_RB_Alloc(sizeof(SBP_PFX_FarFocus));
   pPacket->max_plane = pWork->max_plane;
   pPacket->focus_near = pWork->focus_near;
   pPacket->focus_far = pWork->focus_far;
   BP_RB_AddCommand(kCmd_PostFx_FarFocus, (char*)pPacket);
}
...
static int GetResources( Work *work, int name, int where )
{
   ...
   DG_AS_DMAPackSetCallbacks(dmapack, BP_FarFocusCallback, NULL);
...
static int GetResources_S( Work *work, int name, int where )
{
	...
	work->disable = 1 ;
	work->max_plane = 4 ;
	work->target_focus_near = 1 ;	/* フォーカス開始距離 */
	work->target_focus_far = 1 ;	/* フォーカス終了距離 */
