...
/*
	...
	2000/11/10 K.Takabe
	$Id: eye_ctrl.c,v 1.1.1.3 2002/11/19 11:51:09 Yoshizawa1 Exp $
...
typedef	struct	{
	...
	FMATRIX		*target_pos ;		/* 視線方向用ダミーオブジェクトの位置 */
	/* 視線制御関係 */
...
static void Act( Work *work )
{
	...
	{/* 視線制御処理 */
		...
		/* 視線方向絶対座標を求める */
#if 1
		work->eye_target = *(FVECTOR*)work->target_pos->m[3] ;
#else
...
/* ---------------------------------------------------------------- */
	/*
		キャラクタ初期化関数
	*/
static int GetResources_P( Work *work, int name, OBJECT *body, FMATRIX *target,
					...
					FVECTOR *rmax, FVECTOR *rmin, FVECTOR *lmax, FVECTOR *lmin, int flag )
{
	...
	if ( work != NULL ) {
		...
		if ( GetResources_P( work, name, body, target, rmax, rmin, lmax, lmin ) < 0 ) {
