...
/*
	...
	2001/02/26 K.Takabe
	$Id: wave5.c,v 1.1.1.3 2002/11/19 11:51:17 Yoshizawa1 Exp $

*/
/*

chara プラント海面[NewSeaSurfaceSet] $s:name \
	...
	-flag $w:フラグ
mesg プラント海面 $s:名前 表示状態[2] $b:ＯＮ・ＯＦＦ
mesg プラント海面 $s:名前 slepp[3]
mesg プラント海面 $s:名前 wake[4]
mesg プラント海面 $s:名前 上下運動計算処理[5] $b:ＯＮ・ＯＦＦ
mesg プラント海面 $s:名前 表示位置移動[256] $v:座標
mesg プラント海面 $s:名前 テクスチャ変更[257] $s:テクスチャ名 ...
...
// $b:ＯＮ・ＯＦＦは 0:OFF, 1:ON と指定してください
...
typedef	struct	{
	...
	int			disable_calc_flag ;				/* 波状態非計算フラグ */
...
#else
#define RND( _n )	( ( rand() * (_n) ) >> 15 )
#define RNDS( _n )	( ( ( rand() * (_n) ) >> 14 ) - ( _n ) )
#endif
/* ---------------------------------------------------------------- */
...
static void InitWaveParts( Work *work, WaveParts *wave, int s, int t )
{
...
#else
				patch->flag |= DG_PATCH_REFLECTPLANE2 ;	/* XBOXではVu1プログラム差し替えがないのでこれで対処 */
#endif
				/* 環境マップ補正用マトリクス生成 */
	...
	for ( i = 0 ; i < work->max_patch_plane ; i++ ){
		...
		FlushCache( 0 );
#endif
      memcpy( patch->verts[ 0 ], verts, MESH_X * MESH_Z * 4 * sizeof(u_long128) );
      memcpy( patch->verts[ 1 ], verts, MESH_X * MESH_Z * 4 * sizeof(u_long128) );
...
{
	GV_MSG		*msg ;
	int			n_msg, i, j, k, move_flag = 0 ;
   int      as_lean_and_mean = 0;
   int      as_frame_skip = 0;

   as_frame_skip = DG_Arm_SkipThisFrame();

   if ( gAS_DG_HackArea == kDGHA_w32a )
   {
      as_lean_and_mean = as_frame_skip;
   }
	...
	if ( ( n_msg = GV_ReceiveMessage( work->name, &msg ) ) > 0 ){
		for ( ; n_msg > 0 ; n_msg--, msg++ ){
			switch ( msg->message[0] ){
			  ...
			  case 4:/* スリープ解除（ダミー） */
				break ;
			  case 5:/* 波シミュレーション計算許可の変更 */
				work->disable_calc_flag = ( msg->message[1] == 0 ) ;
				break ;
			  case 256:/* 表示座標の移動 */
	...
	if ( move_flag ){
		for ( i = 0 ; i < PATCH_Z ; i++ ){
			for ( j = 0 ; j < PATCH_X ; j++ ){
				...
				wave = &work->w_parts[ j + i * PATCH_X ] ;
				wave->offset.vx = work->center.vx + (float)( MESH_WIDTH_X * MESH_DIV_X ) * ( j - PATCH_X * 0.5f + 0.5f ) ;
				wave->offset.vy = work->center.vy ;	/* これは使ってないので意味なし */
				wave->offset.vz = work->center.vz + (float)( MESH_WIDTH_Z * MESH_DIV_Z ) * ( i - PATCH_Z * 0.5f + 0.5f ) ;
				for ( k = 0 ; k < work->max_patch_plane ; k++ ){
					wave->patch[k]->world.m[3][0] = wave->offset.vx ;
					wave->patch[k]->world.m[3][1] = wave->offset.vy ;
   ...
   if ( !as_lean_and_mean )
   {
	   for ( k = 0 ; k <= DM_FrameSkip ; k++ ){
		   if ( work->disable_calc_flag == 0 && !as_lean_and_mean ){
            float ave_height = 0.f;
...
//					   total_ave_height += ave_height ;
	...
	if ( work->disable_calc_flag == 0 && !as_lean_and_mean && !as_frame_skip ){
   ...
   if ( !as_frame_skip )
   {
	   for ( i = 0 ; i < PATCH_Z ; i++ ){
		   for ( j = 0 ; j < PATCH_X ; j++ ){
			   ...
			   FlushCache( 0 );
   #endif
			   DG_StartMemToSpr( (void*)SCRPAD_ADDR, wave->patch[0]->verts[ wave->patch[0]->buffer_clock ],
			   ...
			   for ( k = 0 ; k < work->max_patch_plane ; k++ ){
				   ...
				   DG_StartSprToMem( patch->verts[ patch->buffer_clock ], (void*)SCRPAD_ADDR,
								    SIZEOF_QWORD(DG_PATCH_VERT) * MESH_X * MESH_Z );
				   DG_EndSprToMem();
			   }
		   }
...
static int GetResources( Work *work, int name, int where )
{
   ...
   int in_w32 = !strcmp(GM_GetArea(), "w32a") || !strcmp(GM_GetArea(), "w32b");
	...
	for ( i = 0 ; i < PATCH_Z ; i++ ){
		for ( j = 0 ; j < PATCH_X ; j++ ){
         WaveParts	*wave ;
         // Arm fix:
         // shift around the patches and scale up the parts
         // if we scale up each patch by x%, then we need to move the outer patches by (3/4)*x%*old_patch_width
         // fix for MGSTWO-2855
         float arm_scale = 1.02f;
         float arm_patch_offset = 0.75f * (arm_scale - 1.0f) * (MESH_WIDTH_X * MESH_DIV_X);
         // do it for everything except the area where the player is close to the surface
         if (in_w32)
         {
            arm_scale = 1.0f;
            arm_patch_offset = 0.0f;
         }

			wave = &work->w_parts[ j + i * PATCH_X ] ;
			InitWaveParts( work, wave, j, i );
			wave->offset.vx = pos.vx + (float)( MESH_WIDTH_X * MESH_DIV_X ) * ( j - PATCH_X * 0.5f + 0.5f ) + (j - 1) * arm_patch_offset;
			wave->offset.vy = pos.vy ;
			wave->offset.vz = pos.vz + (float)( MESH_WIDTH_Z * MESH_DIV_Z ) * ( i - PATCH_Z * 0.5f + 0.5f ) + (i - 1) * arm_patch_offset;
			wave->offset.vw = 1.0f ;
			for ( k = 0 ; k < work->max_patch_plane ; k++ ){
            int e;
            for (e = 0; e < 9; ++e)
            {
               wave->patch[k]->world.m[e%3][e/3] = arm_scale*wave->patch[k]->world.m[e%3][e/3];
            }
				wave->patch[k]->world.m[3][0] = wave->offset.vx ;
				wave->patch[k]->world.m[3][1] = wave->offset.vy ;
				wave->patch[k]->world.m[3][2] = wave->offset.vz ;
...
/* ---------------------------------------------------------------- */
	/*
		シナリオ起動インターフェイス
	*/
void *NewSeaSurfaceSet( int name, int where )
{
	Work		*work ;
...
static void LoadVu0MicroProgram( void )
{
	...
	DG_DmaStartCheck( 0 );
	DPUT_D0_MADR( (int)( CalcPatchRayPickVu + 1 ) ) ;
		...
		for ( i = 0 ; i < PATCH_Z ; i++ ){
			for ( j = 0 ; j < PATCH_X ; j++ ){
				...
				if ( last_addr != verts ){
					last_addr = verts ;
					FlushCache( 0 );
					DG_StartMemToSpr( (void*)SCRPAD_ADDR, verts, SIZEOF_QWORD(DG_PATCH_VERT) * MESH_X * MESH_Z );
					DG_EndMemToSpr();
				}
