...
/*
	...
	$Id: manhat3d.c,v 1.1.1.3 2002/11/19 11:47:46 Yoshizawa1 Exp $
*/
...
extern void Big_PutManHatLight_Yw(FMATRIX *world, FVECTOR *cam) ; 
extern void Big_PutManHatLight_L1(FMATRIX *world, FVECTOR *cam) ;
extern void Big_PutManHatLight_L2(FMATRIX *world, FVECTOR *cam) ;
...
static	void  Act(Work *work)
{
	...
	for(i = 0; i < N_UNITS; i++,unit++){
		...
		if(i == 0){
			Big_PutManHatLight_Yw(&world,&cam->position);
			Big_PutManHatLight_L1(&world,&cam->position);
		}else if(i == 1){
			Big_PutManHatLight_L2(&world,&cam->position);
		}
...
static	void  Die(Work *work)
{
	...
	if( work->fog_wall_work2 != NULL ){
		GV_DestroyOtherActor( work->fog_wall_work2 );
		work->fog_wall_work2 = NULL;
	}
...
static	int   GetResources(Work *work, int fog_flag)
{
	...
	unit = work->unit ;
	/* マンハッタン */
	def[0] = (DG_DEF*)GV_GetCache(GV_CacheID(1673890 /*"w00_l1"*/,'k')) ;
	if(def[0] == NULL){
		printf("w00_l1 not exit\n") ;
		return -1 ;
	}
	def[1] = (DG_DEF*)GV_GetCache(GV_CacheID(1673891 /*"w00_l2"*/,'k')) ;
	if(def[1] == NULL){
		printf("w00_l2 not exit\n") ;
		return -1 ;
	}
	def[2] = (DG_DEF*)GV_GetCache(GV_CacheID(1673892 /*"w00_l3"*/,'k')) ;
	if(def[2] == NULL){
		printf("w00_l3 not exit\n") ;
		return -1 ;
	}
	def[3] = (DG_DEF*)GV_GetCache(GV_CacheID(1673893 /*"w00_l4"*/,'k')) ;
	if(def[3] == NULL){
		printf("w00_l4 not exit\n") ;
		return -1 ;
	}
	...
	for(i = 0; i < N_UNITS ; i++,unit ++){
		...
		for(j = 0; j < N_OBJS; j++,pos++){
			if(i == 0){
				...
				pos->vx = FIRST_POS_X - 30000.0F - 20000.0F * (i - 1) ;
			}
			if( height_flag ){
				pos->vy = (float)height ;
			}else{
...
void *NewManhattan3D(int name,int map )
{
	Work		*work ;
...
void *NewManhattan3D_Fog(int name, int map )
{
	Work		*work ;
