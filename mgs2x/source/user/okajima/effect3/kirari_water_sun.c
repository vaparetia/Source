...
/*
	...
	$Id: kirari_water_sun.c,v 1.1.1.3 2002/11/19 11:47:33 Yoshizawa1 Exp $
*/
//rnd
...
static	void	Act( Work *work )
{
	...
	if( GM_CheckGameStatus( STATE_DEMO ) ){
		_sceVu0SubVector( &fvtemp2, &cam_pos, &work->before_cam_pos );
		ftemp0 = GV_VecLen3F( &fvtemp2 );
...
#if 0
	col_r = (int)(DG_ColorMatrix.m[3][0])&255;
	col_g = (int)(DG_ColorMatrix.m[3][1])&255;
	col_b = (int)(DG_ColorMatrix.m[3][2])&255;
#else
...
/* 0DIV回避 2002.03.11 ヤノ */
	{ 
		float dmdm;
		dmdm =  cam_pos.vy - sun_pos_mirror.vy;
		if( dmdm == 0.0f ) dmdm = 0.000001f; 
		_sceVu0ScaleVector( &fvtemp3, &fvtemp2, (cam_pos.vy - GM_WaterLevel)/(dmdm) );
	}
...
//	AN_Test_Eye2( &sun_pos_mirror, 2 );
...
#ifdef KP_XBOX
	if ( sun_pos.vw==0.0f ) sun_pos.vw = 0.00001f ;
#endif
...
#ifdef KP_XBOX
	if ( sun_pos_mirror.vw==0.0f ) sun_pos_mirror.vw = 0.00001f ;
#endif
...
#ifdef KP_XBOX
	if ( near_limit.vw==0.0f ) near_limit.vw = 0.00001f ;
#endif
	...
	for ( i=0; i<N_PARTS; i++ ){
#ifdef KP_XBOX
		if ( cal_pos_after->vw == 0.0f ) cal_pos_after->vw = 0.00001f ;
#endif
...
//				ftemp0*= ftemp0;
		...
		if( flag ){
//rnd
			ftemp2 = rnd()*frnd();	// 太陽鏡面のスクリーン座標からの距離比
//			ftemp2 = (float)(N_PARTS-i*2)/(float)N_PARTS;	// 太陽鏡面のスクリーン座標からの距離比
			...
			cal_pos_after->vy = sun_pos_mirror.vy + limit_diff*ftemp2;	// スクリーンＹ
			cal_pos_after->vz = SCREEN_NEAR;

			DG_PutVector( cal_pos_after, cal_pos, 1 );
			_sceVu0SubVector( &fvtemp2, cal_pos, &cam_pos ) ;
/* 0DIV回避 2002.03.11 ヤノ */
//_sceVu0ScaleVector( cal_pos, &fvtemp2, (cam_pos.vy - GM_WaterLevel)/(cam_pos.vy - cal_pos->vy) );
	{ 
		float dmdm;
...
void *NewKirariWaterSun( FMATRIX *world, int col )
{
	Work		*work ;
