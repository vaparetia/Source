...
/*
	...
	2001/08/01 S.Okajima
	$Id: peter_blood.c,v 1.1.1.3 2002/11/19 11:47:34 Yoshizawa1 Exp $
*/
...
//if(GV_Time%300==0) work->on_flag = 1 - work->on_flag;

	if( work->on_flag ){
		work->fade_time++;
		if( work->fade_time > FADE_TIME ) work->fade_time = FADE_TIME;
	}else{
		work->fade_time--;
		if( work->fade_time < 0 ) work->fade_time = 0;
	}

	work->col>>=8;
	work->col<<=8;
	work->col |= ((work->alpha_max * work->fade_time / FADE_TIME)&255);
...
//	if( !(GV_PadData[ 0 ].status & PAD_X) ) return;
	...
	radius = DG_MAX( work->width, work->height ) * 1.5f;
...
void *NewPeterBlood( int name, FVECTOR *pos, FVECTOR *vec, float width, float height, int col )
{
	Work		*work ;
	SVECTOR		svtemp;
	...
	if ( work != NULL ) {
		...
		OK_DirVecXY( &DG_ZeroVector, vec, &svtemp );

		work->name = name;
		DG_SetPos2( pos, &svtemp );
...
void *NewPeterBlood_Gas( int name, FVECTOR *pos, FVECTOR *vec, float width, float height, int col )
{
	Work		*work ;
	SVECTOR		svtemp;
