...
/*
	...
	mountain_splush.c
	今度こそ沸き上がる水面
	2001/06/30 S.Okajima
	$Id: mountain_splush.c,v 1.1.1.3 2002/11/19 11:47:33 Yoshizawa1 Exp $
...
#define	CHECK_HEIGHT		(2000.0f)

/*----------------------------------------------------------------*/
extern float GM_WaterLevel;
extern int	OK_PutSplush( FVECTOR *center, FVECTOR *force );
/*----------------------------------------------------------------*/
void *NewWaterSurfaceMountainSub( FVECTOR *center, float radius, float intense, int flag );
/* ---------------------------------------------------------------- */
typedef	struct	{
	GV_ACT_EX	actor ;
...
static void Act( Work *work )
{
	...
	if( work->edge_flag==0 ){
		shift = work->radius_add + work->radius_min;

		ftemp1 = TPI * (float)work->count * (1.0f/16.0f);

		fvtemp.vx = shift * sinf(ftemp1)   +  work->center.vx;
		fvtemp.vy = GM_WaterLevel + 1.0f;
		fvtemp.vz = shift * cosf(ftemp1)   +  work->center.vz;
		if( HZX_LevelHazardCheck(
				GM_GetHzxGroupID( GM_CurrentStageMap ),
				&fvtemp,
				HZX_CHK_F_FLOOR,
				HZX_FLOOR_NO_BULLET | HZX_FLOOR_RECOIL_TYPE ) & 2 ){
			HZX_GetLevelHeight( flr_height );
//printf("%f %f\n",flr_height[1],GM_WaterLevel);
			if( flr_height[1] - GM_WaterLevel < CHECK_HEIGHT ){
//				printf("CHECK\n");
//				fvtemp.vy = flr_height[1];
//				AN_Test_Eye2( &fvtemp, 2 );
	...
	if( !work->sub
	 ...
	 && (flag > 30 && flag < 60) ){
		...
		shift = work->radius_add*(1.0f+rnd());
		ftemp1= TPI*rnd();
		center.vx+= shift*sinf( ftemp1 );
		center.vy = GM_WaterLevel;
		center.vz+= shift*cosf( ftemp1 );
		NewWaterSurfaceMountainSub(
			&center,
			work->radius_min*(0.5f+rnd()),
			work->intense*(0.5f+rnd()*0.25f),
			work->flag );
	}
	if( !work->sub
	 && work->life == work->life_max/4 ){
		NewWaterSurfaceMountainSub(
			&work->center,
			work->radius_min*4.0f,
			work->intense*0.5f,
			work->flag );
	}
...
flag:0x02 減算ＯＮ
*/
void *NewWaterSurfaceMountain( FVECTOR *center, float radius, float intense, int flag )
{
	Work		*work ;
