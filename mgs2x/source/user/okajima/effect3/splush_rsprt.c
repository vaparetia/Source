...
/*
	...
	$Id: splush_rsprt.c,v 1.1.1.3 2002/11/19 11:47:37 Yoshizawa1 Exp $
*/
...
//	tex = DG_GetTexture( 3594043 /*"drop01_msk"*/ );
	...
	// 中心位置
#endif
		extern void* NewOneRipple( FVECTOR*	pvecPos,
		    float	fRadius,	// 半径
		    int		nTime,		// 時間
		    int		nAddNum,	// 同時生成数	
		...
		NewOneRipple( &work->center, 100.f, 10, 2, 1, 4, 200.f, 500.f);
	}
#endif
...
void *NewSplushRotateSplush( FVECTOR *pos, float radius, float intense, int col )
{
	Work		*work ;

	OPERATOR() ;

	if( OK_SpNum > 4 ) return NULL;
