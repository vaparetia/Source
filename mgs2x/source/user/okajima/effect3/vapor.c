...
/*
	...
	$Id: vapor.c,v 1.1.1.3 2002/11/19 11:47:38 Yoshizawa1 Exp $
*/
	...
	if( !work->fade_out_started ){
		...
		for( k=0; k<loop*4; k++ ){
			...
			if( (*life)>= LIFE_MAX ){
				...
				sc_pos1->vy+= work->radius*0.25f*rnd();
				sc_pos1->vz+= radius*cosf( angle );

#if 1
				angle = TPI*rnd();
				radius = work->radius*rnd()*0.01f;
				sc_pos2->vx = radius*sinf( angle );
				sc_pos2->vy = work->radius*rnd()*0.05f;
...
static int GetResources( Work *work )
{
	...
	for ( j=0; j<LIFE_MAX; j++ ){
		(*col_dat++) = (int)(col_a * sinf( PI * (float)j / (float)(LIFE_MAX-1) )   );
	}
...
void *NewVapor( FVECTOR *center, float radius, int col, int life )
{
	Work		*work ;
