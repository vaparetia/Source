...
//----------------------------------------------------------------------------
/*
	霧雨（透視変換バージョン）：カメラ高速移動対応：風対応

	2001/08/16 S.Okajima
	$Id: gas2_pers_fast.c,v 1.1.1.3 2002/11/19 11:47:31 Yoshizawa1 Exp $
*/
...
static	void	Act( Work *work )
{
	...
	OK_GetLocalWind2( &cam_pos, &wind );
	wind.vy *= 0.25f;
	_sceVu0ScaleVector( &wind, &wind, OK_slow_param*0.25f );
...
//	NewBoundingBoxView( &bound_1, &bound_0, 255, 128, 64 ) ;
	...
	for ( i = 0 ; i < N_PRIMS*N_POLYS ; i++ ){
		_sceVu0AddVector( space, space, space_vec ) ;
...
static int GetResources( Work *work )
{
	...
	/* 表示範囲 */
	work->bound_0.vx =  BOUND_WIDTH*2 ;
	work->bound_0.vy =  BOUND_WIDTH*2 ;
	work->bound_0.vz =  BOUND_WIDTH*2 ;
	work->bound_1.vx = -BOUND_WIDTH*2 ;
	work->bound_1.vy = -BOUND_WIDTH*2 ;
	work->bound_1.vz = -BOUND_WIDTH*2 ;
...
void *NewRainFogPersFast2( int name, u_int col )
{
	Work		*work ;
