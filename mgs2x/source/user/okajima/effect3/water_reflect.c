...
/*
	...
	$Id: water_reflect.c,v 1.1.1.3 2002/11/19 11:47:39 Yoshizawa1 Exp $
	...
	for ( i = 0 ; i < N_SIDES0 ; i++ ){
		...
		if( f1 > 1.0f ){
			for ( k = 0 ; k < N_SIDES1 ; k++ ){
				...
				p_f = work->rot_shift;
				p_f+= (*(p_i++));
				latice_pos->vy = edge.vy + width.vy;
				latice_pos->vz = 0.0f;
				latice_pos++;
			}
		}else{
...
/* ---------------------------------------------------------------- */
static void Calc_UV( Work *work, DG_PRIM2 *prim )
{
	int	i,j;
	float	ftemp1;
	DG_PRIM2_UVRGB	*uvrgb0_0;
	...
	uvrgb0_0 = prim->uvrgb[0];
	uvrgb0_1 = uvrgb0_0;
	uvrgb0_1++;
	uvrgb1_0 = prim->uvrgb[1];
	uvrgb1_1 = uvrgb1_0;
	uvrgb1_1++;

	for ( i = 0 ; i < N_SIDES0 ; i++ ){

		ftemp1 = work->shift_max * (float)(i  ) / (float)N_LOOP0 + work->offset;
		...
		if( i==0 ){
			for ( j = 0 ; j < N_SIDES1 ; j++ ){
//				uvrgb0_0->a = uvrgb1_0->a = 0;
				uvrgb0_0->u = uvrgb1_0->u = FTOI12( ftemp1 );
				uvrgb0_0+= 2;
				uvrgb1_0+= 2;
			}
		}else if( i==N_SIDES0-1 ){
			for ( j = 0 ; j < N_SIDES1 ; j++ ){
//				uvrgb0_1->a = uvrgb1_1->a = 0;
				uvrgb0_1->u = uvrgb1_1->u = FTOI12( ftemp1 );
				uvrgb0_1+= 2;
				uvrgb1_1+= 2;
			}
		}else{
...
//					alpha = 0;
//				}else{
//					alpha = work->base_alpha;
//				}
//				uvrgb0_0->a = uvrgb0_1->a = uvrgb1_0->a = uvrgb1_1->a = alpha;
				uvrgb0_0->u = uvrgb0_1->u = uvrgb1_0->u = uvrgb1_1->u = FTOI12( ftemp1 );
				uvrgb0_0+= 2;
				uvrgb1_0+= 2;
				uvrgb0_1+= 2;
				uvrgb1_1+= 2;
...
void *NewWaterReflect( int name, int where )
{
	Work		*work ;
