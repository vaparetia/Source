...
//----------------------------------------------------------------------------
/*
	d_fade_io_prog_strong.c
	フェードイン・アウト・プログラム起動のみ：強い

	2001/06/19 S.Okajima
	$Id: d_fade_io_prog_strong.c,v 1.4 2002/11/23 12:16:44 Yoshizawa1 Exp $
*/
...
#ifndef KP_WINDOWS
		prim = DG_SetDmapackBox( prim, 0, 0, 640, 448, col );
#else
...
static int Initialize( Work *work )
{
	...
	work->dmapack_fade = dmapack = (DG_DMAPACK *)DG_MakeDmapack2( DG_DMAPACK_NORMAL, DG_PLUGIN_PHASE_AFTER, 150 );
...
/* フェードインアウト */
void *NewFadeInOut_Demo_Strong( int col_r, int col_g, int col_b, int col_a, int time )
{
	Work		*work ;
...
/* フェードインアウト強制初期化 */
void *NewFadeInOutForce_Demo_Strong( int ini_r, int ini_g, int ini_b, int ini_a,
