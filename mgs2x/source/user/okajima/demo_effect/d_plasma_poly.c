...
/*
	...
	$Id: d_plasma_poly.c,v 1.1.1.3 2002/11/19 11:46:54 Yoshizawa1 Exp $
*/
...
static void Act( Work *work )
{
	...
	if( work->init_flag != 0 ){
		...
		for ( i=N_PRIMS*N_POLYS; i>0; i-- ){
			alpha = P_ALPHA_MAX * i / (N_PRIMS*N_POLYS);
		...
		for ( i=N_PRIMS*N_POLYS; i>0; i-- ){
			uvrgb_x->a = uvrgb_y->a = uvrgb_z->a = P_ALPHA_MAX;
			uvrgb_x++; uvrgb_y++; uvrgb_z++;
			uvrgb_x->a = uvrgb_y->a = uvrgb_z->a = P_ALPHA_MAX;
			uvrgb_x++; uvrgb_y++; uvrgb_z++;
			uvrgb_x->a = uvrgb_y->a = uvrgb_z->a = P_ALPHA_MAX;
...
static int GetResources( Work *work, FVECTOR *pos0, FVECTOR *pos1, int radius, int width, int branch, int life )
{
	FVECTOR	fvtemp;
	float	len;
	FVECTOR	*node;
...
life  :寿命。フレーム指定。０以上。
*/
void *NewPlasmaPoly_Demo( FVECTOR *pos0, FVECTOR *pos1, int radius, int width, int branch, int life )
{
	Work		*work ;
	...
	if ( work != NULL ) {
		...
		if ( GetResources( work, pos0, pos1, radius, width, branch, life ) < 0 ) {
			GV_DestroyActor( work ) ;
			return NULL ;
		}
	}
