...
/*
	...
	$Id: d_plasma_line.c,v 1.1.1.3 2002/11/19 11:46:53 Yoshizawa1 Exp $
*/
...
static void Act( Work *work )
{
	...
	if( work->init_flag != 0 ){
		...
		for ( i=N_PRIMS*N_VERTS; i>0; i-- ){
			uvrgbwh->a = P_ALPHA_MAX * i / (N_PRIMS*N_VERTS);
			uvrgbwh++;
		}
...
static int GetResources( Work *work, FVECTOR *pos0, FVECTOR *pos1, int radius, int life )
{
	FVECTOR	fvtemp;
	float	len;
	FVECTOR	*node;
...
life  :寿命。フレーム指定。０以上。
*/
void *NewPlasmaLine_Demo( FVECTOR *pos0, FVECTOR *pos1, int radius, int life )
{
	Work		*work ;
	...
	if ( work != NULL ) {
		...
		if ( GetResources( work, pos0, pos1, radius, life ) < 0 ) {
			GV_DestroyActor( work ) ;
			return NULL ;
		}
	}
