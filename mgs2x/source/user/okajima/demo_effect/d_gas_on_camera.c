...
/*
	...
	$Id: d_gas_on_camera.c,v 1.1.1.3 2002/11/19 11:46:53 Yoshizawa1 Exp $
*/
		...
		for ( j = 0 ; j < N_VERTS2 ; j++ ){
			if( vu0_CheckBoundingBox( space, &bound_0, &bound_1 ) ){
				...
				if( (*space_alpha) >  work->alpha_sub ){ // 引きすぎないようにする
					...
					space->vx = bound_1.vx + BOUND_WIDTH*2.0f*rnd();
					space->vy = bound_1.vy + BOUND_WIDTH*2.0f*rnd();
					space->vz = bound_1.vz + BOUND_WIDTH*2.0f*rnd();
...
static int GetResources( Work *work, FVECTOR *vec, FVECTOR *col, int tex_name, int size, int life, int mode )
{
	DG_PRIM2	*prim ;
	DG_TEX		*tex ;
...
*/
void *NewSpriteFogOnCamera_Demo( FVECTOR *vec, FVECTOR *col, int tex_name, int size, int life, int mode )
{
	Work		*work ;
	...
	if ( work != NULL ) {
		...
		if ( GetResources( work, vec, col, tex_name, size, life, mode ) < 0 ) {
			GV_DestroyActor( work ) ;
			return NULL ;
		}
	}
