...
//----------------------------------------------------------------------------
/*
	drop_body_splush_prog.c
	体からの水飛沫：水面付近で発生

	2000/04/12 S.Okajima
	$Id: drop_body_splush_prog.c,v 1.1.1.3 2002/11/19 11:47:19 Yoshizawa1 Exp $
*/
#ifdef PSX2
#include <sys/types.h>
#include <stdio.h>
							...
							if( SetData( pos, before_pos, nrm, prim_pos, prim_vec ) ){
								if(k==0){
									...
									if( GM_CheckGameStatus( STATE_DEMO ) ){
										_sceVu0ScaleVector( &fvtemp, prim_vec, 0.50f );
										OK_PutSplushSurface( prim_pos, &fvtemp );
									}
									if( pos->vy <= GM_WaterLevel ){
...
*/
	...
	if( work->owner_flag==1  &&  GM_CheckPlayerStatus( PLAYER_WATCH | PLAYER_INTRUDE) ){
		DG_InvisiblePrim2( prim );
		return;
	}
...
#if 1
	if( work->chara_name == 10163495 /*"ライデン"*/
	||  work->chara_name == 2677990 /*"スネーク"*/ ){
		work->owner_flag = 1;
	}else{
		work->owner_flag = 0;
	}
...
//printf("%f:%f:%f:\n",scr_pos->vx,scr_pos->vy,scr_pos->vz);
...
/* ---------------------------------------------------------------- */
void *NewDropBodySplushScn( int name, int map )
{
	Work		*work ;
