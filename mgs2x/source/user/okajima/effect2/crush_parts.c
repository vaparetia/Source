...
//----------------------------------------------------------------------------
/*
	splush_tidal_parts.c
	落下物の水飛沫
	ストリップで曲面形成

	2001/01/29 S.Okajima
	$Id: crush_parts.c,v 1.1.1.3 2002/11/19 11:47:19 Yoshizawa1 Exp $
...
//		_sceVu0ScaleVector( &fvtemp, &fvtemp, local_width*(1.0f-ftemp) );
		...
		if( pos0->vy < work->floor_height ){
			pos0->vy = work->floor_height;
			count++;
		}
		if( pos1->vy < work->floor_height ){
			pos1->vy = work->floor_height;
			count++;
		}
...
座標のポインタ先を毎フレーム参照
*/
void *NewCrushParts( FVECTOR *center, FVECTOR *force, float width, float floor_height )
{
	Work		*work ;
...
//		DG_COPY_VEC( &work->top_vec, force );
		_sceVu0ScaleVector( &work->top_vec, force, 0.75f );
		work->width = width*1.0f;
		work->floor_height = floor_height;
