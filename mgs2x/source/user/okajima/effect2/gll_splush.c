...
/*
	gll_splush.c
	ゴルルゴン専用版の改造版NewAutoSplush
	オリジナルはauto_splush.c 苦情は重野建一郎まで
	2002/07/18 Sigeno
	$Id: gll_splush.c,v 1.1.1.3 2002/11/19 11:47:21 Yoshizawa1 Exp $
*/
#ifdef PSX2
#include <sys/types.h>
#include <stdio.h>
...
#define MAX_LIMIT		(4000.0f)
#define ADJUST_SCALE	(0.5f)
#else
#define SPLUSH_NUM		(8)
#define LEN_LIMIT		(250.0f)
#define MAX_LIMIT		(800.0f)
#define ADJUST_SCALE	(0.5f)
#endif
/*----------------------------------------------------------------*/
...
#else
		len2 = len*1.50f ;
		if(len2 > MAX_LIMIT ) len2 = MAX_LIMIT ;
		_sceVu0ScaleVector( &norm, &norm, len2 );
//printf("len [%f]\n",len);
#endif
...
void *NewGllSplush( FVECTOR *pos, float radius ,int *sw)
{
	Work		*work ;
