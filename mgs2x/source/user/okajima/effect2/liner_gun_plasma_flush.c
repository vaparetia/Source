...
//----------------------------------------------------------------------------
/*
	liner_gun_plasma_flush.c
	二点を結ぶプラズマ（画面クリップありの点をスプライトで表示）
	先端フェードする
	2001/04/17 S.Okajima
	$Id: liner_gun_plasma_flush.c,v 1.1.1.3 2002/11/19 11:47:21 Yoshizawa1 Exp $
*/
#ifdef PSX2
#include <sys/types.h>
#include <stdio.h>
...
#define	VEC_LENGTH		( SCR_LENGTH / 16   /  4)
#define N_VERTS	(32)
...
//	OK_TotalNum++;
	...
	DG_SetTmpLight2(
		...
		1000.0f,
		2000.0f,
		P_RGB_R_MAX | P_RGB_G_MAX<<8 | P_RGB_B_MAX<<16,
...
*pos1 :発生点（即時コピー）
*/
void *NewLinerGunPlasmaFlush( FVECTOR *pos0, FVECTOR *pos1, int life )
{
	Work		*work ;
