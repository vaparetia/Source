...
/*
	...
	$Id: ripple_bubble.c,v 1.1.1.3 2002/11/19 11:47:23 Yoshizawa1 Exp $
*/
#ifdef PSX2
#include <sys/types.h>
#include <eekernel.h>
...
#define	SIZE_ADD		(500.0f)

#define	RIPPLE_BUBBLE_SIZE_MAX		(STABLE_SIZE+SIZE_ADD)
...
//	float			length;
	...
	if( length > RIPPLE_BUBBLE_SIZE_MAX ){
		length = RIPPLE_BUBBLE_SIZE_MAX;
	}
...
//		_sceVu0AddVector( vec, vec, &fvtemp0 );	vec++;
//	}
//	alpha = (int)(MAX_ALPHA * length * param / RIPPLE_BUBBLE_SIZE_MAX);
//	alpha = (int)(MAX_ALPHA * length / RIPPLE_BUBBLE_SIZE_MAX);
	alpha = (int)(MAX_ALPHA * (0.5f + length*0.5f / RIPPLE_BUBBLE_SIZE_MAX) );
...
static	void	Act( Work *work )
{
	...
	if( work->demo_flag ){
		DM_EftControlMatrix( &work->demo_control, &tmp_world ) ;
	}else{
		//内部条件
...
//	GM_WaterLevel = -3000.0f;
...
#else
				*(pos_add + 3) = *(pos_add + 2) = *(pos_add + 1) = *pos_add;
				*(pos_sub + 3) = *(pos_sub + 2) = *(pos_sub + 1) = *pos_sub = *pos_add;
#endif				
				pos_add+= 4;
				pos_sub+= 4;
				uvrgb  += 4;
	...
	if( work->demo_flag ){
		DG_COPY_VEC( &work->pre_pos[0], (FVECTOR *)tmp_world.m[3] ) ;
	}else{
		DG_COPY_VEC( &work->pre_pos[0], &work->control->mov ) ;
	}
	work->act_clock++;
...
static void Die( Work *work )
{
	if( work->demo_flag ){
		if( work->name != 0 ){
			DM_EftControlDelList( &work->demo_control );
		}
	}
...
void *NewRipBubbleMan( int name, int map )
{
	Work		*work ;
...
void *NewRipBubbleMan_DEMO( int name, float upper, float below )
{
	Work		*work ;
	...
	if ( work != NULL ) {
		...
		work->name = name;
		DM_EftControlAddList( name, &work->demo_control );

		work->demo_flag = 1;

		work->inf_upper = upper;
		work->inf_below = below;
