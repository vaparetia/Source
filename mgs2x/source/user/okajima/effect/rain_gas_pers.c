...
/*
	...
	$Id: rain_gas_pers.c,v 1.4 2002/11/23 12:28:39 Yoshizawa1 Exp $
*/
...
//	OK_DirectionSmoother( &OK_rain_gas_rot_pers, &G_wind_rot, 0.995f );
	...
	if( big_thund_flash_flag ){
		...
		alpha_max = (int)( (float)(P_ALPHA_MAX_SLOW - P_ALPHA_MAX)*(0.5f-OK_slow_param*0.5f) )
		           + P_ALPHA_MAX;
...
//	DG_SetPrim2Alpha( prim, SCE_GS_SET_ALPHA( 0, 1, 0, 1, 0x00 ) );
	...
	GV_ZeroMemory( work->space_pos, sizeof( FVECTOR ) * N_VERTS2 * N_PRIMS2 ) ;
	GV_ZeroMemory( prim->pos[0],    sizeof( FVECTOR ) * N_VERTS2 * N_PRIMS2 );
	GV_ZeroMemory( prim->pos[1],    sizeof( FVECTOR ) * N_VERTS2 * N_PRIMS2 );
...
void *NewRainFogPers( int name, int map )
{
	Work		*work ;
