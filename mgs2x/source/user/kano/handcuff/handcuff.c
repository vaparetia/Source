...
/*
	...
	2001/06/09 K.Kano
	$Id: handcuff.c,v 1.1.1.3 2002/11/19 11:43:12 Yoshizawa1 Exp $
*/
...
static void CalcLocateHandcuff(Work *_work,struct _scratchpad *scrpad)
{
	...
	t.vx=-objs->objs[work->n_points-1].trans.vx;
	t.vy=-objs->objs[work->n_points-1].trans.vy;
	t.vz=-objs->objs[work->n_points-1].trans.vz;
	t.vw=1.0f;
	...
	for(i=1;i<work->n_points-2;i++){
		...
		UTL_MakeQuatM(&(scrpad->q),&(scrpad->x[i]),&(scrpad->x[i+1]),
					  &(scrpad->matt),&(objs->objs[i+1].trans));
...
static void Act(Work *work)
{
	...
	fpu_CopyMatrix(&(work->objs->world),&(scrpad->world[0]));

	for(i=0;i<work->objs->def->n_models;i++){
		fpu_CopyMatrix(&(work->objs->objs[i].world),&(scrpad->world[i]));
	}
...
#ifdef DEBUG
	// NewKmsSkeletonTest(work->objs,0x00ffff00);
#endif
...
static void GetOptionValue(Work *work)
{
    ...
    if(work->target1!=NULL){
		...
		DG_SetLightMatrix(work->objs,work->target1->objs->light);
    }
    else{
...
/* 初期化部メイン */
void *NewHandcuff(int name,int where)
{
    Work *work ;
...
/* 初期化部メイン */
void *NewHandcuff_called(int name,int model_name,OBJECT *target1,int num1,OBJECT *target2,int num2)
{
    Work *work ;
