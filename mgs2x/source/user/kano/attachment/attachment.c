...
/*
	...
	1999/11/04 K.Kano
	$Id: attachment.c,v 1.1.1.3 2002/11/19 11:43:03 Yoshizawa1 Exp $
*/
...
#define	ATTACHMENT1_NAME	GV_StrCode( "装備品Ａ" )
...
static void Act(Work *work)
{
    ...
    n_msg=GV_ReceiveMessage(/* ATTACHMENT1_NAME */ work->name,&msg);
	...
	if(GM_CheckGameStatus(STATE_DEMO) && (DM_EffectInitFlag & DM_EFFECT_INIT)){
		InitCalcAttachment(&(work->mp));
	}

	MoveAttachment(work->objs,&(work->mp));
}
...
static void GetOptionValue(MODEL_PARAMETER *mp,DG_OBJS *objs)
{
    mp->m=0.001f;
    mp->k=-0.0001f/mp->m;
    ...
    if( GCL_GetOption( 'a' ) != NULL ){
		...
		else mp->axis.vz=0.0f;

		if(mp->n_axis==2){
			mp->axis.vx=1.0f-mp->axis.vx;
			mp->axis.vy=1.0f-mp->axis.vy;
			mp->axis.vz=1.0f-mp->axis.vz;
		}
    }
...
static int GetResources(Work *work)
{
    ...
    if(!InitAttachment(work->objs,&(work->mp))) return 0;
...
/* 初期化部メイン */
void *NewAttachment(int name,int where)
{
    Work *work ;
...
void Attachment_GetOptionValue_called(MODEL_PARAMETER *mp,DG_OBJS *objs,
									  FVECTOR *v,SVECTOR *r,
							   ...
							   int *a,int angle_limit,float oval_param)
{
    ...
    if(!InitAttachment(work->objs,&(work->mp))) return 0;
    Attachment_GetOptionValue_called(&(work->mp),work->objs,v,r,target,objnum,
						   ...
						   int *a,int angle_limit,float oval_param)
{
    ...
    if(work!=NULL) {
		...
		if(!GetResources_called(work,model_name,v,r,target,objnum,x,
								a,angle_limit,oval_param)){
			GV_DestroyActor(work) ;
			return NULL ;
		}
    }
