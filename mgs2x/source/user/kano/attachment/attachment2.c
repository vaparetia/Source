...
/*
	...
	1999/11/04 K.Kano
	$Id: attachment2.c,v 1.1.1.3 2002/11/19 11:43:03 Yoshizawa1 Exp $
*/
...
#define	ATTACHMENT2_NAME	GV_StrCode( "装備品Ｂ" )
...
static void Act(Work *work)
{
    ...
    n_msg=GV_ReceiveMessage(/* ATTACHMENT2_NAME */ work->name,&msg);
	...
	MoveAttachment2(work->objs,&(work->mp));
}
...
static void GetOptionValue(MODEL_PARAMETER2 *mp,DG_OBJS *objs)
{
    mp->tmat=NULL;
    mp->tmat2=NULL;
    ...
    if( GCL_GetOption( 'p' ) != NULL ){
		...
		GCL_GetIV( GCL_NextStr(), buf ) ;
		vu0_IV0toFV( (IVECTOR *)buf,&(mp->x[1]) ) ;
    } 
    mp->x[1].vw=1.0f;
    ...
    if( GCL_GetOption( 'q' ) != NULL ){
		mp->tobjnum2=GCL_GetNextInt();
    }
    if(mp->target!=NULL){
		mp->tmat=&(mp->target->objs->objs[mp->tobjnum].world);
		mp->tmat2=&(mp->target->objs->objs[mp->tobjnum2].world);
...
static int GetResources(Work *work)
{
    ...
    if(!InitAttachment2(work->objs,&(work->mp))) return 0;
...
/* 初期化部メイン */
void *NewAttachment2(int name,int where)
{
    Work *work ;
							   ...
							   OBJECT *target,int objnum,FVECTOR *x,int objnum2,FVECTOR *p)
{
    ...
    if(!InitAttachment2(work->objs,&(work->mp))) return 0;
    Attachment2_GetOptionValue_called(&(work->mp),work->objs,v,r,target,objnum,x,objnum2,p);
							...
							OBJECT *target,int objnum,FVECTOR *x,int objnum2,FVECTOR *p)
{
    ...
    if(work!=NULL) {
		...
		if(!GetResources_called(work,model_name,v,r,target,objnum,x,objnum2,p)){
			GV_DestroyActor(work) ;
			return NULL ;
		}
    }
