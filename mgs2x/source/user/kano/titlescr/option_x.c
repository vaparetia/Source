/*
	...
	2002/07/03 Y.YANO 
	$Id: option_x.c,v 1.24 2002/10/01 06:43:07 usr04761 Exp $
*/
...
/*
  ...
  *SE :8～16段階
  *MSC:8～16段階
   CAP:JAPANESE_BP_IGNORE()/ENGLISH/FRENCH/.../OFF
  *CTR 
   OWN:NORMAL/REVERSE
   ITM:GROUP/LINER
...
static void GetOptionValue(Work *work)
{
	if( work->node_flag2 == 1 ){
		...
		work->option_explain_resource[ 3 ] = GV_StrCode("オプションの説明スペイン語" );
		work->option_explain_resource[ 4 ] = GV_StrCode("オプションの説明イタリア語1" );
		...
		work->radar_explain_resource[ 3 ] = GV_StrCode("レーダーの説明スペイン語" );
		work->radar_explain_resource[ 4 ] = GV_StrCode("レーダーの説明イタリア語1" );
		...
		work->quick_explain_resource[ 3 ] = GV_StrCode("クイックチェンジの説明スペイン語" );
		work->quick_explain_resource[ 4 ] = GV_StrCode("クイックチェンジの説明イタリア語1" );
...
static int GetResources(Work *work)
{
	...
	if( work->node_flag2 == 1 ){
      ...
      float safeZoneOffsetY=0;
		work->l2d_handle = L2D_SetupLayout2(ptr, DISP_CHANL, 5, SPR_FLAG_PRIV, safeZoneOffsetY);
	} else {
		/* タイトルオプション */
