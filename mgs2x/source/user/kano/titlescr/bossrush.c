...
/*
	...
	2001/09/19  K.Kano
	$Id: bossrush.c,v 1.4 2002/11/23 12:16:42 Yoshizawa1 Exp $
*/
...
#define E_OPTION_EXPLAIN_RESOURCE	0x0006591e		/* オプションの説明英語 */
#define E_RADAR_EXPLAIN_RESOURCE	0x001bface		/* レーダーの説明英語 */
#define E_QUICK_EXPLAIN_RESOURCE	0x006d6db2		/* クイックチェンジの説明英語 */
...
int sd_set_cli(int);
void ReArrangeBossOptions(Work *work);
...
static void ChangeVValue(Work *work,int strcode,float v,int base_v,int base_pv)
{
	SPR_OBJ *spr;
	...
	if(!work->explain_disp_flag){
		...
		switch(work->option_cursor)
      {
   		case OPTION_ITEM_RAD:
			{
				...
				str=BP_GCL_LOOKUP_NEW_FONT_STRING((char *)GetLocalResource(RADAR_EXPLAIN_RESOURCE,index));
			}
			break;

         case OPTION_ITEM_BLOOD:
            {
               str=BP_GCL_LOOKUP_NEW_FONT_STRING((char *)GetLocalResource(OPTION_EXPLAIN_RESOURCE,2));  
            }
            break;
         ...
         case OPTION_ITEM_REV:
            {
               str=BP_GCL_LOOKUP_NEW_FONT_STRING((char *)GetLocalResource(OPTION_EXPLAIN_RESOURCE,6));  
            }
            break;
         ...
         case OPTION_ITEM_ITEMMENU:
            {
               str=BP_GCL_LOOKUP_NEW_FONT_STRING((char *)GetLocalResource(OPTION_EXPLAIN_RESOURCE,7));  
            }
            break;
	   	...
	   	case OPTION_ITEM_QUICK:
			{
				...
				str=BP_GCL_LOOKUP_NEW_FONT_STRING((char *)GetLocalResource(QUICK_EXPLAIN_RESOURCE,index));
			}
			break;
         ...
         case OPTION_ITEM_EXIT:
            {
               int index=work->option_menu_cursor[OPTION_ITEM_QUICK];
               str=BP_GCL_LOOKUP_NEW_FONT_STRING((char *)GetLocalResource(OPTION_EXPLAIN_RESOURCE,11));
            }
            break;
		}
...
{
	int i;
	for(i=OPTION_ITEM_RAD;i<=OPTION_ITEM_QUICK;i++){
...
static int optionStep(Work *work)
{
	...
	case 2:
...
#if 1
		{
			...
			case PAD_U:
				work->option_cursor--;
            if (work->option_cursor == OPTION_ITEM_ITEMMENU)
               work->option_cursor--;
...
void ReArrangeBossOptions(Work *work)
{
   ...
   // hide vibrate
   pTestObject = L2D_GetObject(work->l2d_handle,OBJ_optVIB);
   if ( pTestObject ) SPR_HIDE(pTestObject);
   pTestObject = L2D_GetObject(work->l2d_handle,CODE_MENUVIB_COL);
   if ( pTestObject ) SPR_HIDE(pTestObject);
   pTestObject = L2D_GetObject(work->l2d_handle,OBJ_swVIB);
   if ( pTestObject ) SPR_HIDE(pTestObject);



   pTestObject = pTestObject = L2D_GetObject(work->l2d_handle,OBJ_optRAD);
   if ( pTestObject ) pTestObject->sprite.pos.y = yTextPos+(yTextSize*0);
   pTestObject = L2D_GetObject(work->l2d_handle,CODE_MENURAD_COL);
   if ( pTestObject ) pTestObject->sprite.pos.y = yTextPos+(yTextSize*0);
   pTestObject = L2D_GetObject(work->l2d_handle,OBJ_swRAD);
   if ( pTestObject ) pTestObject->sprite.pos.y = yTextPos+(yTextSize*0);

   pTestObject = pTestObject = L2D_GetObject(work->l2d_handle,OBJ_optBLD);
   if ( pTestObject ) pTestObject->sprite.pos.y = yTextPos+(yTextSize*1);
   pTestObject = L2D_GetObject(work->l2d_handle,CODE_MENUBLD_COL);
   if ( pTestObject ) pTestObject->sprite.pos.y = yTextPos+(yTextSize*1);
   pTestObject = L2D_GetObject(work->l2d_handle,OBJ_swBLD);
   if ( pTestObject ) pTestObject->sprite.pos.y = yTextPos+(yTextSize*1);

   pTestObject = pTestObject = L2D_GetObject(work->l2d_handle,OBJ_optOWN);
   if ( pTestObject ) pTestObject->sprite.pos.y = yTextPos+(yTextSize*5)+2;
   pTestObject = L2D_GetObject(work->l2d_handle,CODE_MENUOWN_COL);
   if ( pTestObject ) pTestObject->sprite.pos.y = yTextPos+(yTextSize*5)+2;
   pTestObject = L2D_GetObject(work->l2d_handle,OBJ_swOWN);
   if ( pTestObject ) pTestObject->sprite.pos.y = yTextPos+(yTextSize*5)+2;

   pTestObject = pTestObject = L2D_GetObject(work->l2d_handle,OBJ_optITEM);
   if ( pTestObject ) 
      pTestObject->sprite.pos.y = yTextPos+(yTextSize*6)+2;
   if ( pTestObject ) 
      SPR_HIDE(pTestObject);
   pTestObject = L2D_GetObject(work->l2d_handle,CODE_MENUITEM_COL);
   if ( pTestObject ) 
      pTestObject->sprite.pos.y = yTextPos+(yTextSize*6)+2;
   if ( pTestObject ) 
      SPR_HIDE(pTestObject);
   pTestObject = L2D_GetObject(work->l2d_handle,OBJ_swITEM);
   if ( pTestObject ) 
      pTestObject->sprite.pos.y = yTextPos+(yTextSize*6)+2;
   if ( pTestObject ) 
      SPR_HIDE(pTestObject);

   pTestObject = pTestObject = L2D_GetObject(work->l2d_handle,OBJ_optQUICK);
   if ( pTestObject ) pTestObject->sprite.pos.y = yTextPos+(yTextSize*6)+3;
   pTestObject = L2D_GetObject(work->l2d_handle,CODE_MENUQUICK_COL);
   if ( pTestObject ) pTestObject->sprite.pos.y = yTextPos+(yTextSize*6)+3;
   pTestObject = L2D_GetObject(work->l2d_handle,OBJ_swQUICK);
   if ( pTestObject ) pTestObject->sprite.pos.y = yTextPos+(yTextSize*6)+3;
