...
/*
    wormcorpse.c
    カメラにつくふなむしの死骸 : かもめのフンから流用
    2001/07/12 Masafumi Okuta 
    $Id: wormcorpse.c,v 1.1.1.3 2002/11/19 11:47:55 Yoshizawa1 Exp $
*/
...
#define INIT_BASE_RAD	(15.f)
#define INIT_ADD_RAD	(0.33f)
...
#define DROP_SPD	(0.12f)	// 落下速度
#define DIR_RATE	(1.6f)	// 指向移動レート
...
static	void	Act( Work *work )
{
	...
	_sceVu0ScaleVector( &vecZ, &vecZ, work->fDirSpd );
    }
	...
	for ( k = 0 ; k < N_VERTS_CIR; k++ ){
		...
		pvecPos->vy = work->fCenterY + fSin * (*pfRad) + (*pfAdd);
		pfAdd++;
		pvecPos->vz = SCREEN_NEAR;

	    }
	    // α設定
	    puvrgb->a = work->nAlpha;
	...
	for ( i = 0 ; i < N_PRIMS; i++ ){
		...
		if ( k % 2 == 0){
		    ...
		    pvecPos->vx = work->fCenterX + fCos * work->fSizeSpread + (*pfAdd);
		    pfAdd++;
		    (*pfAdd) += work->fDropSpd * fRateDrop * 0.5f;
		    pvecPos->vy = work->fCenterY + fSin * work->fSizeSpread + (*pfAdd);
		    pfAdd++;
		    pvecPos->vz = SCREEN_NEAR;
		}
		pvecPos++;
	...
	// α減衰
	if ( work->nLife <= work->nAlpha){
	    work->nAlpha = work->nLife;
	    if ( work->nAlpha < 0 ) work->nAlpha = 0;
	}
	if ( work->nLife <= work->nAlphaSpr){
...
/*----------------------------------------------------------------*/
static int InitPacket( DG_PRIM2 *prim, DG_TEX *tex, int nFlag, u_int unRGBA )
{
    int			i, k;
    int			col_r;
    int			col_g;
    ...
    col_r = ( (unRGBA & 0xff000000) >> 24 );
    col_g = ( (unRGBA & 0x00ff0000) >> 16 );
    col_b = ( (unRGBA & 0x0000ff00) >> 8 );
    col_a = ( (unRGBA & 0x000000ff) );
...
static int GetResources( Work*		work, 		// 
			 FVECTOR* 	pvecCenter,	// 座標
			 int		nBodyTexCode,	// 本体
			 int		nSpreadTexCode,	// 飛抹
			 u_int		unBodyRGBA,	// 本体RGBA
			 u_int		unSpreadRGBA,	// 飛抹RGBA
			 float		fSizeBase,	// サイズ最小
			 float		fSizeAdd,	// サイズ最大
			 int 		nLife )		// 生存時間
{
    int			i, k;
    DG_PRIM2*		prim;
    ...
    work->nAlpha 	= (unBodyRGBA & 0x000000ff); 	// 本体α値
    work->nAlphaSpr 	= (unSpreadRGBA & 0x000000ff); 	// 飛抹α値
    ...
    work->fSizeSpread = (fSizeBase + (fSizeAdd * frnd())) * 1.3f;
    ...
    // 本体プリミティブ作成
    tex = DG_GetTexture( nBodyTexCode);
    ...
    if( !InitPacket( prim, tex, 0, unBodyRGBA ) ) return -1; // パケット初期化
    DG_SetPrim2Alpha( prim, SCE_GS_SET_ALPHA( 0, 1, 0, 1, 0x80 ) ); // Half
    prim->raise = RAISE_ADD; // 優先

    // 飛抹プリミティブ作成
    tex = DG_GetTexture( nSpreadTexCode);
    prim_spr = work->prim_spr = GM_MakePrim2( DG_PRIM2_POLY|DG_PRIM2_SHADE|DG_PRIM2_TEX|
					      DG_PRIM2_ALPHA|DG_PRIM2_ON_CAMERA, N_PRIMS, N_VERTS );
    if ( prim_spr == NULL ){
	printf("null prim_spr\n");
	return -1;
    }
    DG_InvisiblePrim2( prim_spr ); // 非表示
    if( !InitPacket( prim_spr, tex, 1, unSpreadRGBA ) ) return -1; // パケット初期化
...
// 舟虫の死骸がカメラ付着
void* NewWormCorpseCamera( FVECTOR* 	pvecCenter,	// 中心
			   int		nBodyTexCode,	// 本体
			   int		nSpreadTexCode,	// 飛抹
			   u_int	unBodyRGBA,	// 本体RGBA
			   u_int	unSpreadRGBA,	// 飛抹RGBA
			   ...
			   int 		nLife )		// 表示時間
{
	...
	if ( GetResources( work, pvecCenter, nBodyTexCode, nSpreadTexCode, unBodyRGBA, 
			   unSpreadRGBA, fSizeBase, fSizeAdd, nLife ) < 0 ) {
	    GV_DestroyActor( work ) ;
	    return NULL;
	}
    }
