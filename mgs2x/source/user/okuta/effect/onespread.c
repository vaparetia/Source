...
/*
    ...
    $Id: onespread.c,v 1.1.1.3 2002/11/19 11:47:52 Yoshizawa1 Exp $
*/
...
static void Act( Work *work )
{
	...
	for ( i = 0; i < work->nPrims; i++ ){
		...
		if ( (*pucAlpha) > 0 ){ // 動作
		    ...
		    pvecStep++;
		}else if ( nNum >= work->nActNum && nAddNum < work->nAddNum){ // α = 0 でリセット
		    FVECTOR	vecCenter;
		    FVECTOR	vecDir;
		    FVECTOR*	pvec;
	...
	work->nActNum += nAddNum;
    }

    // カウンタ
    work->nCntr++;
    if ( work->nCntr >= work->nLifeMax + COUNT_VMODE(120)){
...
// 水飛抹
void* NewOneSpread( FMATRIX*	pmatOrg, 		// 出現座標系
