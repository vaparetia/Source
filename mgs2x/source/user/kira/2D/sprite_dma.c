...
/*
  ...
  $Id: sprite_dma.c,v 1.1.1.3 2002/11/19 11:43:49 Yoshizawa1 Exp $
*/
/*
  ...
  for(bros = prim; bros != NULL; bros = bros->head.next)
      ...
      if(!(now_attrib & SPR_FLAG_HIDDEN))
	{
	  ...
	  switch(bros->head.id)
		...
		rgb = SCE_GS_SET_RGBAQ(bros->point.col.r, bros->point.col.g,
				       bros->point.col.b, bros->point.col.a,0);
		point[DG_Clock].gif.data.rgbq0 = rgb;

		xyz = SCE_GS_SET_XYZ(ChanlX(now_x), ChanlY(now_y), DEFAULT_Z);
		point[DG_Clock].gif.data.xyz0 = xyz;

		prim_d |= (SCE_GS_PRIM_POINT & 7);
		point[DG_Clock].gif.alpha.giftag.tag =
		  SCE_GIF_SET_TAG(1, 0, 1, prim_d, 0, 1);
		point[DG_Clock].gif.alpha.alpha.data = alpha;
	      }
	      break;
		...
		rgb = SCE_GS_SET_RGBAQ(bros->line.col[0].r,
				       bros->line.col[0].g,
				       bros->line.col[0].b,
				       bros->line.col[0].a, 0);
		line[DG_Clock].gif.data.rgbq0 = rgb;

		xyz = SCE_GS_SET_XYZ(ChanlX(now_x), ChanlY(now_y), DEFAULT_Z);
		line[DG_Clock].gif.data.xyz0 = xyz;
		
		rgb = SCE_GS_SET_RGBAQ(bros->line.col[1].r,
				       bros->line.col[1].g,
				       bros->line.col[1].b,
				       bros->line.col[1].a, 0);
		line[DG_Clock].gif.data.rgbq1 = rgb;
		...
		line[DG_Clock].gif.data.xyz1 = xyz;

		prim_d |= (SCE_GS_PRIM_LINE & 7);
		line[DG_Clock].gif.alpha.giftag.tag =
		  SCE_GIF_SET_TAG(1, 0, 1, prim_d, 0, 1);
		line[DG_Clock].gif.alpha.alpha.data = alpha;
	      }
	      break;
	    case SP_LINESTRIP:
	      {
		...
		x = base_x + PosX(bros->box.rect.end.x);
		y = base_y + PosY(bros->box.rect.end.y);

		rgb = SCE_GS_SET_RGBAQ(bros->box.col.r,
				       bros->box.col.g,
				       bros->box.col.b,
				       bros->box.col.a, 0);

		xyz = SCE_GS_SET_XYZ(ChanlX(now_x), ChanlY(now_y), DEFAULT_Z);
		box[DG_Clock].gif.data.xyz0 = xyz;
		box[DG_Clock].gif.data.xyz4 = xyz;
		
		xyz = SCE_GS_SET_XYZ(ChanlX(x), ChanlY(now_y), DEFAULT_Z);
		box[DG_Clock].gif.data.xyz1 = xyz;
		
		xyz = SCE_GS_SET_XYZ(ChanlX(x), ChanlY(y), DEFAULT_Z);
		box[DG_Clock].gif.data.xyz2 = xyz;

		xyz = SCE_GS_SET_XYZ(ChanlX(now_x), ChanlY(y), DEFAULT_Z);
		box[DG_Clock].gif.data.xyz3 = xyz;

		box[DG_Clock].gif.data.rgbq = rgb;

		box[DG_Clock].gif.alpha.alpha.data = alpha;
		prim_d |= (SCE_GS_PRIM_LINESTRIP & 7);
		box[DG_Clock].gif.alpha.giftag.tag =
		  SCE_GIF_SET_TAG(1, 0, 1, prim_d, 0, 1);
	      }
	      break;
	    case SP_SPRITE:
		...
		rgb = SCE_GS_SET_RGBAQ(bros->sprite.col.r,
				       bros->sprite.col.g,
				       bros->sprite.col.b,
				       bros->sprite.col.a, 0);
		
		xyz = SCE_GS_SET_XYZ(x[0], y[0], DEFAULT_Z);
		sprite[DG_Clock].gif.data.xyz0  = xyz;
		sprite[DG_Clock].gif.data.rgbq0 = rgb;

		xyz = SCE_GS_SET_XYZ(x[1], y[1], DEFAULT_Z);
		sprite[DG_Clock].gif.data.xyz1  = xyz;
		sprite[DG_Clock].gif.data.rgbq1 = rgb;

		sprite[DG_Clock].gif.attrib.data.pabe.data = pabe;
		/*
		 * テクスチャ関係のパラメータ設定
		 */
		/* テクスチャがなければ、PRIMレジスタに、
		   TME を OFF にした設定を書き込む */
		if(txsw)
		  {
		    SPR_FIX u0, v0;
		    SPR_FIX u1, v1;

		    u0 = bros->ex_hd.tex.u, v0 = bros->ex_hd.tex.v;
		    u1 = bros->ex_hd.tex.u + bros->ex_hd.tex.w;
		    v1 = bros->ex_hd.tex.v + bros->ex_hd.tex.h;

		    if(bros->head.flags & SPR_FLAG_H_REV) SWAP(u0, u1);
		    if(bros->head.flags & SPR_FLAG_V_REV) SWAP(v0, v1);

		    /* テクスチャパラメタを DMA パケットに設定する */
		    sprite[DG_Clock].gif.data.tex0 = tex0;
#ifndef NO_TEX1
		    sprite[DG_Clock].gif.attrib.data.tex1.data = tex1;
#endif /* NO_TEX1 */
		    sprite[DG_Clock].gif.attrib.data.tex2.data = tex2;
		    sprite[DG_Clock].gif.data.uv0 = SCE_GS_SET_UV(u0, v0);
		    sprite[DG_Clock].gif.data.uv1 = SCE_GS_SET_UV(u1, v1);

		    sprite[DG_Clock].gif.data.clamp = clamp;
		  }
		prim_d |= (SCE_GS_PRIM_SPRITE & 7);
		sprite[DG_Clock].gif.attrib.giftag.tag =
		  SCE_GIF_SET_TAG(SIZEOF_QWORD(struct _txat_data),
				  0, 1, prim_d, 0, 1);
		sprite[DG_Clock].gif.attrib.data.alpha.data = alpha;
	      }
	      break;
		...
		rgb = SCE_GS_SET_RGBAQ(bros->zoom.col.r,
				       bros->zoom.col.g,
				       bros->zoom.col.b,
				       bros->zoom.col.a, 0);
		
		xyz = SCE_GS_SET_XYZ(x[0], y[0], DEFAULT_Z);
		zoom[DG_Clock].gif.data.xyz0  = xyz;
		zoom[DG_Clock].gif.data.rgbq0 = rgb;

		xyz = SCE_GS_SET_XYZ(x[1], y[1], DEFAULT_Z);
		zoom[DG_Clock].gif.data.xyz1  = xyz;
		zoom[DG_Clock].gif.data.rgbq1 = rgb;
		zoom[DG_Clock].gif.attrib.data.pabe.data = pabe;
		...
		if(txsw)
		  {
		    ...
		    /* テクスチャパラメタを DMA パケットに設定する */
		    zoom[DG_Clock].gif.data.tex0 = tex0;
		    
#ifndef NO_TEX1
		    zoom[DG_Clock].gif.attrib.data.tex1.data = tex1;
#endif /* NO_TEX1 */
		    
		    zoom[DG_Clock].gif.attrib.data.tex2.data = tex2;
		    zoom[DG_Clock].gif.data.uv0 = SCE_GS_SET_UV(u0, v0);
		    zoom[DG_Clock].gif.data.uv1 = SCE_GS_SET_UV(u1, v1);

		    zoom[DG_Clock].gif.data.clamp = clamp;
		  }
		prim_d |= (SCE_GS_PRIM_SPRITE & 7);
		zoom[DG_Clock].gif.attrib.giftag.tag =
		  SCE_GIF_SET_TAG(SIZEOF_QWORD(struct _txat_data),
				  0, 1, prim_d, 0, 1);
		zoom[DG_Clock].gif.attrib.data.alpha.data = alpha;
	      }
	      break;
		...
		y[0] = -bros->spin.center.y;

		x[1] = x[0] + bros->spin.dw;
		y[1] = y[0];

		x[2] = x[1];
		y[2] = y[0] + bros->spin.dh;
		...
		rgb = SCE_GS_SET_RGBAQ(bros->spin.col.r,
				       bros->spin.col.g,
				       bros->spin.col.b,
				       bros->spin.col.a,
				       0);

		spin[DG_Clock].gif.data.rgbq0 = rgb;
		xyz = SCE_GS_SET_XYZ(x[0], y[0], DEFAULT_Z);
		spin[DG_Clock].gif.data.xyz0 = xyz;


		spin[DG_Clock].gif.data.rgbq1 = rgb;
		xyz = SCE_GS_SET_XYZ(x[1], y[1], DEFAULT_Z);
		spin[DG_Clock].gif.data.xyz1 = xyz;
		spin[DG_Clock].gif.data.rgbq2 = rgb;
		xyz = SCE_GS_SET_XYZ(x[2], y[2], DEFAULT_Z);
		spin[DG_Clock].gif.data.xyz2 = xyz;
		spin[DG_Clock].gif.data.rgbq3 = rgb;
		xyz = SCE_GS_SET_XYZ(x[3], y[3], DEFAULT_Z);
		spin[DG_Clock].gif.data.xyz3 = xyz;
		spin[DG_Clock].gif.attrib.data.pabe.data = pabe;
		/*
		 * テクスチャ関連パラメタの設定
		 */
		/* テクスチャが設定されていない場合は、処理をスキップする */
		if(txsw)
		  {
		    SPR_FIX u0, v0;
		    SPR_FIX u1, v1;
		    ...
		    /* テクスチャパラメタを DMA パケットに設定する */
		    spin[DG_Clock].gif.data.tex0 = tex0;
		    
#ifndef NO_TEX1
		    spin[DG_Clock].gif.attrib.data.tex1.data = tex1;
#endif /* NO_TEX1 */

		    spin[DG_Clock].gif.attrib.data.tex2.data = tex2;
		    spin[DG_Clock].gif.data.uv0 = SCE_GS_SET_UV(u0, v0);
		    spin[DG_Clock].gif.data.uv1 = SCE_GS_SET_UV(u1, v0);
		    spin[DG_Clock].gif.data.uv2 = SCE_GS_SET_UV(u1, v1);
		    spin[DG_Clock].gif.data.uv3 = SCE_GS_SET_UV(u0, v1);

		    spin[DG_Clock].gif.data.clamp = clamp;
		  }

		prim_d |= (SCE_GS_PRIM_TRIFAN & 7);
		spin[DG_Clock].gif.attrib.giftag.tag =
		  SCE_GIF_SET_TAG(SIZEOF_QWORD(struct _txat_data),
				  1, 1, prim_d, 0, 1);
		spin[DG_Clock].gif.attrib.data.alpha.data = alpha;
	      }
	      break;
	    case SP_POLY:
	      {
		...
		/* 変形スプライトであっても、上下左右反転はサポート */
		/*
		if(now_attrib & SPR_FLAG_H_REV)
		  {
		    SWAP(x[0], x[1]); SWAP(y[0], y[1]);
		    SWAP(x[3], x[2]); SWAP(y[3], y[2]);
		  }
		if(now_attrib & SPR_FLAG_V_REV)
		  {
		    SWAP(x[0], x[3]); SWAP(y[0], y[3]);
		    SWAP(x[1], x[2]); SWAP(y[1], y[2]);
		  }
		*/
		rgb = SCE_GS_SET_RGBAQ(bros->poly.col[0].r,
				       bros->poly.col[0].g,
				       bros->poly.col[0].b,
				       bros->poly.col[0].a,
				       0);
		poly[DG_Clock].gif.data.rgbq0 = rgb;

		xyz = SCE_GS_SET_XYZ(x[0], y[0], DEFAULT_Z);
		poly[DG_Clock].gif.data.xyz0 = xyz;


		rgb = SCE_GS_SET_RGBAQ(bros->poly.col[1].r,
				       bros->poly.col[1].g,
				       bros->poly.col[1].b,
				       bros->poly.col[1].a,
				       0);
		poly[DG_Clock].gif.data.rgbq1 = rgb;

		xyz = SCE_GS_SET_XYZ(x[1], y[1], DEFAULT_Z);
		poly[DG_Clock].gif.data.xyz1 = xyz;

		rgb = SCE_GS_SET_RGBAQ(bros->poly.col[2].r,
				       bros->poly.col[2].g,
				       bros->poly.col[2].b,
				       bros->poly.col[2].a,
				       0);
		poly[DG_Clock].gif.data.rgbq2 = rgb;

		xyz = SCE_GS_SET_XYZ(x[2], y[2], DEFAULT_Z);
		poly[DG_Clock].gif.data.xyz2 = xyz;

		rgb = SCE_GS_SET_RGBAQ(bros->poly.col[3].r,
				       bros->poly.col[3].g,
				       bros->poly.col[3].b,
				       bros->poly.col[3].a,
				       0);
		poly[DG_Clock].gif.data.rgbq3 = rgb;

		xyz = SCE_GS_SET_XYZ(x[3], y[3], DEFAULT_Z);
		poly[DG_Clock].gif.data.xyz3 = xyz;


		poly[DG_Clock].gif.attrib.data.pabe.data = pabe;
		/*
		 * テクスチャ関連パラメタの設定
		 */
		/* テクスチャが設定されていない場合は、処理をスキップする */
		if(txsw)
		  {
		    ...
		    /* テクスチャパラメタを DMA パケットに設定する */
		    poly[DG_Clock].gif.data.tex0 = tex0;
		    
#ifndef NO_TEX1
		    poly[DG_Clock].gif.attrib.data.tex1.data = tex1;
#endif /* NO_TEX1 */

		    poly[DG_Clock].gif.attrib.data.tex2.data = tex2;
		    
		    poly[DG_Clock].gif.data.uv0 = SCE_GS_SET_UV(u0, v0);
		    poly[DG_Clock].gif.data.uv1 = SCE_GS_SET_UV(u1, v0);
		    poly[DG_Clock].gif.data.uv2 = SCE_GS_SET_UV(u1, v1);
		    poly[DG_Clock].gif.data.uv3 = SCE_GS_SET_UV(u0, v1);

		    poly[DG_Clock].gif.data.clamp = clamp;
		  }

		prim_d |= (SCE_GS_PRIM_TRIFAN & 7);
		poly[DG_Clock].gif.attrib.giftag.tag =
		  SCE_GIF_SET_TAG(SIZEOF_QWORD(struct _txat_data),
				  1, 1, prim_d, 0, 1);
		poly[DG_Clock].gif.attrib.data.alpha.data = alpha;
	      }
	      break;
	    case SP_TRIANGLE:
	      {
		...
		rgb = SCE_GS_SET_RGBAQ(bros->triangle.col[0].r,
				       bros->triangle.col[0].g,
				       bros->triangle.col[0].b,
				       bros->triangle.col[0].a,
				       0);
		triangle[DG_Clock].gif.data.rgbq0 = rgb;

		xyz = SCE_GS_SET_XYZ(x[0], y[0], DEFAULT_Z);
		triangle[DG_Clock].gif.data.xyz0 = xyz;


		rgb = SCE_GS_SET_RGBAQ(bros->triangle.col[1].r,
				       bros->triangle.col[1].g,
				       bros->triangle.col[1].b,
				       bros->triangle.col[1].a,
				       0);
		triangle[DG_Clock].gif.data.rgbq1 = rgb;

		xyz = SCE_GS_SET_XYZ(x[1], y[1], DEFAULT_Z);
		triangle[DG_Clock].gif.data.xyz1 = xyz;

		rgb = SCE_GS_SET_RGBAQ(bros->triangle.col[2].r,
				       bros->triangle.col[2].g,
				       bros->triangle.col[2].b,
				       bros->triangle.col[2].a,
				       0);
		triangle[DG_Clock].gif.data.rgbq2 = rgb;

		xyz = SCE_GS_SET_XYZ(x[2], y[2], DEFAULT_Z);
		triangle[DG_Clock].gif.data.xyz2 = xyz;

		triangle[DG_Clock].gif.attrib.data.pabe.data = pabe;
		/*
		 * テクスチャ関連パラメタの設定
		 */
		/* テクスチャが設定されていない場合は、処理をスキップする */
		if(txsw)
		  {
		    ...
		    /* テクスチャパラメタを DMA パケットに設定する */
		    triangle[DG_Clock].gif.data.tex0 = tex0;
		    
#ifndef NO_TEX1
		    triangle[DG_Clock].gif.attrib.data.tex1.data = tex1;
#endif /* NO_TEX1 */

		    triangle[DG_Clock].gif.attrib.data.tex2.data = tex2;
		    
		    triangle[DG_Clock].gif.data.uv0 = SCE_GS_SET_UV(u0, v0);
		    triangle[DG_Clock].gif.data.uv1 = SCE_GS_SET_UV(u1, v0);
		    triangle[DG_Clock].gif.data.uv2 = SCE_GS_SET_UV(u0, v1);

		    triangle[DG_Clock].gif.data.clamp = clamp;
		  }

		prim_d |= (SCE_GS_PRIM_TRI & 7);
		triangle[DG_Clock].gif.attrib.giftag.tag =
		  SCE_GIF_SET_TAG(SIZEOF_QWORD(struct _txat_data),
				  1, 1, prim_d, 0, 1);
		triangle[DG_Clock].gif.attrib.data.alpha.data = alpha;
	      }
	      break;
	    }
	}
...
void * (* spr_get_func_malloc(void))(long64 size)
{
  Work * work = sys_work;
  if(NULL == work) return NULL;
...
void (* spr_get_func_free(void))(void * ptr)
{
  Work * work = sys_work;
  if(NULL == work) return NULL;
