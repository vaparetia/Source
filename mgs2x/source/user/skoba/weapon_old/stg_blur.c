...
/*
	...
	2000/07/12 S.Okajima
	$Id: stg_blur.c,v 1.5 2002/11/23 12:16:41 Yoshizawa1 Exp $
	...
	if ( GV_PauseLevel == 0 ){
		...
		*/
		...
		if ( work->tick_count >= work->interval ){
			...
			vec.vx = 0.0f ;
			vec.vy = -work->start_speed ;
			vec.vz = 0.0f ;
			vec.vw = 1.0f ;
			svtemp.vx = 0;
			svtemp.vy = irnd()%4096;
			svtemp.vz = irnd()%work->spot_angle;
...
{
	work->prim[0] = OK_FreePrim2( work->prim[0] );
	work->prim[1] = OK_FreePrim2( work->prim[1] );
}
...
#ifdef PSX2
		prim = work->prim[k] = 
		  GM_MakePrim2( prim_flag, work->n_prims, N_VERTS );
		if ( prim == NULL ){
			return ( -1 );
		}
...
#else
		prim = work->prim[k] = 
		  GM_MakePrim2( prim_flag|DG_PRIM2_FRAMETEX, work->n_prims, N_VERTS );
		if ( prim == NULL ){
			return ( -1 );
		}
...
void *NewSTG_SmokeBlurEffect( FMATRIX *world, int start_speed, int end_speed, int start_size, int end_size, int spot_size, int spot_angle, int n_prims, int interval, int color, int flag, int life )
{
	Work		*work ;
