...
/*
	単発銃のマズルフラッシュ 
	2001/04/13 S.Kobayashi
	$Id: mazzleflash_sub2.c,v 1.1.1.3 2002/11/19 11:50:36 Yoshizawa1 Exp $
*/
...
static int color_mng( int value )
{
	if ( value == 1 ){
		on = 1;
		return on;
...
static int color_func( int value )
{

	static int sign = 1;
	// デバッグメニューが値を取得するときには引数-1でコールされる。
	if( on == 0 ){
		r = g = b = a = 128;
		return value;
	}
	...
	MENU_S_Color( 192 , 192 , 192 , 128 );
	MENU_S_Locate( 40 , 40 , 1 );
	MENU_S_Printf( "%d\n" , r );
	MENU_S_Printf( "%d\n" , g );
	MENU_S_Printf( "%d\n" , b );
	MENU_S_Printf( "%d\n" , a );
...
static GM_DEBUG_MENU debug_color = {
	class:  "SKOBA",
	menu:   "COLOR",
	max:    2,
	items:  ( char *[] ){ "OFF", "ON" },
	values: ( int [] ){ 0, 1 },
	target: ( int * )color_mng,	// 関数へのポインタをキャスト 
	type:   GM_DEBUG_MENU_FUNC,		// 必ず設定 
};
...
static void Init_ScrPad( FMATRIX  *world, DG_TEX *tex)
{
	...
	for(i = 0; i < N_PRIMS; i++ ){
		...
		for(j = 0; j < N_VERTS; j++){
			...
			offset_x = 64 * irnd() % 2;
			offset_y = 64 * irnd() % 2;
			uvrgbwh->u0 = FTOI12(0.0F * tex->u_scale + tex->u_offset + offset_x) ;
			uvrgbwh->v0 = FTOI12(0.0F * tex->v_scale + tex->v_offset + offset_y) ;
			uvrgbwh->u1 = FTOI12(0.5F * tex->u_scale + tex->u_offset + offset_x) ;
			uvrgbwh->v1 = FTOI12(0.5F * tex->v_scale + tex->v_offset + offset_y) ;
			uvrgbwh->q0 = 4096 ;
			uvrgbwh->q1 = 4096 ;
			uvrgbwh->f0 = 0x0fff ;
			uvrgbwh->f1 = 0x0fff ;
			uvrgbwh->r  = 64 + irnd() % 10 ;
			uvrgbwh->g  = 64 + irnd() % 10 ;
			uvrgbwh->b  = 64 + irnd() % 10 ;
			uvrgbwh->a  = 64 + irnd() % 10 - j * 15 ;
			uvrgbwh->w = ( int )( tmp * cosf( angle ) );
			uvrgbwh->h = ( int )( tmp * sinf( angle ) );
			uvrgbwh++;
		}
	}
}
...
static  int  GetResources(Work *work, FMATRIX *world)
{
	...
	tex = DG_GetTexture( GV_StrCode("muzzlefrash_01_alp") ) ;
	if( tex == NULL ){
		return ( -1 );
	}
...
/*  -----------------------------------------------------------------  */
void *SubMazzleFlash2(FMATRIX *world)
{
	Work		*work ;
