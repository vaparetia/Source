...
//BP
//----------------------------------------------------------------------------
/*スパスマズルフラッシュスパスマズルフラッシュスパスマズルフラッシュスパスマズルフラッシュ

	sps_mazzle.c
	スパスマズルフラッシュ
	2000/06/29 S.Okajima
	$Id: sps_mazzle.c,v 1.1.1.3 2002/11/19 11:50:37 Yoshizawa1 Exp $
...
static void InitPacket( Work *work, DG_PRIM2 *prim, DG_TEX *tex, FMATRIX *world )
{
	...
	for ( i=0; i<N_PRIMS; i++ ){
		for ( j=0; j<N_POLYS; j++ ){
			_sceVu0ScaleVector( vec, &unit_vec, VEC_MAX*rnd() );
			vec->vx += VEC_RND * frnd();
			vec->vy += VEC_RND * frnd();
			vec->vz += VEC_RND * frnd();
			vec->vw  = ALP_MIN + ALP_RND*rnd();

			DG_COPY_VEC( &fvtemp, (FVECTOR *)world->m[3] );
...
static	int GetResources( Work *work )
{
	...
	{
		extern void *NewSPS_Smoke( FMATRIX *hand );
		NewSPS_Smoke( work->world );
	}
	{
		extern void *NewConnectSmoke( FMATRIX *world, FVECTOR *shift, int life, float size_rnd );
		GV_SetActorChild( work, NewConnectSmoke( work->world, &sps_shift_mazzule, 30, 16.0f ) );
	}
...
void *NewSPS_MazzleFlush( FMATRIX *hand )
{
	Work		*work ;
