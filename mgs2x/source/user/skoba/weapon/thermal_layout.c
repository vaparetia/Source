...
/*
   ...
   $Id: thermal_layout.c,v 1.2 2002/12/05 18:41:58 takaki Exp $
*/
...
#define     NUMBER_MAX ( 1 )
#define 	NUM_WIDTH (60)
#define 	DEFAULT_Y ( DIRECT_SCREEN_Y( -20 ) )
...
static int MsgChack( Work *pWork )
{
	...
	while( --msg_num >= 0 ){
		...
		if( num == VISIBLE_FLAG ){
			...
			pWork->effect = NewIRMode() ;
			if ( pWork->effect != NULL ) {
				GV_SetActorChild( pWork, pWork->effect ) ;
			}
		}
		if( num == INVISIBLE_FLAG ){
			GM_SetSightStatus( SGT_Invisible );
			if ( pWork->effect != NULL ) {
				GV_CallChildSignalFunc( pWork , GV_SIGNAL_KILL, 0 );
				GV_DestroyChild( pWork->effect );
			}
		}
		msg--;
	}
...
#endif
	if ( pWork->name != 0 ){
		parent_y = DIRECT_SCREEN_Y( 309.0f ) + DEFAULT_Y;
	} else {
...
static int AnimetionAct( Work *pWork ) // 通常実行
{
	...
	if ( pWork->action < pWork->action_num ){
		...
		if ( pWork->name != 0 && pWork->action == 1 ){
...
static int Act( Work *pWork )
{
	...
	if ( GM_CheckSightStatus( SGT_Thermal ) || ( GM_IsGameOver() == 1 ) ){
		invisible( pWork );
	}
 	pWork->act( pWork );
