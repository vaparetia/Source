...
/*
	...
	$Id: arms_con.c,v 1.1.1.3 2002/11/19 11:50:26 Yoshizawa1 Exp $
...
static void Muzzle_HandGunDefault( Work *pWork , int id, int flags )
{
	...
	if(flags&(EFFECT_FLAGS_LINE|EFFECT_FLAGS_REC)){
		...
		NewBullet( &temp, bullet_flags, BOTH_SIDE, 25, 0, 20000, 1000, WP_Usp );
	}
...
static void Muzzle_SOCOM( Work *pWork , int id, int flags )
{
	...
	if( flags & ( EFFECT_FLAGS_LINE | EFFECT_FLAGS_REC ) ){
		...
		NewBullet( &temp, bullet_flags, BOTH_SIDE, 25, 0, 20000, 1000, WP_Socom );
	}
	if ( flags & EFFECT_FLAGS_ON_TMP ){
		light_flag |= EFFECT_FLAGS_ON_TMP;
	}
	if ( flags & EFFECT_FLAGS_NO_MZL ){
		mazzle_flag |= EFFECT_FLAGS_NO_MZL;
	}
	MazzleSOCOM( pWork->world , mazzle_flag , light_flag );
}

// demo_usp
static void Muzzle_DemoHandGunDefault( FMATRIX *pWorld , int id, int flags )
{
	char light_flag;
	char mazzle_flag;
	...
	mazzle_flag = 0;
	if(flags&EFFECT_FLAGS_AMO){		
		AN_CartridgeUSP_E( pWorld, NULL, NULL );
	}
	if(flags&(EFFECT_FLAGS_LINE|EFFECT_FLAGS_REC)){
		FMATRIX	temp;
		u_int 	bullet_flags = BUL_TYPE_SCAR;

		DG_COPY_MAT( &temp, pWorld );
		DG_SetPos( pWorld );
		DG_PutVector( &_MuzzlePos[id] , (FVECTOR *)temp.m[3] , 1 );
		if(flags&EFFECT_FLAGS_LINE) bullet_flags |= BUL_TYPE_VISIBLE;
		if(flags&EFFECT_FLAGS_REC) bullet_flags |= BUL_TYPE_SPARK;
	...
	MazzleUSP( pWorld , mazzle_flag , light_flag );
}

// demo Socom
static void Muzzle_DemoSOCOM( FMATRIX *pWorld , int id, int flags )
{
	char light_flag;
	char mazzle_flag;
	...
	mazzle_flag = 0;
	if( flags & EFFECT_FLAGS_AMO ){		
		AN_CartridgeSOCOM_E( pWorld, NULL, NULL );
	}
	if( flags & ( EFFECT_FLAGS_LINE | EFFECT_FLAGS_REC ) ){
		FMATRIX	temp;
		u_int 	bullet_flags = BUL_TYPE_SCAR;
	...
	MazzleSOCOM( pWorld , mazzle_flag , light_flag );
}
...
static void Muzzle_M92_Sea( Work *pWork , int id, int flags )
{
	...
	if( flags & ( EFFECT_FLAGS_LINE | EFFECT_FLAGS_REC ) ){
		...
		NewBullet( &temp, bullet_flags, BOTH_SIDE, 25, 0, 20000, 1000, WP_m92 );
	}
	if ( flags & EFFECT_FLAGS_ON_TMP ){
		light_flag |= EFFECT_FLAGS_ON_TMP;
	}
...
static void Muzzle_Ray_Vlcn( Work *pWork , int id , int flags )
{
	...
	if ( flags & EFFECT_FLAGS_ON_SPARK ){
		NewRayValcanFireNoVRArea( pWork->world , side_flag , vlcn_pos ); // 六角なし
	} else {
		NewRayValcanFire( pWork->world , side_flag , vlcn_pos );
	}
}
	...
	while( --mes_num >= 0 ){
		...
		switch( num ){
		  ...
		  case MSG_ARMSCON_AFTSMOKE:
			AftEftFunc[msg->message[1]]( work->world, work->arms_id, msg->message[2],
										 msg->message[3], msg->message[4], msg->message[5] );
			break;
		  default:
			printf("ArmsEffectControl Msg Err!!\n");
			break;
		}
		msg--;
	}
...
void *NewArmsEffectControl( int name, int arms_id, FMATRIX *world )
{
	...
	if ( work != NULL ) {
		...
		work->world = world;
		printf("%d\n",arms_id);
		if( arms_id >= ARMS_DATA_BOTTOM ) return NULL;
	}

	return (void *)work ;
}

void NewArmsEffectUsp( int name , int flags , FMATRIX *world )
{
	Muzzle_DemoHandGunDefault( world , 1 , flags );
}

void NewArmsEffectSocom( int name , int flags , FMATRIX *world )
{
	Muzzle_DemoSOCOM( world , 2 , flags );
}
