/*
	...
	$Id: sk_jimaku.c,v 1.1.1.3 2002/11/19 11:50:22 Yoshizawa1 Exp $
*/
 ...
 */
static int SK_codec_Create_DMA_PacketForTextTrans(CODEC_MESG_PANEL * panel , int x, int y , CVECTOR *color )
{
  int dx, dy, w, h, ofs;
  DG_DMAPACK * dmapack;
...
static void SK_codec_centering( CODEC_MESG_PANEL * panel, int tw)
{
  textarea_packet *packet;
  int dx, dy, w, h;
...
void SK_CODEC_DrawMesgPanel( CODEC_MESG_PANEL *panel, int x, int y , CVECTOR *color )
{
  SK_codec_Create_DMA_PacketForTextTrans(panel , x , y , color);
}

void SK_CODEC_DrawMessage( CODEC_MESG_PANEL *panel, char *mesg )
{
  FONT_DRAWINFO dr;
  int w;
  ...
  if( mesg != NULL ){
    ...
    SK_codec_centering(panel, w);
    printf("cjimaku.c: w = %d success\n", w);
  }
}

void SK_CODEC_OpenMesgPanel( CODEC_MESG_PANEL *panel, int width, int height )
{
  int vramsize;
...
/* メッセージパネルを隠す */
void SK_CODEC_HideMesgPanel( CODEC_MESG_PANEL * panel)
{
  panel->dmapack->flag |= DG_DMAPACK_INVISIBLEMENU;
}

/* メッセージパネルを再表示する */
void SK_CODEC_ShowMesgPanel(CODEC_MESG_PANEL * panel)
{
  panel->dmapack->flag &= ~DG_DMAPACK_INVISIBLEMENU;
}

void SK_CODEC_CloseMesgPanel( CODEC_MESG_PANEL *panel )
{
