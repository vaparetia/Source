...
/*
   ...
   2002/09/12	K.Cigeno retouched
   $Id: boss_etc2.c,v 1.2 2002/11/25 01:54:55 Yoshizawa1 Exp $
*/
...
typedef	struct _boss_etc2__work {
	GV_ACT_EX			actor ;
	int					handle_2d;
	GV_PAD              *pad; 
	int                 action;
...
static int gBP_BossName = -1; //BP_GAMEPLAY - record name of boss being played
...
static void BossNameDisp( Work *pWork , int flag )
{
	...
	case olga_stage : // オルガ
		ShowHideControl( pWork->handle_2d , L2D_boss_telop_OBJ_vs_olga , flag );
		break;
	case fat_stage : // ファットマン
		ShowHideControl( pWork->handle_2d , L2D_boss_telop_OBJ_vs_fatman , flag );
		break;
	case har_stage : // ハリアー
		ShowHideControl( pWork->handle_2d , L2D_boss_telop_OBJ_vs_harrier , flag );
		break;
	case vamp_stage : // ヴァンプ
		ShowHideControl( pWork->handle_2d , L2D_boss_telop_OBJ_vs_vamp , flag );
		break;
	case ray_stage : // RAY
		ShowHideControl( pWork->handle_2d , L2D_boss_telop_OBJ_vs_ray , flag );
		break;
	case soli_stage : // ソリダス
		ShowHideControl( pWork->handle_2d , L2D_boss_telop_OBJ_vs_solidus , flag );
		break;
	case w03_stage : // 長廊下
		ShowHideControl( pWork->handle_2d , L2D_boss_telop_OBJ_vs_soldiers , flag );
		break;
	case tng_stage : // 天狗
		ShowHideControl( pWork->handle_2d , L2D_boss_telop_OBJ_vs_tengus , flag );
		break;
...
static void KeySetPic( Work *pWork ) // キーを設定する関数
{
	...
	case olga_stage : // オルガ
		...
		break;
	case w03_stage : // 長廊下兵
		pWork->boss_pic[ 2 ].code1 = STR_KEY_BOSS_W03ENE;
		pWork->boss_pic[ 2 ].code2 = STR_KEY_BOSS_W03ENE;
		break;
	case fat_stage : // ファットマン
	...
	case vamp_stage : // ヴァンプ
		...
		break;
	case tng_stage : // 天狗兵
		pWork->boss_pic[ 2 ].code1 = STR_KEY_BOSS_TNG;
		pWork->boss_pic[ 2 ].code2 = STR_KEY_BOSS_TNG;
		break;
	case ray_stage : // RAY
...
static void KeySetName( Work *pWork ) // キーを設定する関数
{
	...
	case olga_stage : // オルガ
		...
		break;
	case w03_stage : // 長廊下
		pWork->boss_pic[ 3 ].code1 = STR_KEY_BOSS_NAME_W03ENE;
		pWork->boss_pic[ 3 ].code2 = STR_KEY_BOSS_NAME_W03ENE;
		break;
	case fat_stage : // ファットマン
	...
	case vamp_stage : // ヴァンプ
		...
		break;
	case tng_stage : // 天狗兵
		pWork->boss_pic[ 3 ].code1 = STR_KEY_BOSS_NAME_TNG;
		pWork->boss_pic[ 3 ].code2 = STR_KEY_BOSS_NAME_TNG;
		break;
	case ray_stage : // RAY
...
static int AnimationAct( Work *pWork ) // 通常実行
{
	...
	if ( ( status != L2D_STAT_BUSY ) && ( pWork->flag & SK_ACTION_OK ) ){
		...
		case STR_SHOWWIN : // default
         ...
         //BP_TROPHY - unlock "No Boss of Mine" ?
         if( gBP_BossName == soli_stage )
         {
            if( bp_trophy_get_tanker_cleared_by_user() && bp_trophy_get_plant_cleared_by_user() )
            {
               BP_TrophySystem_UnlockTrophy( kTRP_BeatBossSurvival );
            }
         }
         //BP_TROPHY - unlock "No Boss of Mine" ?
...
static	int	GetResources( Work *work )
{
   ...
   //BP_GAMEPLAY - record name of boss being played
   //NOTE: 
   // This actor type gets created/destroyed multiple times per boss battle. Only the 1st time,
   // is the boss name present (for the versus screen boss name).
   // It's stored off only if present to perform correct trophy logic.
   if( GCL_GetOption( 'b' ) != NULL )
   {
      gBP_BossName = GCL_GetOptionValue( 'b' , 0 );
   }
   //BP_GAMEPLAY - record name of boss being played
...
void *NewSK_BossTelop2( int name )
{
	Work		*work ;
