...
int videoDecCreate(VideoDec *vd,
    ...
    int tagSize, TimeStamp *pts, int n_pts, VoBuf *voBuf)
{
#if 1 //BP
   BP_MPEG_TODO_BREAK;
   ...
   vd->voBuf = voBuf;
    ...
    sceMpegAddCallback(&vd->mpeg,
    	sceMpegCbError, (sceMpegCallback)mpegError, vd);
    sceMpegAddCallback(&vd->mpeg,
    	sceMpegCbNodata, mpegNodata, vd);
    sceMpegAddCallback(&vd->mpeg,
    	sceMpegCbStopDMA, mpegStopDMA, vd);
    sceMpegAddCallback(&vd->mpeg,
    	sceMpegCbRestartDMA, mpegRestartDMA, vd);
    sceMpegAddCallback(&vd->mpeg, sceMpegCbTimeStamp,
    	(sceMpegCallback)mpegTS, vd);
...
//
int videoDecPutTs(VideoDec *vd, long64 pts_val,
    long64 dts_val, u_char *start, int len)
{
    TimeStamp ts;
...
void videoDecMain(VideoDec *vd)
{
    ...
    while (vd->voBuf->count)
    	;

     videoDecSetState(vd, VD_STATE_END);
printf( "videoDecMain end\n" );
}
...
int decBs0(VideoDec *vd)
{
   ...
   return 1;
    ...
    while (!sceMpegIsEnd(&vd->mpeg)) {
		...
		//
		while (!(voData = voBufGetData(vd->voBuf))) {
			switchThread();
		}
		...
		//
        ret = sceMpegGetPicture(mp, rgb32, vd->buf_width/16 * vd->buf_height/16);
...
int videoDecDecode( VideoDec *vd )
{
#if 1//BP
   BP_MPEG_TODO_BREAK;
   videoDecSetState(vd, VD_STATE_END);
...
//	printf( "end\n" );
    ...
    if( !( voData = voBufGetData( vd->voBuf ) ) ){
		printf( "vo\n" );
		return 0;
    }
    ...
    rgb32 = ( sceIpuRGB32 * )voData->v;
//printf( "COUNTER = %d\n", counter ++ );
    if( sceMpegGetPicture( mp, rgb32, vd->buf_width/ 16 * vd->buf_height / 16 ) < 0 ){
