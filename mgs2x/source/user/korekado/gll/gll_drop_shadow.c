...
//    }
    ...
    for ( i=0  ;  i<work->shadow->def->n_models  ; i++, o++, p++ ) {
		_sceVu0MulMatrix( &o->world, &drop_shadow, &p->world ) ;
		MakePosition( work, work->body->objs, work->shadow, i, &drop_shadow ) ;
		if ( work->shadow->def->n_models > 1 ) {
			if ( work->body->objs->objs[ i ].flag & DG_FLAG_INVISIBLE ) {
				work->shadow->objs[ i ].flag |= DG_FLAG_INVISIBLE ;
			} else {
				work->shadow->objs[ i ].flag &= ~DG_FLAG_INVISIBLE ;
			}
		}
...
static int GetResources( Work *work, OBJECT *body, CONTROL *control,  FMATRIX *lights, int *flag )
{
    ...
    if ( !(work->shadow = DG_MakeObjs( body->objs->def, DG_FLAG_SHADE|DG_FLAG_FINISHCALC|DG_FLAG_NOFOG|DG_FLAG_SEMITRANS|DG_FLAG_FORCEMSAA, 0 )) ) {
...
void *NewGllDropShadow( OBJECT *body, CONTROL *control, FMATRIX *lights, int *flag )
{
    Work *work ;
