...
/*
	...
	$Id: putatach_scn.c,v 1.1.1.3 2002/11/19 11:44:06 Yoshizawa1 Exp $
*/
#include <sys/types.h>
#include <stdio.h>
#include <stdlib.h>
...
/*----------------------------------------------------------------*/
void		*NewPutAttachments_Scn( int name, int map )
{
	Work		*work;
	OBJECT		*body = NULL;
	CONTROL		*ctrl;
	...
	if ( work != NULL ) {
		...
		body_name = GCL_GetOptionValue( 'c', 0 );
		at_id = GCL_GetOptionValue( 'a', 0 );
		flag = GCL_GetOptionValue( 'f', 0 );

		ctrl = SearchControl( body_name );
		if( ctrl ) body = ctrl->object;

		if( body && GetResources( work, NULL, body, at_id, flag ) < 0 ) {
			GV_DestroyActor( work ) ;
			return NULL ;
		}
	}
