...
/*
   ...
   2000/06/07 T.Morita
   $Id: orga_bar.c,v 1.1.1.3 2002/11/19 11:46:27 Yoshizawa1 Exp $
*/
#include <stdio.h>
#include <stdlib.h>
...
static const FVECTOR ORG_BarriarPos = { -12000.0f, 14600.0f, -18674.0f, 1.0f } ;
static const FVECTOR ORG_DeckPos    = { -11500.0f, 12750.0f, -17500.0f, 1.0f } ;
...
static void Act( Work *work )
{
    ...
    extern void *NewOrgaBullet( FMATRIX *world, u_int type, u_int side, u_int size,
			    u_int damage, u_int length, u_int speed, int weapon ) ;
    FMATRIX dir ;
    FVECTOR pos ;
    ...
    {
	dir = DG_UnitMatrix ;
	_sceVu0CopyVector( &pos, &ORG_DeckPos ) ;
	pos.vx += 300.0f*frnd() ;
	pos.vz += 300.0f*frnd() ;
	_sceVu0SubVector( (FVECTOR*)&dir.m[Y], work->orga_pos, &pos ) ;
	dir.m[Y][W] = 0.0f ;
	...
	if ( PL_PlayerResetInvincible() )
	{
	    work->shoot = 1 ;
	    dir = DG_UnitMatrix ;
	    _sceVu0CopyVector( (FVECTOR*)&dir.m[W], &ORG_BarriarPos ) ;
	    ...
	    GM_SeSetMode( ORGA_SE_USP_FIRE, work->orga_pos, GM_SEMODE_BOMB ) ;
	    if ( !work->mode )
	    {
...
static int GetResources( Work *work, int mode, FVECTOR *orga_pos )
{
    work->mode = mode ;
    work->tics = mode ? 180 : 0 ;
    work->orga_pos = orga_pos ;
    work->shoot    = 0    ;
    return  0 ;
}

void *NewOrgaWeaponBarriar( int mode, FVECTOR *orga_pos )
{
    Work *work ;
    ...
    if( work != NULL )
    {
        ...
        if( GetResources( work, mode, orga_pos ) < 0 )
        {
            GV_DestroyActor( work ) ;
            return NULL ;
        }
