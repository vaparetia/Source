...
/*
   ...
   2000/01/11 T.Morita
   $Id: orga_hzd.c,v 1.1.1.3 2002/11/19 11:46:28 Yoshizawa1 Exp $
*/
#include <stdio.h>
#include <stdlib.h>
...
#define ORGA_HZX_S_SPHERE 500

int ORG_HzdSegLineCheck( HZX_SEG *seg, FVECTOR *p1, FVECTOR *p2 )
{
    FVECTOR p ;
    float sx, sz, tx, tz ;
    float r, s, rd ;
...
void ORG_HzdSegReactVector( HZX_SEG *seg, FVECTOR *p  )
{
    float dx, dz, f ;
...
int ORG_HzdCheck( HZX_GROUP_ID hzx, FVECTOR *pos, FVECTOR *pos_v,
		  float min, FVECTOR *bounce, float sphere )
{
    ...
    if ( HZX_NearHazardCheck( hzx, pos, ORGA_HZX_S_SPHERE, HZX_CHK_ALL, HZX_SEG_NO_BULLET, (int)sphere ) )
    {
        HZX_GetNearHazard( seg, atr ) ;
        if ( ORG_HzdSegLineCheck( seg, pos, &new ) )
            ORG_HzdSegReactVector( seg, pos_v ), flag |= 1 ;
    }
...
int ORG_HzdOnlineCheck( HZX_GROUP_ID hzx, FVECTOR *pos, FVECTOR *pos_v,	float min, FVECTOR *bounce )
{
    int   atr[2] ;
    HZX_SEG seg[2] ;
    int flag = 0 ;
	...
	flag = HZX_OnlineHazardCheck( hzx, pos, &new, HZX_CHK_ALL,
				      HZX_SEG_NO_BULLET |  HZX_SEG_NO_MISSILE |HZX_SEG_RECOIL_TYPE,
	...
	ORG_HzdSegReactVector( seg, pos_v ) ;
