...
/*
  ...
  2001/01/27 T.Morita
  $Id: fort_b_line.c,v 1.1.1.3 2002/11/19 11:45:56 Yoshizawa1 Exp $
*/
#include <stdio.h>
#include <stdlib.h>
...
typedef struct work_t
{
    ...
    SPL_POINT points[DEMO_BUL_N_POINT+2] ;
} Work ;
...
static void CalcTangentVector( TAN_VEC *t,   /* tan_vec buffer     */
			       SPL_POINT   *src, /* current key frame  */
			       SPL_POINT   *dst, /* next key frame     */
			       int      frms /* number of frames   */ )
{
    ...
    if ( frms )
	t->base = dst->frmno - (t->frms = (float)frms      ) ;
    else
	t->frms = dst->frmno - (t->base = (float)src->frmno) ;
...
*/
static void CalcSplineVectorPos( SPL_POINT *head, float frm, FVECTOR *p )
{
    TAN_VEC tan_vec ;
			     ...
			     float front, float vertical )
{
    ...
    FVECTOR v, h, dir ;
    int     i   ;
    static SPL_POINT points_org[DEMO_BUL_N_POINT+2] = {
...
void *NewFortSplineBulletDemo( FVECTOR *pos, FVECTOR *pos_v, FVECTOR *avoid,
			       float front, float vertical )
{
    Work *work ;
    ...
    if( work != NULL )
    {
        ...
        if( GetResourcesCall( work, pos, pos_v, avoid, front, vertical ) < 0 )
        {
            GV_DestroyActor( work ) ;
            return NULL ;
        }
