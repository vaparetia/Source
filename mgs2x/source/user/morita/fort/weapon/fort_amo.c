...
/*
   ...
   1999/12/18 T.Morita
   $Id: fort_amo.c,v 1.1.1.3 2002/11/19 11:46:18 Yoshizawa1 Exp $
*/
#include <stdio.h>
#include <stdlib.h>
...
#define FORT_AMO_GRAVITY  3
#define FORT_AMO_BOUNCE   0.58f
#define FORT_AMO_SPHERE   40
#define FORT_AMO_R_VEL    0.8f

static FVECTOR BounceVector = { -1.0, -FORT_AMO_BOUNCE, -1.0 } ;


typedef	struct work_lnramo_t Work ;
struct work_lnramo_t
{
    GV_ACT	 actor  ;
    HZX_GROUP_ID hzx    ;
...
static void Act( Work *work )
{
	...
	*work->flag = 0 ;
	...
	break ;
    case 1:
	work->vel.vy -= FORT_AMO_GRAVITY ;  /* 重力加速度 */
	...
	if ( FRT_HzdCheck( work->hzx, (FVECTOR*)&work->drop->world.m[W], &work->vel,
			   -FORT_AMO_SPHERE, &BounceVector, FORT_AMO_SPHERE ) & 2 )
	{
	    _sceVu0ScaleVector( &work->vel, &work->vel, FORT_AMO_R_VEL ) ;/* 摩擦処理 */
...
static int GetResources( Work *work, FMATRIX *right, FMATRIX *left, short *flag )
{
    DG_DEF  *def = GV_GetCache( GV_CacheID( GV_StrCode( "lnr_amo" ), 'k' ) ) ;
...
void *NewFortWeaponLNRAMO( FMATRIX *right, FMATRIX *left, short *flag )
{
    Work *work ;
    ...
    if( work != NULL )
    {
        ...
        if( GetResources( work, right, left, flag ) < 0 )
        {
            GV_DestroyActor( work ) ;
            return NULL ;
        }
