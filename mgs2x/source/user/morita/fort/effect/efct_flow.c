...
/*
  ...
  $Id: efct_flow.c,v 1.1.1.3 2002/11/19 11:46:10 Yoshizawa1 Exp $
*/
...
#define	SCR_FVEC0	(void*)(SCRPAD_ADDR)
#define	SCR_FVEC1	(void*)(SCRPAD_ADDR+16*1)
#define	SCR_FVEC2	(void*)(SCRPAD_ADDR+16*2)
#define	SCR_FVEC3	(void*)(SCRPAD_ADDR+16*3)
#define	SCR_FVEC4	(void*)(SCRPAD_ADDR+16*4)
#define	SCR_WORK0	(void*)(SCRPAD_ADDR+1024)
#define	SCR_WORK1	(void*)(SCRPAD_ADDR+2048)
#define	SCR_WORK2	(void*)(SCRPAD_ADDR+3072)
...
{
	FVECTOR		*sc_min,  *sc_max;
	FVECTOR		*sc_plus, *sc_minus;
	FVECTOR		*sc_temp0;
	FVECTOR		*vec;
	FVECTOR		*pos0;
	FVECTOR		*pos1;
	...
	int		i, k, clock ;
	DG_PRIM2	*prim ;
	DG_PRIM2_UVRGBWH *prim_uvrgb0 ;	/* スプライト用 */
	DG_PRIM2_UVRGBWH *prim_uvrgb1 ;	/* スプライト用 */
	int   flag  ;
	float angle ;
	...
	DG_COPY_VEC( sc_plus,  FRT_MiscBulletPos() );
	DG_COPY_VEC( sc_minus, sc_plus );
	sc_minus->vy += 3000.0f ;
   ...
   if( !DG_SwitchBuffPrim2( work->prim ) )
   {
	...
	prim_vec  = work->vec;
	prim_uvrgb0 = prim->uvrgb[1-clock];
	prim_uvrgb1 = prim->uvrgb[  clock];

	if ( !(GV_Time & 0x03) )
	    if ( work->n_prim < N_PRIMS )
		work->n_prim++ ;
	flag = 0 ;
	for ( i=work->n_prim ; --i>=0 ; )
	{
		...
		for ( k = 0 ; k < N_VERTS ; k++ ){
			...
			angle  = (GM_StagePlayTime & 0xff) ;
			angle -= angle>128 ? -256.0f : 0.0f ;
			angle *= i&1 ? -1.0f : 1.0f ;
			angle  = angle / 128.0f * M_PI + 0.5f * i*k ;
			prim_uvrgb1->w = (int)(work->size * vu0_Cos( angle ));
			prim_uvrgb1->h = (int)(work->size * vu0_Sin( angle ));
			prim_uvrgb1->a = prim_uvrgb0->a ;

			if ( work->life )
			{
			    if ( pos0->vy < sc_min->vy+200.0f )
			    {
				if ( prim_uvrgb1->a > work->alpha/4 )
				    prim_uvrgb1->a-- ;
			    }
			    else if ( prim_uvrgb1->a < work->alpha )
				prim_uvrgb1->a++ ;
			}
			else if ( prim_uvrgb1->a )
			{
			    if ( !((GV_Time+i*k) & ((0xffffffff >> prim_uvrgb1->a) & 0x1f )) )
				prim_uvrgb1->a-- ;
			}
			flag |= prim_uvrgb1->a ;
			if ( !i && !k )
			    FRT_SmokeClouded = prim_uvrgb1->a ;
...
//	DG_SetPrim2Alpha( prim, SCE_GS_SET_ALPHA( 0, 2, 0, 1, 0x00 ) );
	...
	for ( i = 0 ; i < N_PRIMS ; i++ ){
		for ( k = 0 ; k < N_VERTS ; k++ ){
			...
			angle        = rnd()*TPI;
			uvrgbwh1->w  = uvrgbwh0->w  = (int)(work->size * vu0_Cos( angle ));
			uvrgbwh1->h  = uvrgbwh0->h  = (int)(work->size * vu0_Sin( angle ));
...
static int GetResourcesCalled( Work *work, FVECTOR *min, FVECTOR *max, float size, int life )
{
	DG_PRIM2	*prim ;
	DG_TEX		*tex ;
	FVECTOR		fvtemp0;
	...
	_sceVu0CopyVector( &fvtemp0, min ) ;
	_sceVu0CopyVector( &fvtemp1, max ) ;
...
static int GetResources( Work *work )
{
	...
	if ( GetResourcesCalled( work,
				 &fvtemp0,
				 &fvtemp1,
				 (float)GCL_GetOptionValue( 's', 1000 ),
				 60*20 ) < 0 )
	    return -1 ;
	return 0 ;
}

void *NewFortDynamicFlow( int name, int map )
{
	Work		*work ;
...
void *NewFortDynamicFlowCalled( FVECTOR *min, FVECTOR *max, float size, int life )
{
	Work		*work ;
	...
	if ( work != NULL ) {
		...
		if ( GetResourcesCalled( work, min, max, size, life ) < 0 ) {
			GV_DestroyActor( work ) ;
			return NULL ;
		}
	}
