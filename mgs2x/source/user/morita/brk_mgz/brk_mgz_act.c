...
/*
   ...
   2000/06/20 T. Morita
   $Id: brk_mgz_act.c,v 1.1.1.3 2002/11/19 11:45:35 Yoshizawa1 Exp $
*/
#include <stdio.h>
#include <stdlib.h>
...
FVECTOR BRK_MGZ_Size = { BRK_MGZER_RAD,BRK_MGZER_RAD,BRK_MGZER_RAD, 0 } ;
...
float BRK_MGS_GetCurrentSize( MAGAZINE *mgz, FMATRIX *m, FVECTOR *size, FVECTOR *cen )
{
    int     i, j ;
    FVECTOR f, min, max ;
...
/***
  ...
  ***/
/* 当たりを見て移動 */
static int MoveObject( Work *work, FMATRIX *m, MOVE *p, MAGAZINE *mgz, float gravity )
{
    int     i, r, flag = 0 ;
	...
	/*雑誌は Targetよりモデルの一番最下点を求める*/
        BRK_MGS_GetCurrentSize( mgz, m, &size, &v ) ;
    else
    {
	/*ゴミは現在の向きに進む速度を現在の速度に足して平均化していく。
          これによりフワフワする */

	_sceVu0ScaleVector( &v, (FVECTOR*)&m->m[X],
			    ((p->rot_vx+1024)&2047)/32.0f ) ;
	/* 一度下向きになったら,もう上がらない */
	if ( p->pos_v.vy < 0.0f && v.vy > 0.0f )
	    v.vy = -v.vy ;
...
#endif
	p->pos_v.vy = p->pos_v.vy>=0.0f ? -2.0f : p->pos_v.vy ;
	p->pos.vy += p->pos_v.vy ;
	break ;
    case 1:
    case 3:
...
static int MoveObjectNonHit( Work *work, FMATRIX *m, MOVE *p, MAGAZINE *mgz, float gravity )
{
    int     i, r ;
...
/* 0°or 180°に倒れ込み */
static int FallDownObject( Work *work, FMATRIX *m, MOVE *p, MAGAZINE *mgz, float gravity )
{
    int     r, flag ;
    FVECTOR cen ;
    FVECTOR size = { BRK_DUST_SPHERE, BRK_DUST_SPHERE, BRK_DUST_SPHERE, 0 } ;
    ...
    RotateMatrixZY( m, &DG_UnitMatrix, p->rot_x, p->rot_y ) ;
    if ( mgz )
        BRK_MGS_GetCurrentSize( mgz, m, &size, &cen ) ;
...
/***
  ...
  ***/
/* 小さい破片のアクト起動関数 */
void BRK_MGZ_ActPiece( Work *work )
{
    int    i, flag = 0 ;
    PIECE *p ;

    for ( i=BRK_MGZ_N_PIECE, p=work->piece ; --i>=0 ; p++ )
    {
        switch ( p->flag )
        {
        case 1:
...
#if MAKING
	{
	    void AN_Test_Eye2( FVECTOR *, int ) ;
	    void AN_Test_Eye3( FVECTOR * ) ;
	    //AN_Test_Eye2( &p->mov.pos, 2 ) ;
	    AN_Test_Eye3( &p->mov.pos ) ;
	}
#endif
...
#if MAKING
	GV_DestroyOtherActor( p->pursuit ) ;
#endif
    }
}

void BRK_MGZ_StartActPiece( Work *work, int i, FVECTOR *pos, FVECTOR *pos_v, float width, float pop )
{
    PIECE  *p ;
    FVECTOR r ;
...
#if MAKING
	{
	    extern void *NewLinePursuit( FVECTOR *pos ) ;
	    p->pursuit = NewLinePursuit( &p->mov.pos ) ;
	}
#endif

        if ( ++work->n_piece >= BRK_MGZ_N_PIECE )
            work->n_piece = 0, p -= BRK_MGZ_N_PIECE ;
    }    
}
...
/***
  ...
  ***/
/* ページのアクト関数 */
void BRK_MGZ_ActPage( Work *work )
{
    int   i, flag = 0 ;
    float open ;
    PAGE *p ;
    ...
    {
	if ( p->flag < -BRK_MGZ_SPOOLTIME )
	    continue ;
	else if ( p->flag == -BRK_MGZ_SPOOLTIME )
	    BRK_MGZ_FreePage( p ) ;
        else if ( p->flag > 0 )
        {
	    if ( p->anime->count )
		...
		if( p->book->open_v == 0.0f ) p->book->open_v = 0.0000001f; /* 0DIV 2002.03.22 Yano add */
...
#if 0 /* 0DIV 2002.03.22 Yano modify */
		    p->anime->p[0] = 1.0f, p->anime->p[1] = 0.0f,
			p->anime->count = 1-(int)(DEG2RAD(15.0f)/p->book->open_v) ;
#else
		{
		    p->anime->p[0] = 1.0f, p->anime->p[1] = 0.0f;
			if( p->book->open_v == 0.0f ) p->book->open_v = 0.0000001f;
			p->anime->count = 1-(int)(DEG2RAD(15.0f)/p->book->open_v) ;
		}
#endif
...
/* ページのアクト起動関数 */
void BRK_MGZ_StartActPage( Work *work, MAGAZINE *m, int frame )
{
    PAGE   *p = work->page ;
    int i ;

    for( i=BRK_MGZ_N_PAGE, p=work->page ; p->flag>0 && --i>0 ; p++ ) ;
...
/***
  ...
  ***/
//obj毎の回転
void BRK_MGZ_DispMagazine( MAGAZINE *m )
{
    FMATRIX mtx ;
    float rot_z=0.0f ;

    if ( m->mov.rot_x> 0 && m->open > (float)M_PI*0.5f )
	rot_z = DEG2RAD(2.5f) ;
    _sceVu0RotMatrixY( &mtx, &DG_UnitMatrix, rot_z ) ;
    _sceVu0MulMatrix( &mtx, &m->objs->world, &mtx ) ;

    _sceVu0CopyMatrix( &m->objs->objs[0].world, &mtx ) ;
    _sceVu0RotMatrixY( &m->objs->objs[1].world, &DG_UnitMatrix,  m->open ) ;
    _sceVu0MulMatrix( &m->objs->objs[1].world, &mtx, &m->objs->objs[1].world ) ;
}

void BRK_MGZ_ActMagazine( Work *work )
{
	...
	case -2:
		...
		p->mov.pos_v.vz = 100 ;//+(irnd()&31) ;
	    else
		p->mov.pos_v.vx = 100 ;//+(irnd()&31) ;
	    p->mov.pos_v.vy = 30 ;
	    p->mov.rot_vx = (irnd()&0x00e0)-128 ;
	    p->mov.rot_vy = (irnd()&0x00e0)-128 ;
	    p->open   = DEG2RAD( 0.0f) ;/*本が開く角度    */
	    p->open_v = DEG2RAD(20.0f) ;/*本が開く角度速度*/

	    /* p->flag は 4 以上が当たりを無視するので 10 を入れると 7フレーム無視する */
	    p->flag = 10 ; /* ignore collison for 7 frames */
	    flag = 1 ;
	    break ;
	case -1:
	case  0:
	...
	case 4:
	    /* ゴミを出して宙を舞う */
	    BRK_MGZ_StartActPiece( p->work, 10, &p->mov.pos, &pop, 50.0f, 40.0f ) ;

	default:
	    /* 当たりなしで宙を舞う */
	    p->flag-- ;
	    MoveObjectNonHit( work, &p->objs->world, &p->mov, p, BRK_MGZ_GRAVITY ) ;
	    if ( (p->open += p->open_v) > DEG2RAD(30.0f) && !(GV_Time & 7) )
		BRK_MGZ_StartActPage( work, p, 20 ) ;
	    if ( p->open > DEG2RAD(160.0f) )
		p->open_v *= -0.8f, p->open = DEG2RAD(160.0f) ;
	...
	case  3:
	    /* 当たりつきで宙を舞う */
            if ( MoveObject( work, &p->objs->world, &p->mov, p, BRK_MGZ_GRAVITY ) == 1 )
	    {
		/* 敵兵見つかり用 */
	...
	case  2:
	    /* 地面についたので倒れる */
	    if ( FallDownObject( work, &p->objs->world, &p->mov, p, BRK_MGZ_GRAVITY ) )
	    {
		if ( p->open + p->open_v > DEG2RAD(90.0f) ){  /*開く*/
		    GM_SeSetMode( SD_A_BOOKFA01, &p->mov.pos, GM_SEMODE_BOMB ) ;
		}else{	/*閉じる*/
		    GM_SeSetMode( SD_A_BOOKFA02, &p->mov.pos, GM_SEMODE_BOMB ) ;
		}
                p->flag = 1 ;
	...
	case  1:
	    ...
	    if (  p->mov.rot_x > 0 ) /* うつぶせDEG2RAD(170.0f)*/
		p->open += p->open_v = ((p->open + p->open_v > DEG2RAD(90.0f) ? DEG2RAD(175.0f) : DEG2RAD(0.0f)) - p->open) * 1.0f ;
	    else                     /* あおむけ */
		p->open += p->open_v = ((p->open + p->open_v > DEG2RAD(90.0f) ? DEG2RAD(180.0f) : DEG2RAD(0.0f)) - p->open) * 0.125f ;
	    if ( !(int)(p->open_v/DEG2RAD(0.1f)) ){
                p->flag = p->flag==1 ? 0 : 2 ;		
	    }
	    break ;
        }
	BRK_MGZ_DispMagazine( p ) ;
	BRK_UTL_GetLightMatrix( &p->mov.pos, p->lights, 0.6f, work->where ) ;
...
/***
  ...
  ***/
/* 埃のアクト関数 */
void BRK_MGZ_ActDust( Work *work )
{
    ...
    for ( i=BRK_MGZ_N_DUST ; --i>=0 ; nxt_u++, nxt_p++, prv_u++, prv_p++, v++ )
	if ( (nxt_u->a = prv_u->a) )
	{
	    flag = 1 ;
	    if ( GV_Time & 1 )
...
/* 埃のアクト起動関数 */
void BRK_MGZ_StartActDust( Work *work, int i, FVECTOR *pos, FVECTOR *pos_v,
...
//	nxt_u->h = (short)(100.0f  * sinf(work->rad[i]));
	v->vw = 20.0f ;
	if ( ++work->n_dust >= BRK_MGZ_N_DUST )
	{
	    work->n_dust = 0 ;
	    nxt_u -= BRK_MGZ_N_DUST ;
	    nxt_p -= BRK_MGZ_N_DUST ;
	    v     -= BRK_MGZ_N_DUST ;
	}
	else
	    nxt_u++, nxt_p++, v++ ;
