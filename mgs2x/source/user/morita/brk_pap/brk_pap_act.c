...
/*
   ...
   2000/04/25 T. Morita
   $Id: brk_pap_act.c,v 1.1.1.3 2002/11/19 11:45:39 Yoshizawa1 Exp $
*/
#include <stdio.h>
#include <stdlib.h>
...
static FVECTOR BRK_PAP_NoBounce = { 1.01f, 1.0f, 1.01f, 0.0f } ;
...
static int MoveObject( Work *work, FMATRIX *m, MOVE *p, DG_MDL *mdl, int r_rot, float r_vel )
{
    ...
    switch( i=BRK_CheckHazard( work->hzd, &p->pos, &p->pos_v, &BRK_PAP_NoBounce, &size ) )
    {
    case 3:
    case 1:
	flag = 1 ;
...
/***
  ...
  ***/
/* 小さい破片のアクト起動関数 */
void BRK_PAP_ActPiece( Work *work )
{
    int    i, flag = 0 ;
    PIECE *p ;

    for ( i=BRK_PAP_N_PIECE, p=work->piece ; --i>=0 ; p++ )
    {
        switch ( p->flag )
        {
        case 1:
            if ( MoveObject( work, &p->pos->world, &p->mov, NULL, BRK_ROT_R, BRK_VEL_R ) == 1 )
	    {
		p->mov.rot_x &= 4095 ;
...
void BRK_PAP_StartActPiece( Work *work, int i, FVECTOR *pos, FVECTOR *pos_v, float width, float pop )
{
    PIECE  *p ;
    FVECTOR r ;
    ...
    for ( p=&work->piece[work->n_piece] ; --i>=0 ; p++ )
    {
        ...
        if ( ++work->n_piece >= BRK_PAP_N_PIECE )
            work->n_piece = 0, p -= BRK_PAP_N_PIECE ;
    }
}


void BRK_PAP_ActPaper( Work *work )
{
    PAPER *p ;
    int    i, flag =0 ;
	...
	switch ( p->flag )
		...
		p->flag = 2 ;
            ...
            break ;
        case 2:
            switch( FallDownObject( work, &p->objs->world, &p->mov, p->objs->objs->model,
				 BRK_ROT_R, BRK_VEL_R ) )
	    {
	    case 1:
		p->flag = 0 ;
	...
	/* ライトマトリックスを取得する */
	//BRK_UTL_GetLightMatrix( &p->mov.pos, p->lights, 0.8f, work->where ) ;
	DG_GetLightMatrix( &p->mov.pos, p->lights ) ;
	_sceVu0CopyMatrix( &p->objs->objs[0].world, &p->objs->world ) ;
	_sceVu0CopyMatrix( &p->objs->objs[1].world, &p->objs->world ) ;
...
void BRK_PAP_StartActPaper( Work *work, int i, FVECTOR *pos, FVECTOR *pos_v, float width, float pop )
{
    PAPER  *p ;
    FVECTOR r ;
    ...
    while( --i>=0 )
    {
	p = &work->paper[work->n_paper] ;
        r.vx = width*frnd() ;
        r.vy = width* rnd() + pop ;/* 上半球にするため rnd() を使う */
        r.vz = width*frnd() ;
        p->mov.rot_x  = (irnd()&511)-256 ;
	...
	DG_VisibleObjs( p->objs ) ;
        if ( ++work->n_paper >= BRK_PAP_N_PAPER )
            work->n_paper = 0 ;
    }    
}
...
/***
  ...
  ***/
/* 埃のアクト関数 */
void BRK_PAP_ActDust( Work *work )
{
    ...
    for ( i=BRK_PAP_N_DUST ; --i>=0 ; nxt_u++, nxt_p++, prv_u++, prv_p++, v++ )
	if ( (nxt_u->a = prv_u->a) )
	{
	    flag = 1 ;
	    if ( GV_Time & 1 )
		nxt_u->a-- ;
	    if ( (v->vw -= 2.0f) > 0.0f )
		  nxt_u->w = nxt_u->h = prv_u->h += (int)v->vw ;/*ある程度まで拡大*/
...
/* 埃のアクト起動関数 */
void BRK_PAP_StartActDust( Work *work, int i, FVECTOR *pos, FVECTOR *pos_v,
			   float width, float pop )
{
	...
	nxt_u->w = nxt_u->h = 100 ;
	v->vw = 20.0f ;
	if ( ++work->n_dust >= BRK_PAP_N_DUST )
	{
	    work->n_dust = 0 ;
	    nxt_u -= BRK_PAP_N_DUST ;
	    nxt_p -= BRK_PAP_N_DUST ;
	    v     -= BRK_PAP_N_DUST ;
	}
	else
	    nxt_u++, nxt_p++, v++ ;
...
/*シェードを計算させるため*/
void BRK_PAP_ActPiled( Work *work )
{
    int    i ;
    PILED *p = work->piled ;
