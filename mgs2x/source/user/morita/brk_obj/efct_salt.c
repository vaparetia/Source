...
/*
   ...
   メリケン粉の固まりの飛び散り
	...
	$Id: efct_salt.c,v 1.1.1.3 2002/11/19 11:45:38 Yoshizawa1 Exp $
...
#define	  MEM_UVRGB		((void *)( (int)MEM_POS   + sizeof(FVECTOR) * N_NUM))
#define	  MEM_MOVE		((void *)( (int)MEM_UVRGB + sizeof(DG_PRIM2_UVRGB) * N_NUM))
#define	  MEM_TEMP		((void *)( (int)MEM_MOVE  + sizeof(FVECTOR) * N_NUM ))
#define	  MAX_COUNT		(60)
#define	  SCALE			(15)
#define	  GRAVITY		(-0.5F)
...
static  void  Act(Work *work)
{
    ...
    for( i=N_PRIMS*N_VERTS ; --i>=0 ; uvrgb++ )
	uvrgb->a = (i&3)==0 ? 0 : color ;
...
static  void  InitScrPad(Work *work,DG_TEX *tex)
{
    ...
    /* ゴミデータを保持 */
    local_move = MEM_TEMP ;
    tex_data = (void*)((int)MEM_TEMP + sizeof(FVECTOR) * N_VERTS) ;
    //BP - Commented out following useless line - has horrible behavior since tex_data is not char* or void*
...
/* キャラ起動 */
void  *NewSaltSplay( FMATRIX *world )
{
    Work  *work ;
