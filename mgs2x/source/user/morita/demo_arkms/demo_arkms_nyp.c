...
/*
   demo_arkms_nyp.c
   ARKMS専用 キャラ（森田さんのコピーを置換）

   2000/02/04 T. Shibata
   $Id: demo_arkms_nyp.c,v 1.1.1.3 2002/11/19 11:45:55 Yoshizawa1 Exp $
*/
#include <stdio.h>
#include <stdlib.h>
...
static void Die( Work *work )
{
	...
	//FreeObject_NYPD( work->object, &work->changed ) ;
}
...
static int GetResources( Work *work, int name, OBJECT *object, int type )
{
    ...
    objs = object->objs ;
    if ( !InitObject_NYPD( object, DG_FLAG_SHADE|DG_FLAG_FINISHCALC, &work->changed, type ) )
	PERROR( "Cannnot initialize InitObject_NYPD()  : NewDemoArkms" ) ;
    if ( objs )
...
void *NewDemoArkms_NYPD( int name, OBJECT *object, int type )
{
    Work *work ;
