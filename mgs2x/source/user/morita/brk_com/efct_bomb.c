...
/*
	...
	$Id: efct_bomb.c,v 1.1.1.3 2002/11/19 11:45:26 Yoshizawa1 Exp $
...
#define	VEC_RAND			(100.0f)
#define	VEC_DECAY_RATIO		(0.97f)

#define	VIBLATION_LENGTH_1		(60)
#define	VIBLATION_LENGTH_2		(8)
...
//			fvtemp.vy   = SIZE + ((VEC_MIN + VEC_RAND) - fvtemp.vy) * 0.3f;	/* 速い時小さい */
			fvtemp.vy   = SIZE;
			angle       = rnd()*TPI;
			uvrgbwh->w  = (int)(fvtemp.vy * vu0_Cos( angle ));
			uvrgbwh->h  = (int)(fvtemp.vy * vu0_Sin( angle ));
...
void *NewBreakComputerBombKasu( FVECTOR *pos, SVECTOR *pole_rot )
{
	Work		*work ;
