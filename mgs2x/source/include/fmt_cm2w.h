/*
	...
	1999/07/07 K.Takabe
	$Id: fmt_cm2w.h,v 1.2 2002/11/20 05:35:54 takaki Exp $
...
typedef struct _tri_fileheader {
	...
	unsigned long32		clut_addr ;		/* ＣＬＵＴデータへのポインタ（現在未使用） */
#else
	unsigned long32		file_type ;		/* ファイル識別ＩＤ（現在未使用） */
	unsigned long32		compress_flag ;	/* 圧縮フラグ（現在は単なるフラグとして使用） */
	unsigned long32		n_info ;		/* XTIテクスチャ情報数 */
	unsigned long32		info_addr ;		/* XTIテクスチャ情報へのオフセット */
	unsigned long32		n_textures ;	/* テクスチャデータ数 */
	unsigned long32		texel_addr ;	/* テクセルデータへのポインタ */
	unsigned long32		clut_addr ;		/* ＣＬＵＴデータへのポインタ */
	unsigned long32		n_clut ;		/* 格納ＣＬＵＴエントリ数 */
#endif
} TRI_FILEHEADER ;
