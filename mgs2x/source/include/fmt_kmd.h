/*
	...
	1999/07/07 K.Takabe
	$Id: fmt_kmd.h,v 1.1.1.3 2002/11/19 11:41:59 Yoshizawa1 Exp $
...
enum {
	...
	DG_EVMTYPE_FORMAT_V2	= 0x01000000,	/* フォーマットVer.2.00（マトリクスパレット化） */
	DG_EVMTYPE_FORMAT_V3	= 0x02000000,	/* フォーマットVer.3.00（スケルトンバウンディング追加） */
};
...
/*----------------------------------------------------------------*/
/*----------------------------------------------------------------*/
#define NEW_KMS_FORMAT
#ifndef NEW_KMS_FORMAT
/* ＰＳＹメタルギア用新モデルフォーマット */

#define DG_MDLPACK DG_OBJPACK
typedef	struct _DG_OBJPACK	{
...
typedef struct _dg_old_mwdef {
	...
	int				n_packs ;		/* ポリゴンパケット数				*/
	EVM_PACK		*packet ;		/* ポリゴンパケット					*/
	OLD_EVM_SKEL		skeleton[ NULL_ARRAY ] ;
} OLD_EVM_DEF ;
...
/*
	...
	u_char	index0 ;	マトリクス０に対するマトリクスインデックス番号(0~7)）
	u_char	index1 ;	マトリクス１に対するマトリクスインデックス番号(0~7)）
	u_char	index2 ;	マトリクス２に対するマトリクスインデックス番号(0~7)）
	u_char	index3 ;	マトリクス３に対するマトリクスインデックス番号(0~7)）
...
typedef struct _CV2_MDL{
	...
	NORM_USRDATA	*norm_usrdata_le;	/* BP - NOTHING APPEARS TO USE THIS */	/* ユーザー定義情報 */
