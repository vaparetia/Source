/*
	...
	1999/07/07 K.Takabe
	$Id: fmt_lit.h,v 1.1.1.3 2002/11/19 11:41:59 Yoshizawa1 Exp $
...
typedef	struct	{
	u_int		n_lights ;	/* 光源数		*/
	DG_LIT_OLD		lights[ NULL_ARRAY ] ;	/* 光源配列		*/
} DG_LITS_OLD ;

typedef	struct	{
	u_int		n_lights ;	/* 光源数		*/
	DG_LIT		lights[ NULL_ARRAY ] ;	/* 光源配列		*/
} DG_LITS ;
...
typedef struct {
	...
	int			pad ;				/* リザーブ（現在は０でなければならない） */
   LIT_GRP		grp[ NULL_ARRAY ] ;
} LIT_DEF ;

//BP_PS2 - define for all platforms
#if 1 //BP def KP_XBOX
typedef struct {
	FVECTOR		dir ;				/* グローバル平行光源の方向（vwに光源強度） */
	CVECTOR		color ;				/* グローバル平行光源の光源色 */
...
enum {
	...
	/*
		LIT_TYPE_POINT、LIT_TYPE_SPOT、LIT_TYPE_LINE、及びLIT_TYPE_BLACKPOINTは共に排他である。
	*/
};
