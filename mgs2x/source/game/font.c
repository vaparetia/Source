...
﻿/*
	font.c
		フォント描画ルーチン
	1999/12/20	K.Uehara
	$Id: font.c,v 1.1.1.3 2002/11/19 11:41:48 Yoshizawa1 Exp $
*/
...
static inline unsigned char const *getnextchar( int *code, unsigned char const *m )
{
	int a;
...
void font_resident_rubi_set( unsigned char const *rubitop )
{
}
...
static inline int rubi_get_bit( unsigned char const *datatop, int pos )
{
	return ( datatop[ pos / 4 ] & ( 3 << ( ( 3 - ( pos % 4 ) ) * 2 ) ) ) ? 1 : 0;
}

static inline int rubi_get_pattern( unsigned char const *datatop, int width, int x, int y )
{
	/* x, yに対応するデータを返す */
	/* xはオーバーランする可能性がある */
...
static int draw_rubi_string( FONT_DRAWINFO *out, int x, int y, unsigned char const *string )
{
	int width;
	int count;
	int basewidth, basey;
...
static int draw_rubi( FONT_DRAWINFO *dr, int x, int y, unsigned char const *str )
{
...
int font_draw_string( FONT_DRAWINFO *work, char const *string )
{
