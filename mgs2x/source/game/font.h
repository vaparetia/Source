/*
	font.h
		フォント描画ルーチンヘッダ
	2000/01/05	K.Uehara
	$Id: font.h,v 1.1.1.3 2002/11/19 11:41:48 Yoshizawa1 Exp $
*/
...
typedef int ( *rubi_draw_func )( FONT_DRAWINFO *out, int x, int y, unsigned char const *rubi );
...
struct _drawinfo {
	...
	unsigned char const *(*callback)( FONT_DRAWINFO *dr, unsigned char const *now, int no );
	short now_x;
	short now_y;
	int yc;

	unsigned char const *prev_char;
};
...
static inline int font_get_draw_width( FONT_DRAWINFO *draw )
{
	return draw->outinfo->max_width;
}

static inline void font_set_draw_callback( FONT_DRAWINFO *draw,
										             unsigned char const *( *callback )( FONT_DRAWINFO *dr, unsigned char const *now, int no ) )
{
