...
sprStatus * mode_2d_new(char * name)
{
  ...
  if(NULL == (block = spr_layout->blockTarget)) return NULL;

  /* 同じブロック以下に、同一名称の状態が登録されていないかをチェックする */
  for(stat = block->d.st.begin; stat != NULL; stat = stat->next)
    /* 同じ名称があればエラー */
    if(!strcmp(stat->label, name)) return NULL;
  
  /* メモリを確保できなければエラー */
  if(NULL == (stat = MALLOC(sizeof(sprStatus)))) return NULL;
...
sprTrack * track_2d_new(char * name)
{
  ...
  for(track = spr_layout->actTarget->begin; track != NULL; track = track->next)
    if(track->target == obj)
      {
