/*
	...
	1999/07/07 K.Takabe
	$Id: fmt_kms.h,v 1.4 2002/06/06 08:05:27 usr02774 Exp $
...
enum {
	...
	DG_EVMTYPE_FORMAT_V3	= 0x02000000,	/* フォーマットVer.3.00 */
	DG_EVMTYPE_FORMAT_V4	= 0x04000000,	/* フォーマットVer.3.00 */
};
...
typedef struct _evx_pack {
	...
	unsigned int	n_mats ;		/* 使用マトリクス数			*/
	unsigned char	mat_id[8] ;		/* 使用マトリクス番号		*/
	void			*verts ;		/* 頂点データ				*//* = NULL */
	void			*norms ;		/* 法線データ				*//* = NULL */
	void			*uvs[3] ;		/* ＵＶデータ				*//* = NULL */
	void			*weight ;		/* ウェイトデータ			*//* = NULL */
	void			*rgbs ;			/* 頂点カラーデータ（現在未使用）*/
	void	 		*index;			/* インデックスバッファ情報 */
} EVX_PACK ;

/* モデルスケルトン情報 */
typedef struct _evx_skel {
	int				flag ;
	int				parent ;		/* 親スケルトン番号				*/
	float			tx,ty,tz ;		/* 親からのオフセット位置		*/
	float			rt_tx, rt_ty, rt_tz ;	/* ルートからのオフセット */
...
typedef struct _evx_def {
	...
	int				texture ;		/* 使用テクスチャグループＩＤ		*/
	int				n_packs ;		/* ポリゴンパケット数				*/
	EVX_PACK		*packet ;		/* ポリゴンパケット					*/
	void			*vbuff ;		/* 共有頂点データ */
	int				stride ;		/* 頂点サイズ */
	int				n_verts ;		/* 共有頂点数 */
	EVX_SKEL		skeleton[ NULL_ARRAY ] ;
} EVX_DEF ;
