/*
	font.c
		フォント描画ルーチン
	1999/12/20	K.Uehara
	$Id: font.c,v 1.2 2002/05/14 00:32:16 usr01475 Exp $
*/
...
#ifdef CHECK_ONLY
#define ASSERT( a )
typedef unsigned __int32   u_int; //BP
#else
#include "libgv.h"
#endif
#include "font.h"
...
static __inline int getcode( int code )
{
	return ( code & ~( TOP_KINSOKU_MASK | BACK_KINSOKU_MASK ) );
}

static __inline char *getnextchar( int *code, unsigned char *m )
{
	int a;
...
void font_set_top_addr( int type, void *addr )
{
	...
	if( type == FONT_TYPE_GCL ){
		zendata[ FONT_TYPE_GCL2 ] = ( char * )addr + FONT_ONE_SIZE * ( 1024 - 4 );
	}
}
...
static __inline unsigned char *get_han_top( int hancode )
{
	return handata + ( hancode & 0x000FFFFF );
}

static __inline int get_han_width( int hancode )
{
	return ( hancode >> 20 ) & 0x3F;
}

static __inline int get_han_offset( int hancode )
{
	return ( hancode >> 26 ) & 0x3F;
}

static __inline int get_font_width( int code )
{
	if( code < 0x8100 ){
		/* 半角 */
		int han;
...
static __inline int rubi_get_bit( char *datatop, int pos )
{
	return ( datatop[ pos / 4 ] & ( 3 << ( ( 3 - ( pos % 4 ) ) * 2 ) ) ) ? 1 : 0;
}

static __inline int rubi_get_pattern( char *datatop, int width, int x, int y )
{
	/* x, yに対応するデータを返す */
	/* xはオーバーランする可能性がある */
