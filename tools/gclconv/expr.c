...
#ifdef USE_GCL_COMMAND

static char *set_expr_command_call( NODE *node, EXPR_INFO *exinfo )
{
	char *p;
	WORDBUF word;
	int mode;
...
static int get_node( EXPR_INFO *exinfo, NODE *node )
{
	...
	if( strlen( buf ) == 1 && ( p = strchr( alone_operands_char, *buf ) ) != NULL ){
		...
		/* ユーザー関数の呼出し */
		set_expr_command_call( node, exinfo );
#endif
	} else if( *buf == '\'' ){
		/* 文字列はエラー */
		ERROR( "式の中で文字列はつかえません\n" );
	} else if( *buf == '(' ){
		char term_char;
		...
		node->value = get_symbol_code( buf + 2, 2 );

	} else {
		/* 数字が来るはず */
		if( is_expr_num( buf, &node->value ) ){
			node->type = TYPE_VALUE;
		} else {
			/* 文字列コードにする */
...
static int calc_node( EXPR_INFO *exinfo, NODE *node )
{
	...
	switch( node->type ){
	  ...
	  case TYPE_OPERAND:
		{
			int value;
			if( calc_expr( exinfo, ( OPERAND * )node->value, &value ) ){
				node->type = TYPE_VALUE;
				node->value = value;
				return 1;
			}
