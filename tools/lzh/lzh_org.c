...
//#define	USE_ASM	1				//	Assembler が使用可能な時定義する
...
#ifdef USE_ASM
static int	get_matchlen(char *,char *);
#endif
...
static void send_block(void)
{
	...
	for( i = 0 ; i < h_char->tableno ; i++ )
	{
		if( !h_char->code[i].code_bits )continue;
		printf("%d(%d):",i,h_char->code[i].code_bits );
		for( k = 0 ; k < h_char->code[i].code_bits ; k++ )
			printf("%d", h_char->code[i].code >> (h_char->code[i].code_bits - k -1) & 1 );
		printf("\n");
	}
#endif
	...
	for( i = 0 ; i < h_ptr->tableno ; i++ )
	{
		if( !h_ptr->code[i].code_bits )continue;
		printf("%d(%d):",i,h_ptr->code[i].code_bits );
		for( k = 0 ; k < h_ptr->code[i].code_bits ; k++ )
			printf("%d", h_ptr->code[i].code >> (h_ptr->code[i].code_bits - k -1) & 1 );
		printf("\n");
	}
#endif
...
static void search( uint r )
{
	...
	for( k = 0 ; k < cnt ; k++ )
	{
		...
		{
#ifdef USE_ASM
			j = get_matchlen( DICSIZ + g_text + i, g_text + r );
#else
		...
		{
#ifdef USE_ASM
			j = get_matchlen( g_text + i, g_text + r );
#else
			while( j < g_remainder && j < F )
			{
...
void decode( unsigned long count )
{
	...
	for( i = 0 ; i < h_char->tableno ; i++ )
	{
		if( !h_char->code[i].code_bits )continue;
		printf("%d(%d):",i,h_char->code[i].code_bits );
		for( c = 0 ; c < h_char->code[i].code_bits ; c++ )
			printf("%d", h_char->code[i].code >> (h_char->code[i].code_bits - c -1) & 1 );
		printf("\n");
	}
	...
	for( i = 0 ; i < h_ptr->tableno ; i++ )
	{
		if( !h_ptr->code[i].code_bits )continue;
		printf("%d(%d):",i,h_ptr->code[i].code_bits );
		for( c = 0 ; c < h_ptr->code[i].code_bits ; c++ )
			printf("%d", h_ptr->code[i].code >> (h_ptr->code[i].code_bits - c -1) & 1 );
		printf("\n");
	}
#endif

		for( l = 0 ; l < g_hufsize ; l++ )
		{
			...
			{
				j = node - D256 + THRESHOLD;

				node = huff_decode( h_ptr, p_root);
...
#ifdef USE_ASM
static int get_matchlen(char * cmp1, char * cmp2)
{
	asm	push	ds
	asm	push	es
	asm	mov		ax, F
