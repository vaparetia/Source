...
typedef struct {
	...
	int			jinfo_size ;	/* 関節キー情報のサイズ((motion_length+1)*motion_joints)*4) */
	int			sound_size ;	/* サウンド情報のサイズ（将来の拡張用） */
	int			anime_size ;	/* アニメーションのサイズ（将来の拡張用） */
	int			pad[3] ;
	int			error_length ;	/* 圧縮時のエラー許容距離（デフォルト25） */
	int			error_angle ;	/* 圧縮時のエラー許容角度（ＰＳ固定小数点角度） */
} MTN_FILE_HEADER ;


typedef struct {
	unsigned int	key:1 ;		/* 移動量及び関節回転用キーフラグ */
	unsigned int	fix:1 ;		/* 移動軌跡キー及び関節固定フラグ（固定フラグは同一フレーム内で１つのみ） */
	unsigned int	tkey:1 ;	/* 関節におけるトランスレーション用キーフラグ */
	unsigned int	pad:29 ;
} KEY_INFO ;
...
enum {
	...
	MTN_FLAG_TRANS		= 0x0010,		/* 関節移動量圧縮有効フラグ */
	/* バージョン管理 */
	MTN_FLAG_VER_MASK	= 0xff00,		/* バージョンマスク */
	MTN_FLAG_VER_000	= 0x0000,		/* 旧バージョンフォーマット */
	MTN_FLAG_VER_001	= 0x0100,		/* 新バージョンフォーマット */
	/* 特殊属性 */
...
typedef struct {
	...
	long	root_offset ;		/* 軌跡トレース情報のオフセット */
...
#define MT3_FLAG_MOVE2_PART	(0x00000020)	/* 新方式圧縮型 */
