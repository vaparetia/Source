/*
	$Id: loadtpk.c,v 1.7 2002/08/06 09:46:37 usr02774 Exp $
 */
...
int				MaxClutEntry = 0 ;
unsigned int	ClutEntry[ 256 * 4096 ];

/* ---------------------------------------------------------------- */
static int GetDataSize( int width, int height, int format, int mip_level )
{
	int		size, s_tmp, i ;
	switch ( format ){
...
/* ---------------------------------------------------------------- */
/* 共通CLUTデータエリアに新規ＣＬＵＴを追加し、そのインデックス番号を取得する */
int AddClutData( unsigned int *clut, int size )
{
	int		entry ;
	entry = MaxClutEntry ;
	memcpy( &ClutEntry[ MaxClutEntry * 256 ], clut, 4 * 256 );
	MaxClutEntry++ ;
	return ( entry );
}
/* ---------------------------------------------------------------- */
static int LoadTpkData( FILE *fp, int code, int size )
{
	...
	tpk_info->image_size = image_size = GetDataSize( tpk_info->width, tpk_info->height, tmp_header->format, tpk_info->mip_level );
	/* イメージデータ領域の確保 */
	tpk_info->image_data = malloc( image_size );
	memset( tpk_info->image_data, 0, image_size );
	/* パレット領域の確保 */
	switch ( tmp_header->format ){
	case TEXPACK_FORMAT_PAL8:
		clut_size = 256 ;
	...
	/* パレットデータのコピー */
	if ( tpk_info->clut_data != NULL ){
		void		*org_clut_data ;
		org_clut_data = (void*)( (char*)tmp_header->clut_addr + (int)tmp_header );
		memcpy( tpk_info->clut_data, org_clut_data, tpk_info->clut_size );
	}
	...
	{/* ヘッダー及び各テクスチャ情報をコピーする */
		...
		for ( i = tpk_info->n_textures ; i > 0 ; i--, src_tex++, texture++ ){
			...
			if ( clut_size != 0 ){
				texture->clut_num = 
					AddClutData( (void*)( (char*)src_tex->clut_addr + (int)tmp_header ), 1 ) ;
			}
		}
	}
