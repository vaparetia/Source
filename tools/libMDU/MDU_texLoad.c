/*
   ...
   $Id: MDU_texLoad.c,v 1.19 2002/08/26 11:22:09 usr01363 Exp $
...
FILE		*fp ;
{
	...
	if ( header->info.biWidth % 2 ) {
		printf( "wrong width %ld, Must be even!!!!\n", header->info.biWidth ) ;
		return -1 ;
	}
    colors = Colors ;
...
//printf( "%x\n", sizeof( BITMAPFILEHEADER ) + sizeof( BITMAPINFOHEADER ) - 2 ) ;
    fread( colors, sizeof( RGBQUAD2 ), 16, fp ) ;
#if 0
	{
		int		i ;
...
FILE		*fp ;
{
    ...
    fseek( fp, header->header.bfOffBits - sizeof( RGBQUAD2 ) * 256, SEEK_SET ) ;
    fread( colors, sizeof( RGBQUAD2 ), 256, fp ) ;
    N_Colors = 256 ;
	size = header->info.biSizeImage ;
	if ( size == 0 ) {
