/*
   ...
   $Id: MDU_mdlDivideKm4.c,v 1.11 2000/03/28 01:09:38 usr01363 Exp $
...
/*----------------------------------------------------------
   ...
   モデル分割関数 
   KM4_DEF2		*MDU_DivideKm4( orgdef2, div )
   KM4_DEF2		*orgdef2 ;
   SVECTOR		*div ; 		 分割数 

   属性による分割 
   KM4_DEF2		*MDU_SplitKm4( orgdef2 )
   KM4_DEF2		*orgdef2 ;
...
int		n ; /* src内 n番目のポリゴン */
{
    ...
    //printf("AddPolygon Start : %d\n",dst->n_prims);

    /* 領域の再確保 */
    if ( dst->n_prims == 0 ) {
#ifdef MEM_SIZE
	verts = dst->verts = ( FVECTOR * )malloc( MEM_SIZE ) ;
	envs = dst->envs = ( FVECTOR * )malloc( MEM_SIZE ) ;
	norms = dst->norms = ( FVECTOR * )malloc( MEM_SIZE ) ;
	uvs = dst->uvs = ( TVECTOR * )malloc( MEM_SIZE ) ;
	tid = dst->tid = ( u_int * )malloc( MEM_SIZE ) ;
	vid = dst->vid = ( u_short * )malloc( MEM_SIZE ) ;
	nid = dst->nid = ( u_short * )malloc( MEM_SIZE ) ;

	vert_usrdata = dst->vert_usrdata = ( VERTEX_USERDATA * )malloc( MEM_SIZE ) ;
	norm_usrdata = dst->norm_usrdata = ( NORMAL_USERDATA * )malloc( MEM_SIZE ) ;
	prim_usrdata = dst->prim_usrdata = ( PRIMITIVE_USERDATA * )malloc( MEM_SIZE ) ;
#else
	verts = dst->verts = ( FVECTOR * )malloc( sizeof( FVECTOR ) * 4 ) ;
	envs = dst->envs = ( FVECTOR * )malloc( sizeof( FVECTOR ) * 4 ) ;
	norms = dst->norms = ( FVECTOR * )malloc( sizeof( FVECTOR ) * 4 ) ;
	uvs = dst->uvs = ( TVECTOR * )malloc( sizeof( TVECTOR ) * 4 ) ;
	tid = dst->tid = ( u_int * )malloc( sizeof( u_int ) ) ;
	vid = dst->vid = ( u_short * )malloc( sizeof( u_short ) * 4 ) ;
	nid = dst->nid = ( u_short * )malloc( sizeof( u_short ) * 4 ) ;

	vert_usrdata = dst->vert_usrdata
	    = ( VERTEX_USERDATA * )malloc( sizeof( VERTEX_USERDATA ) * 4 ) ;
	norm_usrdata = dst->norm_usrdata
	    = ( NORMAL_USERDATA * )malloc( sizeof( NORMAL_USERDATA ) * 4 ) ;
	prim_usrdata = dst->prim_usrdata
	    = ( PRIMITIVE_USERDATA * )malloc( sizeof( PRIMITIVE_USERDATA ) ) ;
#endif
//	vid[ 0 ] = nid[ 0 ] = 0 ;
//	vid[ 1 ] = nid[ 1 ] = 1 ;
//	vid[ 2 ] = nid[ 2 ] = 2 ;
//	vid[ 3 ] = nid[ 3 ] = 3 ;
...
#ifndef MEM_SIZE
	verts = ( FVECTOR * )malloc( ( dst->n_prims + 1 ) * sizeof( FVECTOR ) * 4 ) ;
	envs = ( FVECTOR * )malloc( ( dst->n_prims + 1 ) * sizeof( FVECTOR ) * 4 ) ;
	norms = ( FVECTOR * )malloc( ( dst->n_prims + 1 ) * sizeof( FVECTOR ) * 4 ) ;
	uvs = ( TVECTOR * )malloc( ( dst->n_prims + 1 ) * sizeof( TVECTOR ) * 4 ) ;
	tid = ( u_int * )malloc( ( dst->n_prims + 1 ) * sizeof( u_int ) ) ;
	vid = ( u_short * )malloc( ( dst->n_prims + 1 ) * sizeof( u_short ) * 4 ) ;
	nid = ( u_short * )malloc( ( dst->n_prims + 1 ) * sizeof( u_short ) * 4 ) ;

	vert_usrdata
	    = ( VERTEX_USERDATA * )malloc( ( dst->n_prims + 1 ) * sizeof( VERTEX_USERDATA ) * 4 ) ;
	norm_usrdata
	    = ( NORMAL_USERDATA * )malloc( ( dst->n_prims + 1 ) * sizeof( NORMAL_USERDATA ) * 4 ) ;
	prim_usrdata
	    = ( PRIMITIVE_USERDATA * )malloc( ( dst->n_prims + 1 )
	...
	memcpy( vert_usrdata, dst->vert_usrdata, sizeof( VERTEX_USERDATA ) * 4 * dst->n_prims ) ;
	memcpy( norm_usrdata, dst->norm_usrdata, sizeof( NORMAL_USERDATA ) * 4 * dst->n_prims ) ;
	memcpy( prim_usrdata, dst->prim_usrdata, sizeof( PRIMITIVE_USERDATA ) * dst->n_prims ) ;
	...
	free( dst->vert_usrdata ) ;
	free( dst->norm_usrdata ) ;
	free( dst->prim_usrdata ) ;
	...
	dst->vert_usrdata = vert_usrdata ;
	dst->norm_usrdata = norm_usrdata ;
	dst->prim_usrdata = prim_usrdata ;
#else
	size = ( dst->n_prims + 1 ) * sizeof( FVECTOR ) * 4 ;
	if ( mallocblksize( dst->verts ) < size ) {
	    size = mallocblksize( dst->verts ) + MEM_SIZE ;
	    verts = ( FVECTOR * )malloc( size ) ;
	    memcpy( verts, dst->verts, sizeof( FVECTOR ) * 4 * dst->n_prims ) ;
	    free( dst->verts ) ;
	    dst->verts = verts ;
	}
	size = ( dst->n_prims + 1 ) * sizeof( FVECTOR ) * 4 ;
	if ( mallocblksize( dst->envs ) < size ) {
	    size = mallocblksize( dst->envs ) + MEM_SIZE ;
	    envs = ( FVECTOR * )malloc( size ) ; 
	    memcpy( envs, dst->envs, sizeof( FVECTOR ) * 4 * dst->n_prims ) ;
	    free( dst->envs ) ;
	    dst->envs = envs ;
	}
	size = ( dst->n_prims + 1 ) * sizeof( FVECTOR ) * 4 ;
	if ( mallocblksize( dst->norms ) < size ) {
	    size = mallocblksize( dst->norms ) + MEM_SIZE ;
	    norms = ( FVECTOR * )malloc( size ) ; 
	    memcpy( norms, dst->norms, sizeof( FVECTOR ) * 4 * dst->n_prims ) ;
	    free( dst->norms ) ;
	    dst->norms = norms ;
	}
	size = ( dst->n_prims + 1 ) * sizeof( TVECTOR ) * 4 ;
	if ( mallocblksize( dst->uvs ) < size ) {
	    size = mallocblksize( dst->uvs ) + MEM_SIZE ;
	    uvs = ( TVECTOR * )malloc( size ) ; 
	    memcpy( uvs, dst->uvs, sizeof( TVECTOR ) * 4 * dst->n_prims ) ;
	    free( dst->uvs ) ;
	    dst->uvs = uvs ;
	}
	size = ( dst->n_prims + 1 ) * sizeof( u_int ) ;
	if ( mallocblksize( dst->tid ) < size ) {
	    size = mallocblksize( dst->tid ) + MEM_SIZE ;
	    tid = ( u_int * )malloc( size ) ; 
	    memcpy( tid, dst->tid, sizeof( u_int ) * dst->n_prims ) ;
	    free( dst->tid ) ;
	    dst->tid = tid ;
	}
	size = ( dst->n_prims + 1 ) * sizeof( u_short ) * 4 ;
	if ( mallocblksize( dst->vid ) < size ) {
	    size = mallocblksize( dst->vid ) + MEM_SIZE ;
	    vid = ( u_short * )malloc( size ) ; 
	    memcpy( vid, dst->vid, sizeof( u_short ) * 4 * dst->n_prims ) ;	
	    free( dst->vid ) ;
	    dst->vid = vid ;
	}
	size = ( dst->n_prims + 1 ) * sizeof( u_short ) * 4 ;
	if ( mallocblksize( dst->nid ) < size ) {
	    size = mallocblksize( dst->nid ) + MEM_SIZE ;
	    nid = ( u_short * )malloc( size ) ; 
	    memcpy( nid, dst->nid, sizeof( u_short ) * 4 * dst->n_prims ) ;	
	    free( dst->nid ) ;
	    dst->nid = nid ;
	}
	size = ( dst->n_prims + 1 ) * sizeof( VERTEX_USERDATA ) * 4 ;
	if ( mallocblksize( dst->vert_usrdata ) < size ) {
	    size = mallocblksize( dst->vert_usrdata ) + MEM_SIZE ;
	    vert_usrdata = ( VERTEX_USERDATA * )malloc( size ) ; 
	    memcpy( vert_usrdata, dst->vert_usrdata,
		    sizeof( VERTEX_USERDATA ) * 4 * dst->n_prims ) ;
	    free( dst->vert_usrdata ) ;
	    dst->vert_usrdata = vert_usrdata ;
	}
	size = ( dst->n_prims + 1 ) * sizeof( NORMAL_USERDATA ) * 4 ;
	if ( mallocblksize( dst->norm_usrdata ) < size ) {
	    size = mallocblksize( dst->norm_usrdata ) + MEM_SIZE ;
	    norm_usrdata = ( NORMAL_USERDATA * )malloc( size ) ; 
	    memcpy( norm_usrdata, dst->norm_usrdata,
		    sizeof( NORMAL_USERDATA ) * 4 * dst->n_prims ) ;
	    free( dst->norm_usrdata ) ;
	    dst->norm_usrdata = norm_usrdata ;
	}
	size = ( dst->n_prims + 1 ) * sizeof( PRIMITIVE_USERDATA ) ;
	if ( mallocblksize( dst->prim_usrdata ) < size ) {
	    size = mallocblksize( dst->prim_usrdata ) + MEM_SIZE ;
	    prim_usrdata = ( PRIMITIVE_USERDATA * )malloc( size ) ; 
	    memcpy( prim_usrdata, dst->prim_usrdata,
		    sizeof( PRIMITIVE_USERDATA ) * dst->n_prims ) ;
	    free( dst->prim_usrdata ) ;
	    dst->prim_usrdata = prim_usrdata ;
	}
#endif
	...
	vert_usrdata = dst->vert_usrdata + dst->n_prims * 4 ;
	norm_usrdata = dst->norm_usrdata + dst->n_prims * 4 ;
	prim_usrdata = dst->prim_usrdata + dst->n_prims ;
    }
    ...
    *vert_usrdata = *( src->vert_usrdata + src->vid[ n * 4 + 0 ] ) ; vert_usrdata ++ ;
    *vert_usrdata = *( src->vert_usrdata + src->vid[ n * 4 + 1 ] ) ; vert_usrdata ++ ;
    *vert_usrdata = *( src->vert_usrdata + src->vid[ n * 4 + 2 ] ) ; vert_usrdata ++ ;
    *vert_usrdata = *( src->vert_usrdata + src->vid[ n * 4 + 3 ] ) ;
    *norm_usrdata = *( src->norm_usrdata + src->nid[ n * 4 + 0 ] ) ; norm_usrdata ++ ;
    *norm_usrdata = *( src->norm_usrdata + src->nid[ n * 4 + 1 ] ) ; norm_usrdata ++ ;
    *norm_usrdata = *( src->norm_usrdata + src->nid[ n * 4 + 2 ] ) ; norm_usrdata ++ ;
    *norm_usrdata = *( src->norm_usrdata + src->nid[ n * 4 + 3 ] ) ;
    *prim_usrdata = *( src->prim_usrdata + n ) ;

    dst->n_verts += 4 ;
    dst->n_norms += 4 ;
    dst->n_prims ++ ;

    //printf("AddPolygon End\n");
}
...
#endif
    /* 分配用モデル領域の確保 */
    Model = ( KM4_MDL * )malloc( sizeof( KM4_MDL ) * N_Types *  Div->vx * 
				  Div->vy * Div->vz ) ;
...
//		    Model[ n ].tx = def2->def->models[ 0 ].tx + 1000.0F ;
#endif
		    n ++ ;
		}
	    }
...
SVECTOR		*div ; /* 分割数 */
{
    ...
    Div = div ; Model = NULL ;
    /* 一度コピー */
    memcpy( def2, orgdef2, sizeof( KM4_DEF2 ) ) ;
    /* 整数化しておく */
//    MDU_ModelVertsInt( def2->def ) ;
    ...
    def = ( KM4_DEF * )malloc( sizeof( KM4_DEF ) + sizeof( KM4_MDL ) * 
			       N_Models ) ;
    ...
    while( N_Models > 0 ) {
	if ( mdl->n_prims > 0 ) {
	    memcpy( &def->models[ i ], mdl, sizeof( KM4_MDL ) ) ;
	    i ++ ;
	    N_Models -- ;
	}
	mdl ++ ;
    ...
    /* 頂点＆法線共有化 */
    MDU_Km4JointModel( def ) ;
    /* 今までのdefをフリー */
    MDU_mdlFreeKm4Def2( orgdef2 ) ;
    /* 新しいdefを接続 */
    def2->def = def ;
...
KM4_DEF2	*def2 ;
{
    ...
    /* 領域確保 */
    N_Models += n_extends ;
    Model = ( KM4_MDL * )malloc( sizeof( KM4_MDL ) * N_Models ) ;
    memset( Model, 0x00, sizeof( KM4_MDL ) * N_Models ) ;
