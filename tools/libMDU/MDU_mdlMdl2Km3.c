...
int	MDU_M2K_N_ObjFlags ;
char	MDU_M2K_ObjNames[ 256 ][ 256 ] ;
int	MDU_M2K_ObjFlags[ 256 ] ;
...
/* テクスチャ名格納用 */
char	Textures[ MAX_TEXTURES ][ MAX_NAME_LEN ] ;
float	TexAlphaAverage[ MAX_TEXTURES ];
int	N_Textures ;
...
printf( "rot %f %f %f\n", P3DObject( hP3DObjectL )->p3dXYZRotate.x,
...
#else
	{
	    ...
	    RotMatZYX( &v1, &m1 ) ;
	    v2.v[ 0 ] = P3DObject( hP3DObjectL )->p3dXYZMove.x ;
	    v2.v[ 1 ] = P3DObject( hP3DObjectL )->p3dXYZMove.y ;
	    v2.v[ 2 ] = P3DObject( hP3DObjectL )->p3dXYZMove.z ;
	    MulMatVec( &m1, &v2, &v2 ) ;
	    mdl->tx = ( float )( v2.v[ 0 ] * lpp3dXYZScale->x ) ;
	    mdl->ty = ( float )( v2.v[ 1 ] * lpp3dXYZScale->y ) ;
	    mdl->tz = ( float )( v2.v[ 2 ] * lpp3dXYZScale->z ) ;
	}
#endif
	...
	for( i = 0, lpP3DVertex = P3DObject( hP3DObjectL )->lpP3DVertex;
	     i < mdl->n_verts; i ++, lpP3DVertex++, verts ++, envs ++ ) {
#if 0
...
HP3DOBJECT hP3DObject ;	/* 親になるオブジェクトハンドル(NULL でトップレベル) */
{
	...
	return;
    do {
	mdl = ( KM3_MDL * )P3DObject( hP3DObjectL )->lParam ;
	if( hP3DObject != NULL )
...
KM3_MDL	*mdl ;
{
    ...
    center.vx = ( float )floor( ( double )( ( mdl->lx + mdl->ux ) / 2.0F ) + mdl->tx ) ;
    center.vy = ( float )floor( ( double )( ( mdl->ly + mdl->uy ) / 2.0F ) + mdl->ty ) ;
    center.vz = ( float )floor( ( double )( ( mdl->lz + mdl->uz ) / 2.0F ) + mdl->tz ) ;
    if ( mdl->n_prims > 0 ) {
	verts = mdl->verts ;
	verts->vx += mdl->tx ;
	verts->vy += mdl->ty ;
...
/* 拡張モデルの生成 */
static KM3_DEF	*CreateExtendModel( def, mdl, hP3DModel )
KM3_DEF		*def ;
KM3_MDL		*mdl ;
HP3DMODEL       hP3DModel ;
{
	...
	for( j = 0; j < MAX_PIXEL; j ++ ) {
	    ...
	    for( k = 0, verts2 = mdl->verts, envs2 = mdl->envs ;
		 ( k < n_verts2 ) && 
		( verts->vx != verts2->vx ||
		  verts->vy != verts2->vy ||
		  verts->vy != verts2->vy ) ;
...
gotoEND :
    ...
    def = ( KM3_DEF * )reallocEx( ( void * )def, sizeof( KM3_DEF ) + 
				  sizeof( KM3_MDL ) * ( def->n_x_models + 1 ) ) ;
    if ( def == NULL ) {
	CreateExtendUnitExit( 0, &model ) ;
	return NULL ;
    }
    ...
    memcpy( ext, &model, sizeof( KM3_MDL ) ) ;
    ext->n_prims = 0 ;
    ext->n_verts = 0 ;
    ext->verts = NULL ;
    ext->vid = NULL ;
	...
	for( j = 0; j < MAX_PIXEL; j ++ ) {
	    ...
	    verts = model.verts + vid[ j ] ;
	    envs = model.envs + vid[ j ] ;
	    for ( k = 0, verts2 = ext->verts, envs2 = ext->envs ;
		  ( k < ext->n_verts ) &&
		  ( ( verts->vx != verts2->vx ) ||
		    ( verts->vy != verts2->vy ) ||
		    ( verts->vz != verts2->vz ) ) ;
...
/* ユニットの分割 */
static KM3_DEF	*SplitModel( def, hP3DModel, hP3DObject )
KM3_DEF		*def ;
HP3DMODEL       hP3DModel ;	/* モデルハンドル */
HP3DOBJECT      hP3DObject ;	/* 親になるオブジェクトハンドル(NULL でトップレベル) */
{
...
/* 変換 */
static	KM3_DEF	*Convert( hP3DModel, lpp3dXYZScale )
HP3DMODEL	hP3DModel ;
LPP3DXYZ	lpp3dXYZScale ;
{
    KM3_DEF	*def ;
    ...
    def = ( KM3_DEF * )MDU_Alloc( sizeof( KM3_DEF ) + sizeof( KM3_MDL ) * nNo ) ;
    if ( def == NULL ) return NULL ;

    /* ０クリア */
    memset( def, 0x00, sizeof( KM3_DEF ) + sizeof( KM3_MDL ) * nNo ) ;
	...
	def->lx = ( ( KM3_MDL * )( P3DObject( hP3DObject )->lParam ) )->lx ;
	def->ly = ( ( KM3_MDL * )( P3DObject( hP3DObject )->lParam ) )->ly ;
	def->lz = ( ( KM3_MDL * )( P3DObject( hP3DObject )->lParam ) )->lz ;
	def->ux = ( ( KM3_MDL * )( P3DObject( hP3DObject )->lParam ) )->ux ;
	def->uy = ( ( KM3_MDL * )( P3DObject( hP3DObject )->lParam ) )->uy ;
	def->uz = ( ( KM3_MDL * )( P3DObject( hP3DObject )->lParam ) )->uz ;
    }
    if ( SkeltonModel == 0 ) {
...
KM3_DEF2	*MDU_Mdl2Km3( name, scale, hP3DModel, skel )
LPCSTR		name ;
LPP3DXYZ	scale ;
HP3DMODEL	hP3DModel ;
int		skel ;
{
    ...
    D( printf( ".mdl -> .km3 conversion start\n" ) ; ) ;
    def = NULL ; def2 = NULL ;
...
no_skelton_model :
    ...
    if ( SkeltonModel ) MDU_ModelVertsInt( def ) ;

    return def2 ;
}

/* MDU_texExtMdl.c からテクスチャ名を参照 */
int	MDU_M2K_FindTexture( name )
char		*name ;
{
    int		i ;
    char	root[ 256 ], *ext ;
    ...
    ext[ 0 ] = '\0' ;
    for ( i = 0; i < N_Textures; i ++ ) {
	if ( !strcmp( Textures[ i ], root ) ) return i ;
    }
    return -1 ;
}
