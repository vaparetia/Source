/*
   ...
   $Id: arkms.c,v 1.1 1999/12/27 01:15:21 usr01363 Exp $
...
static int read_option(KMS_DEF **def)
{
    KMS_DEF *tdef;
    int flag=0;
    union {
	...
	if(objnum0>objnum1 && tdef->n_x_models<=objnum1){
	    printf("Error : KMS does not have this(these) object(s).\n");
	    continue;
	}
	...
	case 0:
	    if(*def==NULL){
		if((*def=malloc(sizeof(KMS_DEF)+sizeof(KMS_OBJ)))==NULL){
		...
		KMS_DEF *sdef;
		if((sdef=realloc(*def,sizeof(KMS_DEF)
				 +sizeof(KMS_OBJ)*((*def)->n_models+1)))==NULL){
		...
		*def=sdef;
	    ...
	    memcpy(&((*def)->objs[(*def)->n_models]),&(tdef->objs[objnum0]),sizeof(KMS_OBJ));
	    (*def)->n_models++;
	    (*def)->n_x_models++;
	    break;
	case 1:
	...
	case 4:
		...
		if(*def==NULL){
		    if((*def=malloc(sizeof(KMS_DEF)+sizeof(KMS_OBJ)*sz))==NULL){
			printf("Memory Overflow\n");
			return 0;
		    }
		    (*def)->n_models=(*def)->n_x_models=0;
		    ...
		    KMS_DEF *sdef;
		    if((sdef=realloc(*def,sizeof(KMS_DEF)
				     +sizeof(KMS_OBJ)*((*def)->n_models+sz)))==NULL){
		...
		memcpy(&((*def)->objs[(*def)->n_models]),&(tdef->objs[objnum0]),sizeof(KMS_OBJ)*sz);
		(*def)->n_models+=sz;
		(*def)->n_x_models+=sz;
	    }
	    break;
...
{
    printf( "arkms : Get objects you need from KMSs for linux\n" ) ;
    printf( "\t Usage : arkms outputfile(*.kms) [inputfile0(*.kms) -n objnum0 -n objnum1 ...] [inputfile1(*.kms) -n objnum0 -n objnum1 ...] ... \n" ) ;
