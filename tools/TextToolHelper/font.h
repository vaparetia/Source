/*
	font.h
		フォント描画ルーチンヘッダ
	2000/01/05	K.Uehara
	$Id: font.h,v 1.7 2005/11/09 10:37:28 usr03692 Exp $
*/
...
#ifdef ONLINE
	unsigned char d_width;
	unsigned char d_height;
#endif	
};
...
struct _drawinfo {
	...
	char *(*callback)( FONT_DRAWINFO *dr, char *now, int no );
	short now_x;
	short now_y;
	int yc;
...
#define FONT_TYPE_RESIDENT_2	0x05
#define FONT_TYPE_RESIDENT_3	0x06	   
#define MAX_FONT_TYPE		0x07
...
EXTERN_INLINE int font_get_draw_width( FONT_DRAWINFO *draw )
{
	return draw->outinfo->max_width;
}

EXTERN_INLINE void font_set_draw_callback( FONT_DRAWINFO *draw
										  , char *( *callback )( FONT_DRAWINFO *dr, char *now
																, int no ) )
{
