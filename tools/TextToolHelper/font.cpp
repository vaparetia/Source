//----------------------------------------------------------------------------
//BP - added stdafx.h to get precompiled common.h in Windows
#include "stdafx.h"
//BP
//----------------------------------------------------------------------------
...
{
	rubi_data = (unsigned char*)rubitop;
}

void font_set_top_addr( int type, void *addr )
{
	zendata[ type ] = (unsigned char*)addr;
	if( type == FONT_TYPE_GCL ){
		zendata[ FONT_TYPE_GCL2 ] = ( unsigned char * )addr + FONT_ONE_SIZE * ( 1024 - 4 );
	}
}
	  ...
	  default:		// 漢字 : コードによってフォントのメモリ位置が違う
		{
			...
			pos = pos - ( pos / 256 );
			top = (char*)zendata[ type ];
		}
	}
	return (unsigned char*)top + pos * FONT_ONE_SIZE;
}
...
void font_resident_load_set( char *fonttop )
{
	...
	table = ( unsigned int * )fonttop;
	hantable = ( unsigned int * )( fonttop + sizeof( int ) * 2 );
	handata = ( unsigned char * )( fonttop + ( /*BP_LE_SwapSInt*/(table[ 0 ]) & 0x03FFFFFF ) );
	zendata0 = ( unsigned char * )( fonttop + /*BP_LE_SwapSInt*/(table[ 1 ]) );
	zendata[ FONT_TYPE_RESIDENT ] = zendata0 + FONT_ZEN_TOP * FONT_ONE_SIZE;
	zendata[ FONT_TYPE_RESIDENT_2 ] = zendata[ FONT_TYPE_RESIDENT ] + (0x400-4) * FONT_ONE_SIZE;
	zendata[ FONT_TYPE_RESIDENT_3 ] = zendata[ FONT_TYPE_RESIDENT_2 ] + (0x400-4) * FONT_ONE_SIZE;
	font_han_max_offset = ( /*BP_LE_SwapSInt*/(table[ 0 ]) >> 26 );
...
static unsigned int get_hantable( int code )
{
	...
	return /*BP_LE_SwapUInt*/( hantable[ code ] );
}
...
static int put_zenkaku_4bpp( FONT_VRAMINFO *out, int x, int y, int code, int flag )
{
   ...
   vram = (unsigned char*)out->vramtop;
	row = out->row;
...
static int put_hankaku_4bpp( FONT_VRAMINFO *out, int xx, int yy, int code, int flag )
{
...
//			ytop = ( out->rubi_display_flag ) ? ( y - 8 ) : ( y - 4 );
		...
		if( code == 0 ){
			...
			if( code == CODE_BLACK ){
				int wsum = 0;
				wsum += put_hankaku_4bpp( out, xx, yy, 'b', flag );
				xx += wsum;
				wsum += put_hankaku_4bpp( out, xx, yy, 'l', flag );
				return wsum;
			}
#endif
			return 0;
...
// 描画せずに文字幅チェックを行うために使用
int put_zenkaku_4bpp_no_draw( FONT_VRAMINFO *out, int x, int y, int code, int flag )
{
	return FONT_SIZE_W;
}

int put_hankaku_4bpp_no_draw( FONT_VRAMINFO *out, int xx, int yy, int code, int flag )
{
	unsigned int han;
	int width;
...
#ifdef ONLINE
static int put_zenkaku_4bpp_scalable( FONT_VRAMINFO *out, int x, int y, int code, int flag )
{
#ifndef CHECK_ONLY
	int i,j;
	int row;
...
//		ytop = ( out->rubi_display_flag ) ? ( y - 6 ) : ( y - 2 );
	...
	int put_w = out->d_width;
	int put_h = out->d_height;
	...
	for( i = 0; i < put_h; ++i ) {
		for( j = 0; j < put_w; ++j ) {
			...
			u_int sum = 0;
//			u_int area = 0;
			for( ii = i * FONT_SIZE_H;
				 ii < (i + 1)* FONT_SIZE_H;
				 ii = nextii ) {
				nextii = (ii + put_h) / put_h * put_h;
				if( nextii > (i + 1)* FONT_SIZE_H ) nextii = ( i + 1) * FONT_SIZE_H;
				for( jj = j * FONT_SIZE_W;
					 jj < (j + 1)* FONT_SIZE_W;
					 jj = nextjj ) {
					nextjj = (jj + put_w) / put_w * put_w;
					if( nextjj > (j + 1)* FONT_SIZE_W ) nextjj = (j + 1) * FONT_SIZE_W;
//					area += ( nextjj - jj ) * ( nextii - ii );

					sum += ( nextjj - jj ) * ( nextii - ii ) *
							((p[ (jj / put_w  + ii / put_h * FONT_SIZE_W)/4 ] >> ((3 - (jj / put_w & 3)) * 2)) & 3);
					
				}
			}
//			printf("area %d sum %d\n", area, sum );
//			printf("data %d \n", sum / ( FONT_SIZE_W * FONT_SIZE_H * 3 / 4 + 1) );
			u_char data = vram[ i * row + (x + j) / 2 ];
			u_char col = (sum * 5 + (( FONT_SIZE_W * FONT_SIZE_H ) - 1)) /( FONT_SIZE_W * FONT_SIZE_H);
			if( (x + j) & 1 ) {
				data &= 0xf;
				data |= col << 4;
			} else {
...
static int put_hankaku_4bpp_scalable( FONT_VRAMINFO *out, int xx, int yy, int code, int flag )
{
	unsigned char *vtop, *vram, *v;
	unsigned char *p, d;
	unsigned char color;
...
//			ytop = ( out->rubi_display_flag ) ? ( y - 8 ) : ( y - 4 );
		...
		int put_w = out->d_width;
		int put_h = out->d_height;
		int draw_w = (out->d_width * width + FONT_SIZE_W - 1) / FONT_SIZE_W;
			...
			for( j = 0; j < draw_w; ++j ) {
				...
				for( ii = i * FONT_SIZE_H;
					...
					for( jj = j * FONT_SIZE_W;
						...
						if( jj / put_w < width ) {
							indx = jj / put_w + ii / put_h * width;
							sum += ( nextjj - jj ) * ( nextii - ii ) *
									((p[ indx/4 ] >> ((3 - (indx & 3)) * 2)) & 3);
						}
					}
				}
//				printf("area %d sum %d\n", area, sum );
...
static int put_rubi( FONT_VRAMINFO *out, int x, int y, int code )
{
	...
	*/
	...
	for( i = 0; i < height; i += 2 ){
		...
		for( j = 0; j < width; j += 2 ){
			unsigned char data;
			data = rubi_get_pattern( (char*)p, width, j, i );
			*v |= ( color | data ) << ( 4 - cp );
			if( cp == 0 ){
				v ++;
				cp = 4;
...
static int get_rubi_width( int *count, unsigned char *string )
{
	...
	for( ;; ){
		int code;
		m = (unsigned char*)getnextchar( &code, m );
		code = getcode( code );
...
//	int w, h;
	...
	if( !( vinfo->flag & FONT_NO_KINSOKU ) ){
		/* 禁則あり */
		vinfo->width = width - ( FONT_SIZE_W  + c_skip );
	}
#endif
...
#ifdef ONLINE
// ONLINE のＬＡでのみ使用する
void font_set_vraminfo_scalable( FONT_VRAMINFO *vinfo
								 , void *vramtop, int width, int height
								 , int c_skip, int l_skip, int flag,
...
//	int w, h;
	...
	vinfo->color = 0;
	vinfo->max_width = 0;
	vinfo->y_step = d_height + l_skip;

	vinfo->han_func = put_hankaku_4bpp_scalable;
	vinfo->zen_func = put_zenkaku_4bpp_scalable;
	vinfo->rubi_func = draw_rubi_string;

	vinfo->d_width = d_width;
	vinfo->d_height = d_height;
}
#endif
...
int font_draw_string( FONT_DRAWINFO *work, char *string )
{
	...
	for( ;; ){
		...
		if( mdata < CODE( 0x20 ) ){
			/* コントロールコード */
         m = (unsigned char*)next;
		} else if(( code == CODE( '\\' ) ) &&
			(( outinfo->flag & FONT_NO_META_CHARA ) == 0 )){
			// めた。
			m = (unsigned char*)next;
			next = getnextchar( &mdata, m );
			code = getcode( mdata );
			...
			if( code == CODE( '#' ) ){
				int d;
				/* # の後は制御文字 */
				m = (unsigned char*)getnextchar( &d, (unsigned char*)next );
				d = getcode( d );
				switch( d ){
				  case CODE( 'R' ):
					m = (unsigned char*)getnextchar( &d, m );
				  ...
				  case CODE( 'C' ):
					...
					if( work->callback != NULL ){
						...
						work->now_x = x;
						work->now_y = y;
						res = ( *work->callback )( work, (char*)m, d );
						if( res != NULL ){
							m = (unsigned char*)res;
						}
						// 状況の復帰
...
//					mdata = code = code_convert( mdata );
				...
				if( IS_BLOCK_END_CODE( code ) ){
					if( work->rubi_flag && code != CODE( '}' ) ){
						// 、。, で区切られたうしろ
						draw_rubi( work, x, y, (unsigned char*)next );
					}
		...
		/* ここから通常文字描画 */
		work->prev_char = (char*)m;
		m = (unsigned char*)getnextchar( &mdata, m );
		code = getcode( mdata );
DRAW:
		/* 描画処理 */
		...
		/* 禁則処理とスキップ処理 */
		{
			...
			if(( code == CODE( ' ' )) && ( outinfo->flag & FONT_WORD_WRAP )) {
				...
				do {
					...
					nn2 = getnextchar( &next_mdata2, (unsigned char*)nn2 );
					next_code2 = getcode( next_mdata2 );
					next_width2 = get_font_width( next_code2 );
			...
			if( next_width > 0
				...
				if( ! ( outinfo->flag & FONT_NO_KINSOKU ) ){
					...
					if( mdata & BACK_KINSOKU_MASK ){
						...
						m = (unsigned char*)work->prev_char;
						dx = 0;
						...
						if( work->block_flag && IS_BLOCK_END_CODE( next_code ) ){
							...
							/* 先頭禁則 */
							int nn_mdata;
							getnextchar( &nn_mdata, (unsigned char*)nn );
				...
				if( outinfo->flag & FONT_NO_NEW_LINE_SPACE ) {
					int tmp;
					if( next_code == CODE( ' ' )) {
						m = (unsigned char*)getnextchar( &tmp, m );
					}
				}
DO_CR:
				// 改行処理
				...
				y = y + outinfo->y_step;
				work->yc ++;
				work->cr_flag = TRUE;		// 実はバグ。ほんとはDO_CRの上にあるべき。
