...
MTN_MOTION* MTN_AllocMotion( int n_joints, int frame )
{
	...
	key[ 0 + 1 ].fix = 1 ;
	key[ motion->header.motion_length ].fix = 1 ;
	/* キーフレーム情報の領域確保＆初期化 */
	size = sizeof(KEY_INFO) * ( motion->header.motion_length + 1 ) ;
	...
	for ( i = 0 ; i < motion->header.motion_joints ; i++ ){
		...
		key[ 0 + 1 ].tkey = 1 ;
		key[ motion->header.motion_length ].key = 1 ;
		key[ motion->header.motion_length ].tkey = 1 ;
	}
...
MTN_MOTION* MTN_OpenMTNFile( char *filename )
{
	...
	/* クォータニオン列の符号を揃える */
	for ( i = 0 ; i < motion->header.motion_joints ; i++ ){
		SetupQuat( &motion->rots[ i * ( motion->header.motion_length + 1 ) ], motion->header.motion_length );
	}

	/* 旧バージョンとの互換性を取るための細工 */
	if ( ( motion->header.flags & MTN_FLAG_VER_MASK ) == MTN_FLAG_VER_000 ){
		/* VER000 > VER001 の変更点 */
		/* error_length パラメータの追加 */
		motion->header.error_length = 25 ;
		/* バージョンアップ完了 */
		motion->header.flags &= ~MTN_FLAG_VER_MASK ;
		motion->header.flags |= MTN_FLAG_VER_001 ;
	}
