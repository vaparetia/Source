...
printf("%s\n", argv[i]);
	...
	for(i = 1; i < argc; i++)
	{
		...
		else if(strcmp(argv[i], COMMAND_GO) == 0)
		{
			/* 引数がgoコマンドだった */
			*lpnCommand |= MGDEMO_GO;
			argv[i][0] = '\0';
		}
		...
		else if(strcmp(argv[i], COMMAND_MODELPLUS) == 0)
		{
			/* Auto Model Plus が指定された */
			*lpnCommand |= MGDEMO_MODELPLUS;
			argv[i][0] = '\0';
		}
	}
	...
	if((*lpnCommand & MGDEMO_HELP) || (*lpnCommand & MGDEMO_GO))
		return 1;
