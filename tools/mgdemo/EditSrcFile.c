...
char* lpszCheckBeginCodeSrc = "\n# Add Demo Model List by mgdemo.\n";
	...
	char* lpszListDirectory)	/* list ディレクトリへのパス */
{
	...
	/* キーワードが出るまでテンポラリファイルにコピーする */
	while(fgets(szLineString, _MAX_TEXT, fFile) != NULL)
	{
		/* キーワード検索 */
		if(strstr(szLineString, lpszCheckBeginCodeSrc) != NULL)
			break;
	...
	/* スタートキーワードを書き出す */
	if(fputs(lpszCheckBeginCodeSrc, fTmpFile) < 0)
	{
		printf("Error! Don\'t write tmp file.\n");
		return 0;
	}
...
/******************************************************************************
 * mgdemoが作成した、scr編集時のテンポラリファイルを削除する
 */
	...
	char* lpszDemoTmpFileName)		/* mgdemo テンポラリファイルの名称 */
{
	...
	for(i = 0; i < MAXDEMOTMPFILE; i++)
	{
		...
		// 存在しなかったら削除
		if(CheckFile(szTmpFileName) == 0) break;
