/*
	conv.c
	.mtn to .mst 変換プログラム
	$Id: conv2.c,v 1.3 2001/12/04 10:18:22 usr02774 Exp $
...
/* ---------------------------------------------------------------- */
	...
	*/
void *MotionPacketCompress2( void *write_buffer, MST_INFO *info, DEMO_MOTION *src_motion )
{
	FVECTOR		*src, *old ;
...
/* ---------------------------------------------------------------- */
	/*
		.mtnファイルの圧縮＆ストリーム化処理
	*/
int Convert2( char *out_filename, PROGRAM_OPTION *option )
{
	...
	for ( i = 0 ; i < n_motions ; i++ ){
		...
		//printf("%s\n%d frame %d joints\n", option->input_data[i].filename,
		//	   motion->header.motion_length, motion->header.motion_joints );
	...
	for ( i = 0 ; i < max_frame ; i++ ){
		...
		MTN_MOTION	*motion ;
		MST_PACKET	*dst_packet ;
		void		*write_buffer, *top_addr ;
		int			total_motion = 0 ;
		...
		FrameBufferSize = 0 ;
		dst_packet = (MST_PACKET*)FrameBuffer ;
		//top_addr = write_buffer = (void*)&dst_packet->info_list[n_motions] ;
		top_addr = write_buffer = CopyBuffer ;
		...
		for ( j = 0 ; j < n_motions ; j++ ){
			...
			if ( ( i % option->frame_skip ) != 0 ) continue ;

			motion = option->input_data[j].motion ;
			frame = i + ( option->frame_skip - 1 ) ;
			if ( frame >= motion->header.motion_length ) continue ;
			/* 無圧縮モーションパケットの生成 */
			...
			packet->pad[1] = 0 ;
			/* モーションデータの設定 */
			MTN_GetMotionData( motion, frame, &mov, trans, qrots );
			for ( k = 0 ; k < motion->header.motion_joints ; k++ ){
				packet->motion[ k * 2 + 0 ] = qrots[k] ;
				packet->motion[ k * 2 + 1 ] = trans[k] ;
			}
			dst_packet->info_list[total_motion].object_id = option->input_data[j].strcode ;
			dst_packet->info_list[total_motion].start_joint = 0 ;
			dst_packet->info_list[total_motion].n_joints = motion->header.motion_joints ;
			dst_packet->info_list[total_motion].offset = (int)write_buffer - (int)top_addr ;
			dst_packet->info_list[total_motion].flag = MST_FLAG_COMPROTS|MST_FLAG_COMPTRANS ;
			dst_packet->info_list[total_motion].frame = option->frame_skip ;
			//printf("id:%d, n_joints:%d\n", option->input_data[total_motion].strcode, motion->header.motion_joints );
			write_buffer = MotionPacketCompress2( write_buffer, &dst_packet->info_list[total_motion], (DEMO_MOTION*)packet );
			total_motion++ ;
		}
		{
			int		size ;
			size = (int)write_buffer - (int)top_addr ;
			memcpy( &dst_packet->info_list[total_motion], top_addr, size );

			dst_packet->packet_type = DEMO_PACKET_MST ;
			dst_packet->size = sizeof(MST_PACKET) ;
			dst_packet->size += sizeof(MST_INFO) * total_motion ;
			dst_packet->size += size ;
			dst_packet->size = ( dst_packet->size + 15 ) & ~15 ;
			dst_packet->id = 0 ;
			dst_packet->n_lists = total_motion ;
		}
