...
static BOOL WriteMotionMove(int fFile, HP3DMODEL hP3DModel, HP3DMOTION hP3DMotion, LPP3DXYZ lpp3dXYZScale);
static BOOL WriteMotionRotate(int fFile, HP3DMODEL hP3DModel, HP3DOBJECT hP3DObject, HP3DMOTION hP3DMotion);
static BOOL WriteMotionTrans( int fFile, HP3DMODEL hP3DModel, HP3DOBJECT hP3DObject, HP3DMOTION hP3DMotion, LPP3DXYZ lpp3dXYZScale );
...
static void GetAbsRotateMatrix( P3DMATRIX AbsMatrix, HP3DOBJECT hP3DObject, HP3DMOTION hP3DMotion, int nFrame );
 ...
 */
BOOL P3DWriteFileMETALGEAR2Motion2(	/* 0以外	正常 */
									/* 0 */
	...
	LPARAM    lParam)							/* fnInterrupt への引数 */
{
	...
	if((_writeEx(fFile,
		...
		|| (WriteMotionMove(
			fFile, hP3DModelNew, hP3DMotion, lpp3dXYZScale) == 0)
		|| (WriteMotionRotate(
			fFile, hP3DModelNew, NULL, hP3DMotion) == 0)
		|| (WriteMotionTrans(
			fFile, hP3DModelNew, NULL, hP3DMotion, lpp3dXYZScale) == 0))
	{
		P3DDestroyModel(hP3DModelNew);
		_closeEx(fFile);
	...
	HP3DMOTION hP3DMotion)		/* モーションハンドル */
{
	...
	printfConsole(NULL, "OBJECT ROTATE = %s\n", P3DObject(hP3DObjectL)->szName);
		...
		if(((hP3DMotionObject = P3DFindMotionObjectObject(
				hP3DMotion, hP3DObjectL)) == NULL))
			return 0;
	...
	printfConsole(NULL, " %d : %6.2f %6.2f %6.2f\n", i,
		...
		if(WriteMotionRotate(
			fFile, hP3DModel, hP3DObjectL, hP3DMotion) == 0)
			return 0;
	...
	LPP3DXYZ     lpp3dXYZScale)	/* スケール値 */
{
	...
	/* 拡張 */
	FVECTOR          fvec, quat ;
	LPP3DOBJECT      lpP3DObject ;
	HP3DOBJECT       hP3DParentObject;
	...
	printfConsole(NULL, "OBJECT ROTATE = %s\n", P3DObject(hP3DObjectL)->szName);
		...
		lpP3DObject = P3DObject( hP3DObjectL ) ;

		/* 親オブジェクトのハンドル取得 */
		hP3DParentObject = P3DGetParentObject( hP3DObjectL );
		if ( hP3DParentObject != NULL ){
			/* 親オブジェクトのモーションオブジェクト取得 */
			hP3DParentMotionObject = P3DFindMotionObjectObject( hP3DMotion, hP3DParentObject ) ;
	...
	printfConsole(NULL, " %d : %6.2f %6.2f %6.2f\n", i,
		lpP3DMotionFrame->p3dXYZRotate.x, lpP3DMotionFrame->p3dXYZRotate.y, lpP3DMotionFrame->p3dXYZRotate.z);
			...
			if ( hP3DParentObject != NULL ){
				...
				GetAbsRotateMatrix( P3DMatrix1, hP3DParentObject, hP3DMotion, i );
				...
				P3DXyz.x = P3DMatrix2[3][0] - lpP3DObject->p3dXYZMove.x ;
				P3DXyz.y = P3DMatrix2[3][1] - lpP3DObject->p3dXYZMove.y ;
				P3DXyz.z = P3DMatrix2[3][2] - lpP3DObject->p3dXYZMove.z ;
#if 0
				if ( ( i % 20 ) == 0 ){
					printf("%6.2f %6.2f %6.2f\n", P3DMatrix2[3][0], P3DMatrix2[3][1], P3DMatrix2[3][2] );
					printf("%6.2f %6.2f %6.2f\n",
						   lpP3DObject->p3dXYZMove.x,
						   lpP3DObject->p3dXYZMove.y,
						   lpP3DObject->p3dXYZMove.z );
		...
		if(WriteMotionTrans(
			fFile, hP3DModel, hP3DObjectL, hP3DMotion, lpp3dXYZScale) == 0)
			return 0;
	}
	while((hP3DObjectL = P3DGetNextObject(hP3DObjectL)) != NULL);
	...
	int        nFrame)
{
	...
	/* 親オブジェクトのハンドル取得 */
	if ( ( hP3DParentObject = P3DGetParentObject( hP3DObject ) ) != NULL ){
		/* 親の絶対回転マトリクスを取得 */
		GetAbsRotateMatrix( AbsMatrix, hP3DParentObject, hP3DMotion, nFrame);
	} else {
		P3DInitMatrix( AbsMatrix );
	}

	/* モーションオブジェクト取得 */
	hP3DMotionObject = P3DFindMotionObjectObject( hP3DMotion, hP3DObject ) ;
	/* モーションフレーム取得 */
