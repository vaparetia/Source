	...
	for(st = 0; st < stat_cnt; st++)
	  {
		...
		switch(cmd & SEQ_STAT_MASK)
		  ...
		  case SEQ_STAT_MAGNI:
		    {
		      float magni;
		      magni = float_short(cmd & 0xffff);
		      printf("\t\tMagni: %f\n", magni);
		    }
		    break;
		  case SEQ_STAT_PARTUV:
		    {
		      unsigned short u, U, v, V;
		      u = (unsigned short)(cmd >> 16) & 0x0fff;
		      U = cmd & 0x0fff;
		      fgetw(&V, rfp);
		      fgetw(&v, rfp);
		      printf("\t\tPart UV:\n");
		    ...
		    break;
		  case SEQ_STAT_PTXSIZ:
		    {
		      unsigned short w, W, h, H;
		      w = (unsigned short)(cmd >> 16) & 0x0fff;
		      W = cmd & 0x0fff;
		      fgetw(&H, rfp);
		      fgetw(&h, rfp);
		      printf("\t\tPart Texture size:\n");
		      printf("\t\t\tWidth : %3d / %3d\n", w, W);
		      printf("\t\t\tHeight: %3d / %3d\n", h, H);
		    }
		    break;
