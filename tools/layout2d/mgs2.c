	...
	for(stat = block->d.st.begin; stat != NULL; stat = stat->next)
	  {
	    ...
	    {
	      l = ((int)stat->disp & 1);
	      l |= ((int)stat->h_rev & 1) << 1;
	      l |= ((int)stat->v_rev & 1) << 2;
	      fputl(SEQ_STAT_DISP | l, wfp);
	    }
	   
	    /* プライオリティ補正値 */
	    if(stat->pri > 0)
	      fputl(SEQ_STAT_PRI | (stat->pri & PRI_MASK), wfp);
		...
		if(!stat->iu && !stat->iv)
		  ...
		  {
		    fputl(SEQ_STAT_PARTUV | stat->iu, wfp);
		    fputl(stat->iv, wfp);
		  }

		/* 正規化テクセルサイズ */
		if(!stat->iuw && !stat->ivh)
		  {
		  ...
		  {
		    fputl(SEQ_STAT_PTXSIZ | stat->iuw, wfp);
		    fputl(stat->ivh, wfp);
		  }
	      }
...
static unsigned long write_action(sprLayout * layout, FILE *wfp)
{
  ...
  for(act = layout->actBegin; act != NULL; act = act->next)
	...
	{
	  w = (NULL != track->target) ? track->target->serial_id : 0xffff;
	  fputw(w, wfp);
	  ...
	  for(i = 0; i < track->seq_step; i++)
		...
		break;
	      case TR_SIGN:  /* シグナル     */
		l = SEQ_ACT_SIGN | (track->seq[i].signal & 0xffffff);
		fputl(l, wfp);
		fputl(track->seq[i].sigvalue, wfp);
		break;
	      case TR_END:   /* 終了         */
...
static int out_tri(triInfo * info, char * sdir, char * odir, int opt)
{
  triTex * tex;
  ...
  for(tex = info->tex; NULL != tex; tex = tex->next) {
	  tri_AddTexture(info->tri_name, tex->tex_name);
	  xti_AddTexture(info->tri_name, tex->tex_name);
  }
		  ...
		  char * search_dir, char * out_dir, int opt)
{
  ...
  for(info = layout->tri_info; NULL != info; info = info->next)
    if(0 != (ret = out_tri(info, sdir, odir, opt))) break;
