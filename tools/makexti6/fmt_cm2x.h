/*
	...
	1999/07/07 K.Takabe
	$Id: fmt_cm2x.h,v 1.2 2002/08/05 06:35:44 usr02774 Exp $
...
typedef struct _tri_fileheader {
	unsigned long32		tex_offset ;	/* テクスチャデータ開始オフセット（ブロック単位） */
	...
	unsigned long32		compress_flag ;	/* 圧縮フラグ */
	//unsigned long32		texel_addr ;	/* テクセルデータへのポインタ（現在未使用） */
	unsigned long32		format ;		/* Xbox: テクスチャフォーマット */
...
typedef struct _tri_fileheader {
	...
	unsigned long32		clut_size ;		/* ＣＬＵＴデータサイズ（ブロック単位） */
	unsigned long32		n_textures ;	/* テクスチャデータ数 */
	unsigned long32		compress_flag ;	/* 圧縮フラグ（現在は単なるフラグとして使用） */
	unsigned long32		texel_addr ;	/* テクセルデータへのポインタ（現在未使用） */
	unsigned long32		clut_addr ;		/* ＣＬＵＴデータへのポインタ（現在未使用） */
#else
	unsigned long32		tex_offset ;	/* テクスチャデータ開始オフセット（未使用） */
	unsigned long32		tex_size ;		/* テクスチャデータサイズ（未使用） */
	unsigned long32		clut_offset ;	/* ＣＬＵＴデータ開始オフセット（未使用） */
	unsigned long32		clut_size ;		/* ＣＬＵＴデータサイズ（未使用） */
	unsigned long32		n_textures ;	/* テクスチャデータ数 */
	unsigned long32		compress_flag ;	/* 圧縮フラグ（現在は単なるフラグとして使用） */
	unsigned long32		info_addr ;		/* XTIテクスチャ情報へのオフセット */
	unsigned long32		n_info ;		/* XTIテクスチャ情報数 */
#endif
} TRI_FILEHEADER ;
#endif
