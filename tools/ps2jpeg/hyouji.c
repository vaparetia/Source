...
void SetDisplayMode( int width, int height, int offset, in
t col_mode, int v_zoom, int page_offset ){
	u_long	dispfb ;
	u_long	display ;
	...
	if ( offset >= ( 1024*1024/2048 ) ) offset = 1024*1024/20
48 - 1 ;
	offset &= 511 ;
	dispfb = (col_mode<<15)|((width>>6)<<9)|(offset);
	display = ( (unsigned long)(height*1-1) << 44 ) | ((unsig
ned long)0x9ff<<32)
	  | ((( 2560+width-1)/width-1) << 23 ) | ((50)<<12) | (0x
28c+(0*(2560/width) | (v_zoom<<27) )) ;
...
void main ( void )
{
	...
	/* グラフィック初期化 */
	sceGsResetGraph(0, SCE_GS_INTERLACE, SCE_GS_NTSC, SCE_GS_
FIELD);
