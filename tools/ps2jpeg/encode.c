...
static int *bigbuff , bigbuff_top[512 * 448];
	      ...
	      unsigned char *code , float qn[2])
{
    ...
    memset(code , 0 , width*height);
    nbyte0 = dct_huff(image , width , height ,
		      (unsigned char *) code , qn[0] , Y);
    nbyte1 = dct_huff(image ,width , height ,
		      (unsigned char *)code+width*height  ,qn[1],C );
    //printf(" %d , %d \n", nbyte0 ,nbyte1);
    //Yのcodeと、Cのcodeをcodeにまとめる。
    memmove(code + sizeof(int)*2 , code , nbyte0 );
    memmove(code + sizeof(int)*2 + nbyte0 , code + width*height , nbyte1);
    *((int*)(code)) = nbyte0;
		    ...
		    unsigned char *code,float qn , int ycflag)
{
    ...
    huffman_get(( float *)prb_ac,16*16, pc_ac, (int *)bits_ac);
