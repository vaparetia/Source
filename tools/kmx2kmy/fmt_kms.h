/*
	...
	$Id: fmt_kms.h,v 1.1 2002/06/05 05:23:18 usr02774 Exp $
*/
...
enum {
	...
	MGS_MODEL_FLAG_NEWXVER	= 0x02000000,	/* XBOX用新フォーマットフラグ */
} ;
...
typedef	struct _DG_MDLPACK	{
	...
	unsigned short	n_verts ;		/* ポリゴン数						*/
	unsigned short	n_indices ;		/* ポリゴン数						*/
	unsigned int	tex_id[3] ;		/* テクスチャＩＤ（３セット）		*/
	short			*verts ;		/* 頂点データ						*/
	short			*norms ;		/* 法線データ						*/
	short			*uvs[3] ;		/* ＵＶデータ（３セット）			*/
	CVECTOR			*rgbs ;			/* プリシェードデータへ（未使用）	*/
	unsigned short	*index ;			/* リザーブ */
} KMS_MDLPACK ;
...
typedef	struct _DG_MDL	{
	...
	void			*vbuff ;		/* パック化されたオブジェクトの共有頂点データ */
	int				stride ;		/* 頂点データのストライド */
	int				n_verts ;		/* 共有頂点データの頂点数 */
} KMS_MDL ;
