/*
	...
	$Id: fmt_kms.h,v 1.1 2002/11/18 07:22:48 usr02774 Exp $
*/
...
enum {
	...
	MGS_MODEL_HYBRID		= 15066104,	/* バイブリッドモデル("MGS_MODEL_HYBRID") */
...
/*----------------------------------------------------------------*/
	...
	*/
/* 頂点情報 */
typedef struct _khm_vert {
	short			vx, vy, vz, flag ;
	short			nx, ny, nz, w ;
	short			u0, v0, q0, index ;
	short			u1, v1, q1, p0 ;
	short			u2, v2, q2, p1 ;
} KHM_VERT ;

/* モデルパケット情報 */
typedef struct _khm_packet {
	int				flag ;				/* 各種フラグ */
	int				n_verts ;			/* 頂点数 */
	int				v_size ;			/* 頂点サイズ */
	int				tex[3] ;			/* 参照テクスチャＩＤ */
	KHM_VERT		*verts ;			/* 頂点データへのポインタ */
	int				pad[1] ;			/* リザーブ */
} KHM_PACKET ;

/* モデル情報 */
typedef struct _khm_model {
	int				id ;				/* モデルＩＤ */
	int				n_mats ;			/* 最大スケルトン参照数 */
	u_char			mats[8] ;			/* 参照スケルトン番号 */
	int				n_packets ;			/* パケット数 */
	KHM_PACKET		*packet ;			/* パケットへのポインタ */
	int				pad[2] ;			/* リザーブ */
	FVECTOR			bound_max ;			/* モデルバウンディング */
	FVECTOR			bound_min ;			/* モデルバウンディング */
} KHM_MODEL ;

/* モデルスケルトン情報 */
typedef struct _khm_skel {
	int				id ;				/* 関節ＩＤ */
	int				flag ;				/* 関節フラグ */
	int				parent ;			/* 親関節番号 */
	int				pad[1] ;			/* リザーブ */
	FVECTOR			trans ;				/* 親からのオフセット量 */
	FVECTOR			root_trans ;		/* モデルルートからのオフセット量 */
	FVECTOR			bound_max ;			/* スケルトンのバウンディング */
	FVECTOR			bound_min ;			/* スケルトンのバウンディング */
} KHM_SKEL ;

/* ファイルヘッダー */
typedef struct _khm_header{
	int				format_type ;		/* ファイルフォーマット */
	int				n_skeletons ;		/* 関節数 */
	int				n_models ;			/* 基本ユニット数 */
	int				texture ;			/* 使用テクスチャパックＩＤ */
	KHM_SKEL		*skeletons ;		/* スケルトンデータ */
	KHM_MODEL		*models ;			/* 関節に対応したモデルデータ */
	int				pad[2] ;			/* リザーブ */
	FVECTOR			bound_max ;			/* 全体バウンディング */
	FVECTOR			bound_min ;			/* 全体バウンディング */
} KHM_HEADER ;
