...
namespace Paloma
{
...

    {
        ...
        /// <summary>
        /// Loads the Targa header into a TargaHeader object.
        /// </summary>
        /// <param name="sFileName">The Targa image filename</param>
        /// <returns>A TargaHeader object with the Targa information loaded into it.</returns>
        public static TargaHeader LoadTargaHeader(string sFileName)
        {
           // make sure we have a .tga file
           if (System.IO.Path.GetExtension(sFileName).ToLower() == ".tga")
           {
              // make sure the file exists
              if (System.IO.File.Exists(sFileName) == true)
              {
                  char[] filebytes = new char[100];
                  TargaImage image = new TargaImage();

                  StreamReader fileReader = new StreamReader(sFileName);
                  fileReader.Read(filebytes, 0, 100);

                  BinaryReader binReader = null;
                  using (binReader = new BinaryReader(fileReader.BaseStream))
                  {
                     image.LoadTGAHeaderInfo(binReader);
                  }
                  return image.Header;
              }
              else
                 throw new Exception(@"Error loading file, could not find file '" + sFileName + "' on disk.");

           }
           else
              throw new Exception(@"Error loading file, file '" + sFileName + "' must have an extension of '.tga'.");
        }
