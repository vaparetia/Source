...
def build_vertex_blob(positions, colors_rgba_le):
    """Build CVertexArray blob (LE) for Float3 positions + UByte4N colours."""
    num_verts   = len(positions)
    num_streams = 2
    header_size = 4 + num_streams * STREAM_SIZE
    pos_size    = num_verts * 12
    col_size    = num_verts * 4
    blob_size   = header_size + pos_size + col_size
...
def build_modl(positions, colors_rgba_le, indices, bounds_min, bounds_max,
    ...
    """
    blob = build_vertex_blob(positions, colors_rgba_le)
